...
```
:warning: The path in the example `--with-boot-jdk=` option is appropriate for the Docker installation. If you're not using the Docker environment, or using the image from the github container registry, set the path that's appropriate for your setup, such as `<my_home_dir>/bootjdk8`.
