...
public class Test_J9VMInternals {
	...
	public static class SuperclassRestricted extends sun.misc.BASE64Decoder {
		static {
			logger.debug("#####################################");
...
//			new sun.misc.BASE64Encoder();
	...
	public static class TestHelper2 extends TestRestricted implements Runnable {
		public void run() {
			...
			try {
				Class cl2 = Class.forName("sun.misc.BASE64Encoder");
				Assert.fail("successfully loaded sun.misc.BASE64Encoder using forName");
			} catch (SecurityException e) {
				logger.debug("GOOD forName of sun.misc.BASE64Encoder: " + e);
			...
			try {
				Object o = new Object();
				sun.misc.BASE64Encoder encoder = (sun.misc.BASE64Encoder)o;
				Assert.fail("successfully cast to sun.misc.BASE64Encoder");
			} catch (ClassCastException e) {
				Assert.fail("cast to sun.misc.BASE64Encoder: " + e);
			} catch (SecurityException e) {
				logger.debug("GOOD casting to sun.misc.BASE64Encoder: " + e);
			}

			startTest("try testing instanceof a restricted package");
			try {
				Object o = new Object();
				if (o instanceof sun.misc.BASE64Encoder) {
				}
				Assert.fail("successfully tested instanceof sun.misc.BASE64Encoder");
			} catch (SecurityException e) {
				logger.debug("GOOD instanceof sun.misc.BASE64Encoder: " + e);
			}

			startTest("try calling a static method in a restricted package");
			try {
				sun.misc.Version.init();
				Assert.fail("called static method on sun.misc.Version");
			} catch (SecurityException e) {
				logger.debug("GOOD calling static method on sun.misc.Version: " + e);
			}

			startTest("try accessing a static field in a restricted package");
			try {
				Comparator comparator = sun.misc.ASCIICaseInsensitiveComparator.CASE_INSENSITIVE_ORDER;
				Assert.fail("accessed static field on sun.misc.ASCIICaseInsensitiveComparator");
			} catch (SecurityException e) {
				logger.debug("GOOD accessing static field on sun.misc.ASCIICaseInsensitiveComparator: " + e);
			}
		}
	}
