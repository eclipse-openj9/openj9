...
public class fw001
{
    ...
    public native int endTest();
    public native int startTest(String name);

    // Add or remove field watches based on the information provided.
    public static native int modifyWatches(String className, String fieldName, String fieldSignature, boolean isStatic, boolean isRead, boolean isWrite, boolean isAdd);
    ...
    {
        setNumReports(0);
        return startTest(this.getClass().getName().replace('.', '/')) == JNI_OK ? true : false;
    }

    public boolean teardown()
    {
        return endTest() == JNI_OK ? true : false;
    }
    ...
    public void runTest(MyObject myObj, boolean isStatic, boolean isRead) throws InterruptedException
    {
        ...
        for (String[] currRow : fieldData)
        {
            ...
            // Add field watch.
            if (!assertEquals(JNI_OK, modifyWatches(myObj.getClass().getName().replace('.', '/'), prefix + currRow[0], currRow[1], isStatic, isRead, !isRead /*isWrite*/, true /* isAdd */)))
            {
            ...
            if (!assertEquals(JNI_OK, modifyWatches(myObj.getClass().getName().replace('.', '/'), prefix + currRow[0], currRow[1], isStatic, isRead, !isRead, false /* isAdd */)))
            {
