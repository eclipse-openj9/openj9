 ...
 *******************************************************************************/
package com.ibm.jvmti.tests.eventSampledObjectAllocation;
...
public class soae001 {
	...
	public boolean testDefaultInterval() {
		...
		jvmtiResult = enable();
		if ( jvmtiResult != 0) {
			System.out.println("com.ibm.jvmti.tests.eventSampledObjectAllocation.soae001.enable() failed with: " + jvmtiResult);
		} else {
			byte[] bytes;
			...
			if (samplingResult != 1) {
				System.out.println("com.ibm.jvmti.tests.eventSampledObjectAllocation.soae001.check() failed, expected 1 but got: " + samplingResult);
			} else {
				jvmtiResult = disable();
				if ( jvmtiResult != 0) {
					System.out.println("com.ibm.jvmti.tests.eventSampledObjectAllocation.soae001.disable() failed with: " + jvmtiResult);
				} else {
					reset();
					bytes = new byte[DEFAULT_SAMPLING_RATE];
					...
					if (samplingResult != 0) {
						System.out.println("com.ibm.jvmti.tests.eventSampledObjectAllocation.soae001.check() failed, expected 0 but got: " + samplingResult);
					} else {
						result = true;
					}
				}
