...
public class LinConHashCodeGenerator extends HashCodeGenerator {
	int previousHashCode = 0;
	boolean changedSeed;
	LinCon gen;

	public LinConHashCodeGenerator(int initialSeed) {
		gen = new LinCon();
	}
	...
	public int processHashCode(int hashCode) {
		...
		if (changedSeed) {
			seed = gen.scrambleSeed(seed);
		}
		previousHashCode = HashCodeGenerator.doPresquare(hashCode);
		int temp = hashCode;
		for (int i = 0; i < HashCodeGenerator.LINCON_CYCLES; ++i) {
			temp = gen.runLinCon(temp + seed);
		}
		return temp;
	}
