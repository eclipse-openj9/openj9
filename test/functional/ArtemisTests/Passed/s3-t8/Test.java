...
public class Test {
    ...
    public static int iMeth(short s) {
        if (ax$0) {
            try {
                DatagramChannel ax$3 = DatagramChannel.open();
                ax$3.socket().bind(new InetSocketAddress(0));
                ax$3.configureBlocking(false);
                ByteBuffer ax$1 = ByteBuffer.allocateDirect(256);
                SocketAddress ax$2 = ax$3.receive(ax$1);
                if (ax$2 != null)
                    ;
                ax$3.close();
