 ...
 */
public class DirectHandleTest {

	@Test(groups = { "level.extended" })
	public void testIfDirectHandleTestIsJitted() throws Throwable {
		try {
			TestClass_DH clazz = new TestClass_DH();
			MethodHandle mh = MethodHandles.lookup().findVirtual(TestClass_DH.class, "test", MethodType.methodType(void.class));
			AssertJUnit.assertTrue(mh.getClass().toString().equals("class java.lang.invoke.DirectHandle"));
			try {
				while (true) {
					mh.invokeExact(clazz);
				}
			} catch (Error t) {
				t.printStackTrace();
			}
