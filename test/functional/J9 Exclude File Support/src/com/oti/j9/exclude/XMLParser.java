...
public class XMLParser {
	...
	private static Integer decode(String string) throws NumberFormatException {
	...
	private boolean _scan_cdata_or_eof() throws XMLException {
		...
		while (_fScan != '<') {
			...
			default:
				if (XMLSpec.isLineDelimiter(_fScan) || XMLSpec.isWhitespace(_fScan)) {
	...
	private Hashtable<String, String> _scan_attributes() throws XMLException {
	...
	private void _scan_xml_header() throws XMLException {
		_scan_for_all("<?xml");
		Hashtable<String, String> attributes = _scan_attributes();
	...
	private void _scan_element_close() throws XMLException {
		...
		_fLevel -= 1;
		if (_fLevel < 0) {
			parseError("Unbalanced tag " + closedTag);
		}
	}
	...
	void parseXML() throws XMLException, IOException {
		...
		while (true) {
			...
			} else if (_scan_cdata_or_eof() == EOF_ENCOUNTERED) {
				break;
			}
		}
	...
	public boolean parse(String uri, IXMLDocumentHandler handler) {
		_fURI = uri;
		try (InputStream uriStream = new FileInputStream(uri)) {
