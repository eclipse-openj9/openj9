...
foreach my $argv (@ARGV) {
	if ( $argv =~ /^--graphSpecs=(.*)/ ) {
		...
		print "Usage:\n"
			...
			. "  --validate                Validation mode, it is used to check if the java tkg is correct.\n" 
			. "                            Append .tkgj in the generated files and compare the files with the ones generated by java tkg.\n";
...
runmkgen( $projectRootDir, \@allLevels, \@allGroups, \@allTypes, $output, $graphSpecs, $jdkVersion, \@allImpls, $impl, $modesxml, $ottawacsv, $buildList, $iterations, $testFlag, $validateMode );
