...
rem
rem Copyright (c) 2004, 2017 IBM Corp. and others
rem
...
rem @summary check the name command _%u_%g line option does not work 
...
set TESTSCRIPT=nameOption4
set NAME=_%%u_%%g

%1\java -Xshareclasses:name=%NAME% HelloWorld 2> %TESTSCRIPT%.out

if exist %TESTSCRIPT%.out (
    %1\java SimpleGrep "JVMSHRC154E Escape character g not valid for cache name" %TESTSCRIPT%.out 1
) else (
    echo %TESTSCRIPT%.out: TEST FAILED: No file created
)
