...
# @summary check the Xscmx name command line option
# @summary and that the destroy command works

. ./cmdlineConfig.sh
export TESTSCRIPT=XscmxOption5
export NAME=$CFGUSER

$1/java -Xshareclasses:name=$NAME,verbose -Xscmx20M HelloWorld 2> $TESTSCRIPT.out
...
then
    ...
    #
    if ( grep "is full" $TESTSCRIPT.out >/dev/null )
    then
        echo $TESTSCRIPT: TEST FAILED
        echo $TESTSCRIPT: Expected no cache full error
    else
		echo TEST PASSED
    fi
fi
