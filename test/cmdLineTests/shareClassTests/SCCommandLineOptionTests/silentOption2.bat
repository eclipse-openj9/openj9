...
rem @summary check the silent name command line option overrides verbose

setlocal
call cmdlineConfig.bat
set TESTSCRIPT=silentOption2
set NAME=%CFGUSER%
%1\java -Xshareclasses:name=%NAME%,verbose,silent HelloWorld 2> %TESTSCRIPT%.out

if exist %TESTSCRIPT%.out (
    %1\java SimpleGrep JVMSHRC158I %TESTSCRIPT%.out 0
