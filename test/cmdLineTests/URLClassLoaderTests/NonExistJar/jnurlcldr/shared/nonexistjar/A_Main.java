...
public class A_Main {
	...
	private static boolean changeJarFileName() {
		File f = new File("NonExistJar/B_.jar");
		File f2 = new File ("NonExistJar/B.jar");
		if (f.isFile()==false) {
			try {
			System.out.println("SETUP ERROR: Error isFile()==false: "+f.getCanonicalFile());
			}
			catch(Exception e) {
		...
		if (f.canWrite()==false) {
			try {
			System.out.println("SETUP ERROR: Error canWrite()==false: "+f2.getCanonicalFile());
			}
			catch(Exception e) {
