...
public class ELFS39064DumpReader extends ELFDumpReader {
	...
	protected SortedMap<String, Number> readRegisters() throws IOException {
		...
		registers.put("mask", _reader.readLong());
		registers.put("addr", _reader.readLong());		
		for (int i = 0; i < 16; i++)
			registers.put("gpr" + i, _reader.readLong());
		for (int i = 0; i < 16; i++)
			registers.put("acr" + i, _reader.readLong());
		registers.put("origgpr2", _reader.readLong());
		registers.put("trap", _reader.readLong());
		registers.put("old_ilc", _reader.readLong());
		return registers;
	}
	
	/*
	 * https://refspecs.linuxbase.org/ELF/zSeries/lzsabi0_zSeries.html#DWARFREG
