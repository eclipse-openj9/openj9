...
public class DumpAllOffHeapEntriesCommand extends Command
{
	...
	{
		boolean bOffHeapDataTable = false;
		if (0 != args.length) {
		...
		try {
			...
			bOffHeapDataTable = false;
		}
		if (!bOffHeapDataTable) {
			out.printf("This command only works with the core file, which contains off-heap,%n off heap is only been used in -Xgcpolicy:balanced and -XXgc:enableVirtualLargeObjectHeap case.%n");
		}
	}
