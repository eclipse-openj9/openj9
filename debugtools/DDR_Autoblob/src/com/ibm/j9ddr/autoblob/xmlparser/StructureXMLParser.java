...
public class StructureXMLParser implements ITypeCollection {
	...
	static final class XMLHandler extends DefaultHandler implements ContentHandler {
		...
		private static final Map<String, Type> primitiveTypesByName;
		...
		final Map<String, Typedef> typedefsByName = new HashMap<>();
		final Map<String, StructType> structsByName = new HashMap<>();
		final Map<String, EnumType> enumsByName = new HashMap<>();
		final Map<String, UnionType> unionsByName = new HashMap<>();

		final Map<Long, StructType> anonymousStructsById = new HashMap<>();
		final Map<Long, EnumType> anonymousEnumsById = new HashMap<>();
		final Map<Long, UnionType> anonymousUnionsById = new HashMap<>();

		final List<EnumType> topScopeAnonymousEnums = new LinkedList<>();

		private final ILineNumberConverter converter;

		/* State variables. */

		private Stack<UserDefinedType> typeStack = new Stack<>();

		/* Stack of runnables to be executed every time a tag closes. Will cope very badly with broken XML. */
		private final Stack<Runnable> deferredWorkStack = new Stack<>();
