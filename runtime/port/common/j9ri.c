...
/**
 * Used for platforms (zlinux) that require registration with signal handler first
 * before turning on Runtime Instrumentation.
 * @param[in] portLibrary The port library
 * @return negative error code on failure, 0 on success.
 * Currently all platforms other than zlinux return 0
 */
int32_t
...
/**
 * Used for platforms (zlinx) to clean up work done during j9ri_enableRISupport
 * @param[in] portLibrary The port library
 * @return negative error code on failure, 0 on success.
 * Currently all platforms other than zlinux return 0
 */
