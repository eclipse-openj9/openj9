...
void
j9gc_report_j9_hot_field_info(int32_t reducedCpuUtil, J9Class* clazz, U_8 fieldOffset,  U_32 reducedFrequency)
{
	//create hot field pool for class loader if null
	if (clazz->classLoader->hotFieldPool == NULL || clazz->classLoader->hotFieldClassInfoPool == NULL) {
		printf("MODRON: Failed to allocate memory for hot field pool for classloader \n");
		return;
	}
	...
	if(NULL == clazz->hotFieldsInfo) { 
		...
		printf("MODRON: Add hotFieldsInfo pool element. Class:%p, ClassLoader:%p\n", clazz, clazz->classLoader);
		if(NULL == clazz->hotFieldsInfo || NULL == newHotField) {
			...
			newHotField->hotness = reducedFrequency;
			...
			omrthread_monitor_exit(clazz->classLoader->hotFieldPoolMutex);
			printf("MODRON: Add newHotField to class. Class:%p; hotFieldListLength:%u\n", clazz, clazz->hotFieldsInfo->hotFieldListLength);
			//J9Class* newHotClass = ext->GetNewHotClassPoolElement(); //add class to hotClassPool
		}
		return; 
	} 	
	else {	
		...
		while (NULL != temp) {
			printf("MODRON: FieldOffset:%u Hotness = %u, reducedFrequency = %u, reducedCpuUtil = %u, class:%p, hotFieldListLength:%u\n",temp->hotFieldOffset, temp->hotness, reducedFrequency, reducedCpuUtil, clazz, clazz->hotFieldsInfo->hotFieldListLength);
			if(temp->hotFieldOffset == fieldOffset) {
				printf("MODRON: FieldOffset:%u InitialHotness = %u, reducedFrequency = %u, InitialCpuUtil = %u, reducedCpuUtil = %u, class:%p\n",temp->hotFieldOffset, temp->hotness, reducedFrequency, temp->cpuUtil, reducedCpuUtil, clazz);
				omrthread_monitor_enter(clazz->classLoader->hotFieldPoolMutex);
				temp->hotness += reducedFrequency;
				temp->cpuUtil += reducedCpuUtil;
				clazz->hotFieldsInfo->isClassHotFieldListDirty = true;
				omrthread_monitor_exit(clazz->classLoader->hotFieldPoolMutex); 
				printf("MODRON: FieldOffset:%u, FinalHotness = %u, FinalCpuUtil = %u, class:%p\n",temp->hotFieldOffset , temp->hotness, temp->cpuUtil, clazz);
				return; 
			}
			if(NULL == temp->next) {
				//create hot field pool element and add to classes linked list of hot fields if hot field does not exist
				omrthread_monitor_enter(clazz->classLoader->hotFieldPoolMutex);
				J9HotField* newHotField = (J9HotField *)pool_newElement(clazz->classLoader->hotFieldPool);
				if(NULL == newHotField) {
					omrthread_monitor_exit(clazz->classLoader->hotFieldPoolMutex);
					printf("MODRON: Failed to allocate memory for new hot field\n");
					return;
				}
				newHotField->next = NULL;
				newHotField->hotFieldOffset = fieldOffset;
				...
				clazz->hotFieldsInfo->isClassHotFieldListDirty = true;
				omrthread_monitor_exit(clazz->classLoader->hotFieldPoolMutex);
				printf("MODRON: Add newHotField to class hotfield list that already exists. Class:%p; hotFieldListLength: %u\n", clazz, clazz->hotFieldsInfo->hotFieldListLength);  
				return; 	
			}
			temp = temp->next;
		}
