	...
	case HELPER_TYPE_THREAD_PARK:
		{
			...
			|| ((UDATA)(currentTime - vm->prevProcTimestamp) > vm->recalcThresholdNanos)
				...
				if (vm->prevSysCPUTime.timestamp == 0) {
					...
					Trc_VM_ThreadHelp_timeCompensationHelper_parkMachine(vmThread, currentSysCPUTime.cpuTime, vm->prevSysCPUTime.cpuTime, numberOfCpus, currentSysCPUTime.timestamp, vm->prevSysCPUTime.timestamp);
				}
			...
			Trc_VM_ThreadHelp_timeCompensationHelper_parkWaited(vmThread, j9time_nano_time()-currentTime, policy, earlyBreak);
			break;
		}
