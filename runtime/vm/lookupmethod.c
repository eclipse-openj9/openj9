...
illegalAccessMessage(J9VMThread *currentThread, IDATA badMemberModifier, J9Class *senderClass, J9Class *targetClass, IDATA errorType)
{
	...
	|| (J9_VISIBILITY_NEST_MEMBER_NOT_CLAIMED_ERROR == errorType)
		...
		srcModuleMsg = getModuleNameUTF(currentThread, J9OBJECT_ADDRESS_LOAD(currentThread, srcModuleObject, vm->modulePointerOffset),
				srcModuleBuf, J9VM_PACKAGE_NAME_BUFFER_LENGTH);
		if (NULL == srcModuleMsg) {
			goto allocationFailure;
		}
		destModuleMsg = getModuleNameUTF(currentThread, J9OBJECT_ADDRESS_LOAD(currentThread, destModuleObject, vm->modulePointerOffset),
