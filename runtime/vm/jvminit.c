...

	{
		...
		if (0 <= disclaimIndex) {
			...
			if (4096 != j9vmem_supported_page_sizes()[0]) {
				...
				return JNI_ERR;
			} else if (optionValueOperations(PORTLIB, vm->vmArgsArray, disclaimIndex, GET_OPTION, &disclaimOption, 0, '=', 0, NULL) == OPTION_OK) {
				if (0 == strncmp(disclaimOption, VMOPT_XXCLASSMEMORYDISCLAIM_ALL, strlen(VMOPT_XXCLASSMEMORYDISCLAIM_ALL))) {
					/* TODO just ramClass for the time being. */
				...
				} else if (0 == strncmp(disclaimOption, VMOPT_XXCLASSMEMORYDISCLAIM_NONE, strlen(VMOPT_XXCLASSMEMORYDISCLAIM_NONE))) {
