 ...
 * Currently does not allow MAP_FIXED as it is not supported in all architectures.
 */
#include <sys/mman.h>
...
bool
VMSnapshotImpl::setupRestoreRun(void)
{
...
bool
VMSnapshotImpl::setupSnapshotRun(void)
{
...
bool
VMSnapshotImpl::setupSnapshotRunPostInitialize(void)
{
...
{
	Trc_VM_SubAllocateSnapshotMemory_Entry(_snapshotHeader, byteAmount);
	...
	Trc_VM_SubAllocateSnapshotMemory_Exit(memStart);
 ...
 */
bool
VMSnapshotImpl::readSnapshotFromFile(void)
{
	bool rc = true;
	Trc_VM_ReadSnapshotFromFile_Entry(_heap, _vmSnapshotFilePath);
	PORT_ACCESS_FROM_PORT(_portLibrary);
	...
	_vm->vmSnapshotFilePath = (char*)_vmSnapshotFilePath;
	...
	Trc_VM_ReadSnapshotFromFile_Exit();
...
bool
VMSnapshotImpl::writeSnapshotToFile(void)
{
	PORT_ACCESS_FROM_PORT(_portLibrary);
	Trc_VM_WriteSnapshotToFile_Entry(_heap, _vmSnapshotFilePath);
	...
	Trc_VM_WriteSnapshotToFile_Exit();

	return rc;
}

void
VMSnapshotImpl::writeSnapshot(void)
{
