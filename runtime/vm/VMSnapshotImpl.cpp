...
VMSnapshotImpl::fixupConstantPool(J9Class *ramClass)
{
	...
	const UDATA ramConstantPoolCount = romClass->ramConstantPoolCount;
	...
	if (0 != ramConstantPoolCount) {
		...
		for (UDATA i = 0; i < ramConstantPoolCount; ++i) {
			if (0 == descriptionCount) {
				description = *cpShapeDescription;
				cpShapeDescription += 1;
			...
			case J9CPTYPE_FIELD:
				{
					volatile UDATA &flags = ((J9RAMFieldRef *) ramConstantPool)[i].flags;
					volatile UDATA &valueOffset = ((J9RAMFieldRef *) ramConstantPool)[i].valueOffset;
...
teardownVMSnapshotImpl(J9JavaVM *javaVM)
{
	...
	/* This thread must have the exclusive VMAccess to cache JVM state to ensure data integrity. */
