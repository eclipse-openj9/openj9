...
/**
 * Merges two sorted free lists into a single sorted list.
 *
 * @param a  Pointer to the head of the first sorted free list.
 * @param b  Pointer to the head of the second sorted free list.
 *
 * @return A pointer to the head of the merged sorted free list, or NULL if both inputs are NULL.
 *
 * This function traverses both free lists in ascending order of memory address
 * and stitches them together into one sorted list.
 */
static J9RAMClassFreeListBlock * mergeTwoSortedFreeLists(J9RAMClassFreeListBlock *a,
														   J9RAMClassFreeListBlock *b) {
	if (a == NULL && b == NULL) {
		return NULL;
	}
	...
	if (tail != NULL) {
		tail->nextFreeListBlock = (a != NULL) ? a : b;
	}
	return dummy.nextFreeListBlock;
}

/**
 * Sorts the linked list of free list blocks using the merge sort algorithm.
 *
 * @param head  Pointer to the head of the free list.
 *
 * @return A pointer to the head of the sorted free list.
 */
static J9RAMClassFreeListBlock * mergeSortFreeBlocks(J9RAMClassFreeListBlock *head) {
	if (head == NULL || head->nextFreeListBlock == NULL)
		return head;
	...
	while (fast != NULL && fast->nextFreeListBlock != NULL) {
		slow = slow->nextFreeListBlock;
		fast = fast->nextFreeListBlock->nextFreeListBlock;
	}

	// Split the list.
	J9RAMClassFreeListBlock *mid = (slow != NULL) ? slow->nextFreeListBlock : NULL;
	if (slow != NULL) {
...
/**
 * Coalesces adjacent free list blocks in the given sorted free list.
 *
 * @param freeListHead  Pointer to the head pointer of the free list.
 *
 * This function first ensures that the free list is sorted (by calling mergeSortFreeBlocks)
 * and then iterates through the list to merge adjacent blocks. The free list is updated in place.
 */
static void coalesceFreeBlocks(J9RAMClassFreeListBlock **freeListHead) {
	if (freeListHead == NULL || *freeListHead == NULL)
		return;
	...
	*freeListHead = mergeSortFreeBlocks(*freeListHead);
	...
	while (current != NULL) {
		while (current->nextFreeListBlock != NULL &&
 ...
 *
 * @param freeLists  Pointer to the structure containing various free lists.
 *
...
/**
 * Allocates the remaining RAM class fragments by allocating a new memory segment to cover all the
 * outstanding RAM class allocation requests and then carving that segment into the requested fragments.
 *
 * @param requests                   Pointer to a linked list of RAMClassAllocationRequest structures.
 * @param allocationRequestCount     The total number of allocation requests.
 * @param javaVM                     Pointer to the Java VM instance.
 * @param classLoader                Pointer to the class loader performing the allocation.
 * @param allocationRequests         Array of RAMClassAllocationRequest structures used for tracking allocations.
 * @param j9RamClassFreeList         Pointer to the structure containing RAM class free lists.
 * @param ramClassUDATABlockFreelist Pointer to the free-list for UDATA blocks.
 * @param segmentKind                The segment kind under which the requests must be allocated.
 *
 * @return TRUE if all fragments were successfully allocated, FALSE otherwise.
 */
static BOOLEAN
