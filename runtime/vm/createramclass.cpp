...
markInterfaces(J9ROMClass *romClass, J9Class *superclass, J9ClassLoader *classLoader, BOOLEAN *foundCloneable, UDATA *markedInterfaceCount, UDATA *inheritedInterfaceCount, IDATA *maxInterfaceDepth)
{
	...
	if (interfaceCount != 0) {
		...
		for (i = 0; i<interfaceCount; i++) {
			...
			J9Class *interfaceClass = hashClassTableAt(classLoader, J9UTF8_DATA(interfaceName), J9UTF8_LENGTH(interfaceName), J9_HASH_TABLE_LOOKUP_FLAG_JCL_DEFINE_CLASS);
	...
	if (state->ramClass != NULL) {
		...
		if (failed) {
			...
			if (elementClass == NULL) {
				alreadyLoadedClass = hashClassTableAt(classLoader, J9UTF8_DATA(className), J9UTF8_LENGTH(className), J9_HASH_TABLE_LOOKUP_FLAG_JCL_DEFINE_CLASS);
			} else {
...
fail:
	...
	if (!hotswapping) {
		if (elementClass == NULL) {
			ramClass = hashClassTableAt(classLoader, J9UTF8_DATA(className), J9UTF8_LENGTH(className), J9_HASH_TABLE_LOOKUP_FLAG_JCL_DEFINE_CLASS);
		} else {
