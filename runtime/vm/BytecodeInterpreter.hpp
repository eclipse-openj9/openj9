...
recursiveOOM:
	...
	inlUnsafeNotifyStrictStaticAccess0(REGISTER_ARGS_LIST)
	{
		...
		entry = VM_ValueTypeHelpers::findJ9FlattenedClassCacheEntryForStaticFieldByOffset(ramClass, (UDATA)valueAddress);
...
retry:
		...
		if (J9_ARE_ALL_BITS_SET(classAndFlags, J9StaticFieldRefStrict)
			...
			J9FlattenedClassCacheEntry *entry = VM_ValueTypeHelpers::findJ9FlattenedClassCacheEntryForStaticFieldByOffset(ramConstantPool->ramClass, (UDATA)valueAddress);
			if (J9_VM_FCC_ENTRY_IS_STRICT_STATIC_UNSET(entry)) {
				rc = THROW_GET_STRICT_STATIC_NOT_SET;
				goto done;
			} else {
	...
	if (J9_ARE_ALL_BITS_SET(classAndFlags, J9StaticFieldRefStrict)
		...
		J9FlattenedClassCacheEntry *entry = VM_ValueTypeHelpers::findJ9FlattenedClassCacheEntryForStaticFieldByOffset(ramClass, (UDATA)valueAddress);
		if (J9_VM_FCC_ENTRY_IS_STRICT_STATIC_UNSET(entry)) {
			Assert_VM_true(ramClass->flattenedClassCache->strictStaticFieldCounter > 0);
			ramClass->flattenedClassCache->strictStaticFieldCounter -= 1;
			J9_VM_FCC_ENTRY_SET_AS_WRITTEN(entry);
