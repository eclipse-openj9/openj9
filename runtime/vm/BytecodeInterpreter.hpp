...
retry:
	...
	VMINLINE void
	heapifyObjectIfRequired(REGISTER_ARGS_LIST, j9object_t object)
	{
		if ((UDATA)object >= (UDATA)(_currentThread->stackObject) && (UDATA)object < (UDATA)(_currentThread->stackObject->end)) {
			updateVMStruct(REGISTER_ARGS);
			VM_VMHelpers::heapifyObject(_currentThread, object);
			VMStructHasBeenUpdated(REGISTER_ARGS);
		}
	}
