...
/* Must match jdk.internal.misc.Unsafe field layout and array layout constants */
#define J9_LAYOUT_REFERENCE 0 /* Unsafe.FIELD_LAYOUT_REFERENCE / Unsafe.ARRAY_LAYOUT_REFERENCE */
#define J9_LAYOUT_FLATTENED 1 /* Unsafe.FIELD_LAYOUT_FLATTENED / Unsafe.ARRAY_LAYOUT_FLATTENED */
...
recursiveOOM:
	...
	inlUnsafeNotifyStrictStaticAccess0(REGISTER_ARGS_LIST)
	{
		...
		U_64 staticFieldOffset = *(U_64 *)(_sp + 1);
		...
		/* staticFieldOffset must reference a static field. */
		if (J9_ARE_NO_BITS_SET(staticFieldOffset, J9_SUN_STATIC_FIELD_OFFSET_TAG)) {
			rc = THROW_ILLEGAL_ARGUMENT_EXCEPTION;
			goto done;
		}

		ramClass = J9VM_J9CLASS_FROM_HEAPCLASS(_currentThread, clz);
		/* ramClass must not be fully initialized meaning clinit is still running.
		 * ramClass must have a flattenedClassCache otherwise this class has no strict static fields.
		 */
	...
	/* jdk.internal.misc.Unsafe: public native boolean isFlatPayloadBinary(Class<?> valueType); */
	VMINLINE VM_BytecodeAction
	inlUnsafeIsFlatPayloadBinary(REGISTER_ARGS_LIST)
	{
		j9object_t cls = *(j9object_t*)_sp;

		I_32 result = (I_32)FALSE;
		if (NULL != cls) {
			J9Class *clazz = J9VM_J9CLASS_FROM_HEAPCLASS(_currentThread, cls);
			if (!J9CLASS_HAS_REFERENCES(clazz)) {
				result = (I_32)TRUE;
			}
		}
...
#define PERFORM_ACTION(functionCall) \
	do { \
		...
		case THROW_ILLEGAL_ARGUMENT_EXCEPTION: \
...
runMethod: {
	...
	JUMP_TARGET(J9_BCLOOP_SEND_TARGET_INL_UNSAFE_ISFLATPAYLOADBINARY):
		PERFORM_ACTION(inlUnsafeIsFlatPayloadBinary(REGISTER_ARGS));
#endif /* defined(J9VM_OPT_VALHALLA_FLATTENABLE_VALUE_TYPES) */
