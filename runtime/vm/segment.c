...
static J9MemorySegment * allocateVirtualMemorySegmentInListInternal(J9JavaVM *javaVM, J9MemorySegmentList *segmentList, UDATA size, UDATA type, J9PortVmemParams *vmemParams, U_32 memoryCategory)
{
	...
	if(segment == NULL) {
		...
		if (NULL == allocatedBase) {
			...
			if (J9_ARE_ALL_BITS_SET(type, MEMORY_TYPE_RAM_CLASS | MEMORY_TYPE_DISCLAIMABLE_TO_FILE)) {
				segment->heapBase = (U_8 *)ROUND_TO(pageSize, (UDATA)allocatedBase);
			} else {
...
disclaimClassMemory(J9JavaVM *vm, UDATA flags)
{
	...
	BOOLEAN romClass = J9_ARE_ANY_BITS_SET(vm->extendedRuntimeFlags3, J9_EXTENDED_RUNTIME3_DISCLAIM_ROM_CLASS_MEMORY);
	BOOLEAN ramClass = J9_ARE_ANY_BITS_SET(vm->extendedRuntimeFlags3, J9_EXTENDED_RUNTIME3_DISCLAIM_RAM_CLASS_MEMORY);
	...
	while (NULL != classLoader) {
		...
		while (NULL != segment) {
			...
			if ((ramClass && isRAM && J9_ARE_ANY_BITS_SET(segment->type, MEMORY_TYPE_DISCLAIMABLE_TO_FILE))
			|| (romClass && isROM)
			) {
				I_32 result = madvise(segment->heapBase, segment->size, MADV_PAGEOUT);
				Trc_Segment_disclaimClassMemory_result(segment, segment->heapBase,
