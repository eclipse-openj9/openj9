...
VM_JFRChunkWriter::writeNarrowOOPModeTypesEvent()
{
	...
	/* number of states */
	_bufferWriter->writeLEB128(OOPMODETYPE_COUNT);

	for (int i = 0; i < OOPMODETYPE_COUNT; i++) {
		/* constant index */
		_bufferWriter->writeLEB128(i);
		...
		/* string length */
		UDATA len = strlen(oopModeTypeNames[i]);
		_bufferWriter->writeLEB128(len);

		/* write string */
		_bufferWriter->writeData((U_8 *)oopModeTypeNames[i], len);
	}
...
VM_JFRChunkWriter::writeGCHeapConfigurationEvent()
{
	...
	_bufferWriter->writeLEB128(gcConfig->usesCompressedOops);
