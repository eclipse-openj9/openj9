...
/* Get thread name */
	...
	bufferWriter->writeFormattedString("\"%s\" J9VMThread: %p tid: %zd nid: %zd prio: %zd state: %s ", threadName, walkThread, javaTID, osTID, javaPriority, stateStr);
...
VM_JFRChunkWriter::writeThreadDumpEvent()
{
	...
	const U_64 bufferSize = THREAD_DUMP_EVENT_SIZE * _vm->peakThreadCount;
	U_8 *resultBuffer = (U_8 *)j9mem_allocate_memory(sizeof(U_8) * bufferSize, OMRMEM_CATEGORY_VM);

	if (NULL != resultBuffer) {
		VM_BufferWriter resultWriter(resultBuffer, bufferSize);
