...
/**
 * Hook for old garbage collection. Called without VM access.
 *
 * @param hook[in] the VM hook interface
 * @param eventNum[in] the event number
 * @param eventData[in] the event data
...
jfrOldGarbageCollection(J9HookInterface **hook, UDATA eventNum, void *eventData, void* userData)
{
	...
	J9VMThread *currentThread = (J9VMThread *)event->omrVMThread;
	J9JavaVM *javaVM = (J9JavaVM *)currentThread->javaVM;
	PORT_ACCESS_FROM_VMC(currentThread);
	...
	if (NULL != jfrEvent) {
		...
		jfrEvent->gcID = javaVM->memoryManagerFunctions->j9gc_get_unique_GC_ID(currentThread);
	}
}
...
initializeJFR(J9JavaVM *vm, BOOLEAN lateInit)
{
	...
	if ((*gcOmrHooks)->J9HookRegisterWithCallSite(vmHooks, J9HOOK_MM_OMR_GC_CYCLE_END, jfrOldGarbageCollection, OMR_GET_CALLSITE(), NULL)) {
		goto fail;
	}
...
tearDownJFR(J9JavaVM *vm)
{
	...
	(*vmHooks)->J9HookUnregister(vmHooks, J9HOOK_MM_OMR_GC_CYCLE_END, jfrOldGarbageCollection, NULL);
