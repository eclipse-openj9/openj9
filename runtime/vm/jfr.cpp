...
jfrOldGarbageCollection(J9HookInterface **hook, UDATA eventNum, void *eventData, void *userData)
{
	...
	if (OMR_GC_CYCLE_TYPE_SCAVENGE == event->cycleType || OMR_GC_CYCLE_TYPE_VLHGC_PARTIAL_GARBAGE_COLLECT == event->cycleType) { // Wrong cycle type, is not an old garbage collection.
		return;
	}
