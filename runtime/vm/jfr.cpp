...
jfrThreadCPULoad(J9VMThread *currentThread, J9VMThread *sampleThread)
{
	...
	if (-1 != rc) {
		int64_t sysTime = threadTime.sysTime;
		int64_t userTime = threadTime.userTime;
		U_64 currentTime = (U_64)j9time_nano_time();
		...
		if (NULL != jfrEvent) {
			...
			if (-1 == *prevTimestamp) {
				...
				jfrEvent->user = OMR_MIN((userTime - jfrState->prevUserTime) / ((double)timeDelta), 1.0);
				jfrEvent->system = OMR_MIN((sysTime - jfrState->prevSysTime) / ((double)timeDelta), 1.0);
			}
			*prevTimestamp = currentTime;
			jfrState->prevUserTime = userTime;
			jfrState->prevSysTime = sysTime;
		}
	}
}
