...
findJ9ClassForROMClass(J9VMThread *vmThread, J9ROMClass *romClass, J9ClassLoader **resultClassLoader)
{
	...
	if (j9shr_Query_IsAddressInCache(vm, romClass, romClass->romSize)
		 ...
		 */
		ramClass = hashClassTableAt(*resultClassLoader, (U_8 *)J9UTF8_DATA(utfClassName), J9UTF8_LENGTH(utfClassName), J9_HASH_TABLE_LOOKUP_FLAG_JCL_DEFINE_CLASS);
		if ((NULL != ramClass)
		...
		ramClass = hashClassTableAt(vm->extensionClassLoader, (U_8 *)J9UTF8_DATA(utfClassName), J9UTF8_LENGTH(utfClassName), J9_HASH_TABLE_LOOKUP_FLAG_JCL_DEFINE_CLASS);
		if ((NULL != ramClass)
			&& (romClass == ramClass->romClass)
		) {
			ret = ramClass;
		...
		ramClass = hashClassTableAt(vm->applicationClassLoader, (U_8 *)J9UTF8_DATA(utfClassName), J9UTF8_LENGTH(utfClassName), J9_HASH_TABLE_LOOKUP_FLAG_JCL_DEFINE_CLASS);
		if ((NULL != ramClass)
			&& (romClass == ramClass->romClass)
		) {
			ret = ramClass;
		...
		while (NULL != classLoader) {
			if ((classLoader != *resultClassLoader)
				...
				ramClass = hashClassTableAt(classLoader, (U_8 *)J9UTF8_DATA(utfClassName), J9UTF8_LENGTH(utfClassName), J9_HASH_TABLE_LOOKUP_FLAG_JCL_DEFINE_CLASS);
				if ((NULL != ramClass)
					&& (romClass == ramClass->romClass)
				) {
					ret = ramClass;
