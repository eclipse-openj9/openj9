...
spaceIteratorCallback(J9JavaVM* virtualMachine, J9MM_IterateSpaceDescriptor* spaceDescriptor, void* userData)
{
	...
	if (J9_GC_POLICY_BALANCED == ((OMR_VM *)virtualMachine->omrVM)->gcPolicy) {
		...
		if (virtualMachine->memoryManagerFunctions->j9gc_off_heap_allocation_enabled(virtualMachine)) {
			...
			virtualMachine->memoryManagerFunctions->j9gc_get_offheap_data(virtualMachine, (void **)&offheapControlStructure, &offheapBase, &offheapTop, &offheapUsage);
			...
			jcw->_OutputStream.writeCharacters(" VirtualLargeObjectHeap (Offheap)\n");
		}
#endif /* J9VM_GC_VLHGC */
	}
	...
	if (virtualMachine->memoryManagerFunctions->j9gc_off_heap_allocation_enabled(virtualMachine)) {
		...
		jcw->_OutputStream.writeVPrintf(FORMAT_SIZE_DECIMAL, decimalLength + 7 - strlen(spaceDescriptor->name), allocTotal - offheapUsage);
		jcw->_OutputStream.writeCharacters(" (");
		jcw->_OutputStream.writeVPrintf(FORMAT_SIZE_HEX, sizeof(void *) * 2, allocTotal - offheapUsage);
		jcw->_OutputStream.writeCharacters(")\n");
		jcw->_OutputStream.writeCharacters("1STHEAPINUSE   ");
		jcw->_OutputStream.writeCharacters("Offheap memory in use: ");
