...
isPackageExportedToModuleHelper(J9VMThread *currentThread, J9Module *fromModule, J9Package *j9package, J9Module *toModule, BOOLEAN toUnnamed)
{
	...
	if (J9_ARE_NO_BITS_SET(vm->extendedRuntimeFlags3, J9_EXTENDED_RUNTIME3_MODULE_PACKAGES_INITIALIZED)) {
		/*
		 * If a module access check occurs before the modules are fully initialized, there is not enough information
		 * to do a package access check. Fortunately, the only code that can be running at this point is either java.base
		 * code or code generated by java.base (hidden or anonymous classes). In this case, allow all package access.
		 */
		isExported = TRUE;
	} else if (J9_IS_J9MODULE_UNNAMED(vm, fromModule) || J9_IS_J9MODULE_OPEN(fromModule)) {
