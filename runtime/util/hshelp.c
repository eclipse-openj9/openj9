	...
	while (NULL != classPair) {
		...
		if ((NULL != originalRAMClass) && (NULL != replacementRAMClass)) {
			...
			for (i = 0; i < nestMemberCount; i++) {
				...
				J9Class *nestMember = vmFuncs->hashClassTableAt(classLoader, J9UTF8_DATA(nestMemberName), J9UTF8_LENGTH(nestMemberName), J9_HASH_TABLE_LOOKUP_FLAG_JCL_DEFINE_CLASS);
				if (NULL != nestMember) {
