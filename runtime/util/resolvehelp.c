...
isDirectSuperInterface(J9VMThread *vmStruct, J9Class *resolvedClass, J9Class *currentClass) {
	...
	if (J9CLASS_IS_EXEMPT_FROM_VALIDATION(currentClass)) {
		...
		for (i = 0; i < interfaceCount; i++) {
			...
			interfaceClass = vm->internalVMFunctions->hashClassTableAt(resolvedClass->classLoader, J9UTF8_DATA(interfaceName), J9UTF8_LENGTH(interfaceName), J9_HASH_TABLE_LOOKUP_FLAG_JCL_DEFINE_CLASS);
