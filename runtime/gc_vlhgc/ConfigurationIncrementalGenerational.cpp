...
MM_ConfigurationIncrementalGenerational::createHeapWithManager(MM_EnvironmentBase *env, UDATA heapBytesRequested, MM_HeapRegionManager *regionManager)
{
	...
	if (extensions->virtualLargeObjectHeap._wasSpecified) {
		if (extensions->isVirtualLargeObjectHeapEnabled && !extensions->virtualLargeObjectHeap._valueSpecified) {
			extensions->isVirtualLargeObjectHeapEnabled = false;
		} else if (!extensions->isVirtualLargeObjectHeapEnabled && extensions->virtualLargeObjectHeap._valueSpecified) {
			extensions->isVirtualLargeObjectHeapEnabled = true;
		}
	}
