...
MM_CompactGroupPersistentStats::updateProjectedSurvivalRate(MM_EnvironmentVLHGC *env, MM_CompactGroupPersistentStats *persistentStatsArray, UDATA compactGroup)
{
	...
	if (liveBeforeCollect > 0) {
		...
		thisSurvivalRate = OMR_MIN(1.0, (double)liveBytesInCollectedSetAfterCollect / (double)liveBeforeCollect);
	...
	if (projectedLiveBytesSelected > 0) {
		...
		thisSurvivalRate = 0.5;
		if (0 < persistentStats->_projectedLiveBytesAfterPreviousPGCInCollectedSet) {
			thisSurvivalRate = (double)(persistentStats->_measuredLiveBytesAfterCollectInCollectedSet) / (double)persistentStats->_projectedLiveBytesAfterPreviousPGCInCollectedSet;
		...
		/* Store survival rate */
		persistentStats->_projectedInstantaneousSurvivalRateThisPGC = thisSurvivalRate;
		...
		persistentStats->_projectedInstantaneousSurvivalRate = (observedWeight * thisSurvivalRate) + ((1.0 - observedWeight) * persistentStats->_projectedInstantaneousSurvivalRate);

		/* TODO: need to retire this old TRC and create more meaningful new TRC */
