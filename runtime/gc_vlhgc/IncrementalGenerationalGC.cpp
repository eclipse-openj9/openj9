...
MM_IncrementalGenerationalGC::verifyHeapSizing(MM_EnvironmentVLHGC *env)
{
	Assert_GC_true_with_message3(env, (((MM_GlobalAllocationManagerTarok *)_extensions->globalAllocationManager)->getFreeRegionCount() >= _schedulingDelegate.getCurrentEdenSizeInRegions(env)),
				"verifyHeapSizing freeRegionCount(%zu) is less than EdenRegionCount(%zu), allocatedSinceLastPGC=%zu bytes\n",
				((MM_GlobalAllocationManagerTarok *)_extensions->globalAllocationManager)->getFreeRegionCount(), _schedulingDelegate.getCurrentEdenSizeInRegions(env), getAllocatedSinceLastPGC());
}
