...
ifdef({WIN32},{
	...
	dnl $1 = opcode
	dnl $2 = register number
	dnl $3 = stack displacment
	dnl low 3 bits of register number are stored in modR/M[5:3]
	dnl bits 3 and 4 are stored inverted in bits 7 and 4 in the second byte of the EVEX prefix
	define({ZMM_MOVE},{
		dnl EVEX prefix
		BYTE 062h
		BYTE 061h OR ((NOT $2 AND 8) SHL 4) OR (NOT $2 AND 010h)
		BYTE 0feh, 048h
		dnl opcode
		BYTE $1
		dnl modR/M  byte
		BYTE 084h OR (($2 AND 7) SHL 3 )
		dnl SIB
		BYTE 024h
		dnl displacement
		DWORD $3
	...
	define({SAVE_ZMM_REG}, {ZMM_MOVE(07fh, {$1}, {$2})})
	define({RESTORE_ZMM_REG}, {ZMM_MOVE(06fh, {$1}, {$2})})
