...
/* Use acquireVMAccess and releaseVMAccess only on the current thread */

#ifdef J9_INTERNAL_TO_VM
#if defined(J9VM_INTERP_ATOMIC_FREE_JNI)
#define exitVMToJNI(vmThread) internalExitVMToJNI(vmThread)
#define enterVMFromJNI(vmThread) internalEnterVMFromJNI(vmThread)
#else
#define exitVMToJNI(vmThread) internalReleaseVMAccess(vmThread)
#define enterVMFromJNI(vmThread) internalAcquireVMAccess(vmThread)
#endif
