		...
		while (NULL != *link) {
			...
			if (J9VMJAVALANGTHREAD_DEADINTERRUPT(vmThread, current->vthread)) {
				...
				*link = current->nextWaitingContinuation;
				...
				/* Set the notified and onWaitingList flag for virtual threads that have not been interrupted. */
				...
				if (!notifyAll) {
					/* For Object.notify, exit the loop with current pointer set to notified Continuation. */
					break;
				}
