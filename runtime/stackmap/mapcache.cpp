...
getROMMethodInfoForROMMethod(J9StackWalkState *walkState, J9ROMMethod *romMethod)
{
	...
	/* Method frames require argBits, but not local/stack maps. */
...
getROMMethodInfoForBytecodeFrame(J9StackWalkState *walkState)
{
	...
	void *bytecodePC = (void *)walkState->pc;
	...
	UDATA pcOffset = (UDATA)bytecodePC - (UDATA)J9_BYTECODE_START_FROM_RAM_METHOD(method);
	...
	if (J9_ARE_ANY_BITS_SET(romMethod->modifiers, J9AccSynchronized)
		|| J9ROMMETHOD_MODIFIERS_IS_NON_EMPTY_OBJECT_CONSTRUCTOR(romMethod->modifiers)
	) {
		localsSize += 1;
	}
