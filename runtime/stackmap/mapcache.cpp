...
checkCache(J9JavaVM *vm, J9ClassLoader *classLoader, void *key, J9HashTable *mapCache, U_32 *resultArrayBase, UDATA mapWords)
{
	...
	if (J9_ARE_ANY_BITS_SET(vm->extendedRuntimeFlags3, J9_EXTENDED_RUNTIME3_CACHE_MAPS)) {
		if ((mapWords > 0) && (mapWords <= 2)) {
			...
			if (NULL != mapCache) {
				J9MapCacheEntry exemplar = { { 0, 0 }, key };
				J9MapCacheEntry *entry = (J9MapCacheEntry*)hashTableFind(mapCache, &exemplar);
	
				if (NULL != entry) {
					resultArrayBase[0] = entry->bits[0];
					if (mapWords > 1) {
						resultArrayBase[1] = entry->bits[1];
	#if defined(DEBUG)
						j9tty_printf(PORTLIB, "!!! Get CL %p HT %p KY %p RA %08X %08X EN %08X %08X\n", classLoader, mapCache, key, resultArrayBase[0], resultArrayBase[1], entry->bits[0], entry->bits[1]);
					} else {
						j9tty_printf(PORTLIB, "!!! Get CL %p HT %p KY %p RA %08X EN %08X\n", classLoader, mapCache, key, resultArrayBase[0], entry->bits[0]);
	#endif /* DEBUG */
					}
					found = true;
				}
...
updateCache(J9JavaVM *vm, J9ClassLoader *classLoader, void *key, J9HashTable **cacheSlot, U_32 *resultArrayBase, UDATA mapWords)
{
	...
	if (J9_ARE_ANY_BITS_SET(vm->extendedRuntimeFlags3, J9_EXTENDED_RUNTIME3_CACHE_MAPS)) {
		if ((mapWords > 0) && (mapWords <= 2)) {
			...
			if (NULL != mapCache) {
				J9MapCacheEntry entry = { { resultArrayBase[0], 0 }, key };
				if (mapWords > 1) {
					entry.bits[1] = resultArrayBase[1];
	#if defined(DEBUG)
					j9tty_printf(PORTLIB, "!!! Put CL %p HT %p KY %p RA %08X %08X EN %08X %08X\n", classLoader, mapCache, key, resultArrayBase[0], resultArrayBase[1], entry.bits[0], entry.bits[1]);
				} else {
					j9tty_printf(PORTLIB, "!!! Put CL %p HT %p KY %p RA %08X EN %08X\n", classLoader, mapCache, key, resultArrayBase[0], entry.bits[0]);
	#endif /* DEBUG */
				}
