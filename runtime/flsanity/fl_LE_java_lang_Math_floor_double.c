...
#include "floatsanityg.h"
   ...
   void group_java_lang_Math_floor_double() {
      int i=0, groupNum=36,numTests=21;
      ...
      unsigned int op_0 [] = {
         ...
         0x00000000, 0x80000000,		/* check if -0.0d == -0.0d , (one of TCK tests) */
         0x0, 0x0};
      unsigned int result [] = {
         0x0,0x0, 
         0x00000000,0x3ff00000, 
      ...
      for (i=0; i<numTests; i++) {
         r = java_lang_Math_floor_double(*(p0));
         if ( IS_DNAN(erp) ){
            if ( !(IS_DNAN(&r)) ){
      ...
      printf("[%d] %s Passed %d Failed %d Total %d \n", groupNum,"java_lang_Math_floor_double", testsPassed, testsFailed, numTests);
