...
/* End of options relating to dynamicBreadthFirstScanOrdering */
		...
		if (try_scan(&scan_start, "PruneContinuationFromListOnGCClearable")) {
			extensions->timingPruneContinuationFromList = MM_GCExtensions::onGCClearable;
			continue;
		}

		if (try_scan(&scan_start, "PruneContinuationFromListOnLastUnmount")) {
			if ((gc_policy_metronome != extensions->configurationOptions._gcPolicy) &&
				(gc_policy_nogc != extensions->configurationOptions._gcPolicy) &&
				(gc_policy_balanced != extensions->configurationOptions._gcPolicy))
			{
				extensions->timingPruneContinuationFromList = MM_GCExtensions::onLastUnmount;
			}
			continue;
		}
