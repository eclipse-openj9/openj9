      ...
      if (isArray)
         ...
         if (TR::Compiler->om.isOffHeapAllocationEnabled())
            ...
            if (isVariableLen)
               ...
               trprintf(trace, log, "Node (%p): Dealing with full/compressed refs variable length array.\n", node);
...
J9::Power::CodeGenerator::inlineDirectCall(TR::Node *node, TR::Register *&resultReg)
   ...
   if (comp->getSymRefTab()->isNonHelper(node->getSymbolReference(), TR::SymbolReferenceTable::singlePrecisionSQRTSymbol))
      ...
      case TR::java_lang_Thread_onSpinWait:
         ...
         if (!disableOSW)
            {
            trprintf(comp->getOption(TR_TraceCG), comp->log(), "Inlining Thread.onSpinWait call node %p as yield instruction.\n", node);
      ...
      case TR::sun_misc_Unsafe_compareAndSwapLong_jlObjectJJJ_Z:
         trprintf(comp->getOption(TR_TraceCG), comp->log(), "In evaluator for compareAndSwapLong. node = %p node->isSafeForCGToFastPathUnsafeCall = %p\n",
