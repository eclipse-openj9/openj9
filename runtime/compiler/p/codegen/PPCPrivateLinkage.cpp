    ...
    TR::RegisterDependencyConditions *dependencies, TR_LinkageConventions linkage)
{
    ...
    /* Prior to OpenJ9's adoption OpenJDK MethodHandles, MethodHandle dispatch used the ComputedCalls methods.
     * OpenJ9 now uses JITHelpers for (unrefined) virtual dispatch. Both the Computed and JITHelper dispatchVirtual
     * targets are dummy resolved methods that dispatch through the VFT, where the pre-computed VFT entry address is
     * placed in gr12.
     *
     * The old static/special dispstach sequence uses the ComputedCalls dispatchJ9Method target. This goes through the
     * interpreter to call the final target so it places the J9Method pointer in gr3 and the args on the stack. The new
     * dispatch sequence uses a helper symbol (which is inlined by CG) called jitDispatchJ9Method. This uses private
     * linkage so the args go in the argument registers, and the J9Method pointer in gr11. If the target is compiled, it
     * is called directly. If not, the args are placed on the stack and we transition to the interpreter for the call
     */
    ...
    // we use gr11 instead of gr3 (from the related ComputedCalls_dispatchJ9Method above) to avoid overlap
