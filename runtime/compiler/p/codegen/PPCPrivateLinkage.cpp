...
void J9::Power::PrivateLinkage::createPrologue(TR::Instruction *cursor)
   ...
   if (!comp()->isDLT())
      ...
      {
      // For DLT compilations that have a gc-map for the last instruction it causes an issue
      // as the return-address for that gc-map would be outside the compilation body range.
      // To solve it add a buffer nop instruction at the end of the compilation body. 
