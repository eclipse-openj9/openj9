...
#if defined(J9VM_OPT_MICROJIT)
/* These methods all use MicroJIT, so we cannot assume all the assumptions about
...
void
J9::CFG::getInterpreterProfilerBranchCountersOnDoubletonMicroJIT(TR::CFGNode *cfgNode, int32_t *taken, int32_t *nottaken)
   {
   // We have not Real Tree Tops, so we need to work around that.
   TR::Node *node = cfgNode->asBlock()->getExit()->getNode();
...
void
J9::CFG::setSwitchEdgeFrequenciesOnNodeMicroJIT(TR::CFGNode *node, TR::Compilation *comp)
   {
   TR::Block *block = node->asBlock();
...
void
J9::CFG::setBlockFrequenciesBasedOnInterpreterProfilerMicroJIT()
   {
   TR::StackMemoryRegion stackMemoryRegion(*trMemory());
   int32_t numBlocks = getNextNodeNumber();
   ...
   if ((temp->getSuccessors().size() == 2) &&
      ...
      {
      getInterpreterProfilerBranchCountersOnDoubletonMicroJIT(temp, &taken, &nottaken);
      startFrequency = taken + nottaken;
      self()->setEdgeFrequenciesOnNode(temp, taken, nottaken, comp());
      }
   else if (temp->asBlock()->getEntry() &&
      ...
      if (!node->asBlock()->isCold() && (node != start))
         ...
         if ((node->getSuccessors().size() == 2) &&
            ...
            if (!isVirtualGuard(node->asBlock()->getLastRealTreeTop()->getNode()) &&
               ...
               {
               seenNodesInCycle->empty();
               getInterpreterProfilerBranchCountersOnDoubletonMicroJIT(node, &taken, &nottaken);
