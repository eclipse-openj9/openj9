...
struct ILOfCrashedThreadParamenters
   {
   ILOfCrashedThreadParamenters(J9VMThread *vmThread, TR::Compilation *comp, TR::FILE *jitdumpFile, TR::Logger *logger)
...
static void
traceILOfCrashedThread(J9VMThread *vmThread, TR::Compilation *comp, TR::FILE *jitdumpFile, TR::Logger *logger)
   {
...
static void
jitDumpRecompileWithTracing(J9VMThread *vmThread, J9Method *ramMethod, TR::CompilationInfo *compInfo, TR_Hotness optLevel, bool isProfilingCompile, TR::Options *optionsFromOriginalCompile, bool isAOTBody, void *oldStartPC, TR::FILE *jitdumpFile, TR::Logger *logger)
   {
   PORT_ACCESS_FROM_VMC(vmThread);
   J9Class *clazz;
      ...
      if (NULL != bodyInfo)
         {
            jitDumpRecompileWithTracing(
               ...
               reinterpret_cast<TR::Logger*>(walkState->userData3)
            );
         }
      }
...
runJitdump(char *label, J9RASdumpContext *context, J9RASdumpAgent *agent)
   ...
   TR::Logger *logger = TR::CStdIOStreamLogger::create(jitdumpFile->_stream);
   if (NULL == logger)
      {
