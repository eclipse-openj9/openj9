...
struct ILOfCrashedThreadParamenters
   {
   ILOfCrashedThreadParamenters(J9VMThread *vmThread, TR::Compilation *comp, TR::FILE *jitdumpFile, TR::Logger *jitdumpLogger)
...
static void
traceILOfCrashedCompilationThread(J9VMThread *vmThread, TR::Compilation *comp, TR::FILE *jitdumpFile, TR::Logger *jitdumpLogger)
   {
...
runJitdump(char *label, J9RASdumpContext *context, J9RASdumpAgent *agent)
   ...
   TR::Logger *jitdumpLogger = TR::CStdIOStreamLogger::create(jitdumpFile->_stream);
   if (!jitdumpLogger)
      {
