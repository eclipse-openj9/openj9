...
static bool handleServerMessage(JAAS::J9ClientStream *client, TR_J9VMBase *fe)
   ...
   switch (client->read())
      ...
      case J9ServerMessageType::ResolvedMethod_isInterpreted:
         ...
         break;
      case J9ServerMessageType::ResolvedMethod_classModifiers:
         {
         TR_ResolvedJ9Method *method = std::get<0>(client->getRecvData<TR_ResolvedJ9Method *>());
         client->write(method->classModifiers());
         }
         break;
...
TR::CompilationInfoPerThreadBase::preCompilationTasks(J9VMThread * vmThread,
   ...
   if (!entry->isAotLoad())
      ...
      // will just load the code from SCC
      if (eligibleForRelocatableCompile && _compInfo.getPersistentInfo()->getJaasMode() == CLIENT_MODE)
         {
         entry->_doAotLoad = true;
         }
      else
...
TR::CompilationInfoPerThreadBase::compile(
   ...
   try
      ...
      if (_compInfo.getPersistentInfo()->getJaasMode() != CLIENT_MODE || 
         !_methodBeingCompiled->isAotLoad()) // Remote compilation requests appear as AOT loads
         {
