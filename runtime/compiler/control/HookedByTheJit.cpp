...
static void jitHookThreadDestroy(J9HookInterface * * hookInterface, UDATA eventNum, void * eventData, void * userData)
   ...
   if (extractTDB)
      {
#if 1
      printf("jitHookThreadDestroy: please make sure J9VMThread includes a 'tdb' member.\n"); fflush(stdout);
#else
      bool tdbAllZero = true;
      ...
      if (!tdbAllZero)
         {
         //printf("jitHookThreadDestroy: thread name=%s...\n", vmThread->threadName); fflush(stdout);
         for (int ii = 0; ii < 32 ; ++ii)
            {
            printf("* 0x%x  %3d:", vmThread->tdb + ii*8, ii*8);
            for (int jj = 0; jj < 8 ; ++jj)
               {
               printf("  %02x", vmThread->tdb[8*ii+jj]);
               }
            printf("\n");
            }
         ...
         int tdbIndex=0;
         uint64_t tempValLow=0;
         uint64_t tempValHigh=0;

         printf(" [%3d] Format: 0x%02x", tdbIndex, vmThread->tdb[tdbIndex]);
         if (vmThread->tdb[tdbIndex] & 0x01)
            {
            printf(" - Valid TDB");
            }
         else
         ...
         printf(" [%3d] Flags: 0x%02x", tdbIndex, vmThread->tdb[tdbIndex]);
         if (vmThread->tdb[tdbIndex] & 0x80)
            {
            printf(" - Valid CTV");
            }
         if (vmThread->tdb[tdbIndex] & 0x40)
            {
            printf(" - Constrained Transactional Execution");
            }
         else
            {
            printf(" - Nonconstrained Transactional Execution");
            }
         printf(" - Reserved=%X (must be zero)", vmThread->tdb[tdbIndex] & 0x3F);
         printf("\n");
         tdbIndex++;
         ...
         tempValLow = vmThread->tdb[tdbIndex];
         for (int ii = 1; ii < 2 ; ++ii)
            {
            tempValLow = tempValLow << 8;
            tempValLow += vmThread->tdb[tdbIndex+ii];
            }
         printf(" [%3d] Transaction Nesting Depth (TND): %d\n", tdbIndex, tempValLow);
         tdbIndex+=2;

         tempValHigh = vmThread->tdb[tdbIndex];
         for (int ii = 1; ii < 4 ; ++ii)
            {
            tempValHigh = tempValHigh << 8;
            tempValHigh += vmThread->tdb[tdbIndex+ii];
            }
         tempValLow = vmThread->tdb[tdbIndex+4];
         for (int ii = 5; ii < 8 ; ++ii)
            {
            tempValLow = tempValLow << 8;
            tempValLow += vmThread->tdb[tdbIndex+ii];
            }
         printf(" [%3d] Transaction Abort Code (TAC): %d\n", tdbIndex, tempValLow);
         tdbIndex+=8;
         ...
         printf(" [%3d] Conflict Token: ", tdbIndex);
         if (vmThread->tdb[0] & 0x80)
            {
            printf("0x%X%X\n", tempValHigh, tempValLow);
            }
         else
            {
            printf("INVALID\n");
         ...
         printf(" [%3d] Aborted-Transaction Instruction Address (ATIA): 0x", tdbIndex);
         for (int ii = 0; ii < 8 ; ++ii)
            {
            printf("%02x", vmThread->tdb[tdbIndex]);
            tdbIndex++;
            }
         printf("\n");

         printf(" [%3d] Exception Access Identification (EAI): %d\n", tdbIndex, vmThread->tdb[tdbIndex]);
         tdbIndex++;

         printf(" [%3d] Data-Exception Code (DEC): %d\n", tdbIndex, vmThread->tdb[tdbIndex]);
         tdbIndex++;

         tdbIndex+=2;

         printf(" [%3d] Program Interruption Identification (PIID): 0x", tdbIndex);
         ...
         printf(" [%3d] Transaction-Exception Identification (TEID): 0x", tdbIndex);
         for (int ii = 0; ii < 4 ; ++ii)
            {
            printf("%02x", vmThread->tdb[tdbIndex]);
            tdbIndex++;
         ...
         printf(" [%3d] Breaking-Event Address (BEA): 0x", tdbIndex);
         for (int ii = 0; ii < 4 ; ++ii)
            {
            printf("%02x", vmThread->tdb[tdbIndex]);
            tdbIndex++;
         ...
         for (int jj = 0; jj < 16 ; ++jj)
            {
            printf(" [%3d] General Register (GPR%02d): 0x", tdbIndex, jj);
            for (int ii = 0; ii < 8 ; ++ii)
               {
               printf("%02x", vmThread->tdb[tdbIndex]);
               tdbIndex++;
