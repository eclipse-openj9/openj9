...
class BytecodeRange{
    ...
    BytecodeRange(int32_t start)
        ...
        ,_end(-1)
    {}
    inline bool isClosed() { return _end > _start; }
...
class BytecodeRangeList{
    ...
    BytecodeRangeList(BytecodeRange *range)
        ...
        ,prev(NULL)
    {}
    inline uint32_t getStart() { return _range->_start; }
    inline uint32_t getEnd() { return _range->_end; }
    inline BytecodeRangeList *insert(BytecodeRangeList *newListEntry) {
        if (this == newListEntry || next == newListEntry) return this;
        ...
        if (prev) prev->next = newListEntry;
...
public:
    ...
    BytecodeIndexList * appendBCI(uint32_t i){
        if (i == _index) return this;
        if (_next) return _next->appendBCI(i);
    ...
    BytecodeIndexList * getBCIListEntry(uint32_t i){
        ...
        } else if(_next && _next->_index <= i) {  // (i > _index)
    ...
    inline BytecodeIndexList * getPrev(){ return _prev; }
    BytecodeIndexList * getTail(){
        if (_next) return _next->getTail();
        return this;
    }
    inline void cutNext() {
        if (!_next) return;
...
public:
    ...
    BlockList * getBlockListByStartIndex(uint32_t index){
        ...
        if (_range._start == index) return this;
        if (_range._start < index && _next && _next->_range._start > index) return this;

        // If the branch is after this block, and there is more list to search, then continue searching
        if (_next) return _next->getBlockListByStartIndex(index);
    ...
    BlockList * getOrMakeBlockListByBytecodeIndex(uint32_t index){
        ...
        if (_range.isClosed() && index > _range._start && index <= _range._end) return splitBlockListOnIndex(index);
        if (!_range.isClosed() && _next && index > _range._start && index < _next->_range._start) return splitBlockListOnIndex(index);

        // If the branch is after this block, and there is more list to search, then continue searching
        if (_next && index > _next->_range._start) return _next->getOrMakeBlockListByBytecodeIndex(index);
...
public:
    ...
    void addBytecodeIndexToCFG(
        ...
        if(_newBlock){//Current block has ended in a branch, this is a new block now
            nextBlock = new (_cfg.getInternalRegion()) BlockList(_cfg, bci->currentByteCodeIndex(), &_tail);
            _current->insertAfter(nextBlock);
            if(_fallThrough) _current->addSuccessor(&(nextBlock->_range));
            _newBlock = false;
            _fallThrough = true;
            _current = nextBlock;
        } else if(bci->isBranch()) { //Get the target block, make it if it isn't there already
            ...
            if(bci->current() == J9BCgoto || bci->current() == J9BCgotow) _fallThrough = false;
            _current->addSuccessor(&(nextBlock->_range));
            _newBlock = true;
        } else { //Still working on current block
...
MJIT::createInternalMethodMetadata(
    ...
    for(TR_J9ByteCode bc = bci->first(); bc != J9BCunknown; bc = bci->next()){
        ...
        if (profilingCompilation) creator.addBytecodeIndexToCFG(bci);
        gcMapOffset = 0;
        isRef = false;
        switch(bc){
            MakeEntry:
