...
#define PATCH_RELATIVE_ADDR_8_BIT(buffer, absAddress) do {\
...
MJIT::CodeGenerator::generatePrePrologue(
    ...
    // Not sure whether maxLength needs to be changed here for non-static methods or not
...
MJIT::CodeGenerator::generatePrologue(
    ...
    *jitStackOverflowJumpPatchLocation = buffer;
    ...
    InternalMetaData internalMetaData = createInternalMethodMetadata(
        bci,
        localTableEntries,
        localVariableCount,
        firstLocalOffset,
        ...
        pointerSize,
        &resolvedAllCallees);
    if(!resolvedAllCallees) return 0;
...
buffer_size_t
MJIT::CodeGenerator::generateArgumentMoveForStaticMethod(char *buffer, TR_ResolvedMethod *staticMethod, char *typeString, U_16 typeStringLength, U_16 slotCount, struct TR::X86LinkageProperties *properties){
...
MJIT::CodeGenerator::generateInvokeStatic(char *buffer, MJIT::ByteCodeIterator *bci, struct TR::X86LinkageProperties *properties)
{
    ...
    if(!resolved || resolved->isNative()) return 0;
