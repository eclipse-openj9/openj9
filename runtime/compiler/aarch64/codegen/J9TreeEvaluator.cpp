...
static OP::Mnemonic getStoreOpCodeFromDataType(TR::CodeGenerator *cg, TR::DataType dt, int32_t elementSize,
    bool useIdxReg);
    ...
    TR::Register *valueReg)
{
    ...
    if (isWrite) {
        ...
        OP::Mnemonic storeOp = getStoreOpCodeFromDataType(cg, dt, elementSize, false);
    ...
    Inst_Label(cg, OP::b, node, endLabel);
    ...
    TR::Snippet *dataSnippet, bool isWrite, TR::Register *sideEffectRegister, TR::Register *dataSnippetRegister)
{
    ...
    Inst_Label(cg, OP::label, node, unresolvedLabel);
...
static TR::Register *generateSoftwareReadBarrier(TR::Node *node, TR::CodeGenerator *cg, bool isArdbari)
{
    ...
    Inst_Label(cg, OP::label, node, endLabel, deps);
    ...
    TR_ARM64ScratchRegisterManager *srm, TR::LabelSymbol *doneLabel, TR::SymbolReference *wbRef, TR::CodeGenerator *cg)
{
    ...
    if (gcMode != gc_modron_wrtbar_always) {
        ...
        if (doCrdMrk) {
            ...
            Inst_Label(cg, OP::label, node, crdMrkDoneLabel);
    ...
    TR::CodeGenerator *cg)
{
    ...
    Inst_Label(cg, OP::label, node, doneLabel, conditions, NULL);
...
TR::Register *J9::ARM64::TreeEvaluator::awrtbariEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    OP::Mnemonic storeOp = usingCompressedPointers ? OP::strimmw : OP::strimmx;
...
TR::Register *J9::ARM64::TreeEvaluator::DIVCHKEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (!isConstDivisor || (!is64Bit && divisor->getInt() == 0) || (is64Bit && divisor->getLongInt() == 0)) {
        ...
        if (isConstDivisor) {
            ...
            gcPoint = Inst_Label(cg, OP::b, node, snippetLabel);
        } else {
    ...
    TR::Register *addrReg, TR::Register *metaReg, TR_ARM64ScratchRegisterManager *srm, TR::LabelSymbol *callLabel)
{
    ...
    if (comp->getOption(TR_EnableMonitorCacheLookup)) {
        ...
        auto branchInstrToFallThru = Inst_Label(cg, OP::b, node, fallThruFromMonitorLookupCacheLabel);
        if (debugObj) {
            debugObj->addInstructionComment(branchInstrToFallThru,
                "Branch to fall through label as lockOffset is positive");
        }
        Inst_Label(cg, OP::label, node, monitorLookupCacheLabel);
        ...
        Inst_Label(cg, OP::label, node, fallThruFromMonitorLookupCacheLabel);
    } else {
...
TR::Register *J9::ARM64::TreeEvaluator::monexitEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (inlineRecursive) {
        ...
        Inst_Label(cg, OP::label, node, OOLLabel);
        ...
        Inst_Label(cg, OP::label, node, OOLEndLabel, ooldeps);
...
TR::Register *J9::ARM64::TreeEvaluator::VMinstanceofEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    for (auto it = itBegin; it != itEnd; it++) {
        ...
        switch (current) {
            ...
            case HelperCall: {
                ...
                if (it == itBegin) {
                    ...
                    Inst_Label(cg, OP::b, node, callHelperLabel);
                }
            } break;
            default:
                break;
        ...
        if (!isTerminalSequence(it, itEnd)) {
            Inst_Label(cg, OP::label, node, nextSequenceLabel);
...
TR::Register *J9::ARM64::TreeEvaluator::flushEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (op == TR::allocationFence) {
        ...
        OP::AArch64BarrierLimitation lim;
        if (op == TR::loadFence) {
    ...
    TR::Register *tempReg1, TR::Register *tempReg2)
{
    if (isVariableLen) {
        ...
        Inst_Label(cg, OP::label, node, loopStartLabel);
        ...
        Inst_Label(cg, OP::label, node, mediumLabel);
        ...
        if (loopCount > 0) {
            if (loopCount > 1) {
                ...
                Inst_Label(cg, OP::label, node, loopStart);
            }
...
TR::Register *J9::ARM64::TreeEvaluator::VMnewEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    Inst_Label(cg, OP::label, node, callReturnLabel);
    ...
    int32_t leafArrayElementSize)
{
    ...
    OP::Mnemonic loadAddrOp = addrSize == 8 ? OP::ldrimmx : OP::ldrimmw;
    OP::Mnemonic storeAddrOp = addrSize == 8 ? OP::strimmx : OP::strimmw;
    ...
    OP::Mnemonic storeClassOp = use64BitClasses ? OP::strimmx : OP::strimmw;
    ...
    Inst_Label(cg, OP::label, node, nonZeroFirstDimLabel);
    ...
    // === loop
    Inst_Label(cg, OP::label, node, loop1Label);
    ...
    Inst_Label(cg, OP::label, node, nonZeroSecondDimLabel);
    ...
    // === loop
    Inst_Label(cg, OP::label, node, loop2Label);
    // initialise a header in the second dimension with class = component
    Inst_MemSrc1(cg, storeClassOp, node, MRef_disp(cg, temp2Reg, TR::Compiler->om.offsetOfObjectVftField()),
        componentClassReg);
    ...
    Inst_Label(cg, OP::label, node, endLabel, dependencies);
...
TR::Register *J9::ARM64::TreeEvaluator::monentEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (comp->target().cpu.supportsFeature(OMR_FEATURE_ARM64_LSE) && (!disableLSE)) {
         ...
         */
        op = fej9->generateCompressedLockWord() ? OP::casalw : OP::casalx;
        /*
...
TR::Register *J9::ARM64::TreeEvaluator::ZEROCHKEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    cg->decReferenceCount(node->getFirstChild());
    Inst_Label(cg, OP::label, node, restartLabel);
...
TR::Register *J9::ARM64::TreeEvaluator::ArrayStoreCHKEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (!sourceChild->isNull()) {
        ...
        if (nopASC) {
            ...
            if (!disableArrayStoreCHKOpts) {
                ...
                Inst_Label(cg, OP::b, node, helperCallLabel);
            }
        }
    ...
    auto instr = Inst_Label(cg, OP::label, node, wbLabel);
    if (debugObj) {
        debugObj->addInstructionComment(instr, "ArrayStoreCHK Done");
    }
    instr = Inst_Label(cg, OP::label, node, OOLMergeLabel, deps);
    if (debugObj) {
 ...
 * @return the result register
 */
static TR::Register *inlineFPTrg1Src3(TR::Node *node, OP::Mnemonic op, TR::CodeGenerator *cg)
{
...
static TR::Register *inlineCRC32CUpdateBytes(TR::Node *node, TR::CodeGenerator *cg, bool isRawAddress, bool isCRC32C)
{
    ...
    const OP::Mnemonic crc32ByteOp = isCRC32C ? OP::crc32cb : OP::crc32b;
    const OP::Mnemonic crc32HalfWordOp = isCRC32C ? OP::crc32ch : OP::crc32h;
    const OP::Mnemonic crc32WordOp = isCRC32C ? OP::crc32cw : OP::crc32w;
    const OP::Mnemonic crc32DoubleWordOp = isCRC32C ? OP::crc32cx : OP::crc32x;
    ...
    if (isConstEndOrLen && isConstOff && !isRawAddress) {
        ...
        if (alignmentCheckNeeded) {
             ...
             */
            auto align2LabelInstr = Inst_Label(cg, OP::label, node, align2Label);
             ...
             */
            auto align4LabelInstr = Inst_Label(cg, OP::label, node, align4Label);
         ...
         * residualLabel:
         */
        auto align8LabelInstr = Inst_Label(cg, OP::label, node, align8Label);

        TR::LabelSymbol *loopStartLabel = generateLabelSymbol(cg);

        Inst_CompareImm(cg, node, sizeLeftReg, 32, false);
        Inst_ConditionalBranch(cg, node, residualLabel, TR::CC_LT);
        auto loopHeaderLabelInstr = Inst_Label(cg, OP::label, node, loopHeaderLabel);
        ...
        auto loopStartLabelInstr = Inst_Label(cg, OP::label, node, loopStartLabel);
        for (int32_t i = 0; i < 4; i++) {
            TR::MemoryReference *mref = MRef_disp(cg, baseReg, (i + 1) * 8);
        ...
        Inst_Trg1Src1Imm(cg, OP::addimmx, node, baseReg, baseReg, 8);
        auto residualLabelInstr = Inst_Label(cg, OP::label, node, residualLabel);
        if (debugObj) {
        ...
        Inst_Trg1Src2(cg, crc32DoubleWordOp, node, inputReg, inputReg, tmpReg);
        auto left16_23LabelInstr = Inst_Label(cg, OP::label, node, left16_23Label);
        Inst_Trg1Mem(cg, OP::ldrpostx, node, tmpReg, MRef_disp(cg, baseReg, 8));
        Inst_Trg1Src2(cg, crc32DoubleWordOp, node, inputReg, inputReg, tmpReg);
        auto left8_15LabelInstr = Inst_Label(cg, OP::label, node, left8_15Label);
        Inst_Trg1Mem(cg, OP::ldrpostx, node, tmpReg, MRef_disp(cg, baseReg, 8));
        Inst_Trg1Src2(cg, crc32DoubleWordOp, node, inputReg, inputReg, tmpReg);
         ...
         */
        auto residual4LabelInstr = Inst_Label(cg, OP::label, node, residual4Label);
         ...
         */
        auto residual2LabelInstr = Inst_Label(cg, OP::label, node, residual2Label);
         ...
         * crc32cb     inputReg, inputReg, tmpReg
         */
        auto residual1LabelInstr = Inst_Label(cg, OP::label, node, residual1Label);
        ...
        conditions->addPostCondition(inputReg, TR::RealRegister::NoReg);
        srm->addScratchRegistersToDependencyList(conditions);
        auto donesLabelInstr = Inst_Label(cg, OP::label, node, doneLabel, conditions);
        if (debugObj) {
...
static TR::Register *inlineStringHashCode(TR::Node *node, bool isCompressed, TR::CodeGenerator *cg)
{
    ...
    Inst_Label(cg, OP::label, node, residualLabel);
...
static TR::Register *inlineIntrinsicIndexOf(TR::Node *node, TR::CodeGenerator *cg, bool isLatin1)
{
     ...
     */
    TR::LabelSymbol *loopLabel = generateLabelSymbol(cg);
    auto loopLabelInstr = Inst_Label(cg, OP::label, node, loopLabel);
    if (debugObj) {
    ...
    auto branchToMergeLabelInstr = Inst_Label(cg, OP::b, node, mergeLabel);
    auto lessThan8LabelInstr = Inst_Label(cg, OP::label, node, lessThan8Label);
    ...
    auto mergeLabelInstr = Inst_Label(cg, OP::label, node, mergeLabel);
    if (debugObj) {
    ...
    auto foundLabelInstr = Inst_Label(cg, OP::label, node, foundLabel);
    ...
    auto doneLabelInstr = Inst_Label(cg, OP::label, node, doneLabel);
    if (debugObj) {
    ...
    Inst_Label(cg, OP::label, node, generateLabelSymbol(cg), conditions);
...
static TR::Register *inlineIntrinsicStringIndexOfString(TR::Node *node, TR::CodeGenerator *cg, bool isLatin1)
{
    ...
    // Outer loop
    Inst_Label(cg, OP::label, node, outerLoopLabel);
    ...
    Inst_Label(cg, OP::label, node, firstCharLoopLabel);
    if (isLatin1) {
    ...
    Inst_Label(cg, OP::b, node, notFoundLabel);

    // First character matched in vector
    Inst_Label(cg, OP::label, node, firstCharMatchedLabel);
    Inst_Trg1Src1(cg, OP::rbitx, node, tmp1Reg, tmp1Reg);
    Inst_Trg1Src1(cg, OP::clzx, node, tmp1Reg, tmp1Reg);
    ...
    // Vector comparison
    Inst_Label(cg, OP::label, node, arrayCmpVectorLoopLabel);
    if (isLatin1) {
    ...
    Inst_Label(cg, OP::label, node, arrayCmpByteLoopLabel);
    ...
    Inst_Label(cg, OP::b, node, arrayCmpByteLoopLabel);

    // s2 unmatched
    Inst_Label(cg, OP::label, node, unmatchedLabel);
    Inst_Trg1Src1Imm(cg, OP::addimmw, node, resultReg, resultReg, 1);
    Inst_Label(cg, OP::b, node, outerLoopLabel);

    // Not found
    Inst_Label(cg, OP::label, node, notFoundLabel);
    loadConstant32(cg, node, -1, resultReg);
    // fall through to doneLabel

    Inst_Label(cg, OP::label, node, doneLabel, dependencies);
...
static TR::Register *inlineStringLatin1Inflate(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    auto branchToDoneLabelInstr = Inst_Label(cg, OP::b, node, doneLabel);
    srm->reclaimScratchRegister(dataReg);

    auto GEQ2BytesLabelInstr = Inst_Label(cg, OP::label, node, GEQ2BytesLabel);
    ...
    auto GEQ4BytesLabelInstr = Inst_Label(cg, OP::label, node, GEQ4BytesLabel);
    ...
    TR::LabelSymbol *loopLabel = generateLabelSymbol(cg);
    auto GEQ8BytesLabelInstr = Inst_Label(cg, OP::label, node, GEQ8BytesLabel);
...
static TR::Register *inlineHasNegativesOrCountPositives(TR::Node *node, bool isHasNegatives, TR::CodeGenerator *cg)
{
    ...
    Inst_Label(cg, OP::label, node, loop16Label);
    ...
    Inst_Label(cg, OP::label, node, lessThan16Label);
    ...
    Inst_Label(cg, OP::label, node, lessThan8Label);
    ...
    if (!isHasNegatives) {
        ...
        Inst_Label(cg, OP::label, node, foundVec16Label);
        ...
        Inst_Label(cg, OP::b, node, adjustIndexLabel);

        // Found a negative value in 64-bit vector comparison
        Inst_Label(cg, OP::label, node, foundVec8Label);
        Inst_Trg1Src1(cg, OP::rbitx, node, tmpReg, tmpReg);
        Inst_Trg1Src1(cg, OP::clzx, node, tmpReg, tmpReg);
        Inst_LogicalShiftRightImm(cg, node, tmpReg, tmpReg, 3, /* is64bit */ false); // div by 8
        Inst_Label(cg, OP::label, node, adjustIndexLabel);
    ...
    Inst_Label(cg, OP::label, node, foundLabel);
    if (isHasNegatives) {
    ...
    if (methodSymbol) {
        ...
        switch (methodSymbol->getRecognizedMethod()) {
            ...
            case TR::java_lang_ref_Reference_reachabilityFence: {
                ...
                Inst_Label(cg, OP::label, node, label, conditions);
            ...
            case TR::java_lang_StrictMath_fma_D:
                resultReg = inlineFPTrg1Src3(node, OP::fmaddd, cg);
                return true;

            case TR::java_lang_Math_fma_F:
            case TR::java_lang_StrictMath_fma_F:
                resultReg = inlineFPTrg1Src3(node, OP::fmadds, cg);
                return true;
    ...
    TR::Register *arrayletReg, TR::Register *offsetReg, int32_t &offsetVal)
{
    ...
    if (indexReg) {
        ...
        loadOp = spinePointerSize == 8 ? OP::ldrimmx : OP::ldrimmw;
    }
    ...
    bool isUnsigned, bool useIdxReg)
{
    switch (dt) {
        case TR::Int8:
            if (isUnsigned)
                return useIdxReg ? OP::ldrboff : OP::ldrbimm;
            else
                return useIdxReg ? OP::ldrsboffw : OP::ldrsbimmw;
        case TR::Int16:
            if (isUnsigned)
                return useIdxReg ? OP::ldrhoff : OP::ldrhimm;
            else
                return useIdxReg ? OP::ldrshoffw : OP::ldrshimmw;
        case TR::Int32:
            return useIdxReg ? OP::ldroffw : OP::ldrimmw;
        case TR::Int64:
            return useIdxReg ? OP::ldroffx : OP::ldrimmx;
        case TR::Float:
            return useIdxReg ? OP::vldroffs : OP::vstrimms;
        case TR::Double:
            return useIdxReg ? OP::vldroffd : OP::vstrimmd;
        case TR::Address:
            if (elementSize == 8)
                return useIdxReg ? OP::ldroffx : OP::ldrimmx;
            else
    ...
    bool useIdxReg)
{
    switch (dt) {
        case TR::Int8:
            return useIdxReg ? OP::strboff : OP::strbimm;
        case TR::Int16:
            return useIdxReg ? OP::strhoff : OP::strhimm;
        case TR::Int32:
            return useIdxReg ? OP::stroffw : OP::strimmw;
        case TR::Int64:
            return useIdxReg ? OP::stroffx : OP::strimmx;
        case TR::Float:
            return useIdxReg ? OP::vstroffs : OP::vstrimms;
        case TR::Double:
            return useIdxReg ? OP::vstroffd : OP::vstrimmd;
        case TR::Address:
            if (elementSize == 8)
                return useIdxReg ? OP::stroffx : OP::strimmx;
            else
...
TR::Register *J9::ARM64::TreeEvaluator::BNDCHKwithSpineCHKEvaluator(TR::Node *node, TR::CodeGenerator *cg)
{
    ...
    if (needsBoundCheck) {
        ...
        if (arrayLengthReg) {
            ...
            if (arrayLengthChild->getInt() != 0) {
                ...
                // to find out which.
                //
                OOLBranchInstr = Inst_Label(cg, OP::b, node, discontiguousArrayLabel);
                needsBoundCheckOOL = true;
            }
        } else {
    ...
    Inst_Label(cg, OP::label, node, doneMainlineLabel);

    // start of OOL
    //
    discontiguousArrayOOL->swapInstructionListsWithCompilation();
    {
        TR::Instruction *OOLLabelInstr = Inst_Label(cg, OP::label, node, discontiguousArrayLabel);
        ...
        if (doLoadOrStore || doAddressComputation) {
            ...
            if (doLoadOrStore) {
                ...
                if (loadOrStoreChild->getOpCode().isStore()) {
                    OP::Mnemonic storeOp = getStoreOpCodeFromDataType(cg, dt, elementSize, indexReg != NULL);
                    ...
                    OP::Mnemonic loadOp = getLoadOpCodeFromDataType(cg, dt, elementSize, isUnsigned, indexReg != NULL);
        ...
        Inst_Label(cg, OP::label, node, doneOOLLabel, OOLDeps);
        Inst_Label(cg, OP::b, node, doneLabel);
    }
    discontiguousArrayOOL->swapInstructionListsWithCompilation();
    //
