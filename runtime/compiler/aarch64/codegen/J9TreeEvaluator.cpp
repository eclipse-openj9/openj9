...
generateSoftwareReadBarrier(TR::Node *node, TR::CodeGenerator *cg, bool isArdbari)
   ...
   if (cg->getHasResumableTrapHandler() && cg->getCurrentEvaluationTreeTop()->getNode()->getOpCode().isNullCheck())
      ...
      if (cg->getImplicitExceptionPoint() == NULL)
         {
         trprintf(comp->getOption(TR_TraceCG), comp->log(), "Instruction %p throws an implicit NPE, node: %p NPE node: %p\n", faultingInstruction, node, node->getFirstChild());
...
J9::ARM64::TreeEvaluator::VMinstanceofEvaluator(TR::Node *node, TR::CodeGenerator *cg)
   ...
   trprintf(trace, log, "%s:Maximum Profiled Classes = %d\n", node->getOpCode().getName(), maxProfiledClasses);
   ...
   for (auto it = itBegin; it != itEnd; it++)
      ...
      switch (current)
         ...
         case LoadObjectClass:
            ...
            break;
         case NullTest:
            trprintf(trace, log, "%s: Emitting NullTest\n", node->getOpCode().getName());
         ...
         case GoToTrue:
            ...
            trprintf(trace, log, "%s: Emitting GoToTrue\n", node->getOpCode().getName());
         ...
         case GoToFalse:
            ...
            trprintf(trace, log, "%s: Emitting GoToFalse\n", node->getOpCode().getName());
            break;
         case ClassEqualityTest:
            trprintf(trace, log, "%s: Emitting ClassEqualityTest\n", node->getOpCode().getName());
         ...
         case SuperClassTest:
            {
            trprintf(trace, log, "%s: Emitting SuperClassTest\n", node->getOpCode().getName());
         ...
         case ProfiledClassTest:
            {
            trprintf(trace, log, "%s: Emitting ProfiledClassTest\n", node->getOpCode().getName());
            ...
            while (profiledClassesIt != profiledClassesItEnd)
               {
               trprintf(trace, log, "%s: ProfiledClassTest: profiledClass = %p, isProfiledClassInstanceOfCastClass = %s\n",
         ...
         case CompileTimeGuessClassTest:
            trprintf(trace, log, "%s: Emitting CompileTimeGuessClassTest\n", node->getOpCode().getName());
         ...
         case CastClassCacheTest:
            {
            trprintf(trace, log, "%s: Emitting CastClassCacheTest\n", node->getOpCode().getName());
         ...
         case ArrayOfJavaLangObjectTest:
            ...
            trprintf(trace, log, "%s: Emitting ArrayOfJavaLangObjectTest\n", node->getOpCode().getName());
         ...
         case HelperCall:
            ...
            trprintf(trace, log, "%s: Emitting HelperCall\n", node->getOpCode().getName());
   ...
   for (auto it = itBegin; it != itEnd; it++)
      ...
      switch (current)
         ...
         case HelperCall:
            ...
            trprintf(trace, log, "%s: Emitting %s\n", node->getOpCode().getName(), seq);
   ...
   if (isArrayNew)
      ...
      if (TR::Compiler->om.isOffHeapAllocationEnabled())
         ...
         if (isVariableLength && TR::Compiler->om.compressObjectReferences())
             ...
             */
            trprintf(trace, log, "Node (%p): Dealing with compressed refs variable length array.\n", node);
            ...
            {
            trprintf(trace, log, "Node (%p): Dealing with full/compressed refs fixed length zero size array.\n", node);
            ...
            {
            trprintf(trace, log, "Node (%p): Dealing with either full/compressed refs fixed length non-zero size array or full refs variable length array.\n", node);
...
genCAS(TR::Node *node, TR::CodeGenerator *cg, TR_ARM64ScratchRegisterManager *srm, TR::Register *objReg, TR::Register *offsetReg, intptr_t offset, bool offsetInReg, TR::Register *oldVReg, TR::Register *newVReg,
      TR::LabelSymbol *doneLabel, int32_t oldValue, bool oldValueInReg, bool is64bit, bool isReference, bool isExchange, bool casWithoutSync = false)
   {
   TR::Compilation * comp = cg->comp();
   TR::Register *addrReg = srm->findOrCreateScratchRegister();
   ...
   if (comp->target().cpu.supportsFeature(OMR_FEATURE_ARM64_LSE) && (!disableLSE))
      ...
      generateTrg1MemSrc1Instruction(cg, op, node, isExchange ? tempReg : resultReg, TR::MemoryReference::createWithDisplacement(cg, addrReg, 0), newVReg);
...
static TR::Register *
VMinlineCompareAndSwap(TR::Node *node, TR::CodeGenerator *cg, bool isLong, bool isExchange = false)
   {
   TR::Compilation * comp = cg->comp();
   TR::Node *firstChild = node->getFirstChild();
   ...
   // Compare and swap:
   resultReg = genCAS(node, cg, srm, objReg, offsetReg, offset, offsetInReg, oldVReg, newVReg, NULL, oldValue, oldValueInReg, isLong, false, isExchange, casWithoutSync);
...
static TR::Register *VMinlineCompareAndSwapObject(TR::Node *node, TR::CodeGenerator *cg, bool isExchange = false)
   ...
   resultReg = genCAS(node, cg, srm, objReg, offsetReg, offset, offsetInReg, oReg, nReg, doneLabel, 0, true, !comp->useCompressedPointers(), true, isExchange);
   ...
   const bool skipWrtBar = (gcMode == gc_modron_wrtbar_none) || (newVNode->isNull() && (gcMode != gc_modron_wrtbar_always));
   if (!skipWrtBar)
      {
      ...
      switch (methodSymbol->getRecognizedMethod())
         ...
         case TR::java_lang_Thread_onSpinWait:
            ...
            if (!disableOSW)
               ...
               trprintf(comp->getOption(TR_TraceCG), comp->log(), "insert YIELD for onSpinWait: node=%p, %s\n", node, comp->signature());
               return true;
               }
            break;
            }
