...
bool
J9::RV::CPU::isCompatible(const OMRProcessorDesc& processorDescription)
   {
   return self()->getProcessorDescription().processor == processorDescription.processor;
   }

OMRProcessorDesc
J9::RV::CPU::getProcessorDescription()
   {
   ...
   if (!initialized)
      ...
      switch (self()->id())
         {
         case TR_DefaultRISCVProcessor:
            _processorDescription.processor = OMR_PROCESOR_RISCV64_UNKNOWN;
            break;
         default:
            TR_ASSERT_FATAL(false, "Invalid RISC-V Processor ID");
         }
