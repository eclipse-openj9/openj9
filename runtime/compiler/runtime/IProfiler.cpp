...
TR_IProfiler::getSearchPCFromMethodAndBCIndex(TR_OpaqueMethodBlock *method, uint32_t byteCodeIndex, TR::Compilation *comp)
   ...
   if (pc == 0 && comp->getLoggingEnabled())
      ...
      trprints(traceIProfiling, log, "CSI : INLINER STACK :\n");
      for (int i = 0; i < len; i++)
         {
...
TR_IProfiler::getProfilingEntry(TR_OpaqueMethodBlock *method, uint32_t byteCodeIndex, TR::Compilation * comp)
   ...
   trprintf(traceIProfiling, log, "Asked for profiling data on PC=%p, ", getSearchPC (method, byteCodeIndex, comp));

   if (entry)
      {
      if (invalidateEntryIfInconsistent(entry))
         {
         trprints(traceIProfiling, log, "got nothing because it was invalidated\n");
         return 0;
         }
   ...
   trprints(traceIProfiling, log, "got nothing\n");
   ...
   if (data)
      ...
      if (branchHasSameDirection(nodeOpCode, node, comp))
         ...
         *taken = (int32_t) fallThroughCount;
         ...
         trprintf(traceIProfiling, comp->log(), "I couldn't figure out the branch direction after change for node [%p], so I gave default direction \n", node);
         *taken = (int32_t) branchToCount;
         *notTaken = (int32_t) fallThroughCount;
         }
   ...
   if (!bcInfo.doNotProfile())
      ...
      if (!isNewOpCode(thisByteCode))
         ...
         if (csInfo)
            ...
            if (!data)
               {
               trprintf(traceIProfiling, log, "Call-graph 1 No profiling data for bcIndex=%d, callerIndex=%d\n", bcInfo.getByteCodeIndex(), bcInfo.getCallerIndex());
               return NULL;
               }
            ...
            *addrOfTotalFrequency = (*addrOfTotalFrequency) + csInfo->_residueWeight;
            ...
            {
            trprintf(traceIProfiling, log, "Call-graph 2 Set not to profile bcIndex=%d, callerIndex=%d\n", bcInfo.getByteCodeIndex(), bcInfo.getCallerIndex());
            return NULL;
            }
         }
      else
         ...
         if (!data)
            {
            trprintf(traceIProfiling, log, "No profiling data for bcIndex=%d, callerIndex=%d\n", bcInfo.getByteCodeIndex(), bcInfo.getCallerIndex());
            return NULL;
            }
...
TR_IProfiler::getCGProfilingData(TR_OpaqueMethodBlock *method, uint32_t byteCodeIndex, TR::Compilation * comp)
   ...
   if (entry)
      ...
      if (invalidateEntryIfInconsistent(entry))
         {
         trprints(traceIProfiling, comp->log(), "got nothing because it was invalidated\n");
         return NULL;
         }
