...
void *TR_HashTableEntry::operator new[](size_t s, TR_Memory *mem)
{
    return mem->allocateMemory(s, heapAlloc, TR_MemoryBase::HashTableEntry);
}

void *TR_HashTable::operator new(size_t s, TR_Memory *mem)
{
...
bool TR_HashTable::locate(void *key, TR_HashIndex &index, TR_HashCode hashCode)
{
    if (hashCode == 0) {
...
#endif
    while ((_table[index].getHashCode() != hashCode) || !isEqual(key, _table[index].getKey())) {
#ifdef DEBUG
        ++collisions;
#endif
...
bool TR_HashTable::add(void *key, void *data, TR_HashCode hashCode)
{
    TR_HashIndex index;
...
void TR_HashTable::remove(TR_HashIndex index)
{
    ...
    if (index > _mask + 1) {
        ...
        for (collisionIndex = headOfChain; _table[collisionIndex].getCollisionChain() != index;
...
bool TR_HashTable::isEmpty() const
{
    bool empty = true;
    for (TR_HashIndex i = 0; i < _tableSize; i++) {
        if (_table[i].isValid()) {
...
void TR_HashTable::grow(uint32_t newSize)
{
    uint32_t closedAreaSize = 2, openAreaSize;
...
void TR_HashTable::grow()
{
    uint32_t newSize = (_mask << 1) | 1;
    growAndRehash(_table, _tableSize, newSize + 1, (newSize + 1) / 4);
}

void TR_HashTable::growAndRehash(TR_HashTableEntry *oldTable, TR_HashIndex oldSize, TR_HashIndex closedAreaSize,
