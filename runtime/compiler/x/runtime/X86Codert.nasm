...
begin512BitLoop:
        ...
        crc32       rax, qword [rsi+1*rdx+0x8]    ; 64-byte unrolled crc32c loop body
        crc32       rax, qword [rsi+1*rdx+0x10]
        crc32       rax, qword [rsi+1*rdx+0x18]
        crc32       rax, qword [rsi+1*rdx+0x20]
        crc32       rax, qword [rsi+1*rdx+0x28]
        crc32       rax, qword [rsi+1*rdx+0x30]
        crc32       rax, qword [rsi+1*rdx+0x38]
        crc32       rax, qword [rsi+1*rdx+0x40]
...
begin64BitLoop:                                   ;  crc 8 bytes at a time in loop
        ...
        cmp         rdi, rcx
        jg          crc32c_residue
        crc32       rax, qword [rsi+1*rdx+0x8]
        add         rdx, 0x00000008
        jmp         begin64BitLoop

crc32c_residue:                                   ; Duff device implementation
%define CRC32C_INC_INSTRUCTION_SIZE 11            ; the size of crc32 + inc instruction
...
crc7b:
        crc32       rax, byte [rsi+1*rdx+0x8]
        inc         rdx                           ; Inc instruction required because num of bytes remaining is
        crc32       rax, byte [rsi+1*rdx+0x8]     ; variable and the bytes must be processed in order
        inc         rdx
        crc32       rax, byte [rsi+1*rdx+0x8]
        inc         rdx
        crc32       rax, byte [rsi+1*rdx+0x8]
