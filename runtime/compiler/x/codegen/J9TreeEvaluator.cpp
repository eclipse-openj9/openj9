...
static void handleOffHeapDataForArrays(
   ...
   if (TR::Compiler->om.compressObjectReferences() && NULL != sizeReg)
      ...
      // Since array size is capped at 32 bits, we only need to check 32-63 bits of lengthReg.
      ...
      // Since array size is capped at 32 bits, we only need to check 32-63 bits of lengthReg.
      generateRegImmInstruction(TR::InstOpCode::CMP4RegImm4, node, sizeReg, 0, cg);
