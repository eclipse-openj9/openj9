...
J9::Compilation::canAllocateInline(TR::Node* node, TR_OpaqueClassBlock* &classInfo)
   ...
   if (node->getOpCodeValue() == TR::newarray)
      ...
      if (classRef->getOpCodeValue() != TR::loadaddr)
         ...
         if (areValueTypesEnabled)
            {
            trprintf(trace, log, "cannot inline array allocation @ node %p because value types are enabled\n", node);
   ...
   if (TR::Compiler->om.useHybridArraylets() && TR::Compiler->om.isDiscontiguousArray(size))
      {
      trprintf(trace, log, "cannot inline array allocation @ node %p because size %d is discontiguous\n", node, size);
      return -1;
      }
      ...
      trprintf(trace, log, "inline array allocation @ node %p for size 0\n", node);
#else
      trprintf(trace, log, "cannot inline array allocation @ node %p because size 0 is discontiguous\n", node);
      return -1;
#endif
      }
