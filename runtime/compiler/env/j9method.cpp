...
getMethodHandleThunkDetails(TR_J9ByteCodeIlGenerator *ilgen, TR::Compilation *comp, TR::SymbolReference *macro)
   ...
   if (details.isMethodHandleThunk())
      ...
      {
      trprintf(trace, log, "  Conservatively leave ILGen macro '%s' as a native call for peeking\n", comp->getDebug()->getName(macro));
      }
   else
      {
      trprintf(trace, log, "  Conservatively abort compile due to presence of ILGen macro '%s'\n", comp->getDebug()->getName(macro));
...
TR_J9ByteCodeIlGenerator::runFEMacro(TR::SymbolReference *symRef)
   ...
   switch (rm)
         ...
         for (
            ...
            if (targetType[0] == 'L')
               ...
               if (fej9->isInterfaceClass(targetParmClass) && isExplicit)
                  ...
                  {
                  trprintf(trace(), log, "source type and target type are compatible, omit checkcast for arg %d\n", argIndex);
                  }
               else
                  {
                  push(argValue);
      ...
      case TR::java_lang_invoke_BruteArgumentMoverHandle_permuteArgs:
         ...
         trprintf(traceILGen, log, "  permuteArgs: nextHandleSignature is %s\n", nextHandleSignature);
         }
         // FALL THROUGH
      case TR::java_lang_invoke_PermuteHandle_permuteArgs:
         ...
         if (comp()->isOutOfProcessCompilation())
            ...
            newSignature = (char *)"()I";
            trprintf(traceILGen, log, "  permuteArgs: oldSignature is %s\n", oldSignature);
            for (i=0; i < permuteLength; i++)
               ...
               if (argIndex >= 0)
                  ...
                  trprintf(traceILGen, log, "  permuteArgs:   %d: incoming argument\n", argIndex);
                  }
               else
                  {
                  newSignature = artificialSignature(stackAlloc, "(.*.@)I",
            ...
            trprintf(traceILGen, log, "  permuteArgs: permuted placeholder signature is %s\n", newSignature);
            }
         else
#endif /* defined(J9VM_OPT_JITSERVER) */
            {
      ...
      case TR::java_lang_invoke_SpreadHandle_arrayArg:
         ...
         trprintf(traceILGen, log, "  SpreadHandle.arrayArg: Array class name is '%s'\n", arrayClassSignature);
...
TR_J9ByteCodeIlGenerator::walkReferenceChain(TR::Node *node, uintptr_t receiver)
   ...
   if (node->getOpCode().isLoadDirect() && node->getType() == TR::Address)
      ...
      if (symRef->isUnresolved())
         {
         trprintf(traceILGen, log, "  walkReferenceChain hit unresolved symref %s; aborting\n", symRef->getName(comp()->getDebug()));
