...
void *
TR_J9SharedCacheServerVM::getJ2IThunk(char *signatureChars, uint32_t signatureLength, TR::Compilation *comp)
   {
   return (void *)findPersistentThunk(signatureChars, signatureLength);
   }

void *
TR_J9SharedCacheServerVM::setJ2IThunk(char *signatureChars, uint32_t signatureLength, void *thunkptr, TR::Compilation *comp)
   {
   return persistThunk(signatureChars, signatureLength, (U_8*)thunkptr - 8 /* start of thunk */, *((U_32 *)((U_8*)thunkptr -8)) + 8 /* size of thunk */);
   }

void *
TR_J9SharedCacheServerVM::persistJ2IThunk(void *thunk)
   {
...
void *
TR_J9SharedCacheServerVM::persistThunk(char *signatureChars, uint32_t signatureLength, uint8_t *thunkStart, uint32_t totalSize)
   {
   ...
   if (comp && comp->getOption(TR_TraceRelocatableDataDetailsCG))
      {
   ...
   JITaaS::J9ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
   stream->write(JITaaS::J9ServerMessageType::SharedCacheVM_persistThunk, std::string(signatureChars, signatureLength), dataDescriptor);
   const void *store = std::get<0>(stream->read<void *>());
...
void *
TR_J9SharedCacheServerVM::findPersistentThunk(char *signatureChars, uint32_t signatureLength)
   {
#if defined(J9VM_INTERP_AOT_COMPILE_SUPPORT) && defined(J9VM_OPT_SHARED_CLASSES) && (defined(TR_HOST_X86) || defined(TR_HOST_POWER) || defined(TR_HOST_S390) || defined(TR_HOST_ARM))
   JITaaS::J9ServerStream *stream = _compInfoPT->getMethodBeingCompiled()->_stream;
   stream->write(JITaaS::J9ServerMessageType::SharedCacheVM_findPersistentThunk, std::string(signatureChars, signatureLength));
   return std::get<0>(stream->read<void *>());
