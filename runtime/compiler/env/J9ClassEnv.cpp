...
static void addEntryForFieldImpl(TR_VMField *field, TR::TypeLayoutBuilder &tlb, TR::Region &region, J9Class *definingClass,
   ...
   if (TR::Compiler->om.areFlattenableValueTypesEnabled() &&
      ...
      trprintf(trace, log, "field %s:%s is flattened. offset from TR_VMField %d, offset from fcc %d\n",
      ...
      trprintf(trace, log, "type layout definingClass %p field: %s signature: %s field offset: %d offsetBase %d\n",
