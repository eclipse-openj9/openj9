...
J9::CodeGenerator::allocateVariableSizeSymRef(int32_t byteLength)
   ...
   trprintf(trace, log, "\tallocateVariableSizeSymbolReference: length = %d\n", byteLength);
   ...
   if (symRef == NULL)
      ...
      trprintf(trace, log, "\t\tno available symRef allocate symRef #%d : %s (%p) of length = %d\n",
      ...
      trprintf(trace, log, "\t\treuse available symRef #%d : %s (%p) with length = %d\n",
            symRef->getReferenceNumber(), self()->getDebug()->getName(sym), sym, byteLength);
      }
   sym->setActiveSize(byteLength);
   sym->setReferenceCount(0);
   trprintf(trace, log, "\treturning symRef #%d (%s) : activeSize set to %d (length = %d)\n",
...
J9::CodeGenerator::freeVariableSizeSymRef(
   ...
   trprintf(trace, log, "\tfreeVariableSizeSymbol: #%d (%s)%s%s%s\n",
   ...
   if (sym->isAddressTaken() && !freeAddressTakenSymbol)
      {
      trprintf(trace, log, "\t\tsym->isAddressTaken()=true and freeAddressTakenSymbol=false so do not free sym #%d (%s %p)\n",
            symRef->getReferenceNumber(), self()->getDebug()->getName(sym), sym);
      return;
      }
   else
      {
      trprintf(trace, log, "\t\tfree symRef #%d (%s %p)\n", symRef->getReferenceNumber(), self()->getDebug()->getName(sym), sym);
...
J9::CodeGenerator::pendingFreeVariableSizeSymRef(TR::SymbolReference *symRef)
   ...
   trprintf(self()->traceBCDCodeGen(), self()->comp()->log(), "\tpendingFreeVariableSizeSymRef: #%d (%s) %s to pending free list\n",
...
J9::CodeGenerator::getFreeVariableSizeSymRef(int32_t byteLength)
   ...
   trprintf(trace, log, "\tgetFreeVariableSizeSymRef of length %d\n", byteLength);

   if (biggestSymRef)
      {
      trprintf(trace, log, "\t\tset initial biggestSymRef to #%d (%s) with length %d\n",
      ...
      while (i != _variableSizeSymRefFreeList.end())
         ...
         trprintf(trace, log, "\t\texamine free symRef #%d (%s) with length %d\n",
         ...
         if ((*i)->getSymbol()->getSize() >= byteLength)
            ...
            {
            trprintf(trace, log, "\t\tupdate biggest symRef seen to #%d (%s) with length %d\n", (*i)->getReferenceNumber(), self()->getDebug()->getName((*i)->getSymbol()), byteLength);
      ...
      trprintf(trace, log, "\t\tincrease biggestSymRef #%d (%s) size from %d -> %d\n",
...
J9::CodeGenerator::checkForUnfreedVariableSizeSymRefs()
   ...
   for (auto i = _variableSizeSymRefAllocList.begin(); i != _variableSizeSymRefAllocList.end(); ++i)
      ...
      if (!found)
         ...
         trprintf(self()->traceBCDCodeGen(), log, "Variable size symRef #%d (%s) has not been freed (symbol refCount is %d)\n",
