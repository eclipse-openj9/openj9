...
J9::AheadOfTimeCompile::initializeCommonAOTRelocationHeader(TR::IteratedExternalRelocation *relocation,
   ...
   switch (kind)
      ...
      case TR_ValidateDynamicMethodFromCallsiteIndex:
         ...
         dmciRecord->setCalleeID(reloTarget, symValManager->getSymbolIDFromValue(svmRecord->_callee));
      ...
      case TR_ValidateHandleMethodFromCPIndex:
         ...
         hmciRecord->setCalleeID(reloTarget, symValManager->getSymbolIDFromValue(svmRecord->_callee));
   ...
   switch (kind)
      ...
      case TR_ValidateDynamicMethodFromCallsiteIndex:
         ...
         if (isVerbose)
            ...
            traceMsg(
               self()->comp(),
               "\n Validate Dynamic Method From Callsite Index: methodID=%d, calleeID=%d, callsiteIndex=%d, appendixObjectNull=%s, definingClassID=%d, methodIndex=%d ",
                     (uint32_t)dmciRecord->methodID(reloTarget),
                     (uint32_t)dmciRecord->calleeID(reloTarget),
      ...
      case TR_ValidateHandleMethodFromCPIndex:
         ...
         if (isVerbose)
            ...
            traceMsg(
               self()->comp(),
               "\n Validate Handle Method From CP Index: methodID=%d, calleeID=%d, cpIndex=%d, appendixObjectNull=%s, definingClassID=%d, methodIndex=%d ",
                     (uint32_t)hmciRecord->methodID(reloTarget),
                     (uint32_t)hmciRecord->calleeID(reloTarget),
