...
J9::SetMonitorStateOnBlockEntry::addSuccessors(
   ...
   trprintf(traceIt, log, "\tIn SMSOBE::addSuccessors for cfgNode %d, monitorStack %p dontPropagateMonitor %d monitorType = %d callerIndex %d walkOlyExceptionSuccs %d\n",
   ...
   for (TR::CFGEdge *edge = succs.getFirst(); edge; edge = succs.getNext())
      ...
      if (succBlock->getEntry())
         ...
         if (monitorType == MonitorEnter)
            {
            trprintf(traceIt, log, "\tIn J9::SetMonitorStateOnBlockEntry::addSuccessors monitorType = MonitorEnter  block %d\n", succBlock->getNumber());
         ...
         trprintf(traceIt, log, "process succBlock %d propagate (t/f: %d) isCatchBlock=%d monitorType=%d callerIndex=%d entryCallerIndex=%d\n",
         ...
         if (monitorStack)
            ...
            if (succBlock->getVisitCount() != _visitCount)
               ...
               trprintf(traceIt, log, "\tIn SMSOnBE::addSuccesors  created newMonitorStack %p and monitorStack %p\n", newMonitorStack,monitorStack);

               if (popMonitor)
                  {
                  trprintf(traceIt, log, "popping monitor symRef=%d before propagation\n", newMonitorStack->top()->getReferenceNumber());
                  newMonitorStack->pop();
                  }
               ...
               trprintf(traceIt, log, "adding monitorstack to successor %d (%p size %d)\n", succBlock->getNumber(), newMonitorStack, newMonitorStack->size());
               }
            else
               {
               ...
               if (!succBlock->isOSRCatchBlock() && !succBlock->isOSRCodeBlock())
                  ...
                  if (!isMonitorStateConsistentForBlock(succBlock, monitorStack, popMonitor))
                     ...
                     {
                     trprintf(traceIt, log, "verified block_%d monitorState is consistent\n", succBlock->getNumber());
                     }
                  }
               continue;
               }
         ...
         trprintf(traceIt, log, "\tIn SMSOnBE::addSuccessors adding block %d to blocksToVisit\n", succBlock->getNumber());
...
J9::SetMonitorStateOnBlockEntry::isMonitorStateConsistentForBlock(
   ...
   trprintf(traceIt, log, "MonitorState block_%d: oldMonitorStack %p newMonitorStack %p popMonitor %d\n",
   ...
   if (oldMonitorStackEmpty != newMonitorStackEmpty)
      {
      trprintf(traceIt, log, "MonitorState inconsistent for block_%d: oldMonitorStack isEmpty %d, newMonitorStack isEmpty %d\n",
   ...
   if (newSize != oldSize)
      {
      trprintf(traceIt, log, "MonitorState inconsistent for block_%d: oldMonitorStack size %d, newMonitorStack size %d\n",
   ...
   for (int i = oldMonitorStack->topIndex(); i>= 0; i--)
      ...
      if (newMonitorStack->element(i)->getReferenceNumber() != oldMonitorStack->element(i)->getReferenceNumber())
         {
         trprintf(traceIt, log, "MonitorState inconsistent for block_%d: oldMonitorStack(%d) symRef=%d, newMonitorStack(%d) symRef=%d\n",
      ...
      if (callerIndex == -1)
         ...
         if (comp->getJittedMethodSymbol()->isSynchronised())
            ...
            if (monitorStack->size() == 1 &&
                  ...
                  !blockExitsMethod)
               {
               trprintf(traceIt, log, "monitorStack is empty (except for special DLT sync object slot) for DLT compile at monexit %p\n", node);
               return false;
               }
            else if (monitorStack->isEmpty())
               {
               trprintf(traceIt, log, "monitorStack is empty for DLT compile at monexit %p\n", node);
               return false;
               }
            }
         else
            ...
            if (monitorStack->isEmpty())
               {
                trprintf(traceIt, log, "monitorStack is empty for non-synchronized DLT compile at monexit %p\n", node);
                return false;
               }
            }
         }
...
static bool needToPushMonitor(TR::Compilation *comp, TR::Block *block, bool traceIt)
   ...
   for (auto e = block->getSuccessors().begin(); e != block->getSuccessors().end(); ++e)
      ...
      if (comp->getFlowGraph()->compareExceptionSuccessors(block, succ) == 0)
         {
         trprintf(traceIt, log, "found identical exception successors for block %d and succ %d\n", block->getNumber(), succ->getNumber());
         ...
         for (TR::TreeTop *tt = succ->getEntry(); tt != succ->getExit() ; tt = tt->getNextTreeTop())
            ...
            if (aNode && (aNode->getOpCodeValue() == TR::tfinish || aNode->getOpCodeValue() == TR::monexit ||
               ...
               {
               trprintf(traceIt, log, "overriding identical exception decision because node %p in block %d is either monexit or tfinish", aNode, succ->getNumber());
               retval = false;
               break;
               }
            }
...
void J9::SetMonitorStateOnBlockEntry::set(bool& lmmdFailed, bool traceIt)
   ...
   while (!_blocksToVisit.isEmpty())
      ...
      trprintf(traceIt, log, "block to process: %d\n", block->getNumber());
      ...
      if (traceIt && monitorStack && !monitorStack->isEmpty())
         ...
         trprints(traceIt, log, "monitor stack is empty\n");
      ...
      for (TR::TreeTop * tt = block->getEntry(); ; tt = tt->getNextTreeTop())
         ...
         if ((node->getOpCode().isStore() &&            //only monents are represented by this store now
            ...
            if (monitorStack)
               ...
               trprintf(traceIt, log, "adding monitor to stack symbol=%p symRef=%d (size=%d) (node %p)\n",
               ...
               trprintf(traceIt, log, "adding monitor to fresh stack symbol=%p symRef=%d (size=%d) (node %p)\n",
                     node->getSymbol(), node->getSymbolReference()->getReferenceNumber(), monitorStack->size()+1,node);
               }
            ...
            if (monitorStack->topIndex() == 0)
               {
               monitorStack = new (trHeapMemory()) TR_Stack<TR::SymbolReference *>(*monitorStack);
               trprintf(traceIt, log, "popping monitor off stack symRef=%d, BEFORE pop size=%d, ", monitorStack->top()->getReferenceNumber(), monitorStack->size());
               monitorStack->pop();
               trprintf(traceIt, log, "AFTER size=%d\n", monitorStack->size());
               }
            ...
            if (monitorExitFence > 0)
               ...
               for (; edge != block->getExceptionSuccessors().end(); ++edge)
                  ...
                  if (node->getByteCodeInfo().getCallerIndex() ==
                        succBlock->getEntry()->getNode()->getByteCodeInfo().getCallerIndex())
                     {
                     trprintf(traceInitMonitorsForExceptionAfterMonexit, log, "block_%d has exceptions after monexit with catch block in the same method %s\n",
         ...
         if (tt == block->getExit())
            ...
            if ((monitorExitFence+monitorEnterStore)>= 2)
               {
               trprintf(traceIt, log, "block_%d has monitorEnterStore=%d monitorExitFence=%d\n", block->getNumber(), monitorEnterStore, monitorExitFence);
               lmmdFailed = true;
               }
            ...
            if (blockHasMonexit)
               ...
               if ((monitorPoppedForExceptionSucc > 0) &&
                  ...
                  {
                  trprintf(traceIt, log, "pushing monexit symRef=%d back temporarily\n", monitorStackTop->getReferenceNumber());
                  monitorStack->push(monitorStackTop);
                  }
               }

            trprintf(traceIt, log, "blockHasMonent=%d blockHasMonexit=%d dontPropagateMonitor=%d callerIndex=%d monitorPoppedForExceptionSucc=%d\n",
