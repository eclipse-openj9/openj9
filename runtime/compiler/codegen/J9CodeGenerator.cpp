      ...
      case TR::instanceof:
        {
         ...
         TR::Node *ttNode = TR::Node::create(TR::treetop, 1, node);
         tt->getPrevTreeTop()->insertAfter(TR::TreeTop::create(comp, ttNode));
         trprintf(comp->getOption(TR_TraceCG), comp->log(), "Anchoring %s node %s [%p] under treetop [%p]\n", anchoringReason, node->getOpCode().getName(), node, ttNode);
         break;
         }
      default:
         break;
   ...
   if (!storeBlock)
      ...
      if (glRegDeps)
         ...
         if(comp->getOption(TR_MimicInterpreterFrameShape) || comp->getOption(TR_PoisonDeadSlots))
            ...
            trprintf(trace, log, "zeroOutAutoOnEdge: glRegDepsParent is %s\n", self()->getDebug()->getName(glRegDepsParent));
      ...
      if (comp->getOption(TR_PoisonDeadSlots))
         {
         trprintf(trace, log, "POISON DEAD SLOTS --- New Block Created %d\n", newBlock->getNumber());
...
J9::CodeGenerator::doInstructionSelection()
   ...
   if (comp->getMethodSymbol()->mayContainMonitors())
      {
      trprints(traceLiveMon, log, "In doInstructionSelection: Method may contain monitors\n");
      ...
      for (TR::AutomaticSymbol * a = locals.getFirst(); a; a = locals.getNext())
         if (a->holdsMonitoredObject())
            {
            trprintf(traceLiveMon, log, "\tSymbol %p contains monitored object\n", a);
            if (!liveLocals)
               {
               trprintf(traceLiveMon, log, "\tsetting LiveLocalIndex to %d on symbol %p\n", numMonitorLocals+1, a);
               a->setLiveLocalIndex(numMonitorLocals++, self()->fe());
               }
            else if (a->getLiveLocalIndex() + 1 > numMonitorLocals)
               {
               trprintf(traceLiveMon, log, "\tsetting numMonitorLocals to %d while considering symbol %p\n", a->getLiveLocalIndex()+1, a);
      ...
      if (numMonitorLocals)
         ...
         trprintf(traceLiveMon, log, "\tCreated monitorState %p\n", &monitorState);

         monitorState.set(_lmmdFailed, traceLiveMon);
         trprintf(traceLiveMon, log, "found numMonitorLocals %d\n", numMonitorLocals);
         }
      else
         trprintf(traceLiveMon, log, "\tnumMonitorLocals = %d\n",numMonitorLocals);
      }
   ...
   for (TR::TreeTop *tt = comp->getStartTree(); tt; tt = self()->getCurrentEvaluationTreeTop()->getNextTreeTop())
      {
      trprintf(traceLiveMon, log, "\tWalking TreeTops at tt %p with node %p\n", tt, tt->getNode());
      ...
      if (opCode == TR::BBStart)
         ...
         if (!block->isExtensionOfPreviousBlock())
            ...
            if (liveMonitorStack)
               ...
               {
               liveMonitors = 0;
               trprintf(traceLiveMon, log, "no liveMonitorStack for block_%d\n", block->getNumber());
               }
            ...
            if (liveMonitorStack)
               ...
               if (!liveMonitorStack->isEmpty())
                  ...
                  if (comp->getOption(TR_PoisonDeadSlots))
                     ...
                     trprintf(traceLiveMon, log, "popping symref %p (#%u) off monitor stack\n", symref, symref->getReferenceNumber());
                     liveMonitors->reset(symref->getSymbol()->castToRegisterMappedSymbol()->getLiveLocalIndex());
                     }
               ...
               trprintf(traceLiveMon, log, "monitor %p went dead at node %p\n", node->getSymbol(), node);
               }
      ...
      if (comp->getOption(TR_SplitWarmAndColdBlocks) &&
         ...
         if (b->isLastWarmBlock())
            ...
            trprintf(traceCG, log, "%s Last warm instruction is %p\n", SPLIT_WARM_COLD_STRING, lastInstr);
         ...
         while (it != _variableSizeSymRefPendingFreeList.end())
            ...
            if (!sym->isAddressTaken() && !found)
               ...
               if (sym->getReferenceCount() == 0 &&
                  ...
                  {
                  trprintf(self()->traceBCDCodeGen(), log, "\treset nodeToFreeAfter %p->NULL for sym %p with refCount %d > 0\n", nodeToFreeAfter, sym, sym->getReferenceCount());
   ...
   trprints(self()->traceBCDCodeGen(), log, "\tinstruction selection is complete so free all symbols in the _variableSizeSymRefPendingFreeList\n");
...
*/
   ...
   for(TR::TreeTop * tt = comp->getStartTree(); tt; tt = tt->getNextTreeTop())
      ...
      trprintf(traceOSR, log, "Lowering trees in OSR block_%d...\n", block->getNumber());
      ...
      trprintf(traceOSR, log, "callerIndex %d: max pending push slots=%d, # of auto slots=%d, # of arg slots=%d\n",
      ...
      trprintf(traceOSR, log, "%s %s %s: written out bytes in OSR buffer\n",
      ...
      for (auto info = classInfo->begin(); info != classInfo->end(); ++info)
         ...
         if (ramMethod != comp->getCurrentMethod()->getPersistentIdentifier()) // && info->_reloKind != TR_ValidateArbitraryClass)
            ...
            for (i = 0; i < comp->getNumInlinedCallSites(); i++)
               ...
               trprintf(trace, log, "\tinline site %d inlined method %p\n", i, inlinedMethod);
               if (ramMethod == inlinedMethod)
                  {
                  trprints(trace, log, "\t\tmatch!\n");
                  siteIndex = i;
                  break;
                  }
               }
            ...
            if (i >= (int32_t) comp->getNumInlinedCallSites())
               ...
               trprints(trace, log, "\ttransformed into TR_ValidateArbitraryClass\n");
               }
            }

         trprintf(trace, log, "Found inlined site %d\n", siteIndex);
   ...
   switch (site->getType())
      ...
      case TR_ProfiledGuard:
         if (site->getGuard()->getTestType() == TR_MethodTest)
            ...
            trprints(trace, log, "TR_ProfiledMethodGuardRelocation\n");
            }
         else if (site->getGuard()->getTestType() == TR_VftTest)
            {
            type = TR_ProfiledClassGuardRelocation;
            trprints(trace, log, "TR_ProfiledClassGuardRelocation\n");
            }
         else
      ...
      case TR_BreakpointGuard:
         trprints(trace, log, "TR_Breakpoint\n");
         type = TR_Breakpoint;
         break;
            ...
            if ((node->getOpCodeValue() == TR::l2a) &&
               ...
               while (cursorTree)
                  ...
                  if (addTree)
                     ...
                     trprintf(comp->getOption(TR_TraceCG), comp->log(), "Placing treetop %p (to hide delay) after tree %p for l2a %p\n", ttNode, cursorNode, node);
...
J9::CodeGenerator::addInvokeBasicCallSiteImpl(
   ...
   trprintf(comp()->getOption(TR_TraceCG), log, "  arg slots: %u\n", numArgSlots32);
   ...
   if (rm == TR::com_ibm_jit_JITHelpers_dispatchVirtual &&
      ...
      trprintf(comp()->getOption(TR_TraceCG), log, "  J2I thunk: %p\n", j2iThunk);
      }
