...
static void populateOSRCallSiteRematTable(TR::Optimizer* optimizer, TR_CallTarget* calltarget,
   ...
   for (
      ...
      if (child->getOpCode().hasSymbolReference()
         ...
         {
         trprintf(trace, log, "callSiteRemat: found potential pending push #%d with store #%d\n",
      ...
      //
         ...
         {
         trprintf(trace, log, "callSiteRemat: failed to find store for pending push #%d\n", store->getSymbolReference()->getReferenceNumber());
   ...
   for (uint32_t i = 0; i < safetyInfo.size(); ++i)
      ...
      if (!unsafeSymRefs.Intersects(safetyInfo.symRefDependencies(i)))
         ...
         if (storeTree == rematTree)
            {
            trprintf(trace, log, "callSiteRemat: adding pending push #%d with store #%d to remat table\n",
...
bool TR_InlinerBase::inlineCallTarget(TR_CallStack *callStack, TR_CallTarget *calltarget, bool inlinefromgraph, TR_PrexArgInfo *argInfo, TR::TreeTop** cursorTreeTop)
   ...
   if (comp()->getOption(TR_EnableOSR)
      ...
      {
      trprintf(tracePrex, comp()->log(), "callSiteRemat: populating OSR call site remat table for call [%p]\n", calltarget->_myCallSite->_callNode);
   ...
   for (TR_ExtraAddressInfo *profiledInfo = sortedValuesIt.getFirst(); profiledInfo != NULL; profiledInfo = sortedValuesIt.getNext())
      ...
      if ((val >= minProfiledCallFrequency ||
         ...
         if (callSiteClass && !isInterface() && TR::Compiler->cls.isInterfaceClass(comp(), callSiteClass) && isCallingObjectMethod() != TR_yes)
            ...
            trprintf(trace, log, "inliner: callSiteClass [%p] is an interface making it impossible to confirm correct context of the profiled class [%p]\n", callSiteClass, tempreceiverClass);
            callSiteClass = 0;
            }
         ...
         if (profiledClassIsNotInstanceOfCallSiteClass)
            ...
            trprintf(trace, log, "inliner: profiled class [%p] is not instanceof callSiteClass [%p]\n", tempreceiverClass, callSiteClass);
         ...
         if (comp()->compileRelocatableCode())
            ...
            if (!fej9->canRememberClass(tempreceiverClass) ||
               ...
               {
               trprintf(trace, log, "inliner: profiled class [%p] or callSiteClass [%p] cannot be rememberd in shared cache\n", tempreceiverClass, callSiteClass);
               continue;
               }
            }
         ...
         {
         trprints(trace, log, "bailing, below inlining threshold\n");
         break;
         }
...
void TR_ProfileableCallSite::findSingleProfiledMethod(ListIterator<TR_ExtraAddressInfo>& sortedValuesIt, TR_AddressInfo * valueInfo, TR_InlinerBase* inliner)
   ...
   trprints(trace, log, "No decisive class profiling info for the virtual method, we'll try to see if more than one class uses the same method implementation.\n");
   ...
   trprints(trace, log, "OK, all classes check out, we'll try to get their method implementations.\n");
   ...
   trprintf(trace, log, "OK, all classes check out, we'll try to get their method implementations (%d).\n", numMethods);
   ...
   if (bestMethodInfo)
      ...
      trprintf(trace, log, "Failed to find any methods compatible with callsite class %p signature %s\n", callSiteClass, TR::Compiler->cls.classSignature(comp(), callSiteClass, comp()->trMemory()));
   }
