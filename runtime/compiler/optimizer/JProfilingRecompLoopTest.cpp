...
TR_JProfilingRecompLoopTest::perform()
   ...
   if (comp()->getProfilingMode() != JProfiling)
      {
      trprints(trace(), comp()->log(), "JProfiling for profiling compilations has not been enabled, skip JProfilingRecompLoopTest\n");
      return 0;
      }
   ...
   if (!bfi)
      {
      trprints(trace(), comp()->log(), "Block frequency info does not exist, skip JProfilingRecompLoopTest\n");
      return 0;
      }
...
TR_JProfilingRecompLoopTest::addRecompilationTests(TR::Compilation *comp, RecompilationTestLocationsInfo &testLocations, TR_BlockFrequencyInfo *bfi)
   ...
   trprintf(trace(), log, "Loop Recompilation Base Threshold = %d\n", recompileThreshold);
   ...
   for (auto testLocationIter = testLocations.rbegin(), testLocationEnd = testLocations.rend(); testLocationIter != testLocationEnd; ++testLocationIter)
      ...
      trprintf(trace(), log, "\t\t Newly created recompilation Test : Threshold comparison Node n%dn\n\t\tRecompilation Call in block_%d\n",
