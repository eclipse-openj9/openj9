      ...
      if (node && node->getOpCode().isCall()  && !node->getSymbol()->castToMethodSymbol()->isHelper())
         ...
         if (varHandleAccessMethod != TR::unknownMethod)
            ...
            if (comp()->compileRelocatableCode())
               {
               trprintf(doTrace, comp()->log(), "A call to VarHandle access method %s is not supported in AOT. Aborting compile.\n", comp()->getDebug()->getName(symRef));
            ...
            if (comp()->getOption(TR_FullSpeedDebug) && !comp()->isPeekingMethod())
               {
               trprintf(doTrace, comp()->log(), "A call to VarHandle access method %s is not supported in FSD. Failing ilgen.\n", comp()->getDebug()->getName(symRef));
