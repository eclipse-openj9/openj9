...
TR_JProfilingValue::perform()
   ...
   if (comp()->getProfilingMode() == JProfiling)
      {
      trprints(trace(), log, "JProfiling has been enabled for profiling compilations, run JProfilingValue\n");
      }
   else if (comp()->getOption(TR_EnableJProfiling))
      {
      trprints(trace(), log, "JProfiling has been enabled, run JProfilingValue\n");
      }
   else
      {
      trprints(trace(), log, "JProfiling has been disabled, skip JProfilingValue\n");
      return 0;
      }
...
TR_JProfilingValue::addProfilingTrees(
   ...
   trprintf(trace, log, "\t\t\tUsing split mainline return block = %d\n", mainlineReturn->getNumber());
   ...
   trprintf(trace, log, "\t\t\tProfiling value n%dn\n", profilingValue->getGlobalIndex());
   ...
   if (comp->isProfilingCompilation() && !disableJProfilingRecompQueueTest)
      ...
      if (bfi != NULL)
         ...
         trprintf(trace, log, "\t\t\tCheck if queued for recompilation test performed in block_%d: n%dn\n",
   ...
   if (addNullCheck)
      ...
      trprintf(trace, log, "\t\t\tNull test performed in block_%d\n", iter->getNumber());
      }
   ...
   trprintf(trace, log, "\t\t\tQuick Test to check if value is already being profiled is in block_%d\n", quickTestBlock->getNumber());
   ...
   trprintf(trace, log, "\t\t\tQuick increment performed in block_%d\n", quickInc->getNumber());
   ...
   if (valueChildOfHelperCall == NULL)
      ...
      if (profilingValue->getOpCode().isStoreDirect() || (profilingValue->getOpCode().isLoadDirect() && !profilingValue->getOpCode().isLoadConst()))
         ...
         {
         trprintf(trace, log, "\t\t\tNode n%dn needs to be stored on temp slot\n", profilingValue->getGlobalIndex());
   ...
   trprintf(trace, log, "\t\t\tHelper call in block_%d\n", helper->getNumber());
   TR::NodeChecklist checklist(comp);
   checklist.add(value);
