...
J9::Simplifier::isLegalToUnaryCancel(TR::Node *node, TR::Node *firstChild, TR::ILOpCodes opcode)
   ...
   if (node->getOpCode().isConversionWithFraction() &&
      ...
      trprintf(trace(), log, "disallow unaryCancel of node %p and firstChild %p due to mismatch of decimal fractions (%d != %d)\n",
   ...
   if (firstChild->getOpCodeValue() == opcode &&
      ...
      trprintf(trace(), log, "disallow unaryCancel of node %p and firstChild %p due to intermediate truncation of node\n", node, firstChild);
      return false;
      }
