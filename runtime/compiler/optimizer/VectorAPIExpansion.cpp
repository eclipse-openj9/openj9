...
TR_VectorAPIExpansion::alias(TR::Node *node1, TR::Node *node2, bool aliasTemps)
   ...
   trprintf(_trace, comp()->log(), "%s aliasing symref #%d to symref #%d (nodes %p %p) for the whole class\n", OPT_DETAILS_VECTOR, id1, id2, node1, node2);
   ...
   if (aliasTemps)
      ...
      trprintf(_trace, comp()->log(), "%s aliasing symref #%d to symref #%d (nodes %p %p) as temps\n", OPT_DETAILS_VECTOR, id1, id2, node1, node2);
...
TR_VectorAPIExpansion::buildVectorAliases(bool verifyMode)
   {
   trprintf(_trace, comp()->log(), "%s Aliasing symrefs verifyMode=%d\n", OPT_DETAILS_VECTOR, verifyMode);
   ...
   if ((opCodeValue == TR::astore || opCodeValue == TR::astorei) &&
      ...
      if (rhs->getOpCode().hasSymbolReference())
         ...
         if (opCodeValue == TR::astore &&
            ...
            if ((_aliasTable[id1]._elementType != TR::NoType && _aliasTable[id1]._elementType != elementType) ||
               ...
               if (boxingAllowed())
                  ...
                  {
                  trprintf(_trace, log, "Invalidating1 #%d due to rhs %p in node %p\n", id1, rhs, node);
            ...
            trprintf(_trace, log, "Making #%d a box of unknown type due to node %p\n", id1, node);
            }
         ...
         if (boxingAllowed())
            ...
            {
            trprintf(_trace, log, "Invalidating2 #%d due to rhs %p in node %p\n", id1, rhs, node);
            invalidateSymRef(node->getSymbolReference());
            }
         }
      }
      ...
      for (int32_t i = 0; i < numChildren; i++)
         ...
         if (!isVectorAPICall ||
            ...
            if (!hasSymbolReference &&
               ...
               if (boxingAllowed())
                  ...
                  {
                  trprintf(_trace, log, "Invalidating3 #%d due to child %d (%p) in node %p\n",
         ...
         if (!isVectorAPICall)
            ...
            trprintf(_trace, log, "Invalidating4 #%d since it's not a vector API method in node %p\n",
      ...
      if (methodElementType == TR::NoType ||
         ...
         if (boxingAllowed())
            ...
            {
            trprintf(_trace, log, "Invalidating5 #%d (isVectorAPICall=%d) due to unknown elementType=%d or numLanes=%d in node %p\n",
         ...
         if (!canVectorize)
            {
            trprintf(_trace, log, "Can't vectorize #%d due to unsupported opcode in node %p\n",
               node->getSymbolReference()->getReferenceNumber(), node);
            ...
            if (!canScalarize)
               ...
               if (boxingAllowed())
                  ...
                  {
                  trprintf(_trace, log, "Invalidating6 #%d due to unsupported opcode in node %p\n",
                     node->getSymbolReference()->getReferenceNumber(), node);
                  invalidateSymRef(node->getSymbolReference());
                  }
               }
            ...
            {
            trprintf(_trace, log, "Can't scalarize #%d due to unsupported opcode in node %p\n",
               node->getSymbolReference()->getReferenceNumber(), node);
            _aliasTable[methodRefNum]._cantScalarize = true;
            }
         }
         ...
         if (isVectorAPICall)
            {
            trprintf(_trace, log, "Parsed intrinsic call node %p: elementType=%s vecLen=%d objectType=%s\n",
                  node,
      ...
      if (boxingAllowed())
         ...
         {
         trprintf(_trace, log, "Invalidating7 #%d due to its adress used by loadaddr node %p\n", node->getSymbolReference()->getReferenceNumber(), node);
         invalidateSymRef(node->getSymbolReference());
         }
      }
      ...
      if (child->getOpCode().hasSymbolReference())
         ...
         if (boxingAllowed())
            ...
            {
            trprintf(_trace, log, "Invalidating8 #%d due to its address used by %p\n",
      ...
      if (child->getOpCode().hasSymbolReference())
         ...
         if (!boxingAllowed())
            {
            trprintf(_trace, log, "Invalidating9 #%d due to its address used by %p\n",
               child->getSymbolReference()->getReferenceNumber(), node);
            invalidateSymRef(child->getSymbolReference());
            }
         }
      ...
      for (int32_t i = 0; i < node->getNumChildren(); i++)
         ...
         if (child->getOpCode().hasSymbolReference() &&
            ...
            if (boxingAllowed())
               {
               trprintf(_trace, log, "Making #%d boxed since it's used by unsupported node %p (%s)\n",
                  child->getSymbolReference()->getReferenceNumber(), node,
               ...
               {
               trprintf(_trace, log, "Invalidating10 #%d since it's used by unsupported node %p (%s)\n",
                  child->getSymbolReference()->getReferenceNumber(), node,
                  node->getOpCode().getName());
               invalidateSymRef(child->getSymbolReference());
               }
   ...
   while (bvi.hasMoreElements())
      ...
      if (_aliasTable[i].*classField == -1)
         ...
         if (boxingAllowed())
            ...
            {
            trprintf(_trace, log, "Invalidating11 %s class #%d since #%d is already invalid\n", tempAliases ? "temp" : "whole", classId, i);
      ...
      if (_aliasTable[i].*classField != -1 || i != classId)
         {
         trprintf(_trace, log, "Set %s class #%d for symref #%d\n", tempAliases ? "temp" : "whole", classId, i);
...
TR_VectorAPIExpansion::buildAliasClasses()
   {
   trprintf(_trace, comp()->log(), "%s Building alias classes\n", OPT_DETAILS_VECTOR);
   ...
   trprintf(_trace, comp()->log(), "%s Building temp alias classes\n", OPT_DETAILS_VECTOR);
...
TR_VectorAPIExpansion::getObjectTypeFromClassNode(TR::Compilation *comp, TR::Node *classNode)
   ...
   if (!clazz)
      {
      trprintf(_trace, comp->log(), "Could not get OpaqueClassBlock from node %p\n", classNode);
      return Unknown;
      }
   ...
   if ((*_boxingClasses[objectType - 1])[vectorLength - 1] == NULL)
    ...
    if ((*(*_boxingClasses[objectType - 1])[vectorLength - 1])[elementType - 1] == NULL)
       {
       trprintf(_trace, comp->log(), "Caching class for boxing: %d %d %d", objectType, vectorLength, elementType);
...
TR_VectorAPIExpansion::findVectorMethods(TR::Compilation *comp, bool reportFoundMethods)
   ...
   trprintf(trace, log, "%s in findVectorMethods\n", OPT_DETAILS_VECTOR);
      ...
      if (opCode.isFunctionCall())
         ...
         if (isVectorAPIMethod(methodSymbol, reportFoundMethods))
            ...
            if (reportFoundMethods &&
               ...
               {
               trprintf(trace, log, "%s found Vector API method\n", OPT_DETAILS_VECTOR);
               return true;
               }
            }
         }
...
TR_VectorAPIExpansion::validateSymRef(int32_t id, int32_t i, vec_sz_t &classLength, TR::DataType &classType,
   ...
   if (_aliasTable[i].*classField == -1)
      {
      trprintf(_trace, log, "%s invalidating12 class #%d due to symref #%d\n", OPT_DETAILS_VECTOR, id, i);
      return false;
      }
      ...
      trprintf(_trace, log, "%s invalidating13 class #%d due to symref #%d\n", OPT_DETAILS_VECTOR, id, i);
      return false;
      }
      ...
      if (!isVectorAPIMethod(symRef->getSymbol()->castToMethodSymbol()))
         ...
         if (boxingAllowed())
            ...
            {
            trprintf(_trace, log, "%s Invalidating14 class #%d due to non-API method #%d\n", OPT_DETAILS_VECTOR, id, i);
            return false;
            }
         }
      }
      ...
      if (tempLength == vec_len_boxed_unknown)
         ...
         trprintf(_trace, log, "%s making temp class #%d boxed due to symref #%d\n", OPT_DETAILS_VECTOR, id, i);
         }
      else if (classLength == vec_len_default)
         {
         trprintf(_trace, log, "%s assigning length to temp class #%d from symref #%d of length %d\n",
         ...
         {
         trprintf(_trace, log, "%s invalidating15 class #%d due to symref #%d temp length %d, class length %d\n",
      ...
      if (tempLength == vec_len_boxed_unknown)
         ...
         {
         trprintf(_trace, log, "%s assigning element type to temp class #%d from symref #%d of type %s\n",
         ...
         classType = tempType;
               ...
               tempType != classType)
         {
         trprintf(_trace, log, "%s invalidating16 class #%d due to symref #%d temp type %s, class type %s\n",
...
TR_VectorAPIExpansion::validateVectorAliasClasses(TR_BitVector *vectorAliasTableElement::* aliasesField,
   ...
   for (int32_t id = 1; id < symRefCount; id++)
      ...
      if ((_aliasTable[id].*classField) == -1)
         {
         trprintf(_trace, log, "%s class #%d is already invalid\n", tempClasses ? "temp" : "whole", id);
         continue;
         }
      ...
      if (!(_aliasTable[id].*aliasesField))
         ...
         trprintf(_trace, log, "   Validating #%d: %s\n", id, vectorClass ? "OK" : "X");
         }
      else
         {
         ...
         while (bvi.hasMoreElements())
            ...
            if (!vectorClass)
               {
               trprintf(_trace, log, "Class #%d can't be vectorized or scalarized due to invalid symRef #%d\n", id, i);
               break;
               }
            ...
            if (_aliasTable[i]._objectType == Invalid &&
               ...
               {
               trprintf(_trace, log, "Class #%d can't be vectorized or scalarized due to invalid object type of #%d\n", id, i);
            ...
            if (_aliasTable[i]._cantVectorize)
               {
               trprintf(_trace, log, "Class #%d can't be vectorized due to #%d\n", id, i);
            ...
            if (_aliasTable[i]._cantScalarize)
               {
               trprintf(_trace, log, "Class #%d can't be scalarized due to #%d\n", id, i);
               _aliasTable[id]._cantScalarize = true;
      ...
      trprintf(_trace, log, "Setting length and type for %s class #%d to %d and %s\n", tempClasses ? "temp" : "whole",
      ...
      if (tempClasses && wholeClass >= 0)
         ...
         if (!boxingAllowed())
            ...
            trprintf(_trace, log, "Invalidating18 whole class #%d due to temp class #%d\n", wholeClass, id);
...
TR_VectorAPIExpansion::isVectorizedOrScalarizedNode(TR::Node *node, TR::DataType &elementType, vec_sz_t &bitsLength,
   ...
   trprintf(_trace, comp()->log(), "#%d bitsLength=%d\n", refId, bitsLength);
...
TR_VectorAPIExpansion::getClassForBoxing(TR::Node *node, TR::DataType elementType, vec_sz_t bitsLength, vapiObjType objectType)
   ...
   trprintf(_trace, comp()->log(), "Getting class for boxing: %d %d %d\n", objectType, vectorLength, elementType);
...
TR_VectorAPIExpansion::boxChild(TR::TreeTop *treeTop, TR::Node *node, uint32_t i, bool checkBoxing)
   ...
   if (boxingSupported)
      ...
      if (!vecClass)
         {
         trprintf(_trace, comp()->log(), "Missing class for boxing of %d child of node %p\n", i, node);
   ...
   if (!boxingSupported ||
      ...
      trprintf(_trace, comp()->log(), "Invalidated class #%d due to unsupported boxing of %d child of node %p\n", classId, i, node);
   ...
   trprintf(_trace, comp()->log(), "Boxed %s%d%s child %d of node %p into %p\n",
...
TR_VectorAPIExpansion::unboxNode(TR::Node *parentNode, TR::Node *operand, vapiObjType operandObjectType,
   ...
   if (unboxingSupported)
      ...
      if (!vecClass)
         {
         trprintf(_trace, comp()->log(), "Missing class for unboxing of operand %p of node %p\n", operand, parentNode);
   ...
   if (!unboxingSupported)
      ...
      trprintf(_trace, comp()->log(), "Invalidated class #%d due to unsupported unboxing of operand %p of node %p\n",
   ...
   trprintf(_trace, comp()->log(), "Unboxed %s%d%s node %p into new node %p for parent %p\n",
...
TR_VectorAPIExpansion::expandVectorAPI()
   ...
   trprintf(_trace, comp()->log(), "%s In expandVectorAPI\n", OPT_DETAILS_VECTOR);
...
TR_VectorAPIExpansion::transformIL(bool checkBoxing)
   {
   TR::Logger *log = comp()->log();
   trprintf(_trace, log, "%s Starting Expansion checkBoxing=%d\n", OPT_DETAILS_VECTOR, checkBoxing);
   ...
   for (TR::TreeTop *treeTop = comp()->getMethodSymbol()->getFirstTreeTop(); treeTop ; treeTop = treeTop->getNextTreeTop())
      ...
      trprintf(_trace, comp()->log(), "Node %p (%s) vectorizedOrScalarized=%d elementType=%d bitsLength=%d objectType=%d scalarized=%d\n",
                ...
                for (int i = 0; i < 2; i++)
                   ...
                   if (operandVectorizedOrScalarized &&
                      ...
                      if (canVectorizeMethod)
                         ...
                         trprintf(_trace, log, "Vectorized node %p based on operand %d\n", node, i);
      ...
      if (boxingAllowed() &&
         ...
         {
         trprintf(_trace, log, "Checking if children of non-vector node %p need to be boxed\n", node);
      ...
      trprintf(_trace, log, "#%d classId = %d\n", symRefId, classId);

      if (classId <= 0)
         continue;

      trprintf(_trace, log, "#%d classId._classId = %d\n", symRefId, _aliasTable[classId]._classId);
      ...
      if (opCodeValue == TR::astore)
         {
         trprintf(_trace, log, "%s astore %p (temp class #%d) elementType=%d vectorLength=%d objectType=%s\n",
         ...
         if (boxingAllowed())
            ...
            trprintf(_trace, log, "Checking if children of vectorized node %p need to be unboxed\n", node);
...
TR_VectorAPIExpansion::vectorizeLoadOrStore(TR_VectorAPIExpansion *opt, TR::Node *node,
   ...
   if (vecSymRef == NULL)
      ...
      trprintf(opt->_trace, comp->log(), "   created new vector symRef #%d for #%d\n", vecSymRef->getReferenceNumber(), symRef->getReferenceNumber());
...
TR_VectorAPIExpansion::addScalarNode(TR_VectorAPIExpansion *opt, TR::Node *node, int32_t numLanes, int32_t i, TR::Node *scalarNode)
   ...
   trprintf(opt->_trace, comp->log(), "Adding new scalar node %p (lane %d) for node %p\n", scalarNode, i, node);
...
TR::Node *TR_VectorAPIExpansion::loadIntrinsicHandler(TR_VectorAPIExpansion *opt, TR::TreeTop *treeTop,
   ...
   if (mode == checkScalarization)
      ...
      if (objectType == Vector)
         ...
         trprintf(opt->_trace, log, "Vector load with numLanes %d in node %p\n", numLanes, node);
         ...
         {
         trprintf(opt->_trace, log, "Mask load with numLanes %d in node %p\n", numLanes, node);
   ...
   trprintf(opt->_trace, log, "loadIntrinsicHandler for node %p\n", node);
...
TR::Node *TR_VectorAPIExpansion::storeIntrinsicHandler(TR_VectorAPIExpansion *opt, TR::TreeTop *treeTop, TR::Node *node,
   ...
   if (mode == checkScalarization)
      ...
      if (objectType == Vector)
         ...
         {
         trprintf(opt->_trace, comp->log(), "Mask store with numLanes %d in node %p\n", numLanes, node);
   ...
   trprintf(opt->_trace, comp->log(), "storeIntrinsicHandler for node %p\n", node);
...
TR_VectorAPIExpansion::getConvertSourceType(TR_VectorAPIExpansion *opt, TR::Node *node,
   ...
   if (sourceNumLanesNode->getOpCode().isLoadConst())
      ...
      if (supportedOnPlatform(comp, bitsLength) == TR::NoVectorLength)
         {
         trprintf(opt->_trace, comp->log(), "Platform does not support conversion source length %d in node %p\n", bitsLength, node);
         return false;
         }
      }
   ...
   if (sourceElementType == TR::NoType || bitsLength == vec_len_default)
      {
      trprintf(opt->_trace, comp->log(), "Unknown conversion source type in node %p\n", node);
      return false;
      }
...
TR::Node *TR_VectorAPIExpansion::naryIntrinsicHandler(TR_VectorAPIExpansion *opt, TR::TreeTop *treeTop, TR::Node *node,
   ...
   if (opCodeType != Blend)
      ...
      if (!opcodeNode->getOpCode().isLoadConst())
         {
         trprintf(opt->_trace, log, "Unknown opcode in node %p\n", node);
         return NULL;
         }
   ...
   if (mode == checkScalarization || mode == doScalarization)
      ...
      if (mode == checkScalarization)
         ...
         if (scalarOpCode == TR::BadILOp)
            {
            trprintf(opt->_trace, log, "Unsupported scalar opcode in node %p\n", node);
            return NULL;
            }
         else
            {
      ...
      if (mode == checkVectorization)
         ...
         if (vectorOpCode == TR::BadILOp || !isOpCodeImplemented(comp, vectorOpCode))
            {
            trprintf(opt->_trace, log, "Unsupported vector opcode in node %p %s\n", node,
         ...
         if (opCodeType == BroadcastInt)
            ...
            if (!isOpCodeImplemented(comp, splatsOpCode))
               {
               trprintf(opt->_trace, log, "Unsupported vsplats opcode in node %p (no codegen)\n", node);
               return NULL;
               }
            }
            ...
            if (!isOpCodeImplemented(comp, splatsOpCode) ||
               ...
               {
               trprintf(opt->_trace, log, "Unsupported vsplats or vsub opcode in node %p (no codegen)\n", node);
               return NULL;
               }
            }
...
TR::Node *TR_VectorAPIExpansion::fromBitsCoercedIntrinsicHandler(TR_VectorAPIExpansion *opt, TR::TreeTop *treeTop, TR::Node *node,
   ...
   if (!broadcastTypeNode->getOpCode().isLoadConst())
      {
      trprintf(opt->_trace, comp->log(), "Unknown broadcast type in node %p\n", node);
      return NULL;
      }
   ...
   trprintf(opt->_trace, comp->log(), "fromBitsCoercedIntrinsicHandler for node %p\n", node);
