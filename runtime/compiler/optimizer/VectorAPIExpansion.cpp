...
TR::Node *TR_VectorAPIExpansion::fromBitsCoercedIntrinsicHandler(TR_VectorAPIExpansion *opt, TR::TreeTop *treeTop, TR::Node *node,
   ...
   if (objectType == Mask)
      {
      // Currently, library only passes -1 or 0 to mask broadcast and it might be the only mode implemented/tested
      // in codegen evaluators
      TR_ASSERT_FATAL(valueToBroadcast->getOpCode().isLoadConst() &&
                      (valueToBroadcast->get64bitIntegralValue() == -1 ||
                       valueToBroadcast->get64bitIntegralValue() == 0), "Mask broadcast has non-constant value\n");
