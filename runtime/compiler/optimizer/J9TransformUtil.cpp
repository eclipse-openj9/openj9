...
static void *dereferenceStructPointerChain(void *baseStruct, TR::Node *baseNode, bool isBaseStableArray, TR::Node *curNode, TR::Compilation *comp)
   ...
   if (baseNode == curNode)
      ...
      trprintf(trace, log, "Caller has already dereferenced node %p, returning NULL as dereferenceStructPointerChain has no idea what to dereference\n", curNode);
      return NULL;
      }
   else
      {
         ...
         if (verifyFieldAccess((void*)curStruct, symRef, isBaseStableArray, comp))
            ...
            if (isJavaField(symRef, comp))
               ...
               if (offset < minOffset ||
                   offset >= maxOffset)
                  {
                  trprintf(trace, log, "Offset %d is out of bound [%d, %d] for %s on array shadow %p!\n", offset, minOffset, maxOffset, symRef->getName(comp->getDebug()), curNode);
                  return NULL;
                  }
            ...
            {
            trprintf(trace, log, "Unable to verify field access to %s on %p!\n", symRef->getName(comp->getDebug()), curNode);
            return NULL;
            }
         }
      else
...
static void *dereferenceStructPointer(TR::KnownObjectTable::Index baseKnownObject,
   ...
   if (node == baseExpression)
      ...
      trprintf(comp->getOption(TR_TraceOptDetails), comp->log(),
...
static TR_YesNoMaybe safeToAddFearPointAt(TR::Optimization* opt, TR::TreeTop* tt)
   ...
   trprintf(trace, log, "Checking if it is safe to add fear point at n%dn\n", tt->getNode()->getGlobalIndex());
   ...
   if (!cannotAttemptOSRDuring(comp, callerIndex) && !comp->isOSRProhibitedOverRangeOfTrees())
      {
      trprints(trace, log, "Safe to add fear point because there is no OSR prohibition\n");
      return TR_yes;
      }
   ...
   while (tt != firstTT)
      ...
      if (comp->isPotentialOSRPoint(tt->getNode()))
         ...
         trprintf(trace, log, "Found %s potential OSR point n%dn, %s to add fear point\n",
   ...
   if (guardAnalysis)
      ...
      trprintf(trace, log, "%s to add fear point based on HCRGuardAnalysis\n", result == TR_yes ? "Safe" : "Not safe");

      return result;
      }

   trprintf(trace, log, "Cannot determine if it is safe to add fear point at n%dn\n", tt->getNode()->getGlobalIndex());
...
bool J9::TransformUtil::attemptStaticFinalFieldFoldingImpl(TR::Optimization* opt, TR::TreeTop * currentTree, TR::Node *node, bool varHandleOnly)
   ...
   if (J9::TransformUtil::foldReliableStaticFinalField(comp, node))
      {
      trprintf(trace, log, "SFFF fold reliable at node %p\n", node);
      return true;
      }
   ...
   trprintf(trace, log, "Looking at static final field n%dn %.*s declared in class %p\n",
   ...
   if (!varHandleOnly || isVarHandleFolding(comp, declaringClass, fieldSignature, fieldSigLength))
      ...
      if (J9::TransformUtil::foldStaticFinalFieldAssumingProtection(comp, node))
         ...
         trprintf(trace, log, "Static final field n%dn is folded with OSRFearPointHelper call tree n%dn  helper tree n%dn\n",
...
J9::TransformUtil::transformIndirectLoadChainImpl(TR::Compilation *comp,
   ...
   if (!symRef->isUnresolved()
      ...
      if (isServer)
         ...
         trprintf(trace, log, "Looking at node %p with initializeStatusFromClassSymbol, class %p initialize status is %d\n",
   ...
   if (!isBaseStableArray && !fej9->canDereferenceAtCompileTime(symRef, comp))
      {
      trprints(trace, log, "Abort transformIndirectLoadChain - cannot dereference at compile time!\n");
      return false;
      }
   ...
   if (isServer)
      ...
      if (!fieldAddress)
         {
         trprintf(trace, log, "Abort transformIndirectLoadChain - cannot verify/dereference field access to %s in %p!\n",
...
J9::TransformUtil::createTempsForCall(TR::Optimization* opt, TR::TreeTop *callTree)
   ...
   trprintf(trace, log, "Creating temps for children of call node %p\n", callNode);
   for (int32_t i = 0 ; i < callNode->getNumChildren() ; ++i)
      ...
      trprintf(trace, log, "Creating store node %p for child %p\n", storeNode, child);
      ...
      trprintf(trace, log, "Replacing call node %p child %p with %p\n", callNode, callNode->getChild(i), value);
...
J9::TransformUtil::createDiamondForCall(TR::Optimization* opt, TR::TreeTop *callTree, TR::TreeTop *compareTree, TR::TreeTop *ifTree, TR::TreeTop *elseTree, bool changeBlockExtensions, bool markCold)
   ...
   trprintf(trace, log, "Creating diamond for call tree %p with compare tree %p if tree %p and else tree %p\n", callTree, compareTree, ifTree, elseTree);
   ...
   if(callNode->getReferenceCount() > 1)
      {
      trprintf(trace, log, "Creating temps for call node %p before generating the diamond\n", callNode);
   ...
   if (newSymbolReference)
      ...
      trprintf(trace, log, "Two store nodes %p and %p are inserted in the diamond\n", ifStoreNode, elseStoreNode);
      }
   }
...
void J9::TransformUtil::separateNullCheck(TR::Compilation* comp, TR::TreeTop* tree, bool trace)
   ...
   trprintf(trace, comp->log(), "separating null check on n%un from n%un\n",
...
J9::TransformUtil::refineMethodHandleInvokeBasic(TR::Compilation* comp, TR::TreeTop* treetop, TR::Node* node, TR::KnownObjectTable::Index mhIndex, bool trace)
   ...
   if (!comp->fej9()->isResolvedDirectDispatchGuaranteed(comp))
      {
      trprintf(trace, log, "Cannot refine invokeBasic n%un %p without isResolvedDirectDispatchGuaranteed()\n",
         node->getGlobalIndex(),
         node);
   ...
   if (mhIndex == TR::KnownObjectTable::UNKNOWN ||
       ...
       knot->isNull(mhIndex))
      {
      trprintf(trace, log, "MethodHandle for invokeBasic n%dn %p is unknown or null\n", node->getGlobalIndex(), node);
      return false;
      }
...
J9::TransformUtil::refineMethodHandleLinkTo(TR::Compilation* comp, TR::TreeTop* treetop, TR::Node* node, TR::KnownObjectTable::Index mnIndex, bool trace)
   ...
   if (missingResolvedDispatch != NULL)
      ...
      trprintf(trace, log, "Cannot refine %s without isResolved%sDispatchGuaranteed()\n",
            nodeStr,
   ...
   if (mnIndex == TR::KnownObjectTable::UNKNOWN ||
       ...
       knot->isNull(mnIndex))
      {
      trprintf(trace, log, "%s: MemberName is unknown or null\n", nodeStr);
      return false;
      }
   ...
   if (!fej9->getMemberNameMethodInfo(comp, mnIndex, &info) || info.vmtarget == NULL)
      {
      trprintf(trace, log, "%s: Failed to get MemberName method info\n", nodeStr);
      return false;
      }
   ...
   if (rm == TR::java_lang_invoke_MethodHandle_linkToVirtual)
      ...
      if (info.refKind != MH_REF_INVOKEVIRTUAL)
         {
         trprintf(trace, log, "%s: wrong MemberName kind %d\n", nodeStr, info.refKind);
         return false;
         }
