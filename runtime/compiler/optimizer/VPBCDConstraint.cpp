...
TR::VP_BCDSign *TR::VP_BCDSign::create(OMR::ValuePropagation *vp, TR_BCDSignConstraint type, TR::DataType dt)
   ...
   if (constraint)
      {
      trprintf(vp->trace(), log, "return existing BCD sign constraint %p (dt=%s, sign=%s)\n", constraint, TR::DataType::getName(dt), constraint->getName());
      return constraint;
      }
   ...
   trprintf(vp->trace(), log, "created new BCD sign constraint %p (dt=%s, sign=%s)\n", constraint, TR::DataType::getName(dt), constraint->getName());
...
TR::VPConstraint *TR::VP_BCDSign::merge1(TR::VPConstraint *other, OMR::ValuePropagation *vp)
   ...
   if (asBCDSign() && other->asBCDSign())
      ...
      trprintf(vp->trace(), log, "\tthisSign %s thisType %s, otherSign %s otherType %s\n",
   ...
   trprints(vp->trace(), log, "\tcannot merge BCDSign constraints -- return NULL\n\n");

   return NULL;
   }

void TR::VP_BCDSign::print(TR::Logger *log, TR::Compilation *comp)
   {
   log->printf("%s (%s)", getName(), TR::DataType::getName(getDataType()));
   }

const char *TR::VP_BCDSign::name()
   {
   return "BCDSign";
   }
