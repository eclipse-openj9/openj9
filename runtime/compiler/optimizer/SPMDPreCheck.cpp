...
bool SPMDPreCheck::isSPMDCandidate(TR::Compilation *comp, TR_RegionStructure *loop)
   ...
   if (!loop->isNaturalLoop())
      {
      trprintf(trace, log, "SPMD PRE-CHECK FAILURE: region %d is not a natural loop and is discounted as an SPMD candidate\n", loop->getNumber());
      }
         ...
         switch (node->getOpCodeValue())
          ...
          if (opcode.isStore())
             ...
             if (vectorOp == TR::BadILOp)
                {
                trprintf(trace, log, "SPMD PRE-CHECK FAILURE: store op code %s does not have a vector equivalent - skipping consideration of loop %d\n",
                   comp->getDebug()->getName(opcode.getOpCodeValue()), loop->getNumber());
                return false;
                }
             if (!comp->cg()->getSupportsOpCodeForAutoSIMD(vectorOp))
                {
                trprintf(trace, log, "SPMD PRE-CHECK FAILURE: vector op code %s is not supported on the current platform - skipping consideration of loop %d\n",
          ...
          trprintf(trace, log, "SPMD PRE-CHECK FAILURE: found disallowed treetop opcode %s at node %p in loop %d\n",
