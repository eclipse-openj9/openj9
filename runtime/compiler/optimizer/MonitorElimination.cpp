...
class TR_ActiveMonitor
   ...
   void addPath(TR_MonitorPath *path)
      ...
      trprintf(trace(), comp()->log(), "Adding path [%p] in block_%d to monitor [%p]\n", path->getNode(), path->getBlock()->getNumber(), getMonitorNode());
      }
   ...
   void addPartialExitPath(TR_MonitorPath *path)
      ...
      trprintf(trace(), comp()->log(), "Adding partial exit path [%p] in block_%d to monitor [%p]\n", path->getNode(), path->getBlock()->getNumber(), getMonitorNode());
      }
   ...
   void exitMonitor(TR_ActiveMonitor *container)
      ...
      trprintf(trace(), comp()->log(), "Found all exits for monitor [%p]\n", getMonitorNode());
      }
...
TR::MonitorElimination::MonitorElimination(TR::OptimizationManager *manager)
   ...
   if(manager->comp()->getOption(TR_DebugRedundantMonitorElimination))
      ...
      trprintf(trace(), comp()->log(), "setting trace to true.  trace now returns %d\n",trace());
      }
...
int32_t TR::MonitorElimination::perform()
   ...
   if (comp()->getOption(TR_DisableMonitorOpts))
      {
      trprints(trace(), log, "Monitor optimizations explicitly disabled\n");
      return 0;
      }
   ...
   if (valueNumberInfo == NULL)
      {
      trprints(trace(), log, "Can't do Monitor Elimination, no value number information\n");
      return 0;
      }
   ...
   if (comp()->getOption(TR_EnableOSR) && disableOSRwithTM)
      {
      trprints(trace(), log, "Cannot remove redundant monitors: OSR disabled with TM\n");
      attemptRedundantMonitors = false;
      }

   if (comp()->getOSRMode() == TR::voluntaryOSR && comp()->getOption(TR_DisableLiveMonitorMetadata))
      {
      trprints(trace(), log, "Cannot remove redundant monitors: voluntary OSR requires the monitor object stores from live monitor metadata\n");
      attemptRedundantMonitors = false;
      }

   if (attemptRedundantMonitors && findRedundantMonitors())
      {
      trprints(trace(), log, "findRedundantMonitors returned true.  About to remove Redundant Monitors\n");
      ...
      if(!comp()->getOption(TR_DisableTLE) && comp()->cg()->getSupportsTLE())
         {
         trprints(trace(), log, "findRedundantMonitors returned true. about to check for TM candidates\n");

         if (evaluateMonitorsForTMCandidates())
            {
            trprintf(trace(), log, "evaluateMonitorsForTMCandidates returned true. firstPass = %d numPassesCompleted = %d\n", firstPass(), manager()->numPassesCompleted());
      ...
      trprintf(trace(), log, "Bad monitor structure found while compiling %s\n", comp()->signature());
      }
   ...
   while (!_monitorStack->isEmpty())
      ...
      for (treeTop = path->getTreeTop(); treeTop != exitTree; treeTop = treeTop->getNextTreeTop())
         ...
         if (node->getOpCode().isCall() || (node->getNumChildren() > 0 &&  node->getFirstChild()->getOpCode().isCall()))
            {
            trprintf(trace(), log, "Monitor node %p has a call at %p in its monitor region\n",monitor->getMonitorNode(),node);
            ...
            if ((!block->isCold() && block->getFrequency() >= minBlockCount) ||
               ...
               trprintf(trace(), log, "MJ Cannot eliminate block call Freq:%d, CCount:%d\n", block->getFrequency(), minBlockCount);
               }
            else
               {
               trprintf(trace(), log, "MJ Cold block, do not stop TLE Freq:%d, isCold:%d \n", block->getFrequency(), block->isCold());
               }
            }
         ...
         if (node->getOpCode().isStoreDirect() && node->getOpCode().hasSymbolReference() && node->getNumChildren() > 0)
            ...
            if (node->getSymbolReference()->holdsMonitoredObjectForSyncMethod())
               ...
               if (multipleTempsForMonitor)
                  {
                  syncMethodTemp = NULL;
                  trprintf(trace(), log, "Found another temp #%d holding monitored object for sync method, giving up\n", node->getSymbolReference()->getReferenceNumber());
                  }
               else
                  {
                  multipleTempsForMonitor = true;
                  syncMethodTemp = treeTop;
                  trprintf(trace(), log, "Found temp #%d holding monitored object for sync method\n", node->getSymbolReference()->getReferenceNumber());
                  }
               }
               ...
               trprintf(trace(), log, "Found temp #%d holding monitored object for monent [%p]\n",
         ...
         if (node->getOpCodeValue() == TR::monent)
            ...
            if (node->getVisitCount() == comp()->getVisitCount())
               {
               trprintf(trace(), log, "Monitor enter [%p] found on more than one container path\n", node);
               }
         ...
         if (node->getOpCodeValue() == TR::monexit)
            ...
            if (node->getVisitCount() == comp()->getVisitCount())
               {
               resetReadMonitors(_monitorStack->topIndex());
               trprintf(trace(), log, "Monitor exit [%p] found on more than one container path\n", node);
               }
            node->setVisitCount(comp()->getVisitCount());
            ...
            if (container == NULL)
               {
               trprintf(trace(), log, "Monitor exit [%p] found without a corresponding monitor enter\n", node);
               resetReadMonitors(_monitorStack->topIndex());
               return false;
               }

            trprintf(trace(), log, "Monitor exit found at [%p] for monitor [%p]\n", node, monitor->getMonitorNode());
...
bool TR::MonitorElimination::addPath(TR_ActiveMonitor *monitor, TR::Block *block)
   ...
   if (block->getEntry())
      ...
      if (monitor->isBlockSeenInContainers(block->getNumber()))
         {
         trprintf(trace(), comp()->log(), "Monitor enter [%p] loops back to containing monitor scope via block_%d\n", monitor->getMonitorNode(), block->getNumber());
         return false;
         }
...
bool TR::MonitorElimination::evaluateMonitorsForTMCandidates()
   ...
   for (monitor = it.getFirst(); monitor; monitor = it.getNext())
      ...
      if(monitor->_containsCalls)
         {
         trprintf(trace(), log, "TM: monitor at node %p contains calls. Not doing TM\n", monitor->getMonitorNode());
         continue;
         }
      ...
      if (monitor->_numberOfTreeTopsInsideMonitor <= minTTs)
         {
         trprintf(trace(), log, "TM: monitor at node %p only has %d TreeTops. Not doing TM\n", monitor->getMonitorNode(), minTTs);
         continue;
         }

      if(hasMultipleEntriesWithSameExit(monitor))
         {
         trprintf(trace(), log, "TM: monitor at node %p has multiple exits for a given entry (not supported yet. Not doing TM", monitor->getMonitorNode());
         continue;
         }
...
TR::SymbolReference* TR::MonitorElimination::createAndInsertTMRetryCounter(TR_ActiveMonitor *monitor)
   ...
   trprintf(trace(), comp()->log(), "Created tempSymRef (%p) for temporary\n",tempSymRef);
...
bool TR::MonitorElimination::searchDownForOtherMonitorsInSuccessors(TR::TreeTop *tt,int32_t &size, TR::list<TR::TreeTop *> &closeMonitors, int32_t maxDepth, int32_t minNumberOfNodes)
   ...
   trprintf(trace(), log, "Begun search down for monitors in successors  monexit at %p.\n Setting numberOfNodesAtDepth[%d] to %d\n", tt->getNode(), depth, size);
   ...
   do
      ...
      trprintf(trace(), log, "Considering block %d  depth %d\n", currentBlock->getNumber(), depth);
      ...
      if(searchDownForOtherMonitorsInCurrentBlock(currentBlock->getEntry(), currentBlockSize, monitorTT))
         ...
         {
         trprintf(trace(), log, "Setting numberOfNodesAtDepth[%d] to currentBlockSize %d\n", depth, currentBlockSize);
   ...
   for(et = treeIT.getCurrent() ; et ; et = treeIT.getNext())
      ...
      if(searchDownForOtherMonitorsInCurrentBlock(et,numberOfTTs,monitorTT))       // found the closest monent in the current block
         ...
         if (numberOfTTs < minNumberOfNodes )
            ...
            if (monitor)
               {
               trprintf(trace(), log, "Setting Active monitor with monitorNode %p to NOT a TM Candidate because it's too close"
         ...
         for(auto iterator = closeMonitors.begin(); iterator != closeMonitors.end(); ++iterator)
            ...
            if (monitor)
               {
               trprintf(trace(), log, "Setting Active monitor with monitorNode %p to NOT a TM Candidate because it's too close (in another block)"
                  " to previous TM Candidate with monexit %p\n", monitor->getMonitorNode(), et->getNode());
               monitor->setNotTMCandidate();
               }
            }
...
void TR::MonitorElimination::transformMonitorsIntoTMRegions()
   ...
   for (monitor = it.getFirst(); monitor; monitor = it.getNext())
      ...
      if (monitor->getMonitorTree()->getNextTreeTop()->getNode()->getOpCodeValue() == TR::BBEnd)
         {
         trprintf(trace(), log, "next treetop after monitor has node %p. monitorblock = %p. Not splitting\n",
         ...
         trprintf(trace(), log, "next treetop after monitor has node %p. critsectionblock = %d %p. splitting\n",
...
bool TR::MonitorElimination::markBlocksAtSameNestingLevel(TR_Structure *structure, TR_BitVector *blocksAtCurrentNestingLevel)
   ...
   if (structure->asBlock())
      ...
      if (isLoop)
         ...
         trprintf(trace(), comp()->log(), "Block numbered %d is loop entry\n", region->getEntry()->getNumber());
         }
...
//
void TR::MonitorElimination::collectPredsAndSuccs(TR::CFGNode *startNode, TR_BitVector *visitedNodes, TR_BitVector **predecessorInfo, TR_BitVector **successorInfo, List<TR::CFGEdge> *cfgBackEdges, TR_BitVector *loopEntryBlocks, TR::Compilation *comp, bool trace)
   {
               ...
               if (!(loopEntryBlocks->get(node->getNumber()) &&
                  ...
                  *subtraction -= *predecessorInfo[node->getNumber()];
                  ...
                  if (!loopEntryBlocks->get(node->getNumber()) &&
                     ...
                     {
                     trprintf(trace, comp->log(), "Node is %d\n", node->getNumber());
...
void TR::MonitorElimination::coarsenMonitorRanges()
   ...
   virtCall = true;
      ...
      if (virtCall)
         ...
         if (_monentBlockInfo[blockNum] == -2)
            {
            trprintf(trace(), comp()->log(), "Kill monent in block_%d cause %p\n", blockNum, node);
            _monentBlockInfo[blockNum] = -1;
            }

         if (_monexitBlockInfo[blockNum] > -1)
             {
             trprintf(trace(), comp()->log(), "Kill monexit in block_%d cause %p\n", blockNum, node);
      ...
      if (node->getOpCodeValue() == TR::monent)
         ...
         if (_monexitBlockInfo[blockNum] > -1)
             ...
             if (_monexitBlockInfo[blockNum] == lockedObjectValueNumber)
                ...
                if ((!containsCall) && performTransformation(comp(), "%s Success: Coarsening monexit %p locally in block_%d\n", OPT_DETAILS, currentMonexit->getNode(), blockNum))
                   ...
                   trprints(trace(), log, "Success: coarsen locally\n");
                ...
                {
                trprintf(trace(), log, "Kill monexit in block_%d cause %p\n", blockNum, node);
         ...
         if (_monentBlockInfo[blockNum] == -2)
            {
            trprintf(trace(), log, "Kill monent in block_%d cause %p\n", blockNum, node);
            _monentBlockInfo[blockNum] = -1;
            }
   ...
   collectPredsAndSuccs(cfg->getStart(), visitedNodes, _predecessorInfo, _successorInfo, &_cfgBackEdges, _loopEntryBlocks, comp(), trace());
   ...
   for (; treeTop; treeTop = treeTop->getNextTreeTop())
      ...
      if (node->getOpCodeValue() == TR::BBEnd)
        ...
        trprintf(trace(), log, "Coarsening: Considering node %p blockNum %d prevLockedObject = %d\n", node, blockNum, prevLockedObject);

        if (prevLockedObject > -1)
           {

           trprintf(trace(), log, "Coarsening: _multiplyLockedObjects->get(prevLockedObject) = %d _safeValueNumbers[prevLockedObject] = %d _coarsendMonexits->get(blockNum) = %d\n",
           ...
           if (_multiplyLockedObjects->get(prevLockedObject) &&
              ...
              {
              trprintf(trace(), log, "Try to coarsen monexit in block_%d\n", blockNum);
   ...
   if (!_specialBlockInfo.isEmpty())
      ...
      for (block = blocksIt.getFirst(); block != NULL; block = blocksIt.getNext())
         {
         trprintf(trace(), log, "Coarsening for special block_%d\n", block->_succBlock->getNumber());
...
void TR::MonitorElimination::coarsenMonitor(int32_t origBlockNum, int32_t prevLockedObject, TR::Node *prevMonitorNode)
   ...
   while (!_monexitWorkList->isEmpty())
      ...
      *_temp &= *_containsMonents;
      ...
      trprints(trace(), log, "Finished building closure\n");
      if (_matchingMonentBlocks->isEmpty())
         {
         trprints(trace(), log, "Returned matching monent\n");
         return;
         }

      if (_specialBlockInfo.getSize() > numSpecialBlocks)
         {
         trprints(trace(), log, "Special block exists\n");
         ...
         *_inclusiveIntersection |= *_matchingMonentBlocks;
         ...
         while (listIt.hasMoreElements() &&
            ...
            if (!nodesInList)
               {
               trprintf(trace(), log, "Cannot coarsen monexit in block_%d because of unknown catch block structure\n", nextListElement);
               return;
               }
            else
               {
            ...
            if (!nodesInList)
               {
               trprintf(trace(), log, "Cannot coarsen monexit in block_%d because succs of block_%d are NOT in list\n", origBlockNum, nextListElement);
               return;
               }
            }
         ...
         while (listIt.hasMoreElements())
            ...
            if (nodesInList)
               ...
               if (!nodesInList)
                  {
                  trprintf(trace(), log, "Cannot coarsen monexit in block_%d because succ test failed for block_%d\n", origBlockNum, nextListElement);
                  break;
                  }
               }
            else
               {
               trprintf(trace(), log, "Cannot coarsen monexit in block_%d because pred test failed for block_%d\n", origBlockNum, nextListElement);
               break;
               }
            }
         ...
         while (succs.hasMoreElements() &&
            ...
            if (nodesInList)
               ...
               {
               trprintf(trace(), log, "Cannot coarsen monexit in block_%d because preds of succ %d are NOT in list\n", origBlockNum, nextSucc);
               }

            if (!nodesInList)
               {
               trprintf(trace(), log, "Cannot coarsen monexit in block_%d because of matching monent in block_%d\n", origBlockNum, nextSucc);
               }
            }
         ...
         if (nodesInList)
            ...
            *_scratch &= *_guardedVirtualCallBlocks;
            ...
            while (scratchIt.hasMoreElements())
               ...
               trprintf(trace(), log, "Look at virtual call block %d\n", nextBlockNum);
               ...
               for (auto succEdge = nextBlock->getSuccessors().begin(); succEdge != nextBlock->getSuccessors().end(); ++succEdge)
                  {
                  trprintf(trace(), log, "Virtual call block %d and succ block %d\n", nextBlockNum, (*succEdge)->getTo()->getNumber());

                  if (!_inclusiveIntersection->get((*succEdge)->getTo()->getNumber()))
                     {
                     trprintf(trace(), log, "Reset virtual call block %d\n", nextBlockNum);
            ...
            *_scratch &= *_containsExceptions;
            ...
            while (succs.hasMoreElements())
               ...
               if (performTransformation(comp(), "%s Success: Coarsening monexit in block_%d\n", OPT_DETAILS, nextSucc))
                  ...
                  trprintf(trace(), log, "Success: Coarsening monexit in block_%d in %s\n", nextSucc, comp()->signature());
                  }
               }
            ...
            while (succs.hasMoreElements())
               ...
               if (performTransformation(comp(), "%s Success: Coarsening monent in block_%d\n", OPT_DETAILS, nextSucc))
                  ...
                  trprintf(trace(), log, "Success: Coarsening monent in block_%d in %s\n", nextSucc, comp()->signature());
                  }
               }
            }
         }
         ...
         *_subtraction -= *_loopEntryBlocks;
                  ...
                  if (succPostDominatesPred &&
                     ...
                     if (symbolsAreNotWritten(_intersection))
                        ...
                        if (symbolsAreNotWrittenInTrees(_monexitTrees[blockNum]->getNextTreeTop(), _blockInfo[blockNum]->getExit()) &&
                           ...
                           {
                           canRecover = true;
                           trprints(trace(), log, "Found a coarsening opportunity across loop\n");
                           }
                        }
                     }
                  }
               ...
               if (succPostDominatesPred &&
                  ...
                  if (recoveryPossibleInInterveningBlocks)
                      ...
                      if (symbolsAreNotWritten(_intersection) &&
                         ...
                         {
                         canRecover = true;
                         trprints(trace(), log, "Found a coarsening opportunity across call (peek done successfully)\n");
                         }
                      }
                  }
               }
         ...
         if ((simpleCoarsening && !_matchingSpecialBlocks->get(blockNum) && !_matchingSpecialBlocks->get(nextSucc)) ||
            ...
            if (!simpleCoarsening)
               ...
               trprints(trace(), log, "special block info added\n");
...
void TR::MonitorElimination::collectPredecessors(int32_t blockNum, TR_BitVector *monexitBlocks, TR_BitVector *predecessors, int32_t lockedObject)
   ...
   while (preds.hasMoreElements())
      ...
      trprintf(trace(), log, "Block number %d monexit status %d prevLockedObject %d\n", nextPred, _monexitBlockInfo[nextPred], lockedObject);
         ...
         *_subtraction -= *_loopEntryBlocks;
         ...
         if ((simpleCoarsening && !_matchingSpecialBlocks->get(blockNum) && !_matchingSpecialBlocks->get(nextPred)) ||
            ...
            if (!simpleCoarsening)
               ...
               trprints(trace(), log, "1special block info added\n");
               SpecialBlockInfo *specialBlockInfo = (SpecialBlockInfo *) trMemory()->allocateStackMemory(sizeof(SpecialBlockInfo));
...
void TR::MonitorElimination::adjustMonentAndMonexitBlocks(TR::Node *prevMonitorNode, TR_BitVector *blocks, int32_t lockedObject)
   ...
   while (bvi.hasMoreElements())
      ...
      trprintf(trace(), comp()->log(), "Adding monexit and monent in block_%d\n", nextBlockNum);
...
void TR::MonitorElimination::prependMonexitInBlock(TR::Node *prevMonitorNode, TR::Block *nextBlock, bool insertNullTest)
   {
   trprintf(trace(), comp()->log(), "Adding monexit in block_%d\n", nextBlock->getNumber());
...
void TR::MonitorElimination::appendMonentInBlock(TR::Node *prevMonitorNode, TR::Block *nextBlock, bool insertNullTest)
   {
   trprintf(trace(), comp()->log(), "Adding monent in block_%d\n", nextBlock->getNumber());
...
void TR::MonitorElimination::insertNullTestBeforeBlock(TR::Node *prevMonitorNode, TR::Block *nextBlock)
   {
   trprintf(trace(), comp()->log(), "Inserting null test before block_%d\n", nextBlock->getNumber());
...
//      _lastTreeTop = catchBlock->getExit();
      ...
      trprintf(trace(), comp()->log(), "Created catch block_%d(%p)\n", catchBlock->getNumber(), catchBlock);
      ...
      while (listIt.hasMoreElements())
         ...
         if (!block->getLastRealTreeTop()->getNode()->getOpCode().isReturn())
            ...
            trprintf(trace(), comp()->log(), "Added edge from block_%d to catch block_%d\n", block->getNumber(), catchBlock->getNumber());
        ...
        if (_guardedVirtualCallBlocks->get(toNum))
          ...
          _monexitEdges.add(*edge);
           ...
           if (block)
              ...
              {
              trprintf(trace(), comp()->log(), "1Tripped on succ %d\n", (*edge)->getTo()->getNumber());
   ...
   while (cursorTree != endTree)
      ...
      if (node->getOpCode().isCall() &&
         ...
         if (!ipaAnalyzer.analyzeCall(node))
            {
            recoveryPossible = false;
            trprintf(trace(), comp()->log(), "Recovery is NOT possible from call %p to method %s\n", node, resolvedMethod->signature(trMemory()));
            break;
            }
         else
            {
               ...
               while (bvi.hasMoreElements())
                  ...
                  if (symRef->getSymbol()->isStatic() ||
                     ...
                     if ((length == currLength) &&
                        ...
                        {
                        trprintf(trace(), comp()->log(), "Recovery is NOT possible from call %p to method %s due to written symbols\n", node, resolvedMethod->signature(trMemory()));
                        return false;
                        }
                     }
                  }
...
bool TR::CoarseningInterProceduralAnalyzer::analyzeNode(TR::Node *node, vcount_t visitCount, bool *success)
       ...
       if (!(symRef->getSymbol()->isStatic() &&
          ...
          if ((node->getOpCodeValue() == TR::loadaddr) &&
             ...
             if (!className)
                {
                *success = false;
                trprintf(trace(), log, "Found unresolved class object load %p while peeking and unable to add assumption -- peek unsuccessful\n", node);
                }
             else
                {
                addClassThatShouldNotBeLoaded(className, classNameLength);
                trprintf(trace(), log, "Found unresolved class object node %p while peeking -- add assumption -- skip peeking in rest of block\n", node);
             ...
             if (className)
                ...
                if (!declaringClazz)
                   ...
                   trprintf(trace(), log, "Found unresolved class object node %p while peeking -- skip peeking in rest of block\n", node);
                ...
                /*
                  ...
                  //
                   ...
                   {
                   *success = false;
                   trprintf(trace(), log, "Found unresolved node %p while peeking whose class in resolved -- peek unsuccessful\n", node);
                   }
                */
                }
             else
                {
                *success = false;
                trprintf(trace(), log, "Found unresolved node %p while peeking whose class is unresolved and unable to add assumption -- peek unsuccessful\n", node);
                }
             }
   ...
   if (node->getOpCodeValue() == TR::monent ||
       node->getOpCodeValue() == TR::monexit)
      {
      trprintf(trace(), log, "Found monitor node %p while peeking -- peek unsuccessful\n", node);
      *success = false;
      return false;
      }
