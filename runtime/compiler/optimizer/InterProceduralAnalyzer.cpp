...
List<OMR::RuntimeAssumption> *TR::InterProceduralAnalyzer::analyzeCallGraph(TR::Node *callNode, bool *success)
   ...
   if (_sniffDepth >= _maxSniffDepth)
      ...
      *success = false;
      ...
      trprints(trace(), log, "High sniff depth made peek unsuccessful\n");
   ...
   if (!resolvedMethodSymbol &&
      ...
      {
      *success = false;
      trprintf(trace(), log, "Unresolved non-interface call node %p made peek unsuccessful\n", callNode);
   ...
   if (!resolvedMethodSymbol)  // as per current logic, it is an interface method
      ...
      if (!clazz)
         ...
         if (!s)
            {
            *success = false;
            trprintf(trace(), log, "Found unresolved method call node %p while peeking whose class is unresolved and unable to add assumption -- peek unsuccessful\n", callNode);
            }
         else
            {
            addClassThatShouldNotBeLoaded(s, len);
            trprintf(trace(), log, "Found unresolved method call node %p while peeking -- add assumption\n", callNode);
            }
         return 0;
         }
      }
   ...
   if ((*success) &&
      ...
      if (clazz)
         ...
         if (!addClassThatShouldNotBeNewlyExtended(clazz))
            {
            trprints(trace(), log, "Could not add Class That should not be newly extended to assumptions list.\n");

            *success = false;
            return 0;
            }

         trprints(trace(), log, "Found class for this object -- add assumption that the class should not be newly extended\n");
         }
      ...
      if (classInfo)
         ...
         trprintf(trace(), log, "Number of subclasses = %d\n", subClasses.getSize());
...
List<OMR::RuntimeAssumption> *TR::InterProceduralAnalyzer::analyzeMethod(TR::Node *callNode, TR_ResolvedMethod *method, bool *success)
   ...
   trprintf(trace(), log, "Consider method %s for peek\n", method->signature(trMemory()));
   ...
   if (0 && alreadyPeekedMethod(method, &successfulPriorPeek, &priorPeek))
      ...
      if (!successfulPriorPeek)
         ...
         *success = false;
         ...
         trprintf(trace(), log, "Prior peek failure for method %s caused current peek to be unsuccessful\n", method->signature(trMemory()));
         }
      else
         {
         trprintf(trace(), log, "Prior peek success for method %s caused current peek to be successful -- prior assumptions added\n", method->signature(trMemory()));
    ...
    if (bytecodeSize > MAX_SNIFF_BYTECODE_SIZE)
      {
      *success = false;
      trprintf(trace(), log, "Large bytecode size %d made peek unsuccessful\n", bytecodeSize);
         ...
         if (argument->getDataType() == TR::Address)
            ...
            if (s && (c == firstArgIndex))
               ...
               if (!argClazz || !thisClazz)
                  {
                  *success = false;
                  trprints(trace(), log, "The call argument class is NULL, bailing out. (probably because of different class loaders)\n");
   ...
   for (treeTop = resolvedMethodSymbol->getFirstTreeTop(); treeTop; treeTop = treeTop->getNextTreeTop())
      ...
      if (!*success)
         {
         trprintf(trace(), log, "Node %p made peek unsuccessful\n", node);
         break;
         }
   ...
   if (0 && *success)
      ...
      trprintf(trace(), log, "Method %s is successfully peeked\n", resolvedMethodSymbol->getResolvedMethod()->signature(trMemory()));
      }
   else
      {
      if (_sniffDepth == 0)
         {
         trprintf(trace(), log, "1Method %s is unsuccessfully peeked\n", resolvedMethodSymbol->getResolvedMethod()->signature(trMemory()));
         ...
         if (!_maxSniffDepthExceeded)
            {
            trprintf(trace(), log, "2Method %s is unsuccessfully peeked\n", resolvedMethodSymbol->getResolvedMethod()->signature(trMemory()));
