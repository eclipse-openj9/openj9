...
ChangeAlignmentOfRegion(TR_CISCTransformer *trans)
   ...
   for (t = T->getEntryNode(); t->isOutsideOfLoop();)
      ...
      if (trans->analyzeT2P(t, pTop) & _T2P_MatchMask)
         ...
         if (!chk->isOutsideOfLoop())
            {
            trprintf(disptrace, log, "ChangeAlignmentOfRegion : (t:%d p:%d) no need to change alignment\n",t->getID(),pTop->getID());
            return changed;        // Find pTop! Already aligned correctly
            }
         }
      if (t->getNumSuccs() < 1)
         {
         trprintf(disptrace, log, "ChangeAlignmentOfRegion : #succs of tID:%d is 0\n", t->getID());
   ...
   trprintf(disptrace, log, "ChangeAlignmentOfRegion : tTop %d, pTop %d\n",t->getID(),pTop->getID());
   ...
   trprintf(disptrace, log, "ChangeAlignmentOfRegion : foundNode %d, lastNode %d\n",foundNode->getID(),lastNode->getID());
   if (branchCount > 0 &&
       !lastNode->getIlOpCode().isBranch())
      {
      trprints(disptrace, log, "Fail: there is a branch in the region. lastNode must be a branch node.\n");
      return changed;
      }
   for (t = firstNode; ;t = t->getSucc(0))
      {
   ...
   for (t = ri.getFirst(); t; t = ri.getNext()) // each node in the region r
      ...
      for (pn = pi.getFirst(); pn; pn = pi.getNext())   // each parent of t
         ...
         if (!r.isIncluded(pn))
            {
            trprintf(disptrace, log, "ChangeAlignmentOfRegion : There is a parent(%d) of %d in the outside of the region\n", pn->getID(), t->getID());
            return changed; // fail
            }
         }
      }
   ...
   for (t = ri.getFirst(); t; t = ri.getNext())
      ...
      if (t->getIlOpCode().isTreeTop())
         ...
         trprintf(disptrace, log, "add TR::Node 0x%p (tid:%d) to BeforeInsertionList.\n", rep, t->getID());
               ...
               if (trNode->getOpCode().isTreeTop())
                  ...
                  if (trNode->getOpCode().isStoreDirect())
                     ...
                     if (!generateCompensation0)
                        ...
                        trprintf(disptrace, comp->log(), "analyzeMoveNodeForward: append the tree of 0x%p into AfterInsertionIdiomList\n", trNode);
         ...
         if (maxPid >= 0)
            ...
            if (maxPid <= currentPID)
               ...
               if (tgt)
                  ...
                  trprintf(disptrace, log, "We can move the node %d to %p(%d)\n", tID, tgt, tgt->getID());
...
moveStoreOutOfLoopForward(TR_CISCTransformer *trans)
      ...
      for (tParent = tParentIter.getFirst(); tParent; tParent = tParentIter.getNext())
         ...
         if (tParent->isStoreDirect() &&
            ...
            if (!storedVariable) storedVariable = tParent->getChild(1);
               ...
               {
               trprints(disptrace, log, "moveStoreOutOfLoopForward failed because all variables of stores are not same.\n");
               success0 = false;
            ...
            if (checkSuccsSet(trans, tParent, &findBV))
               ...
               {
               trprints(disptrace, log, "moveStoreOutOfLoopForward failed because tParent will not reach either boolTable or optionalCmp.\n");
               success0 = false;
               goto endSpecial0;        // FAIL!
               }
            }
   ...
   if (targetList.isEmpty())
      {
      trprints(disptrace, log, "moveStoreOutOfLoopForward failed because targetList is empty.\n");
      success0 = false;
      }
   // check if descendants of p include an array load
   if (!getThreeNodesForArray(p, &ixload, &aload, &iload, true))
      {
      trprintf(disptrace, log, "moveStoreOutOfLoopForward failed because decendents of pid:%d don't include an array load.\n", p->getID());
      success0 = false;
      }
   ...
   if (success0)
      ...
      trprintf(disptrace, log, "moveStoreOutOfLoopForward: Target nodes ixload=%d, aload=%d, iload=%d\n",
      ...
      if (ixload && aload && iload && (iload->isLoadVarDirect() || iload->getOpcode() == TR_variable))
         ...
         trprintf(disptrace, log, "moveStoreOutOfLoopForward adds %d into compensation code [0] and [1]\n", t->getID());
...
indexContainsArrayAccess(TR::Compilation *comp, TR::Node *aXaddNode)
   ...
   bool trace = comp->trace(OMR::idiomRecognition);
   TR::Logger *log = comp->log();
   trprintf(trace, log, "axaddnode %p\n", aXaddNode);
   ...
   //
   trprintf(trace, log, "aXaddNode %p topLevelIndex %p\n", aXaddNode, topLevelIndex);
...
areArraysInvariant(TR::Compilation *comp, TR::Node *inputNode, TR::Node *outputNode, TR_CISCGraph *T)
   ...
   if (T)
      ...
      trprintf(trace, log, "aNode = %p bNode = %p\n", aNode, bNode);
         ...
         if (aCNode && bCNode)
            ...
            for (ch = aDefI.getFirst(); ch; ch = aDefI.getNext())
               ...
               if (ch->getDagID() == aCNode->getDagID())
                  {
                  trprintf(trace, log, "def %d found inside loop for %d\n", ch->getID(), aCNode->getID());
                  return false;
                  }
               }
...
areDefsOnlyInsideLoop(TR::Compilation *comp, TR_CISCTransformer *trans, TR::Node *outputNode)
   ...
   TR::Logger *log = trans->comp()->log();

   trprintf(extraTrace, log, "finding defs for index used in tree %p\n", outputNode);
      ...
      if (loadNode)
         ...
         if (!defs.IsZero())
            ...
            if (numDefs == 1)
               ...
               trprintf(extraTrace, log, "found single def %p for load %p\n", defTT->getNode(), loadNode);
...
checkForPostIncrement(TR::Compilation *comp, TR::Block *loopHeader, TR::Node *loopCmpNode, TR::Symbol *ivSym)
   ...
   trprintf(comp->trace(OMR::idiomRecognition), comp->log(), "found storeIvload %p cmpFirstChild %p\n", storeIvLoad, cmpFirstChild);
...
CISCTransform2FindBytes(TR_CISCTransformer *trans)
   ...
   if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace))
      {
      trprints(disptrace, log, "Bailing CISCTransform2FindBytes due to null TT - might be a preheader in last block of method\n");
      return false;
      }
   ...
   if (exitIfRep)
      ...
      if (exitIfRep != trans->getP2TInLoopIfSingle(P->getImportantNode(1)))
         {
         trprints(disptrace, log, "Give up because of multiple candidates of ificmpge.\n");
         return false;
         }
      bool isDecrement;
   ...
   if (!target) // multiple successors
      ...
      if (listT->isEmpty() ||
         ...
         {
         trprints(disptrace, log, "Currently, CISCTransform2FindBytes allows only the case where there is an ificmpge node and successor is 2.\n");
         return false;
         }
      }
   ...
   if (!isTRT2Char)
      ...
      if ((count = trans->analyzeByteBoolTable(pBoolTable, tmpTable, P->getImportantNode(1), &retSameExit)) <= 0)
         ...
         if (!go)
            {
            trprints(disptrace, log, "analyzeByteBoolTable failed.\n");
      ...
      if ((count = trans->analyzeCharBoolTable(pBoolTable, tmpTable, P->getImportantNode(1), &retSameExit)) <= 0)
         ...
         if (supportsSRSTU &&               // Confirm that the processor has the SRSTU instruction.
            ...
            {
            trprints(disptrace, log, "analyzeCharBoolTable failed.\n");
            return false;  // fail to analyze
            }
         }
      else
         ...
         if (!supportsSRSTU ||  // If we don't have SRSTU support, we can use SRST/TRT for single byte searches if delimiters are within 1-255.
            ...
            if (tmpTable[0])
               {
               trprints(disptrace, log, "Char array has '0' as an exit condition, loop will not be reduced TRT/SRST (single-byte) instruction.\n");
            ...
            for (int32_t i = 256; i < 65536; i++)
               ...
               if (tmpTable[i])
                  {
                  trprintf(disptrace, log, "Char array has one of 256 through 65535 (%d) as an exit condition, loop cannot be reduced to TRT/SRST (single-byte) instruction.\n", i);
   ...
   if (count != 0 && !retSameExit)    // there is a booltable check and all destinations of booltable are not same
      {
      trprints(disptrace, log, "Multiple targets for different delimiter checks detected.  Abandoning reduction.\n");
      return false;
      }
   ...
   if (retSameExit != NULL && !isNeedGenIcmpge && retSameExit->getEnclosingBlock() != target)
      {
      trprintf(disptrace, log,
         "Target for delimiter check (Treetop: %p / Block %d: %p) is different than loop exit block_%d: %p.  Abandoning reduction.\n",
   ...
   // FIXME: this test is needed because in the TRT2Byte
      ...
      if (indexRequiresAdjustment)
         {
         trprintf(disptrace, log, "headerConst node value doesn't equal contiguous array header size %p. Abandoning reduction.\n", ahConstNode);
         return false;
         }

   if (avoidTransformingStringLoops(comp))
      {
      trprints(disptrace, log, "Abandoning reduction because of functional problems when String compression is enabled in Java 8 SR5\n");
      return false;
      }
   ...
   if (count == -1)           // single delimiter which is not constant value
      ...
      trprintf(disptrace, log, "Single non-constant delimiter found.  Setting %p as tableNode.\n", comp->getDebug()->getName(tableCISCNode->getHeadOfTrNodeInfo()->_node));
      }
      ...
      trprintf(disptrace, log, "Single delimiter found.  Setting 'iconst %d' [%p] as tableNode.\n", i, comp->getDebug()->getName(tableNode));
      }
   else
      {
      ...
      if (comp->compileRelocatableCode())
         {
         trprints(disptrace, log, "Abandoning reduction since we can't relocate the static table\n");
         return false;
         }
   ...
   if (listT->isEmpty())
      ...
      {
      trprints(disptrace, log, "Loop has TR::ificmpge for comparing the index.\n");
      TR_CISCNode *lenNode;
   ...
   if (isNeedGenIcmpge)
      {
      trprints(disptrace, log, "Now assuming that all exits of booltable are identical and the exit of icmpge points different.\n");
      ...
      trprintf(disptrace, log, "Block: okDest=%d failDest=%d\n",
         okDest->getEnclosingBlock()->getNumber(), failDest->getEnclosingBlock()->getNumber());
   ...
   if (listT->isEmpty())        // no NULLCHK
      ...
      {
      trprints(disptrace, log, "NULLCHK is found!\n");
...
CISCTransform2NestedArrayFindBytes(TR_CISCTransformer *trans)
   ...
   if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace))
      {
      trprints(disptrace, log, "Bailing CISCTransform2NestedArrayFindBytes due to null TT - might be a preheader in last block of method\n");
      return false;
      }
   ...
   if (listT->isEmpty())
      ...
      {
      trprints(disptrace, log, "TR::ificmpge for comaring the index is found!\n");
      TR_CISCNode *lenNode;
      TR::Node *lenRepNode;
      if (listT->isSingleton())
         {
...
CISCTransform2CopyingTROx(TR_CISCTransformer *trans)
   ...
   if (!isOutputChar  && genSIMD && !genTRxx){
      trprints(disptrace, log, "Bailing CISCTransform2CopyingTROx : b2b - no proper evaluator available\n");
      return false;
   }
   ...
   if (isOutputChar)
      ...
      if (sx == TR_maybe)
         {
         trprints(disptrace, log, "Bailing CISCTransform2CopyingTROx : unknown integer conversion\n");
         return false;
         }
   ...
   if (additionHigh)
      ...
      if (additionHigh->getChild(0) != loadResult && additionHigh->getChild(1) != loadResult)
         {
         trprints(disptrace, log, "Bailing CISCTransform2CopyingTROx : inscrutable iadd\n");
         return false;
         }
      }
   ...
   */
   trprintf(disptrace, log, "Found graph pattern as %d\n", pattern);
   ...
   if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace))
      {
      trprints(disptrace, log, "Bailing CISCTransform2CopyingTROx due to null TT - might be a preheader in last block of method\n");
      return false;
      }
   ...
   if (trans->countGoodArrayIndex(indexVarSymRef) == 0)
      {
      trprintf(disptrace, log, "countGoodArrayIndex failed for %p\n", indexRepNode);
      return false;
      }
   ...
   if (!target) // multiple successors
      ...
      if (trans->getNumOfBBlistSucc() != 2)
         {
         trprintf(disptrace, log, "current restrictions. The number of successors is %d\n", trans->getNumOfBBlistSucc());
         return false;
         }
      }
   ...
   if (0 && boolTableExit)
      ...
      trprintf(disptrace, log, "boolTableNode : %p of loop %d\n", boolTableNode, block->getNumber());
   ...
   if (comp->cg()->getSupportsArrayTranslateTROTNoBreak()||comp->cg()->getSupportsArrayTranslateTROT())
      ...
      if (!isOutputChar)
         {
         trprints(disptrace, log, "failed because of reason 1 %\n");
         return false;
         }
      ...
      if (!foundLoopToReduce)
         {
         trprints(disptrace, log, "failed because of reason 2\n");
         return false;
         }
      ...
      if (termchar != 0)
         ...
         if (isSignExtending)
            {
            trprints(disptrace, log, "Bailing CISCTransform2CopyingTROx due to sign-extension\n");
            return false;
            }
         }
      }
      ...
      if (isOutputChar)
         ...
         if (isSIMDPossible) {
       	 ...
       	 if (isSIMDPossible) {
             ...
             trprints(disptrace, log, "Bailing CISCTransform2CopyingTROx: b2c - no proper evaluator available\n");
             return false;
       	 } else {
       		 for (int i = 0; i < 256; i++)
       		 	 {
         ...
         if (termchar < 0)
            {
            trprints(disptrace, log, "No terminating character found. Abandoning reduction.\n");
            return false;
            }
         tableNode = createTableLoad(comp, baseRepNode, 8, 8, tmpTable, disptrace);
         }
      ...
      for (n = li.getFirst(); n; n = li.getNext())
         ...
         if (trans->getCandidateRegion()->isIncluded(n))
            ...
            if (icmpgeCISCnode != NULL)
               {
               trprintf(disptrace, log, "Bailing CISCTransform2CopyingTROx: multiple loop tests: %d and %d\n", icmpgeCISCnode->getID(), n->getID());
               return false;
               }
            icmpgeCISCnode = n;
            lenNode = n->getChild(1);
   ...
   if (trans->getOffsetOperand1())
      ...
      trprintf(disptrace, log, "The array header const of inputNode %p is modified. (offset=%d)\n", inputNode, trans->getOffsetOperand1());
      }
   if (trans->getOffsetOperand2())
      ...
      TR_ASSERT(constLoad, "Not implemented yet");
      trprintf(disptrace, log, "The array header const of outputNode %p is modified. (offset=%d)\n", outputNode, offset);
      }
...
isSignExtendingCopyingTROx(TR_CISCTransformer *trans)
   ...
   if (!firstOp.isInteger() && !firstOp.isUnsigned())
      {
      trprints(disptrace, comp->log(), "isSignExtendingCopyingTROx: conversion through non-integer type\n");
      return TR_maybe;
      }
...
CISCTransform2TROTArray(TR_CISCTransformer *trans)
   ...
   if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace))
      {
      trprints(disptrace, comp->log(), "Bailing CISCTransform2TROTArray due to null TT - might be a preheader in last block of method\n");
      return false;
      }
...
CISCTransform2CopyingTRTx(TR_CISCTransformer *trans)
   ...
   if (isOutputChar  && genSIMD && !genTRxx){
      trprints(disptrace, log, "Bailing CISCTransform2CopyingTRTx : c2c - no proper evaluator available\n");
      return false;
   }
   ...
   if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace))
      {
      trprints(disptrace, log, "Bailing CISCTransform2CopyingTRTx due to null TT - might be a preheader in last block of method\n");
      return false;
      }
   ...
   if (trans->countGoodArrayIndex(indexVarSymRef) == 0 &&
      ...
      {
      trprintf(disptrace, log, "countGoodArrayIndex failed for %p, %p\n",indexRepNode,dstIndexRepNode);
      return false;
      }
   TR_ScratchList<TR::Node> variableList(comp->trMemory());
   variableList.add(indexRepNode);
   ...
   if (!target) // multiple successors
      ...
      if (trans->getNumOfBBlistSucc() > 3)
         {
         trprintf(disptrace, log, "trans->getNumOfBBlistSucc() is %d.",trans->getNumOfBBlistSucc());
         return false;
         }
      }
   ...
   if ((count = trans->analyzeCharBoolTable(P->getImportantNode(0), tmpTable, P->getImportantNode(1), &retSameExit)) <= 0)
      {
      trprints(disptrace, log, "trans->analyzeCharBoolTable failed\n");
      return false;
      }
   ...
   if (comp->cg()->getSupportsArrayTranslateTRTO255() || comp->cg()->getSupportsArrayTranslateTRTO()  )
      ...
      if (!isOutputChar)
        ...
        if (isSIMDPossible) {
           ...
           trprints(disptrace, log, "Bailing CISCTransform2CopyingTRTx : c2b - no proper evaluator available\n");
           return false;
        } else {
        	//TRxx
        	 for (i = 256; --i >= 0; )
        	 ...
        	 if (termchar < 0) // no room of termchar
        	 {
        		 ...
        		 termchar = TERMBYTE;
                 trprints(disptrace, log, "setAllowSourceCellIsTermChar: ");
        	 }
             trprintf(disptrace, log, "termchar is 0x%02x\n", termchar);
   ...
   if (necessaryCmp.getSize() >= 3)
      {
      trprintf(disptrace, log, "Too many (%d) loop exit tests to transform correctly.  Transformation only supports up to 2.  Abandoning reduction.\n", necessaryCmp.getSize());
      return false;
      }
   ...
   if (!testExitIF(icmpgeCISCnode1->getOpcode(), &isDecrement, &modLength))
      {
      trprints(disptrace, log, "testExitIF for icmpgeCISCnode1 failed\n");
      return false;
      }
   if (isDecrement)
      {
      trprints(disptrace, log, "Not support a decrement loop. (icmpgeCISCnode1)\n");
      return false;
      }
   TR_ASSERT(modLength == 0 || modLength == 1, "error");
   ...
   if (cmpVarSymRef == NULL)
      {
      trprints(disptrace, log, "Unable to determine the sym ref of induction variable in loop termination node.\n");
      return false;
      }
   ...
   if (P->getImportantNode(4))
      ...
      if (icmpgeCISCnode2)
         {
         trprints(disptrace, log, "Not support yet more than three if-statements. (1)\n");
         return false;
         }
      icmpgeCISCnode2 = trans->getP2TInLoopIfSingle(P->getImportantNode(4));
      if (!icmpgeCISCnode2)
         {
         trprints(disptrace, log, "Not support yet more than three if-statements. (2)\n");
         return false;
         }
      }
   ...
   if (icmpgeCISCnode2)
      ...
      if (!testExitIF(icmpgeCISCnode2->getOpcode(), &isDecrement, &modLength))
         {
         trprints(disptrace, log, "testExitIF for icmpgeCISCnode2 failed\n");
         return false;
         }
      if (isDecrement)
         {
         trprints(disptrace, log, "Not support a decrement loop. (icmpgeCISCnode2)\n");
         return false;
         }
      TR_ASSERT(modLength == 0 || modLength == 1, "error");
   ...
   if (icmpgeCISCnode2)
      ...
      trprints(disptrace, log, "Now assuming that all exits of booltable are identical.\n");
...
CISCTransform2TRTOArray(TR_CISCTransformer *trans)
   ...
   if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace))
      {
      trprints(disptrace, comp->log(), "Bailing CISCTransform2TRTOArray due to null TT - might be a preheader in last block of method\n");
      return false;
      }
...
CISCTransform2ArrayCopySub(TR_CISCTransformer *trans, TR::Node *indexRepNode, TR::Node *dstIndexRepNode,
   ...
   if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace))
      {
      trprints(disptrace, log, "Bailing CISCTransform2ARrayCopySub due to null TT - might be a preheader in last block of method\n");
      return false;
      }
   ...
   if (!inLoadCISCNode || !inStoreCISCNode)
      {
      trprintf(disptrace, log, "CISCTransform2ArrayCopy failed. inLoadCISCNode %x inStoreCISCNode %x\n",inLoadCISCNode,inStoreCISCNode);
      return false;
      }
   ...
   if (T && T->getAspects()->getIfCount() > 1)
      {
      trprintf(disptrace, log, "CISCTransform2ArrayCopySub detected %d if-stmts in loop (> 1).  Not transforming.\n", T->getAspects()->getIfCount());
      return false;
      }
   ...
   if (!getMultiplier(trans, P->getImportantNode(2), &mulFactorNode, &elementSize, inLoadNode->getType()))
      {
      trprints(disptrace, log, "CISCTransform2ArrayCopy getMultiplier failed.\n");
      return false;
      }
   if (elementSize != inLoadNode->getSize() || elementSize != inStoreNode->getSize())
      {
      trprintf(disptrace, log, "CISCTransform2ArrayCopy failed - Size Mismatch.  Element Size: %d InLoadSize: %d inStoreSize: %d\n",
   ...
   if (indexContainsArrayAccess(comp, inLoadNode->getFirstChild()) ||
      ...
      {
      trprintf(disptrace, log, "inputNode %p or outputnode %p contains another arrayaccess, no reduction\n", inLoadNode, inStoreNode);
      return false;
      }
   ...
   if (!testExitIF(cmpIfAllCISCNode->getOpcode(), &isDecrementRet, &modLength, &modStartIdx))
      {
      trprints(disptrace, log, "CISCTransform2ArrayCopy testExitIF failed.\n");
      return false;
      }
   ...
   if (!trans->analyzeArrayIndex(indexVarSymRef))
      {
      trprintf(disptrace, log, "analyzeArrayIndex failed. %x\n",indexRepNode);
      return false;
      }
   TR::SymbolReference * dstIndexVarSymRef = dstIndexRepNode ? dstIndexRepNode->getSymbolReference() : NULL;
   ...
   if (!trans->searchNodeInTrees(inputNode, indexRepNode))
      {
      trprints(disptrace, log, "searchNodeInTrees for inputNode failed.\n");
      return false;
      }
   if (!trans->searchNodeInTrees(outputNode, dstIndexVarSymRef ? convertStoreToLoad(comp, dstIndexRepNode) : indexRepNode))
      {
      trprints(disptrace, log, "searchNodeInTrees for outputNode failed.\n");
      return false;
      }
   ...
   if (indexVarSymRef != exitVarSymRef && dstIndexVarSymRef != exitVarSymRef)
      {
      trprintf(disptrace, log, "CISCTransform2ArrayCopy cannot find exitVarSymRef correctly %x.\n", exitVarRepNode);
      return false;
      }
   ...
   trprintf(disptrace, log, "detected postIncrement %d modLength %d modStartIdx %d\n", postIncrement, modLength, modStartIdx);
   ...
   if (!comp->cg()->getSupportsReferenceArrayCopy() && needWriteBarrier)
      {
      trprints(disptrace, log, "CISCTransform2ArrayCopy: needWriteBarrier but not getSupportsReferenceArrayCopy().\n");
      return false;
      }
...
CISCTransform2ArrayCopyB2CorC2B(TR_CISCTransformer *trans)
   ...
   if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace))
      {
      trprints(disptrace, comp->log(), "Bailing CISCTransform2ArrayCopyB2CorC2B due to null TT - might be a preheader in last block of method\n");
      return false;
      }
...
CISCTransform2ArrayCopyB2CBndchk(TR_CISCTransformer *trans)
   ...
   if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace))
      {
      trprints(disptrace, log, "Bailing CISCTransform2ArrayCopyB2CBndchk due to null TT - might be a preheader in last block of method\n");
      return false;
      }
...
CISCTransform2ArrayCopyC2BMixed(TR_CISCTransformer *trans)
   ...
   if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace))
      {
      trprints(disptrace, log, "Bailing CISCTransform2ArrayCopyC2BMixed due to null TT - might be a preheader in last block of method\n");
      return false;
      }
   ...
   trprintf(disptrace, log, "All parameters: %x %x %x %x %x %x\n",
   ...
   trprintf(disptrace, log, "LEalongJumpPath = %d\n",LEalongJumpPath);
...
CISCTransform2ArrayCopyC2BIf2(TR_CISCTransformer *trans)
   ...
   if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace))
      {
      trprints(disptrace, comp->log(), "Bailing CISCTransform2ArrayCopyC2BIf2 due to null TT - might be a preheader in last block of method\n");
      return false;
      }
...
CISCTransform2ArrayCopyB2I(TR_CISCTransformer *trans)
   ...
   if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace))
      {
      trprints(disptrace, comp->log(), "Bailing CISCTransform2ArrayCopyB2I due to null TT - might be a preheader in last block of method\n");
      return false;
      }
...
CISCTransform2PtrArraySet(TR_CISCTransformer *trans)
   ...
   if (trans->getNumOfBBlistBody() > 1)
      {
      trprints(trace, log, "Need exactly 1 basic block\n");
      return false;
      }
   ...
   if (block->getNumberOfRealTreeTops() != 3)
      {
      trprints(trace, log, "Need exactly 3 real treetops\n");
      return false;
      }
   ...
   if (!astore)
      {
      trprints(trace, log, "astore missing\n");
      return false;
      }
   if (!Store)
      {
      trprints(trace, log, "array element store missing\n");
      return false;
      }
   if (!ifcmp)
      {
      trprints(trace, log, "if compare missing\n");
      return false;
      }
   ...
   if (!(astoreNode->getChild(0)->getChild(0) == StoreNode->getChild(0) &&
      ...
      {
      trprints(trace, log, "node trees not in required form\n");
      return false;
      }
   ...
   if (!ifcmpNode->getChild(0)->getOpCode().isLoadVar() &&
      ...
      {
      trprints(trace, log, "neither comparands are loadvar\n");
      return false;
      }
   ...
   if (ifcmpNode->getChild(0)->getOpCode().isLoadVar() ^
      ...
      if (astoreNode->getChild(0) != nonLoadChild)
         {
         trprints(trace, log, "iv is not a commoned child in if comparand\n");
         return false;
         }
      }
   ...
   if (!ifcmpNode->getOpCode().isCompareForOrder() &&
      ...
      {
      trprints(trace, log, "invalid compare condition\n");
      return false;
      }
   ...
   if (!StoreNode->getOpCode().isStoreIndirect() ||
      ...
      {
      trprints(trace, log, "array element store node is neither indirect store nor matched with addr iv\n");
      return false;
      }
   ...
   switch(StoreNode->getSize())
      ...
      case 8: break;
      default:
         trprints(trace, log, "element size is not power-of-2 <= 8\n");
         return false;
      }

   if (StoreNode->getDataType() == TR::Aggregate)
      {
      trprints(trace, log, "arrayset can't handle aggregate elem type\n");
      return false;
      }
   ...
   if (StoreNode->getSize() != getAbs(increment))
      {
      trprints(trace, log, "increment size does not match element size\n");
      return false;
      }
   ...
   if (!endPtr)
      {
      trprints(trace, log, "Could not get end pointer\n");
      return false;
      }
...
CISCTransform2ArraySet(TR_CISCTransformer *trans)
   ...
   if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace))
      {
      trprints(disptrace, log, "Bailing CISCTransform2ArraySet due to null TT - might be a preheader in last block of method\n");
      return false;
      }
   ...
   if (!cmpIfAllCISCNode)
      {
      trprints(disptrace, log, "Not implemented yet for multiple-if\n");
      return false;
      }
   ...
   switch(cmpIfAllCISCNode->getOpcode())
      ...
      case TR::ificmple:
         ...
         break;
      default:
         trprints(disptrace, log, "Bailing CISCTransform2ArraySet due to unrecognized loop exit comparison.\n");
         return false;
      }

   if (disptrace)
      trprintf(disptrace, log, "Examining exit comparison CICS node %d, and determined required length modifier to be: %d\n", cmpIfAllCISCNode->getID(), lengthMod);
   ...
   for (inStoreNode = iteratorStores.getFirst(); inStoreNode; inStoreNode = iteratorStores.getNext())
      ...
      if (valueNode->getOpCode().isLoadDirect() && valueNode->getOpCode().hasSymbolReference())
         ...
         if (valueNode->getSymbolReference()->getReferenceNumber() == indexNode->getSymbolReference()->getReferenceNumber() ||
            ...
            {
            trprints(disptrace, log, "arraystore tree has induction variable on rhs\n");
            return false;
            }
         }
      }
      ...
      if (!loopIsIncrement)    // count-down loop
         ...
         if (isIndexVarUpdateBeforeArrayset)
            ...
            if (origIndVarLoad == origArrayIndVarLoad)
               {
               trprintf(disptrace, log, "Identified array index to have been referencing original induction variable value: %p\n",origIndVarLoad);
...
bool CISCTransform2Strlen16(TR_CISCTransformer *trans)
   ...
   if (block->getNumberOfRealTreeTops() != 2)
      {
      trprints(trace, log, "Need exactly 2 real treetops\n");
      return false;
      }
   ...
   trprints(trace, log, "Failed one of the requirements\n");
   return false;
   }
...
CISCTransform2MixedArraySet(TR_CISCTransformer *trans)
   ...
   if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace))
      {
      trprints(disptrace, comp->log(), "Bailing CISCTransform2MixedArraySet due to null TT - might be a preheader in last block of method\n");
      return false;
      }
...
CISCTransform2ArrayCmp2Ifs(TR_CISCTransformer *trans)
   ...
   if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace))
      {
      trprints(disptrace, comp->log(), "Bailing CISCTransform2ArrayCmp2Ifs due to null TT - might be a preheader in last block of method\n");
      return false;
      }
...
CISCTransform2ArrayCmp(TR_CISCTransformer *trans)
   ...
   if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace, &preHeader))
      {
      trprints(disptrace, log, "Bailing CISCTransform2ArrayCmp due to null TT - might be a preheader in last block of method\n");
      return false;
      }
   ...
   if (preHeader)
      {
      trprintf(disptrace, log, "found preheader to be %d\n", preHeader->getNumber());
      //
      // obtain a CISCNode of each store for incrementing induction variables
      ...
      trprintf(disptrace, log,
         "searching for stores to loop indices between preheader first tree %p and first matching tree %p, looking for symrefnum %d %d\n",
      ...
      for (TR::TreeTop * tt = preHeader->getFirstRealTreeTop();tt && tt != trTreeTop; tt = tt->getNextRealTreeTop())
         ...
         if (tempNode->getOpCode().isStore() && tempNode->getOpCode().hasSymbolReference() &&
            ...
            {
            trprintf(disptrace, log, "Bailing CISCTransform2ArrayCmp due to unexpected store (%p) of one of the indices prior to the idiom\n", tempNode);
            return false;
            }
         }
      }
   ...
   if (T && T->getAspects()->getIfCount() > 2)
      {
      trprintf(disptrace, log, "CISCTransform2ArrayCmp detected %d if-stmts in loop (> 2).  Not transforming.\n", T->getAspects()->getIfCount());
      return false;
      }
   ...
   if (trans->isBlockInLoopBody(failDest->getNode()->getBlock()))
      {
      trprints(disptrace, log, "CISCTransform2ArrayCmp failing transformer, ifcmpall test branch does not exit the loop.\n");
      return false;
      }
   ...
   if (!areArraysInvariant(comp, inSrc1Node, inSrc2Node, T))
      {
      trprintf(disptrace, log, "input array bases %p and %p are not invariant, no reduction\n", inSrc1Node, inSrc2Node);
      return false;
      }
   ...
   if (inSrc1Node->getType() != inSrc2Node->getType())
      {
      trprintf(disptrace, log, "CISCTransform2ArrayCmp failed - Array access types differ. inSrc1: %s, inSrc2: %s\n",
   ...
   if (elementSize != expectedSize)
      {
      trprintf(disptrace, log, "CISCTransform2ArrayCmp failed - Size Mismatch.  Element Size: %d, Expected Size: %d\n",
         elementSize,
         expectedSize);
      return false;     // Size is mismatch!
      }
   ...
   if (isDecrement)     // count-down loop
      ...
      trprintf(disptrace, log, "isDecrement start1Idx %p start2Idx %p end1Idx %p end2Idx %p\n", start1Idx, start2Idx, end1Idx, end2Idx);
      ...
      trprintf(disptrace, log, "start1Idx %p start2Idx %p end1Idx %p end2Idx %p\n", start1Idx, start2Idx, end1Idx, end2Idx);
   ...
   if (generateArraycmplen)
      ...
      if (compareTo)
         ...
         if (generateArraycmpsign)
            ...
            {
            trprints(disptrace, log, "ArrayCmp: Convert compareTo into equals!\n");
   ...
   if (0 && isCompensateCode)
      ...
      //      arraycmp
      //      ...
      trprintf(disptrace, log, "cmpifallciscnode %d ifcmpge %d\n", cmpIfAllCISCNode->getOpcode(), TR::ificmpge);
...
CISCTransform2BitOpMem(TR_CISCTransformer *trans)
   ...
   if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace))
      {
      trprints(disptrace, log, "Bailing CISCTransform2BitOpMem due to null TT - might be a preheader in last block of method\n");
      return false;
      }
   ...
   if (elementSize != inSrc1Node->getSize() || elementSize != inSrc2Node->getSize())
      {
      trprintf(disptrace, log, "CISCTransform2BitOpMem failed - Size Mismatch.  Element Size: %d InSrc1Size: %d inSrc2Size: %d\n", elementSize, inSrc1Node->getSize(), inSrc2Node->getSize());
      return false;     // Size is mismatch!
      }
   ...
   if (bitOpMem)
      ...
      trprintf(disptrace, log, "CISCTransform2BitOpMem: orgPrevBlock=%d checkSrc1=%d lastpath=%d slowpad=%d orgNextTreeTop=%x\n",
...
CISCTransform2CountDecimalDigit(TR_CISCTransformer *trans)
   ...
   if (comp->compileRelocatableCode())
      {
      trprints(disptrace, log, "Bailing CISCTransform2CountDecimalDigit - not supported for AOT compilations.");
      return false;
      }

   if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace))
      {
      trprints(disptrace, log, "Bailing CISCTransform2CountDecimalDigit due to null TT - might be a preheader in last block of method\n");
      return false;
      }
   ...
   if (!constNode->getIlOpCode().isLoadConst())
      {
      trprintf(disptrace, log, "%p is not isLoadConst().\n", constNode);
      return false;
      }
   ...
   switch(ifcmp->getOpcode())
      ...
      case TR::iflcmpeq:
         if (constNode->getOtherInfo() != 0)
            {
            trprintf(disptrace, log, "The exit-if is TR::if*cmpeq but the constant value is %d.\n", constNode->getOtherInfo());
            return false;
            }
         break;
      case TR::ificmplt:
      case TR::iflcmplt:
         if (constNode->getOtherInfo() != 10)
            {
            trprintf(disptrace, log, "The exit-if is TR::if*cmplt but the constant value is %d.\n", constNode->getOtherInfo());
            return false;
            }
         ...
         break;
      default:
         trprintf(disptrace, log, "The exit-if %p is not as expected. We may be able to implement this case.\n", ifcmp);
         return false;
      }
...
CISCTransform2LongToStringDigit(TR_CISCTransformer *trans)
   ...
   if (comp->compileRelocatableCode())
      {
      trprints(disptrace, log, "Bailing CISCTransform2LongToStringDigit - not supported for AOT compilations.");
      return false;
      }

   if (isLoopPreheaderLastBlockInMethod(comp, block, disptrace))
      {
      trprints(disptrace, log, "Bailing CISCTransform2LongToStringDigit due to null TT - might be a preheader in last block of method\n");
      return false;
      }
