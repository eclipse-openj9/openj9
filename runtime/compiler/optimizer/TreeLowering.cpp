...
TR::TreeLowering::Transformer::moveNodeToEndOfBlock(TR::Block* const block, TR::TreeTop* const tt, TR::Node* const node, bool isAddress)
   ...
   if (iterTT != blockExit)
      {
      trprintf(trace(), log, "Moving treetop containing node n%dn [%p] for helper call to end of prevBlock in preparation of final block split\n",
      ...
      while (iterTT != blockExit)
         ...
         if (isStoreOp && iterTT->getNode()->getFirstChild() == node)
            {
            trprintf(trace(), log, "Moving treetop containing node n%dn [%p] for store of helper call result to end of prevBlock in preparation of final block split\n",
...
AcmpTransformer::lower(TR::Node * const node, TR::TreeTop * const tt)
   ...
   trprintf(trace(), log, "Anchoring call node under treetop n%un (0x%p)\n", anchoredCallTT->getNode()->getGlobalIndex(), anchoredCallTT->getNode());
   ...
   trprintf(trace(), log, "Anchoring call arguments n%un and n%un under treetops n%un and n%un\n",
   ...
   trprintf(trace(), log, "Call node n%un is in block %d, targetBlock is %d\n", node->getGlobalIndex(), callBlock->getNumber(), targetBlock->getNumber());
   ...
   trprintf(trace(), log, "Anchored call has been transformed into %s node n%un\n", anchoredNode->getOpCode().getName(), anchoredNode->getGlobalIndex());
   ...
   if (anchoredNode->getOpCodeValue() == TR::iRegLoad)
      ...
      {
      trprintf(trace(), log, "Storing constant %d to symref %d (%s)\n",
   ...
   trprintf(trace(), log, "Added check node n%un; call node is now in block_%d\n", ifacmpeqNode->getGlobalIndex(), callBlock->getNumber());
   ...
   if (!anchoredCallArg1TT->getNode()->getFirstChild()->isNonNull())
      ...
      {
      trprintf(trace(), log, "Skip fastpath for lhs == NULL because node n%un isNonNull\n", anchoredCallArg1TT->getNode()->getFirstChild()->getGlobalIndex());
      }
   ...
   if (!anchoredCallArg2TT->getNode()->getFirstChild()->isNonNull())
      ...
      {
      trprintf(trace(), log, "Skip fastpath for rhs == NULL because node n%un isNonNull\n", anchoredCallArg2TT->getNode()->getFirstChild()->getGlobalIndex());
      }
   ...
   trprintf(trace(), log, "Call node isolated in block_%d by splitPostGRA\n", callBlock->getNumber());
   ...
   trprints(trace(), log, "Moved call block to end of method\n");
   ...
   trprintf(trace(), log, "Added check node n%un\n", checkRhsIsNotVT->getGlobalIndex());
...
NonNullableArrayNullStoreCheckTransformer::lower(TR::Node* const node, TR::TreeTop* const tt)
   ...
   if (!sourceChild->isNonNull())
      ...
      trprintf(enableTrace, comp()->log(), "checkValueNull n%dn is inserted before  n%dn in prevBlock %d\n",
      ...
      trprintf(enableTrace, comp()->log(), "checkNotNullRestrictedArray n%dn is isolated in checkNotNullRestrictedBlock %d\n",
...
createStoreNodeForAnchoredNode(TR::Node *anchoredNode, TR::Node *nodeToBeStored, TR::Compilation *comp, bool enableTrace, const char *msg)
   ...
   if (anchoredNode->getOpCodeValue() == TR::aRegLoad)
      ...
      trprintf(enableTrace, log, "Storing %s n%dn in register %s storeNode n%dn anchoredNode n%dn\n",
      ...
      storeNode->setSymbolReference(anchoredNode->getSymbolReference());
      trprintf(enableTrace, log, "Storing %s n%dn to symref %d (%s) storeNode n%dn anchoredNode n%dn\n",
...
LoadArrayElementTransformer::lower(TR::Node* const node, TR::TreeTop* const tt)
   ...
   trprintf(enableTrace, log, "Anchored call node under treetop n%un (0x%p), elementIndex under treetop n%un (0x%p), arrayBaseAddress under treetop n%un (0x%p)\n",
      anchoredCallTT->getNode()->getGlobalIndex(), anchoredCallTT->getNode(),
   ...
   trprintf(enableTrace, log, "Isolated the anchored call treetop n%dn in block_%d\n",
   ...
   trprintf(enableTrace, log, "Isolated helper call treetop n%dn node n%dn in block_%d\n",
   ...
   trprintf(enableTrace, log, "Created array element load treetop n%dn node n%dn\n", elementLoadTT->getNode()->getGlobalIndex(), elementLoadNode->getGlobalIndex());
   ...
   trprintf(enableTrace, log, "Store array element load node n%dn to n%dn %s\n",
   ...
   trprintf(enableTrace, log, "Isolated array element load treetop n%dn node n%dn in block_%d\n",
   ...
   trprintf(enableTrace, log, "Append ifNode n%dn to block_%d\n", ifNode->getGlobalIndex(), originalBlock->getNumber());
...
StoreArrayElementTransformer::lower(TR::Node* const node, TR::TreeTop* const tt)
   ...
   trprintf(enableTrace, log, "Anchored elementIndex under treetop n%un (0x%p), arrayBaseAddress under treetop n%un (0x%p), value under treetop n%un (0x%p), \n",
      anchoredElementIndexTT->getNode()->getGlobalIndex(), anchoredElementIndexTT->getNode(),
      anchoredArrayBaseAddressTT->getNode()->getGlobalIndex(), anchoredArrayBaseAddressTT->getNode(),
   ...
   trprintf(enableTrace, log, "Isolated the trees after the helper call in block_%d\n", blockAfterHelperCall->getNumber());
   ...
   if (!skipArrayStoreChecks(comp, node))
      ...
      trprintf(enableTrace, log, "Created arrayStoreCHK treetop n%dn arrayStoreCHKNode n%dn\n", arrayStoreTT->getNode()->getGlobalIndex(), arrayStoreCHKNode->getGlobalIndex());
      ...
      trprintf(enableTrace, log, "Created treetop node with awrtbari child as treetop n%dn elementStoreNode n%dn\n",
   ...
   trprintf(enableTrace, log, "Isolated array element store treetop n%dn node n%dn in block_%d\n",
