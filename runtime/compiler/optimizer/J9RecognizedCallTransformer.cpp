...
void J9::RecognizedCallTransformer::processUnsafeAtomicCall(TR::TreeTop* treetop, TR::SymbolReferenceTable::CommonNonhelperSymbol helper, bool needsNullCheck)
   ...
   if (treetop->getNode()->getOpCode().isNullCheck())
      ...
      trprintf(enableTrace, log, "Created node %p to preserve NULLCHK on unsafe call %p\n", checkNode, unsafeCall);
      }
   ...
   if (isNotStaticField)
      ...
      if (TR::Compiler->om.isOffHeapAllocationEnabled())
         ...
         trprintf(enableTrace, comp()->log(), "Created isObjectNull test node n%dn, non-null object will fall through to Block_%d\n",
         ...
         trprintf(enableTrace, log, "Created isObjectArray test node n%dn, array will branch to array access block\n", isObjectArrayNode->getGlobalIndex());
         ...
         trprints(enableTrace, log, "Field is not static, use the object and offset directly\n");
         }
      }
   else
      {
      ...
      if (needsNullCheck)
         ...
         trprintf(enableTrace, log, "Created NULLCHK tree %p on the first argument of Unsafe call\n", treetop->getPrevTreeTop());
         }
      ...
      trprintf(enableTrace, log, "Created isObjectNull test node n%dn, non-null object will fall through to Block_%d\n",
         isObjectNullNode->getGlobalIndex(), treetop->getEnclosingBlock()->getNumber());
      ...
      trprintf(enableTrace, log, "Created isNotLowTagged test node n%dn, static field will fall through to Block_%d\n",
      ...
      if (!disableIllegalWriteReport
         ...
         trprintf(enableTrace, log, "Created isFinal test node n%dn, non-final-static field will fall through to Block_%d, final field goes to Block_%d\n",
      ...
      trprintf(enableTrace, log, "Created node objectAdjustmentNode n%dn #%d and offsetAdjustmentNode #%d n%dn to adjust object and offset for static field\n",
   ...
   if (isObjectNullTreeTop)
      ...
      if (nextTreeTop)
         ...
         if (!storeReturnNode->getOpCode().isStore() || !(storeReturnNode->getFirstChild() == unsafeCall))
            ...
            trprintf(enableTrace, log, "storeNode n%dn #%d for the return value of atomic method helper\n",
      ...
      if (isNotStaticField && isObjectArrayTreeTop)
         ...
         trprintf(enableTrace, log, "Created array access helper block_%d that loads dataAddr pointer from array object address\n", arrayAccessBlock->getNumber());
         }
      else
#endif /* J9VM_GC_SPARSE_HEAP_ALLOCATION */
      ...
      if (!isNotStaticField)
         ...
         */
         ...
         trprintf(enableTrace, log, "Created atomic method helper block_%d that loads from ramStatics treetop n%dn. returnBlock block_%d\n",
         ...
         trprintf(enableTrace, log, "Block_%d contains call to atomic method helper, and is the target of isObjectNull and isNotLowTagged tests\n", treetop->getEnclosingBlock()->getNumber());
