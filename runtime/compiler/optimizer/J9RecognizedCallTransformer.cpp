...
void J9::RecognizedCallTransformer::process_java_lang_StringLatin1_inflate_BIBII(TR::TreeTop *treetop, TR::Node *node)
   ...
   TR::Node *ifCmpNode1 = TR::Node::createif(TR::ificmplt, dstOff, constZeroNode1);
   ...
   TR::Node *ifCmpNode2 = TR::Node::createif(TR::ificmplt, length, constZeroNode2);
   TR::TreeTop *ifCmpTreeTop2 = TR::TreeTop::create(comp(), ifCmpTreeTop1, ifCmpNode2);
   // if (dstOff < length - dstObj.length) { call the original method }
   TR::Node *arrayLenNode = TR::Node::create(node, TR::arraylength, 1, dstObj);
   TR::Node *isubNode = TR::Node::create(node, TR::isub, 2, length, arrayLenNode);
   TR::Node *ifCmpNode3 = TR::Node::createif(TR::ificmplt, dstOff, isubNode);
   TR::TreeTop *ifCmpTreeTop3 = TR::TreeTop::create(comp(), ifCmpTreeTop2, ifCmpNode3);

   TR::TreeTop *arrayTranslateTreeTop = TR::TreeTop::create(comp(), ifCmpTreeTop3, arrayTranslateNode);
   ...
   TR::Block *fallThroughPathBlock = ifCmpBlock3->split(arrayTranslateTreeTop, cfg, true /* fixUpCommoning */, true /* copyExceptionSuccessors */);
