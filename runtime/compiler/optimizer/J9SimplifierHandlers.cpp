...
static TR_PackedCompareCondition packedCompareCommon(TR::Node *parent, TR::Node *op1, TR::Node *op2, bool ignoreConstSign, TR::Simplifier * s)
   ...
   trprintf(trace, log, "packedCompare: (parent  %s (%p) castedToBCD=%s, op1 %s (%p), op2 %s (%p), absVal=%s) %s vs %s",
   ...
   if (op1IsZero && op2IsZero)
      {
      trprints(trace, log, " -> TR_PACKED_COMPARE_EQUAL (op1 == op2 == zero)\n");
   ...
   if (!ignoreConstSign)
      ...
      if (op1IsNegative && op2_GE_zero)
         {
         trprints(trace, log, " -> TR_PACKED_COMPARE_LESS (op1 < 0 and op2 >= 0)\n");
         return TR_PACKED_COMPARE_LESS;
         }

      if (op1_GE_zero && op2IsNegative)
         {
         trprints(trace, log, " -> TR_PACKED_COMPARE_GREATER (op1 >= 0 and op2 < 0)\n");
...
static TR::Node *simplifyPackedArithmeticOperand(TR::Node *node, TR::Node *parent, TR::Block * block, TR::Simplifier * s)
   ...
   if (node->getType().isAnyPacked() &&
      ...
      if (parent->castedToBCD())
         {
         trprintf(s->trace(), s->comp()->log(), "parent %s (%p) castedToBCD=true for child %s (%p) so do not allow removal of child\n",
      ...
      switch (sign)
         ...
         case raw_bcd_sign_0xf:
            ...
            if (isLoadVarChild &&
               ...
               trprintf(s->trace(), s->comp()->log(), "do not replace %s (%p) with setsign 0xf because parent %s (%p) is a store and child %s (%p) is a loadVar\n",
...
static TR::Node *foldSetSignIntoNode(TR::Node *setSign, bool setSignIsTheChild, TR::Node *other, bool removeSetSign, TR::Block *block, TR::Simplifier *s)
   ...
   if (removeSetSign &&
      ...
      {
      trprintf(s->trace(), s->comp()->log(), "disallow foldSetSignIntoNode of setSign -- %s (%p) with other node -- %s (%p) because child %p (prec %d) truncates and parent %p (prec %d) widens\n",
...
static TR::Node *foldAndReplaceDominatedSetSign(TR::Node *setSign, bool setSignIsTheChild, TR::Node *other, TR::Block * block, TR::Simplifier * s)
   ...
   if (setSignIsTheChild || TR::ILOpCode::setSignVersionOfOpCode(child->getOpCodeValue()) != TR::BadILOp)
      ...
      if (parent->getOpCode().isSetSign() &&
         ...
         //       iconst signB
         //
         trprintf(s->trace(), s->comp()->log(), "disallow setting ignored setsign value on dominated %s [" POINTER_PRINTF_FORMAT "] so sign verification can be skipped\n",
            child->getOpCode().getName(), child);
         return parent;
         }
