...
bool TR_UnsafeFastPath::tryTransformUnsafeAtomicCallInVarHandleAccessMethod(TR::TreeTop* callTree, TR::RecognizedMethod callerMethod, TR::RecognizedMethod calleeMethod)
   ...
   if (isVarHandleOperationMethodOnArray(callerMethod)
      ...
      {
      trprintf(trace(), log, "Call %p n%dn is accessing an element from an array that might be arraylet, quit\n", node, node->getGlobalIndex());
      return false;
      }
   ...
   if (symbol->getMethod()->isUnsafeCAS())
      ...
      if (isVarHandleOperationMethodOnNonStaticField(callerMethod) &&
         ...
         trprintf(trace(), log, "Found Unsafe CAS node %p n%dn on non-static field, set the flag\n", node, node->getGlobalIndex());
         return true;
         }
   ...
   if (!comp()->cg()->supportsNonHelper(helper))
      {
      trprints(trace(), log, "Equivalent atomic intrinsic is not supported on current platform, quit\n");
      return false;
      }
   ...
   if (callTree->getNode()->getOpCode().isNullCheck())
      ...
      trprintf(trace(), log, "Created node %p n%dn to preserve null check on call %p n%dn\n",
   ...
   trprintf(trace(), log, "Transformed the call %p n%dn to codegen inlineable intrinsic\n", node, node->getGlobalIndex());
   ...
   for (TR::TreeTop * tt = methodSymbol->getFirstTreeTop(); tt != NULL; tt = tt->getNextTreeTop())
      ...
      if (node && node->getOpCode().isCall() && !node->getSymbol()->castToMethodSymbol()->isHelper())
         ...
         performTransformation(comp(), "%s turning the call [" POINTER_PRINTF_FORMAT "] into an indirect store\n", optDetailString(), node))
            ...
            trprintf(trace(), log, "Created node [" POINTER_PRINTF_FORMAT "] to store the value [" POINTER_PRINTF_FORMAT "] to target location [" POINTER_PRINTF_FORMAT "]\n",
               node, value, addrCalc);
            continue;
            }
         ...
         if (symbol->getRecognizedMethod() == TR::com_ibm_jit_JITHelpers_byteToCharUnsigned)
            ...
            if (performTransformation(comp(), "%s Found unsafe/JITHelpers calls, turning node [" POINTER_PRINTF_FORMAT "] into a unsigned widening\n", optDetailString(), node))
               ...
               trprintf(trace(), log, "Created node [" POINTER_PRINTF_FORMAT "] to unsigned widen value [" POINTER_PRINTF_FORMAT "] \n", node, value);
         ...
         if (isKnownUnsafeCaller(callerMethod) &&
            ...
            trprintf(trace(), log, "VarHandle operation: isArrayOperation %d type %s value %p access mode %s on node %p\n",
         ...
         if (mightBeArraylets && disableUnsafeForArraylets)
            {
            trprintf(trace(), log, "unsafeForArraylets is disabled, skip unsafeFastPath for node [" POINTER_PRINTF_FORMAT "]\n", node);
            continue;
            }
         ...
         if (type != TR::NoType && performTransformation(comp(), "%s Found unsafe/JITHelpers calls, turning node [" POINTER_PRINTF_FORMAT "] into a load/store\n", optDetailString(), node))
            ...
            if (mightBeArraylets)
               {
               trprintf(trace(), log, "This is an array operation in arraylets mode with array [" POINTER_PRINTF_FORMAT "] and offset [ " POINTER_PRINTF_FORMAT "], creating a load/store and a spineCHK\n",
                  object, offset);
               ...
               if (value)
                  ...
                  trprintf(trace(), log, "Created node [" POINTER_PRINTF_FORMAT "] to store the value [" POINTER_PRINTF_FORMAT "] to target location [" POINTER_PRINTF_FORMAT "] with spineCHK [" POINTER_PRINTF_FORMAT "]\n",
                  ...
                  trprintf(trace(), log, "Created node [" POINTER_PRINTF_FORMAT "] to load from location [" POINTER_PRINTF_FORMAT "] with spineCHK [" POINTER_PRINTF_FORMAT "]\n",
               ...
               if (value)
                  ...
                  trprintf(trace(), log, "Created node [" POINTER_PRINTF_FORMAT "] to load from location [" POINTER_PRINTF_FORMAT "]\n", node, addrCalc);
