...
void TR_EscapeAnalysisTools::insertFakeEscapeForOSR(TR::Block *block, TR::Node *induceCall)
   ...
   while (inlinedIndex > -1)
      ...
      trprintf(trace, log, "Calling processAutosAndPendingPushes:  At %d:%d,  ResolvedMethodSymbol [%p] and OSRMethodData [%p]\n", inlinedIndex, byteCodeIndex, rms, methodData);
         ...
         if ((p->isAuto() || p->isParm()) && p->getDataType() == TR::Address)
            ...
            if (!induceDefiningMap
               ...
               if (deadSymRefs == NULL || !deadSymRefs->isSet(symRefNum))
                  ...
                  trprintf(trace, log, "In processSymbolReferences, adding symRef #%d to symRefsToLoad\n", symRef->getReferenceNumber());
                  }
               else
                  {
                  trprintf(trace, log, "In processSymbolReferences, symRef #%d is dead - not added to symRefsToLoad\n", symRef->getReferenceNumber());
                  }
               }
            else
               {
               ...
               while (definingSymsIt.hasMoreElements())
                  ...
                  if ((definingSym->isAuto() || definingSym->isParm())
                     ...
                     trprintf(trace, log, "In processSymbolReferences, adding definingSymRef #%d to symRefsToLoad\n", definingSymRefNum);
                     }
                  else
                     {
                     trprintf(trace, log, "In processSymbolReferences, definingSymRef #%d - isAuto == %d; isParm == %d; dead == %d - not added to symRefsToLoad\n",
