...
TR::TreeTop* generateArraycopyFromSequentialLoads(TR::Compilation* comp, bool trace, TR::TreeTop* currentTreeTop, TR::Node* rootNode, NodeForwardList* combineNodeList)
   ...
   trprintf(trace, log, "Sequential Load Simplification Candidate - rootNode: %p\n", rootNode);
   ...
   trprintf(trace, log, "Sequential Load Simplification Candidate - rootNode: %p, byteCount: %d\n", rootNode, byteCount);

   if (!is64bitResult && (byteCount > 4))
      {
      trprintf(trace, log, "byteCount is too high for constructing an int value. rootNode: %p, byteCount: %d\n", rootNode, byteCount);
      return currentTreeTop;
      }

   if ((byteCount < 2) || (byteCount > 8))
      {
      trprintf(trace, log, "Sequential Load of size other than 2 to 8 is not supported. rootNode: %p, byteCount: %d\n", rootNode, byteCount);
      return currentTreeTop;
      }
   ...
   for (int i = 0; i < byteCount; i++)
      ...
      trprintf(trace, log, "Sequential Load Simplification Candidate - rootNode: %p, processedByteNodes[%d]: %p\n", rootNode, i, processedByteNodes[i]);

      if (0 == i)
         {
         aloadNode = getALoadReferenceForSeqLoad(processedByteNodes[i]);
         trprintf(trace, log, "Sequential Load Simplification Candidate - rootNode: %p, aloadNode: %p\n", rootNode, aloadNode);
         baseOffsetNode = getBaseOffsetForSeqLoad(processedByteNodes[i]);
         trprintf(trace, log, "Sequential Load Simplification Candidate - rootNode: %p, baseOffsetNode: %p\n", rootNode, baseOffsetNode);
         }
      else
         {
         ...
         if (aloadNode != getALoadReferenceForSeqLoad(processedByteNodes[i]))
            {
            trprintf(trace, log, "Sequential Load Simplification aload mismatch. rootNode: %p, aloadNode(0): %p, aloadNode(%d): %p\n",
         ...
         if (baseOffsetNode != getBaseOffsetForSeqLoad(processedByteNodes[i]))
            {
            trprintf(trace, log, "Sequential Load Simplification baseOffsetNode mismatch. rootNode: %p, baseOffsetNode(0): %p, baseOffsetNode(%d): %p\n",
      ...
      trprintf(trace, log, "Sequential Load Simplification Candidate - rootNode: %p, byteConversionNode: %p\n", rootNode, byteConversionNode);
      shiftValue = getShiftValueForSeqLoad(processedByteNodes[i]);
      trprintf(trace, log, "Sequential Load Simplification Candidate - rootNode: %p, shiftValue: %d\n", rootNode, shiftValue);

      if ((shiftValue > ((byteCount - 1) * 8)) || (shiftValue < 0))
         {
         trprintf(trace, log, "shiftValue is out of range for the given byteCount. rootNode: %p, byteCount: %d, shiftValue: %d\n", rootNode, byteCount, shiftValue);
         return currentTreeTop;
         }
      ...
      if (((byteCount - 1) * 8) == shiftValue)
         ...
         trprintf(trace, log, "Sequential Load Simplification Candidate - rootNode: %p, signExtendResult: %d\n", rootNode, signExtendResult);
         }
      else
         {
         ...
         if (checkForSeqLoadSignExtendedByte(processedByteNodes[i]))
            {
            trprintf(trace, log, "Bytes other than the most significant byte should not be sign extended. rootNode: %p, processedByteNodes[%d]: %p\n", rootNode, i, processedByteNodes[i]);
            return currentTreeTop;
            }
         }
      ...
      if (byteConversionNodes[byteNumber] != NULL)
         {
         trprintf(trace, log, "Duplicate shiftValue. rootNode: %p, shiftValue: %d, byteConversionNodes[%d]: %p, byteConversionNode(%d): %p\n",
   ...
   if (trace)
      {
      log->printf("Sequential Load Simplification Candidate - rootNode: %p, byteOffsets: %ld", rootNode, byteOffsets[0]);
      for (int i = 1; i < byteCount; i++)
         {
   ...
   if (matchLittleEndianSeqLoadPattern(byteOffsets, byteCount))
      ...
      {
      trprintf(trace, log, "byteOffsets array does not match a big or little endian pattern. rootNode: %p\n", rootNode);
      return currentTreeTop;
      }

   trprintf(trace, log, "Sequential Load Simplification Candidate - rootNode: %p, %s\n", rootNode, littleEndianLoad ? "Little Endian Load" : "Big Endian Load");
   trprintf(trace, log, "Sequential Load Simplification Candidate - rootNode: %p, newConvertChildNode: %p, newLoadChildNode: %p\n", rootNode, newConvertChildNode, newLoadChildNode);
   ...
   if (littleEndianLoad && !comp->target().cpu.isLittleEndian())
      ...
      if (!comp->cg()->supportsByteswap())
         {
         trprintf(trace, log, "Little endian load on big endian target without byteswap is not supported. rootNode: %p\n", rootNode);
         return currentTreeTop;
         }
      swapBytes = true;
      }
      ...
      if (!comp->cg()->supportsByteswap())
         {
         trprintf(trace, log, "Big endian load on little endian target without byteswap is not supported. rootNode: %p\n", rootNode);
         return currentTreeTop;
         }
      swapBytes = true;
      }
   ...
   trprintf(trace, log, "Sequential Load reduced at rootNode: %p\n", rootNode);
   ...
   if (((4 == byteCount) && !swapBytes && !is64bitResult) ||
      ...
      if (4 == byteCount)
         ...
         trprintf(trace, log, "Recreating rootNode (%p) as iloadi.\n", rootNode);
      ...
      trprintf(trace, log, "Recreating rootNode (%p) as lbyteswap.\n", rootNode);
      ...
      trprintf(trace, log, "Recreating newLoadChildNode (%p) as lloadi.\n", newLoadChildNode);
      ...
      if (is64bitResult)
         ...
         if (signExtendResult)
            {
            trprintf(trace, log, "Recreating rootNode (%p) as i2l.\n", rootNode);
            TR::Node::recreate(rootNode, TR::i2l);
            }
         else
            {
            trprintf(trace, log, "Recreating rootNode (%p) as iu2l.\n", rootNode);
         ...
         if (!swapBytes)
            ...
            trprintf(trace, log, "Creating ibyteswapNode (%p) with child %p.\n", ibyteswapNode, ibyteswapNode->getFirstChild());
         ...
         {
         /* Inserts byteswap above the load. */
         trprintf(trace, log, "Recreating rootNode (%p) as ibyteswap.\n", rootNode);
      ...
      trprintf(trace, log, "Recreating newLoadChildNode (%p) as iloadi.\n", newLoadChildNode);
      ...
      if (signExtendResult)
         ...
         if (is64bitResult)
            {
            trprintf(trace, log, "Recreating rootNode (%p) as s2l.\n", rootNode);
            TR::Node::recreate(rootNode, TR::s2l);
            }
         else
            {
            trprintf(trace, log, "Recreating rootNode (%p) as s2i.\n", rootNode);
         ...
         if (is64bitResult)
            {
            trprintf(trace, log, "Recreating rootNode (%p) as su2l.\n", rootNode);
            TR::Node::recreate(rootNode, TR::su2l);
            }
         else
            {
            trprintf(trace, log, "Recreating rootNode (%p) as su2i.\n", rootNode);
      ...
      if (!swapBytes)
         ...
         trprintf(trace, log, "Creating sbyteswapNode (%p) with child %p.\n", sbyteswapNode, sbyteswapNode->getFirstChild());

         trprintf(trace, log, "Setting rootNode (%p) child to %p.\n", rootNode, sbyteswapNode);
         rootNode->setAndIncChild(0, sbyteswapNode);
         }

      trprintf(trace, log, "Recreating newLoadChildNode (%p) as sloadi.\n", newLoadChildNode);
      ...
      trprintf(trace, log, "Creating mulNode (%p) with children %p and %p.\n", mulNode, mulNode->getFirstChild(), mulNode->getSecondChild());

      trprintf(trace, log, "Setting rootNode (%p) children to %p and %p.\n", rootNode, mulNode, byteConversionNodes[0]);
      ...
      if (is64bitResult)
         {
         trprintf(trace, log, "Recreating byteConversionNodes[0] (%p) as bu2l.\n", byteConversionNodes[0]);
         TR::Node::recreate(byteConversionNodes[0], TR::bu2l);
         }
      else
         {
         trprintf(trace, log, "Recreating byteConversionNodes[0] (%p) as bu2i.\n", byteConversionNodes[0]);
      ...
      if (signExtendResult)
         ...
         if (is64bitResult)
            {
            trprintf(trace, log, "Recreating newConvertChildNode (%p) as s2l.\n", newConvertChildNode);
            TR::Node::recreate(newConvertChildNode, TR::s2l);
            }
         else
            {
            trprintf(trace, log, "Recreating newConvertChildNode (%p) as s2i.\n", newConvertChildNode);
         ...
         if (is64bitResult)
            {
            trprintf(trace, log, "Recreating newConvertChildNode (%p) as su2l.\n", newConvertChildNode);
            TR::Node::recreate(newConvertChildNode, TR::su2l);
            }
         else
            {
            trprintf(trace, log, "Recreating newConvertChildNode (%p) as su2i.\n", newConvertChildNode);
            TR::Node::recreate(newConvertChildNode, TR::su2i);
            }
         }
      ...
      if (swapBytes)
         ...
         trprintf(trace, log, "Setting newConvertChildNode (%p) child to %p.\n", newConvertChildNode, sbyteswapNode);
      ...
      if (signExtendResult)
         {
         trprintf(trace, log, "Recreating newConvertChildNode (%p) as i2l.\n", newConvertChildNode);
         TR::Node::recreate(newConvertChildNode, TR::i2l);
         }
      else
         {
         trprintf(trace, log, "Recreating newConvertChildNode (%p) as iu2l.\n", newConvertChildNode);
      ...
      trprintf(trace, log, "Recreating shortLoadNode (%p) as sloadi.\n", shortLoadNode);
      TR::Node::recreateWithSymRef(shortLoadNode, TR::sloadi, shortSymRef);

      /* The short is never sign extended. */
      trprintf(trace, log, "Recreating shortConversionNode (%p) as su2l.\n", shortConversionNode);
      ...
      if (swapBytes)
         ...
         trprintf(trace, log, "Setting shortConversionNode (%p) child to %p.\n", shortConversionNode, sbyteswapNode);
      ...
      trprintf(trace, log, "Creating andNode (%p) with children %p and %p.\n", andNode, andNode->getFirstChild(), andNode->getSecondChild());
      ...
      trprintf(trace, log, "Recreating lowerIntLoadNode (%p) as iloadi.\n", lowerIntLoadNode);
      ...
      trprintf(trace, log, "Recreating lowerIntConversionNode (%p) as i2l.\n", lowerIntConversionNode);
      ...
      if (swapBytes)
         ...
         trprintf(trace, log, "Setting lowerIntConversionNode (%p) child to %p.\n", lowerIntConversionNode, ibyteswapNode1);
...
bool mayKillInterferenceBetweenNodes(TR::Compilation * comp,
   ...
   if (n1->referencesMayKillAliasInSubTree(n2, newVisitCount, comp))
      {
      trprintf(trace, log, " ---- node %p is killed somewhere in the subtree of node %p\n", n2, n1);

      if (!fineGrainedOverlap(comp, n1, n2))
         {
         trprintf(trace, log, " ----- n1 %p and n2 %p return false for fineGrainedOverlap\n", n1, n2);
         }
      else
         {
         return true;
