...
class EdgeFrequencyInfo
   ...
   TR::CFGEdge *loopBack;
   bool trace;
   void printEdge(TR::Logger *log, TR_BitVector *toAdd, TR_BitVector *toSub)
      {
      if (!toAdd->isEmpty())
         {
 ...
 */
void TR_JProfilingBlock::dumpCounterDependencies(TR::Logger *log, TR_BitVector **componentCounters)
   {
   TR::CFG *cfg = comp()->getFlowGraph();
   for (CFGNodeIterator iter(cfg, this); iter.currentBlock() != NULL; ++iter)
      {
