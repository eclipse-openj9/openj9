...
TR_CISCTransformer::createLoopCandidates(List<TR_RegionStructure> *loopCandidates)
   ...
   if (!candidate->isEmpty())
      {
      trprints(enableTracing, log, "createLoopCandidates: Evaluating list of loop candidates.\n");
      ...
      for (nextWhileLoop = whileLoopsIt.getFirst(); nextWhileLoop != 0; nextWhileLoop = whileLoopsIt.getNext())
         ...
         if (!loopBlockStructure)
            {
            trprintf(enableTracing, log, "\tRejected loop %d - no block structure.\n", naturalLoop->getNumber());
            continue;
            }
         if (!naturalLoop->containsOnlyAcyclicRegions())
            {
            trprintf(enableTracing, log, "\tRejected loop %d - not inner most loop.\n", naturalLoop->getNumber());
            continue; // inner most loop
            }
         if (loopBlockStructure->getBlock()->isCold())
            {
            trprintf(enableTracing, log, "\tRejected loop %d - cold loop.\n", naturalLoop->getNumber());
            continue;     // cold loop
            }
         loopCount++;
         loopCandidates->add(naturalLoop);

         trprintf(enableTracing, log, "\tAccepted loop %d as candidate.\n", naturalLoop->getNumber());
         }
#if SHOW_STATISTICS
   ...
   trprintf(enableTracing, log, "createLoopCandidates: %d loop candidates found.\n", loopCount);
...
TR_CISCTransformer::addPreHeaderIfNeeded(TR_RegionStructure *region)
   ...
   if (!preHeader)
      ...
      for (auto e = loopEntry->getPredecessors().begin(); e != loopEntry->getPredecessors().end(); ++e)
         ...
         trprintf(trace(), log, "fixing predecessor %d\n", predBlock->getNumber());
      ...
      trprintf(trace(), log, "added preheader block_%d\n", preHeader->getNumber());
      }
...
TR_CISCTransformer::analyzeHighFrequencyLoop(TR_CISCGraph *graph,
   ...
   {
   TR::Logger *log = comp()->log();
   trprints(trace(), log, "\tAnalyzing if loop is frequently iterated\n");
...
TR_CISCTransformer::easyTreeSimplification(TR::Node *const node)
   ...
   if (node->getOpCode().isIf())
      ...
      if (node->getOpCodeValue() == TR::ificmplt &&
          iconstC->getInt() == 1)
         {
         trprintf(trace(), comp()->log(), "\t\teasyTreeSimplification: Node: %p converted from ificmplt with 1 to ifcmple with 0", node);
   ...
   if (modified)
      trprintf(trace(), comp()->log(), "\t\teasyTreeSimplification: The tree %p is simplified.\n", node);
   }
...
TR_CISCTransformer::makeCISCGraphForBlock(TR_CISCGraph *graph, TR::Block *const block, int32_t dagId)
   ...
   trprintf(trace(), log, "\t\tmakeCISCGraphForBlock: Building CISCGraph for block %d.\n", block->getNumber());
   ...
   while(true)
      ...
      if (!addAllSubNodes(graph, block, top, NULL, top->getNode(), dagId))
         {
         trprintf(trace(), log, "\t\tFailed to create CISCNode for Node %p in block %d : %p\n", top->getNode(), block->getNumber(), block);
         return false;
         }
...
TR_CISCTransformer::makeCISCGraph(List<TR::Block> *pred,
   ...
   if (includePreds)
      {
      trprints(trace(), comp()->log(), "\tmakeCISCGraph: Building CISCGraph for Predecessor Blocks.\n");
   ...
   bodyDagId = dagId;
   bi.set(body);
   trprints(trace(), comp()->log(), "\tmakeCISCGraph: Building CISCGraph for Loop Body Blocks.\n");
...
int32_t TR_CISCTransformer::perform()
   ...
   if (createLoopCandidates(&loopCandidates))
      ...
      for (nextLoop = loopIt.getFirst(); nextLoop != 0; nextLoop = loopIt.getNext())
            ...
            if (region != NULL)
               ...
               while(predBlock != cfgStart)
                  ...
                  if (predBlock->getNumberOfRealTreeTops() > 300)
                     {
                     trprintf(trace(), log, "Skip the predecessor %d, because it has too many TreeTops (%d).\n", predBlock->getNumber(), predBlock->getNumberOfRealTreeTops());
                  ...
                  if (region != predBlockRegion)
                     {
                     trprintf(trace(), log, "Skip the predecessor block_%d, because it is within another region/loop.\n", predBlock->getNumber());
                     break;
                     }
                  }
               }
         ...
         if (!graph)
            {
            trprintf(trace(), log, "Loop %d.  Failed to make CISC Graph.\n", nextLoop->getNumber());
                        ...
                        if (tOpc == TR_variable)
                           ...
                           for (parent = parenti.getFirst(); parent; parent = parenti.getNext())
                              ...
                              if (!parent->isOutsideOfLoop() &&
                                 ...
                                 {
                                 trprintf(trace(), log, "pID%d: tID%d isn't loop invariant because of %d\n", p->getID(), t->getID(), parent->getID());
                                 isEmbed = false;
                                 break;
                                 }
                              }
         ...
         for (tle = tlistHead; tle; tle = tle->getNextElement())
            ...
            if (result[tmpIdx + t->getID()] == _Embed)
               ...
               if (!checkParents(p, t, result, &inLoop, &allOptionalParents) || !(inLoop || lightScreening ||
                  ...
                  trprintf(trace(), log, "screening1: set _NotEmbed to (%d, %d)\n", p->getID(), t->getID());
                  }
               else
                  {
                  count ++;
            ...
            for (s = si.getFirst(); s; s = si.getNext())
               ...
               if (mOpcode == s->getOpcode())
                  ...
                  if (result[tmpMIdx + tID] == _Embed)
                     ...
                     trprintf(trace(), log, "screening2, sameOpcode: set _NotEmbed to (%d, %d)\n", m->getID(), tID);
                     }
                  }
               }
               ...
               for (s = si.getFirst(); s; s = si.getNext())
                  ...
                  if (mOpcode != s->getOpcode())
                     ...
                     if (result[tmpMIdx + tID] == _Embed)
                        ...
                        thisScreening = true;
                        result[tmpMIdx + tID] = _NotEmbed;
                        trprintf(trace(), log, "screening2, others: set _NotEmbed to (%d, %d)\n", m->getID(), tID);
                        }
                     }
                  }
               }
      ...
      for (tle = tlistHead; tle; tle = tle->getNextElement())
         ...
         if (result[tmpIdx + t->getID()] == _Embed)
            ...
            if (!checkParents(p, t, result, &inLoop, &allOptionalParents) || !(inLoop || lightScreening))
               {
               result[tmpIdx + t->getID()] = _NotEmbed;
               trprintf(trace(), log, "screening3: set _NotEmbed to (%d, %d)\n", p->getID(), t->getID());
               }
            }
         }
      }
...
TR_CISCTransformer::makeLists()
   ...
   for (p = pi.getFirst(); p; p = pi.getNext())
      ...
      if (pList->isMultipleEntry() &&
         ...
         if (!p->isOptionalNode())
            {
            trprintf(trace(), log, "makeLists: pid:%d a variable corresponds to multiple nodes\n", p->getID());
   ...
   while(true) // check all TR_ahconst
      ...
      for (t = p2ti.getFirst(); t; t = p2ti.getNext()) // for each target node
         ...
         if (invalidate)
            ...
            trprintf(trace(), log, "tid:%d (pid:%d) is invalidated because of failure of analyzeArrayHeaderConst\n", tid,pid);
...
TR_CISCTransformer::alignTopOfRegion(TR_CISCNodeRegion *r)
   ...
   trprintf(trace(), log, "alignTopOfRegion: (pTop, t) is (%d, %d)\n", pTop->getID(), t->getID());
   ...
   trprintf(trace(), log, "alignTopOfRegion failed. Cannot find pTop:%d in the region.\n", pTop->getID());
   return false;
   }
...
TR_CISCTransformer::moveCISCNodesInList(List<TR_CISCNode> *l, TR_CISCNode *from, TR_CISCNode *to, TR_CISCNode *moveTo)
   {
   trprintf(trace(), comp()->log(), "moveCISCNodesInList: r_from:%p(%d) r_to:%p(%d) moveTo:%p(%d)\n", from, from->getID(), to, to->getID(), moveTo, moveTo->getID());
         ...
         if (!_T2P[tID].isEmpty() && (t->isDataConnected() || t->isNegligible()))
            ...
            if (t->getIlOpCode().isIf() && !t->isOutsideOfLoop())
               if (!t->isPredSimplyConnected() && !isSingleLoopBody)
                  ...
                  trprintf(trace(), log, "Predecessor of tID %" OMR_PRIu32 " is different from that of idiom.\n", tID);
                  }
            }
         if (isEmbed)
            {
...
TR_CISCTransformer::verifyCandidate()
   ...
   for (b = bi.getFirst(); b; b = bi.getNext())
      ...
      while (true)
         ...
         if (!le)
            {
            trprintf(trace(), comp()->log(), "Cannot find TR::BBStart of block_%d in the region\n", b->getNumber());
...
TR_CISCTransformer::findFirstNode(TR::TreeTop **retTree, TR::Node **retNode, TR::Block **retBlock)
   ...
   *retNode = trNode;
   *retBlock = block;
   trprintf(trace(), log, "First node in candidate region - node: %p block_%d: %p\n", trNode, block->getNumber(), block);
   return true;
   }
...
TR_CISCTransformer::setSuccessorEdges(TR::Block *block, TR::Block *target0, TR::Block *target1)
   ...
   trprintf(trace(), comp()->log(), "setSuccessorEdges for block_%d [%p]: tgt0=%d tgt1=%d\n", block->getNumber(), block, target0->getNumber(), target1->getNumber());
...
TR_CISCTransformer::modifyBlockByVersioningCheck(TR::Block *block, TR::TreeTop *startTop, List<TR::Node> *guardList)
   ...
   if (guardList && !guardList->isEmpty())
      ...
      trprintf(trace(), comp()->log(), "modifyBlockByVersioningCheck: orgPrevBlock=%d firstBlock=%d lastBlock=%d fastpath=%d slowpad=%d orgNextTreeTop=%x\n",
...
TR_CISCTransformer::insertBeforeNodes(TR::Block *block)
   ...
   trprintf(trace(), log, "insertBeforeNodes added %d node(s) to block_%d [%p]\n", count, block->getNumber(), block);
   if (last && last->getOpCode().isBranch())
      ...
      for (auto edge = block->getSuccessors().begin(); edge != block->getSuccessors().end(); ++edge)
         ...
         if (to != branchDestinationBlock &&
             to != orgNextBlock)
            {
            trprintf(trace(), log, "insertBeforeNodes added the edge (%d, %d).\n", newBlock->getNumber(), to->getNumber());
      ...
      trprintf(trace(), log, "insertBeforeNodes created block_%d [%p]\n", newBlock->getNumber(), newBlock);
      block = newBlock;
      }
   return block;
   }
...
TR_CISCTransformer::insertAfterNodes(TR::Block *block, List<TR::Node> *l, bool prepend)
   ...
   trprintf(trace(), comp()->log(), "insertAfterNodes adds %d node(s)\n", count);
   return block;
   }
         ...
         if (analyzeT2P(n, defNode) & _T2P_MatchMask)
            ...
            if (n->getOpcode() == TR::Case)
               ...
               while (!child1->isInterestingConstant())
                  ...
                  if (!child1)
                     {
                     trprintf(trace(), log, "analyzeBoolTable failed for %p. (no single chain)\n", n->getChild(1));
                     return false;
                     }
                  if (!child1->isStoreDirect())
                     {
                     trprintf(trace(), log, "analyzeBoolTable failed for %p. (%p is not store)\n", n->getChild(1), child1);
                     return false;
                     }
               ...
               if (!n->isOutsideOfLoop())
                  ...
                  if (!initExitTreeTop)
                     {
                     trprintf(trace(), log, "analyzeBoolTable - Delimiter checking node %d targets treetop: %p block_%d: %p\n",
   ...
   if (!info.IsZero())
      ...
      for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne())
         ...
         if (useNode->getReferenceCount() > 1)
            {
            trprintf(trace(), log, "canConvertArrayCmpSign failed because ReferenceCount > 1. %p\n", useNode);
            return false;
            }
         ...
         if (useCISCNode)
            ...
            if (NULL == foundTT || !searchNodeInTrees(foundTT->getNode(), useNode, &parentNode, &retChildNum))
               {
               trprintf(trace(), log, "canConvertArrayCmpSign failed because searchNodeInTrees failed. UseNode: %p with corresponding TreeTop: %p\n", useNode, foundTT);
               return false;
               }
            }
         if (!parentNode)
            {
            trprintf(trace(), log, "canConvertArrayCmpSign failed because parentNode is NULL. %p\n", useNode);
            return false;
            }
         ...
         if (parentNode->getOpCode().isStoreDirect())
            ...
            if (!canConvertArrayCmpSign(parentNode, compareIfs, &convertToArrayCmp))
               {
               trprintf(trace(), log, "canConvertArrayCmpSign failed because canConvertArrayCmpSign(p) failed. %p\n", useNode);
               return false;
               }
            }
            ...
            if (theOtherChild->getInt() != 0 ||
               ...
               {
               trprintf(trace(), log, "canConvertArrayCmpSign failed because theOtherChild is not iconst 0. %p\n", useNode);
               return false;
               }
            ...
            switch(parentNode->getOpCodeValue())
               ...
               case TR::ificmpne:
                  break; // OK!
               default:
                  trprintf(trace(), log, "convertArrayCmp failed because parentNode is %s. %x\n",
            ...
            {
            trprintf(trace(), log, "canConvertArrayCmpSign failed because unhandled opcode %s. %x %x\n",
...
TR_CISCTransformer::computeTopologicalEmbedding(TR_CISCGraph *P, TR_CISCGraph *T)
   ...
   if (disabledPatterns && TR::SimpleRegex::match(disabledPatterns, P->getTitle()))
      {
      trprintf(trace(), log, "%s is disabled by disabledIdiomPatterns={}\n", P->getTitle());
      return false;
      }
   ...
   if (!T->testAllAspects(P))
      {
      trprintf(trace(), log, "%s is skipped since graph properties do not match (%08x)\n", P->getTitle(), P->getAspectsValue());
      return false;                     // No need to analyze
      }
   if (T->testAnyNoAspects(P))
      {
      trprintf(trace(), log, "%s is skipped due to existence of testAnyNoAspects (%08x)\n", P->getTitle(), P->getNoAspectsValue());
      return false;                     // No need to analyze
      }
   if (!T->meetMinCounts(P))
      {
      trprintf(trace(), log, "%s is skipped due to failure of meetMinCounts (%d %d %d)\n",
   ...
   if (T->getNumNodes() >= IDIOM_SIZE_FACTOR*P->getNumNodes())
      {
      trprintf(trace(), log, "%s is skipped due to loop being very large\n", P->getTitle());
      return false;  // No need to analyze
      }
   ...
   if (1)//!_compilation->getOption(TR_EnableIdiomRecognitionWarm))
      ...
      if (T->getHotness() < P->getHotness())
         {
         trprintf(trace(), log, "%s is skipped due to hotness\n", P->getTitle());
         return false;                     // No need to analyze
         }
      }
   ...
   if (!incorrectBBFreqLevel)
      ...
      if (P->isHighFrequency() && !T->isHighFrequency())
         {
         trprintf(trace(), log, "%s is skipped due to the rarely iterated loop (!isHighFrequency)\n", P->getTitle());
         return false;     // No need to analyze
         }
      }
   if (T->getHotness() > warm)  // Because IR is called only one time at the warm level, skip this check.
      ...
      if (isAfterVersioning() ? P->isInhibitAfterVersioning() :
         ...
         {
         trprintf(trace(), log, "%s is skipped due to loop versioning check\n", P->getTitle());
         return false;     // No need to analyze
         }
      }
#endif
   ...
   //
   trprintf(trace(), log, "Computing embedding info for idiom %s in loop %d\n", P->getTitle(), _bblistBody.getListHead()->getData()->getNumber());
   ...
   trprintf(trace(), log, "Detected IL nodes in loop for idiom %s\n", P->getTitle());
   ...
   trprintf(trace(), log, "finished topological embedding for idiom %s\n", P->getTitle());
   ...
   if (!_candidateRegion ||
      ...
      {
      trprints(trace(), log, "computeTopologicalEmbedding: Graph transformations failed. (step 3)\n\n");
      registerCandidates();
   ...
   if (trace() || showMesssagesStdout())
      ...
      trprintf(trace(), log, "***** Transformed *****, %s, %s, %s, loop:%d%s\n",
         comp()->getHotnessName(comp()->getMethodHotness()),
...
TR_CISCTransformer::removeBitsKeepAliveCalls(List<TR::Block> *body)
   {
   TR::Logger *log = comp()->log();
   trprints(trace(), log, "\tScanning for java/nio/Bits.keepAlive(Ljava/lang/Object;)V calls.\n");
         ...
         if (node->getOpCodeValue() == TR::treetop)
            ...
            if (node->getOpCode().isCall())
               ...
               if (methodSym->getRecognizedMethod() == TR::java_nio_Bits_keepAlive
                  ...
                  {
                  trprintf(trace(), log, "\t\tRemoving KeepAlive call found in block %d [%p] @ Node: %p\n", block->getNumber(), block, node);
                  foundCall = true;
...
TR_CISCTransformer::insertBitsKeepAliveCalls(TR::Block * block)
   {
   TR::Logger *log = comp()->log();
   trprints(trace(), log, "\tInserting java/nio/Bits.keepAlive(Ljava/lang/Object;)V calls into reduced loop.\n");
...
TR_CISCTransformer::restoreBitsKeepAliveCalls()
   {
   TR::Logger *log = comp()->log();
   trprints(trace(), log, "\tRestoring for java/nio/Bits.keepAlive(Ljava/lang/Object;)V calls.\n");
   ListIterator<TR_BitsKeepAliveInfo> bi(&_BitsKeepAliveList);
   ...
   for (TR_BitsKeepAliveInfo *info = bi.getFirst(); info != NULL; info = bi.getNext())
      ...
      trprintf(trace(), log, "\t\tInserting KeepAlive call found in block %d [%p] @ Node: %p\n", block->getNumber(), block, tt->getNode());
      prev->insertAfter(tt);
      }
   }
...
TR_CISCTransformer::countFail(const char *fmt, ...)
   ...
   trprintf(trace(), comp()->log(), "failed: %s\n", _countFailBuf.text());
