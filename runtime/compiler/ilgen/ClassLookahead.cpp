...
TR_ClassLookahead::perform()
   ...
   trprintf(_traceIt, comp()->log(), "ATTN: Doing classlookahead for %.*s\n", len, name);
...
TR_ClassLookahead::examineNode(TR::TreeTop *nextTree, TR::Node *grandParent, TR::Node *parent, int32_t childNum, TR::Node *node, vcount_t visitCount)
   ...
   if (node->getOpCode().isStore())
      ...
      if (isProperFieldAccess(node))
          ...
          if (fieldInfo &&
             ...
             if (fieldInfo->isTypeInfoValid())
                ...
                if (classPtrSame || isClassPtrInitialized)
                   ...
                   if (_inClassInitializerMethod)
                      {
                      trprintf(_traceIt, log, "CStatically initializing type info for symbol %x at node %x\n", sym, node);
                      ...
                      if (fieldInfo->isTypeInfoValid() == VALID_AND_INITIALIZED_STATICALLY)
                         ...
                         if (!_inFirstInitializerMethod &&
                             !sym->isStatic())
                            {
                            trprintf(_traceIt, log, "C0Not always initializing type info for symbol %x at node %x\n", sym, node);
                            ...
                            if (!sym->isStatic())
                               {
                               trprintf(_traceIt, log, "C0Always initializing type info for symbol %x at node %x\n", sym, node);
                         ...
                         if (!sym->isStatic())
                            {
                            trprintf(_traceIt, log, "C1Always initializing type info for symbol %x at node %x\n", sym, node);
                            fieldInfo->setIsTypeInfoValid(VALID_AND_ALWAYS_INITIALIZED);
                            }
                         else
                            {
                            trprintf(_traceIt, log, "C0Invalidating type info for symbol %x at node %x\n", sym, node);
             ...
             if (isNewArray &&
                ...
                if (_inFirstBlock &&
                   ...
                   if (oldNumDimensions > -1)
                      ...
                      if (arrayFieldInfo->isDimensionInfoValid())
                         ...
                         if (oldNumDimensions != numberOfDimensions)
                            ...
                            //
                            trprintf(_traceIt, log, "0Invalidating dimension info for symbol %x at node %x\n", sym, node);
                   ...
                   {
                   trprintf(_traceIt, log, "00Invalidating dimension info for symbol %x at node %x\n", sym, node);
                   arrayFieldInfo->setIsDimensionInfoValid(INVALID);
                   }
                ...
                if (arrayFieldInfo->isDimensionInfoValid())
                   ...
                   for (;k<(rhsNumChildren - 1 - firstDimensionChild);k++)
                       ...
                       if (_inFirstBlock &&
                           ...
                           if (_inClassInitializerMethod)
                              {
                              trprintf(_traceIt, log, "Statically initializing dimension info for symbol %x at node %x\n", sym, node);
                              ...
                              if (arrayFieldInfo->isDimensionInfoValid() == VALID_AND_INITIALIZED_STATICALLY)
                                 ...
                                 if (!_inFirstInitializerMethod &&
                                     !sym->isStatic())
                                    {
                                    trprintf(_traceIt, log, "0Not always initializing dimension info for symbol %x at node %x\n", sym, node);
                                    ...
                                    if (!sym->isStatic())
                                       {
                                       trprintf(_traceIt, log, "0Always initializing dimension info for symbol %x at node %x\n", sym, node);
                                 ...
                                 if (!sym->isStatic())
                                    {
                                    trprintf(_traceIt, log, "1Always initializing dimension info for symbol %x at node %x\n", sym, node);
                                    arrayFieldInfo->setIsDimensionInfoValid(VALID_AND_ALWAYS_INITIALIZED);
                                    }
                                 else
                                    {
                                    trprintf(_traceIt, log, "C0Invalidating dimension info for symbol %x at node %x\n", sym, node);
                                    arrayFieldInfo->setIsDimensionInfoValid(INVALID);
                                    }
                                 }
                              }
                           ...
                           {
                           trprintf(_traceIt, log, "C1Invalidating dimension info for symbol %x at node %x\n", sym, node);
                           arrayFieldInfo->setIsDimensionInfoValid(INVALID);
                           arrayFieldInfo->setDimensionInfo(k, -1);
                           }
                       }
                ...
                //
                trprintf(_traceIt, log, "1Invalidating dimension info for symbol %x at node %x\n", sym, node);
                arrayFieldInfo->setIsDimensionInfoValid(INVALID);
                }
             }
          else if (fieldInfo)
             ...
             //
             trprintf(_traceIt, log, "1Invalidating dimension and type info for symbol %x at node %x(%s)\n", sym, node, node->getOpCode().getName());
          ...
          if (fieldInfo)
             {
             trprintf(_traceIt, log, "2Invalidating dimension and type info for symbol %x at node %x\n", sym, node);
   ...
   if ((sym->isShadow() ||
       ...
       if (arrayFieldInfo)
          ...
          if (!parent ||
              ...
              {
              trprintf(_traceIt, comp()->log(), "2Invalidating dimension and type info for symbol %x at node %x\n", sym, node);
              arrayFieldInfo->setIsDimensionInfoValid(INVALID);
      ...
      if (length > -1)
         ...
         if (newFieldInfo)
            ...
            if (newFieldInfo->canChangeToArray())
               ...
               {
               trprintf(_traceIt, comp()->log(), "fieldInfo %p exists already for array field %s, so cannot morph\n", newFieldInfo, sig);
...
void TR_ClassLookahead::makeInfoPersistent()
   ...
   while (fieldInfo)
      ...
      if (isTypeInfoValid || isDimensionInfoValid || isImmutable || (isNotRead && ((!hasBigDecimalAssumption || isBigDecimalType) && (!hasBigIntegerAssumption || isBigIntegerType))))
         ...
         if (arrayFieldInfo)
            {
            trprintf(_traceIt, log, "Creating persistent info for array field %s\n", persistentSig);
            newFieldInfo = new (PERSISTENT_NEW) TR_PersistentArrayFieldInfo(*arrayFieldInfo);
            }
         else
            {
            trprintf(_traceIt, log, "Creating persistent info for field %s\n", persistentSig);
