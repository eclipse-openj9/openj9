...
J9::Z::CodeGenerator::widenUnicodeSignLeadingSeparate(TR::Node *node, TR_PseudoRegister *reg, int32_t endByte, int32_t bytesToClear, TR::MemoryReference *targetMR)
   ...
   if (bytesToClear > 0)
      ...
      trprintf(trace, log, "\twidenUnicodeSignLeadingSeparate: node %p, reg %s targetStorageRef #%d, endByte %d, bytesToClear %d\n",
      ...
      trprintf(trace, log, "\tgen MVC of size 2 to move unicode leading separate sign code left by %d bytes to the widened left aligned position\n", bytesToClear);
...
J9::Z::CodeGenerator::genZeroLeftMostUnicodeBytes(TR::Node *node, TR_PseudoRegister *reg, int32_t endByte, int32_t bytesToClear, TR::MemoryReference *targetMR)
   ...
   if (bytesToClear > 0)
      ...
      bool trace = self()->traceBCDCodeGen();
      TR_StorageReference *storageRef = reg ? reg->getStorageReference() : NULL;
      trprintf(trace, log, "\tgenZeroLeftMostUnicodeBytes: node %p, reg %s targetStorageRef #%d, endByte %d, bytesToClear %d\n",
            node, reg?self()->getDebug()->getName(reg):"0", storageRef?storageRef->getReferenceNumber():0, endByte, bytesToClear);
      ...
      for (int32_t i = 0; i < unpkuCount; i++)
         ...
         trprintf(trace, log, "\tgen %d of %d UNPKUs with dest size of %d destOffset of %d and fixed source size %d\n", i+1, unpkuCount, unpkuSize, destOffset, tempSize);
...
J9::Z::CodeGenerator::widenZonedSignLeadingSeparate(TR::Node *node, TR_PseudoRegister *reg, int32_t endByte, int32_t bytesToClear, TR::MemoryReference *targetMR)
   ...
   if (bytesToClear > 0)
      ...
      bool trace = self()->traceBCDCodeGen();
      TR_StorageReference *storageRef = reg ? reg->getStorageReference() : NULL;
      trprintf(trace, log, "\twidenZonedSignLeadingSeparate: node %p, reg %s targetStorageRef #%d, endByte %d, bytesToClear %d\n",
            node, reg?self()->getDebug()->getName(reg):"0", storageRef?storageRef->getReferenceNumber():0, endByte,bytesToClear);
      targetMR = reuseS390LeftAlignedMemoryReference(targetMR, node, storageRef, self(), endByte);
      trprintf(trace, log, "\tgen MVC of size 1 to move zoned leading separate sign code left by %d bytes to the widened left aligned position\n", bytesToClear);
      TR::MemoryReference *originalSignCodeMR = generateS390LeftAlignedMemoryReference(*targetMR, node, bytesToClear, self(), endByte);
      int32_t mvcSize = 1;
      generateSS1Instruction(self(), TR::InstOpCode::MVC, node,
                             mvcSize-1,
...
J9::Z::CodeGenerator::widenZonedSignLeadingEmbedded(TR::Node *node, TR_PseudoRegister *reg, int32_t endByte, int32_t bytesToClear, TR::MemoryReference *targetMR)
   ...
   if (bytesToClear > 0)
      ...
      bool trace = self()->traceBCDCodeGen();
      TR_StorageReference *storageRef = reg ? reg->getStorageReference() : NULL;
      trprintf(trace, log, "\twidenZonedSignLeadingEmbedded: node %p, reg %s targetStorageRef #%d, endByte %d, bytesToClear %d\n",
            node, reg?self()->getDebug()->getName(reg):"0", storageRef?storageRef->getReferenceNumber():0, endByte, bytesToClear);
      self()->genZeroLeftMostZonedBytes(node, reg, endByte, bytesToClear, targetMR);
      targetMR = reuseS390LeftAlignedMemoryReference(targetMR, node, storageRef, self(), endByte);
      trprintf(trace, log, "\tgen MVZ of size 1 to move leading sign code left by %d bytes to the widened left aligned position\n", bytesToClear);
      TR::MemoryReference *originalSignCodeMR = generateS390LeftAlignedMemoryReference(*targetMR, node, bytesToClear, self(), endByte);
      int32_t mvzSize = 1;
      generateSS1Instruction(self(), TR::InstOpCode::MVZ, node,
         ...
         {
         trprintf(trace, log, "\tgenerate OI 0xF0 to force original leading sign code at offset=bytesToClear=%d\n", bytesToClear);
...
J9::Z::CodeGenerator::genZeroLeftMostZonedBytes(TR::Node *node, TR_PseudoRegister *reg, int32_t endByte, int32_t bytesToClear, TR::MemoryReference *targetMR)
   ...
   if (bytesToClear > 0)
      {
      TR_StorageReference *storageRef = reg ? reg->getStorageReference() : NULL;
      trprintf(self()->traceBCDCodeGen(), self()->comp()->log(), "\tgenZeroLeftMostZoneBytes: (%s) %p, reg %s targetStorageRef #%d, endByte %d, bytesToClear %d\n",
...
J9::Z::CodeGenerator::freeUnusedTemporaryBasedHint(TR::Node *node)
   ...
   if (hint && hint->isTemporaryBased() && hint->getTemporaryReferenceCount() == 0)
      ...
      trprintf(self()->traceBCDCodeGen(), self()->comp()->log(), "\tfreeing (pending) unused hint symRef #%d (%s) on %s (%p)\n",
...
J9::Z::CodeGenerator::storageReferencesMatch(TR_StorageReference *ref1, TR_StorageReference *ref2)
   ...
   if (ref1->isNodeBased() && (ref1->getNode()->getOpCode().isLoadVar() || ref1->getNode()->getOpCode().isStore()) &&
      ...
      if (ref1->getNode()->getSize() != ref2->getNode()->getSize())
         {
         trprintf(trace, log, "\tnode based storageRefs match = false : ref1 (#%d) and ref2 (#%d) addresses match but node1 %s (%p) size=%d != node2 %s (%p) size=%d\n",
         ...
         {
         trprintf(trace, log, "\tnode based storageRefs match = true : ref1 (#%d) %s (%p) == ref2 (#%d) %s (%p)\n",
      ...
      {
      trprintf(trace, log, "\ttemp based storageRefs match = true : ref1 (#%d) == ref2 (#%d) match\n", ref1->getReferenceNumber(), ref2->getReferenceNumber());
      refMatch = true;
      }
   return refMatch;
   }
...
J9::Z::CodeGenerator::processUnusedStorageRef(TR_StorageReference *ref)
   ...
   trprintf(trace, log, "\tprocessUnusedStorageRef ref->node %s (%p) with addrChild %s (%p)\n",
   ...
   if (addrChild)
      ...
      if (ref->getNodeReferenceCount() == 1)
         {
         trprintf(trace, log, "\t\tstorageRef->nodeRefCount %d == 1 so processUnusedAddressNode %s (%p) (refCount %d)\n",
         ...
         trprintf(trace, log, "\t\tstorageRef->nodeRefCount %d > 1 so do not decRefCounts of unusedAddressNode %s (%p) (refCount %d)\n",
            ref->getNodeReferenceCount(), addrChild->getOpCode().getName(), addrChild, addrChild->getReferenceCount());
      }

   trprintf(trace, log, "\tdec storageRef->nodeRefCount %d->%d\n", ref->getNodeReferenceCount(), ref->getNodeReferenceCount()-1);
...
J9::Z::CodeGenerator::canUseSingleStoreAsAnAccumulator(TR::Node *parent, TR::Node *node, TR::Node *store,TR_AliasSetInterface &storeAliases, TR::list<TR::Node*> *conflictingAddressNodes, bool justLookForConflictingAddressNodes, bool isChainOfFirstChildren, bool mustCheckAllNodes)
   ...
   if (!mustCheckAllNodes)
      ...
      if (self()->endAccumulatorSearchOnOperation(node))
         {
         trprintf(trace, log, "\t\t\tallow -- found node %s (%p) with endSearch = yes\n", node->getOpCode().getName(), node);
         if (conflictingAddressNodes->empty())
            ...
            trprints(trace, log, "\t\tconflictingAddressNodes list is not empty so continue searching for conflictingAddressNodes\n");
         ...
         if (!mustCheckAllNodes)
            trprints(trace, log, "\t\tFound a node that could clobber the accumulator before use; must check all children\n");
   ...
   if (!justLookForConflictingAddressNodes)
      ...
      if (node->getOpaquePseudoRegister())
         ...
         trprintf(trace, log, "\t\tfound evaluated reg %s : storageRef #%d ", self()->getDebug()->getName(node->getOpaquePseudoRegister()), storageRef->getReferenceNumber());
         if (storageRef->isTemporaryBased())
            {
            trprints(trace, log, "(tempBased)\n");
            ...
            {
            trprintf(trace, log, "(constNodeBased storageRefNode %s (%p))\n", storageRef->getNode()->getOpCode().getName(), storageRef->getNode());
   ...
   if (symRefForAliasing &&
      ...
      if (parent &&
         ...
         {
         trprintf(trace, log, "\t\t\tallow hint (loadVar case) %s (%p) -- store %s #%d (%p) location = nodeForAliasing %s #%d (%p) location\n",
         ...
         trprintf(trace, log, "\t\t\tallow hint (storeVar case) %s (%p) -- store %s #%d (%p) location = nodeForAliasing %s #%d (%p) location\n",
               parent->getOpCode().getName(), parent,
               store->getOpCode().getName(), store->getSymbolReference()->getReferenceNumber(), store,
               nodeForAliasing->getOpCode().getName(), symRefForAliasing->getReferenceNumber(), nodeForAliasing);
      ...
      // where the store is to the leading bytes of the load.  See RTC 95073
         ...
         {
         trprintf(trace, log, "\t\t\tallow hint (pdshr in place case) %s (%p) -- store %s #%d (%p) location = nodeForAliasing %s #%d (%p) location\n",
               parent->getOpCode().getName(), parent,
               store->getOpCode().getName(), store->getSymbolReference()->getReferenceNumber(), store,
               nodeForAliasing->getOpCode().getName(), symRefForAliasing->getReferenceNumber(), nodeForAliasing);
         ...
         {
         trprintf(trace, log, "\t\t\tallow hint (pdMod in place case) %s (%p) -- store %s #%d (%p) location = nodeForAliasing %s #%d (%p) location\n",
               parent->getOpCode().getName(), parent,
               store->getOpCode().getName(), store->getSymbolReference()->getReferenceNumber(), store,
               nodeForAliasing->getOpCode().getName(), symRefForAliasing->getReferenceNumber(), nodeForAliasing);
         ...
         if (useAliasing &&   // checking useAliasing here because in the no info case the above loadAndStoreMayOverlap already did the pattern match
            ...
            trprintf(trace, log, "\t\t\tcheck children -- useAccum=true aliasing test failed but pattern match passed for nodeForAliasing %s (%p) with symRefForAliasing #%d\n",
                  nodeForAliasing->getOpCode().getName(), nodeForAliasing, symRefForAliasing->getReferenceNumber());
            }
         else
            {
            trprintf(trace, log, "\t\t\tdisallow -- useAccum=false for nodeForAliasing %s (%p) with symRefForAliasing #%d\n",
                  nodeForAliasing->getOpCode().getName(), nodeForAliasing, symRefForAliasing->getReferenceNumber());
            return false;
            }
         }
   ...
   if (node->getOpCode().isLoad())
      {
      trprintf(trace, log, "\t\t\t%s -- found load %s (%p) under store %s (%p)\n", (mustCheckAllNodes ? "check children" : "allow"),
      ...
      {
      trprintf(trace, log, "\t\t\tallow -- found base case evaluated reg %s on node %s (%p) under store %s (%p)\n",
   ...
   if (self()->foundConflictingNode(node, conflictingAddressNodes))
      ...
      trprintf(trace, log, "\t\t\ta^a: disallow -- useAccum=false because node %s (%p) was found commoned from address tree on %s (%p)\n",
            node->getOpCode().getName(), node, store->getOpCode().getName(), store);
      return false;
      }
      ...
      if (self()->foundConflictingNode(child, conflictingAddressNodes))
         ...
         if (!canUseSingleStoreAsAnAccumulator(node, child, store, storeAliases, conflictingAddressNodes, justLookForConflictingAddressNodes, continueChainOfFirstChildren, mustCheckAllNodes))
            ...
            if (!justLookForConflictingAddressNodes && self()->endHintOnOperation(node))
               {
               trprintf(trace, log, "\t\t\ta^a: : endHint mismatch -- node %s (%p)\n", node->getOpCode().getName(), node);
               }
            return false;
            }
         }
...
J9::Z::CodeGenerator::validateAddressOneToAddressOffset(int32_t expectedOffset,
   ...
   if (!canGetOffset)
      {
      trprintf(trace, log, "\tvalidateAddressOneToAddressOffset = false : could not compute offset between addr1 %s (%p) (+%lld) and addr2 %s (%p) (+%lld)\n",
   ...
   if (addrOffset != expectedOffset)
      {
      trprintf(trace, log, "\tvalidateAddressOneToAddressOffset = false : addrOffset %d not the expected value of %d between addr1 %s (%p) (+%lld) and addr2 %s (%p) (+%lld)\n",
...
J9::Z::CodeGenerator::getAddressOneToAddressTwoOffset(bool *canGetOffset,
   ...
   *offset=0;
   ...
   if (!foundOffset &&
      ...
      trprintf(trace, log, "\t\t(addr2 %s (%p) + %lld) = (addr1 %s (%p) + %lld) + offset (%lld) : node matches case\n",
   ...
   if (!foundOffset &&
      ...
      if (addr1->getSecondChild()->getOpCode().isIntegralConst() &&
         ...
         trprintf(trace, log, "\t\t(addr2 %s (%p) + %lld) = (addr1 %s (%p) + %lld) + offset (%lld) : both adds case\n",
               addr2->getOpCode().getName(), addr2, addr2ExtraOffset, addr1->getOpCode().getName(), addr1, addr1ExtraOffset, offset64);
         }
      }
   ...
   if (!foundOffset &&
      ...
      trprintf(trace, log, "\t\t(addr2 %s (%p) + %lld) = addr1 %s (%p) + offset (%lld) : 2nd add case\n",
   ...
   if (!foundOffset &&
    ...
    if (baseLoad1 != NULL && baseLoad2 != NULL &&
         ...
         int64_t addr2Offset = addr2->getSecondChild()->get64bitIntegralValue() + addr2ExtraOffset;
         offset64 = (addr2Offset - addr1Offset);
         trprintf(trace, log, "\t\t(addr2 %s (%p) + %lld) = (addr1 %s (%p) + %lld) + offset (%lld) : baseLoad1 %s (%p) in notKilledList, both adds case\n",
...
J9::Z::CodeGenerator::addStorageReferenceHints(TR::Node *node)
   ...
   trprintf(trace, log, "--end--   examining cg treeTop %s (%p)\n\n", node->getOpCode().getName(), node);
...
J9::Z::CodeGenerator::examineNode(
   ...
   if (!node->hasBeenVisitedForHints()) // check other nodes using hasBeenVisitedForHints
      {
      trprintf(trace, log, "\tvisiting node - %s (%p), bestNode - %s (%p) (endHintOnNode=%s)\n",
      ...
      if (nodeCanHaveHint &&
         ...
         //
         trprintf(trace, log, "\t\tendHintOnNode=true so call processNodeList before examining ending hint node %p\n", node);
      ...
      if (isInterestingStore)
         ...
         if ((bestNode == NULL) ||
            ...
            if (!isAccumStore || node->useStoreAsAnAccumulator())
               ...
               if (trace)
                  {
                  if (isAccumStore)
                     log->printf("\t\tfound new store (canUse = %s) bestNode - %s (%p) with actual size %d and storageRefResultSize %d\n",
                        bestNode->useStoreAsAnAccumulator() ? "yes":"no", bestNode->getOpCode().getName(), bestNode, bestNode->getSize(),
                        bestNode->getStorageReferenceSize(comp));
                  else
                     log->printf("\t\tfound new non-store bestNode - %s (%p) (isConversionToNonAggrOrNonBCD=%s, isForcedTemp=%s) with actual size %d and storageRefResultSize %d\n",
                        bestNode->getOpCode().getName(), bestNode, isConversionToNonAggrOrNonBCD?"yes":"no",
                        self()->nodeRequiresATemporary(node)?"yes":"no", bestNode->getSize(), bestNode->getStorageReferenceSize(comp));
                  }
               }
            }
         ...
         if (!isAccumStore && !isConversionToNonAggrOrNonBCD && !isLoad) // don't add stores or bcd2x or load nodes to the list
            {
            trprintf(trace, log, "\t\tadd node - %s (%p) to list\n", node->getOpCode().getName(), node);
      ...
      if (endHintOnNode)
         ...
         trprintf(trace, log, "\t\tendHintOnNode=true for node - %s (%p) setting storeSize to %d\n", node->getOpCode().getName(), node, storeSize);
         }
      ...
      {
      checkNode = parent;
      trprintf(trace, log, "\tnot descending node - %s (%p) because it has been visited already\n", node->getOpCode().getName(), node);
      TR_OpaquePseudoRegister *reg = node->getOpCodeValue() == TR::BBStart ? NULL : node->getOpaquePseudoRegister();
      if (reg)
         {
         trprintf(trace, log, "\t\tnode - %s (%p) with reg %s is an already evaluated bcd node with refCount=%d\n",
         ...
         if (!reg->getStorageReference()->isTemporaryBased())
            {
            trprintf(trace, log, "\t\t\treg storageRef #%d is not a temp so do not update bestNode with node - %s (%p) but do reset reg %s isInit to false\n",
            ...
            {
            trprintf(trace, log, "\t\t\treg storageRef #%d with a store bestNode so do not update bestNode with node - %s (%p) refCount=%d\n",
                  reg->getStorageReference()->getReferenceNumber(), node->getOpCode().getName(), node, node->getReferenceCount());
            }
         else
            {
            trprintf(trace, log, "\t\t\treg storageRef #%d is a final-use (refCount==1) temp so set bestNode to node - %s (%p) reg->isInit=%s (and reuse temp storageRef))\n",
   ...
   if ((leftMostNodesList.empty()) || (checkNode == leftMostNodesList.front()))       // just finished with a left most path and there are nodes to tag with hint
      ...
      if (leftMostNodesList.empty())
         {
         trprintf(trace, log, "\tleftMostNodesList is empty so clear bestNode - %s (%p->NULL) for current node - %s (%p)\n",
               bestNode?bestNode->getOpCode().getName():"NULL", bestNode, node->getOpCode().getName(), node);
         bestNode = NULL;
         storeSize = 0;
         }
         ...
         {
         trprintf(trace, log, "\tcalling processNodeList with bestNode - %s (%p) because leftMostNodesList is not empty for current node - %s (%p)\n",
               bestNode?bestNode->getOpCode().getName():"NULL", bestNode, node->getOpCode().getName(), node);
         self()->processNodeList(bestNode, storeSize, leftMostNodesList);
         }
      }
...
J9::Z::CodeGenerator::processNodeList(
   ...
   if (bestNode)
      ...
      if (bestNode->getOpaquePseudoRegister())
         ...
         trprintf(trace, log, "\t\tbestNode - %s (%p) already has a register (%s) so use reg->getStorageReference #%d and %s\n",
         ...
         if (reg->getStorageReference()->isTemporaryBased() &&
            ...
            {
            trprintf(trace, log, "\t\treg->getStorageReference #%d is tempBased and requested storeSize %d > regLiveSymSize %d so increase tempSize\n",
         ...
         {
         trprintf(trace, log, "\t\tbestNode - %s (%p) is a store so create a new node based storage reference #%d\n",
         ...
         if (!leftMostNodesList.empty())
            ...
            if (trace)
               {
               log->printf("\t\tbestNode - %s (%p) is a BCD arithmetic or conversion op (isBCDToNonBCDConversion %s) and list is not empty so allocate a new temporary based storage reference\n",
            ...
            if (tempSize == bestNodeSize)
               ...
               trprintf(trace, log, "\t\tsetting keepTrackOfSharedNodes=true because hintSize is based on a non-store operation (bestNode %s - %p)\n",
                     bestNode->getOpCode().getName(), bestNode);
               }
            }
         else
            trprintf(trace, log, "\t\tbestNode %p is a BCD arithmetic or conversion op but list is empty so do not allocate a new temporary based storage reference\n", bestNode);
         }
      for (auto listIt = leftMostNodesList.begin(); listIt != leftMostNodesList.end(); ++listIt)
         ...
         trprintf(trace, log, "\ttag (*listIt) - %s (%p) with storageRefHint #%d (%s)\n",
         ...
         if (TR::ILOpCode::isPackedConversionToWiderType((*listIt)->getOpCodeValue()))
            ...
            if (firstChild->chkSkipPadByteClearing() &&
               ...
               {
               trprintf(trace, log, "\tUnset skipPadByteClearing on node %s (%p): storage ref hint has size %d and converted node has size %d\n",
                     firstChild->getOpCode().getName(), firstChild, storageRefHint->getSymbolSize(),
                     TR::DataType::getSizeFromBCDPrecision((*listIt)->getDataType(), firstChild->getDecimalPrecision()));
...
J9::Z::CodeGenerator::markStoreAsAnAccumulator(TR::Node *node)
   ...
   trprintf(trace, log, "markStoreAsAnAccumulator for node %s (%p) - useAliasing=%s\n", node->getOpCode().getName(), node, "yes");
   ...
   if (node->getOpCode().canUseStoreAsAnAccumulator())
      ...
      if (node->getOpCode().isIndirect())
         ...
         trprintf(trace, log, "\tlook for conflicting nodes in address subtree starting at %s (%p)\n", node->getFirstChild()->getOpCode().getName(), node->getFirstChild());
...
J9::Z::CodeGenerator::correctBadSign(TR::Node *node, TR_PseudoRegister *reg, int32_t endByte, TR::MemoryReference *memRef)
   ...
   if (reg && reg->hasKnownBadSignCode())
      ...
      trprintf(self()->traceBCDCodeGen(), self()->comp()->log(), "\tcorrectBadSign node %p: reg %s hasKnownBadSignCode()=true so force sign to a valid sign 0x%x\n",
...
J9::Z::CodeGenerator::genSignCodeSetting(TR::Node *node, TR_PseudoRegister *targetReg, int32_t endByte, TR::MemoryReference *targetMR, int32_t sign, TR_PseudoRegister *srcReg, int32_t digitsToClear, bool numericNibbleIsZero)
   ...
   if (sign == TR::DataType::getIgnoredSignCode())
      {
      trprintf(trace, log, "\tgenSignCodeSetting: node=%p, sign==ignored case srcReg %s, targetReg %s, srcReg->isSignInit %d, targetReg->isSignInit %d\n",
            node, srcReg?cg->getDebug()->getName(srcReg):"NULL", targetReg?cg->getDebug()->getName(targetReg):"NULL",
            srcReg?srcReg->signStateInitialized():0, targetReg?targetReg->signStateInitialized():0);
      if (targetReg != srcReg)
         {
         if (targetReg)
            {
            if (srcReg)
               ...
               trprintf(trace, log, "\tsign==ignored case and srcReg==NULL so setHasKnownBadSignCode=true on targetReg %s\n", cg->getDebug()->getName(targetReg));
               }
            }
         }
   ...
   trprintf(trace, log, "\tgenSignCodeSetting: node=%p, endByte=%d, sign=0x%x, signCodeOffset=%d, srcReg=%s, digitsToClear=%d, numericNibbleIsZero=%s (srcSign=0x%x, hasCleanSign=%s, hasPrefSign=%s, isEffectiveNop=%s)\n",
   ...
   switch (dt)
      ...
      case TR::ZonedDecimalSignLeadingEmbedded:
         ...
         if (isPacked && digitsToClear >= 3)
            ...
            trprintf(trace, log, "\t\tusing %d byte move imm (%s) for sign setting : set digitsCleared=%d\n", leftMostByte, leftMostByte==8?"MVGHI":(leftMostByte==4)?"MVHI":"MVHHI", digitsCleared);
            }
            ...
            digitsCleared = 1;
            trprints(trace, log, "\t\tusing MVI for sign setting : set digitsCleared=1\n");
            }
         else
            {
            if (knownSignIs0xF)
...
J9::Z::CodeGenerator::widenBCDValueIfNeeded(TR::Node *node, TR_PseudoRegister *reg, int32_t startByte, int32_t endByte, TR::MemoryReference *targetMR)
   ...
   int32_t bytesToClear = endByte - startByte;
   trprintf(self()->traceBCDCodeGen(), self()->comp()->log(), "\twidenBCDValue for node %s (%p) : %d->%d (%d bytes)\n", node->getOpCode().getName(), node, startByte, endByte, bytesToClear);
   if (bytesToClear > 0)
      {
...
J9::Z::CodeGenerator::genZeroLeftMostDigitsIfNeeded(TR::Node *node, TR_PseudoRegister *reg, int32_t endByte, int32_t digitsToClear, TR::MemoryReference *targetMR, bool widenOnLeft)
   ...
   trprintf(trace, log, "\tgenZeroLeftMostDigitsIfNeeded %s #%d for node %p: digitsToClear = %d, endByte = %d (digit range is %d->%d), widenOnLeft=%s\n",
   ...
   if (widenOnLeft &&
      ...
      trprintf(trace, log, "\ttargetMR->getStorageReference() #%d liveSymSize %d > endByte %d so increase endByte %d->%d (endDigit %d->%d) and retrieve the actualDigitsToClear based on this new endDigit\n",
...
J9::Z::CodeGenerator::genZeroLeftMostPackedDigits(TR::Node *node, TR_PseudoRegister *reg, int32_t endByte, int32_t digitsToClear, TR::MemoryReference *targetMR, int32_t memRefOffset)
   ...
   if (digitsToClear)
      ...
      trprintf(trace, log, "\tgenZeroLeftMostPackedDigits: node %p, reg %s targetMemSlot #%d, endByte %d, digitsToClear %d (fullBytesToClear %d), memRefOffset %d\n",
      ...
      if (fullBytesToClear)
         ...
         trprintf(trace, log, "\t\tgen XC with size %d and mr offset %d (destOffset %d + memRefOffset %d)\n", fullBytesToClear, destOffset+memRefOffset, destOffset, memRefOffset);
      ...
      if (digitsToClear&0x1)
         ...
         int32_t destOffset = 0;
            {
            trprintf(trace, log, "\tgen NI for odd clear digits with mr offset %d (fullBytesToClear %d + destOffset %d + memRefOffset %d)\n",
...
J9::Z::CodeGenerator::initializeStorageReference(TR::Node *node,
   ...
   trprintf(trace, log, "\tinitializeStorageReference for %s (%p), destReg %s, srcReg %s, sourceSize %d, destSize %d, performExplicitWidening=%s, trackSignState=%s\n",
         node->getOpCode().getName(), node,
         destReg ? self()->getDebug()->getName(destReg):"NULL", srcReg ? self()->getDebug()->getName(srcReg):"NULL",
         sourceSize, destSize, performExplicitWidening?"yes":"no", trackSignState?"yes":"no");
   ...
   if (!self()->storageReferencesMatch(dstStorageRef, srcStorageRef))
      ...
      trprintf(trace, log, "\t\tnode %p : srcReg %s (hasBadSign %s) on srcNode %p has bytes %d->%d %salready clear (bytesToClear=%d), srcCastedToBCD=%d\n",
      ...
      if (destSize > sourceSize &&
         ...
         trprintf(trace, log, "\tincrease mvcSize %d->%d to account for already cleared %d bytes\n", sourceSize, mvcSize, bytesToClear);
         }

      trprintf(trace, log, "\t\tgen MVC/memcpy to initialize storage reference with size = %d\n", mvcSize);
   ...
   if (destPseudoReg)
      ...
      trprintf(trace, log, "\tset destReg targetPrecision to %d (from %s for node dt %s)\n",
...
J9::Z::CodeGenerator::initializeNewTemporaryStorageReference(TR::Node *node,
                                                             ...
                                                             bool trackSignState)
   {
   trprintf(self()->traceBCDCodeGen(), self()->comp()->log(), "\tinitializeNewTemporaryStorageReference for node %p, destReg %s, srcNode %p, srcReg %s (with size %d), sourceSize %d, destSize %d\n",
         node, destReg ? self()->getDebug()->getName(destReg):"NULL", srcNode, srcReg ? self()->getDebug()->getName(srcReg):"NULL",
         srcReg?srcReg->getSize():0, sourceSize, destSize);
...
J9::Z::CodeGenerator::privatizePseudoRegister(TR::Node *node, TR_OpaquePseudoRegister *reg, TR_StorageReference *storageRef, size_t sizeOverride)
   ...
   trprintf(trace, log, "\ta^a : gen memcpy of size = %d to privatize node %s (%p) with storageRef #%d (%s) to #%d (%s) on line_no=%d\n",
...
J9::Z::CodeGenerator::privatizePseudoRegisterIfNeeded(TR::Node *parent, TR::Node *child, TR_OpaquePseudoRegister *childReg)
   ...
   if (hint && hint->isNodeBased())
      ...
      trprintf(trace, log, "\tprivatizePseudoRegisterIfNeeded for %s (%p) with hint %s (%p) (isInMemoryCopyProp=%s) and child %s (%p) (child storageRef isNonConstNodeBased=%s)\n",
            parent->getOpCode().getName(), parent,
      ...
      if (childStorageRef &&
         ...
         trprintf(trace, log, "\tisInMemoryCopyProp=%s, isUsingStorageRefFromAnotherStore=%s, childRegHasDeadOrIgnoredBytes=%s : childStorageRef %s (%p), hintNode %s (%p)\n",
         ...
         if (hintNode->isInMemoryCopyProp() || isUsingStorageRefFromAnotherStore || childRegHasDeadOrIgnoredBytes)
            {
            bool useAliasing = true;
            trprintf(trace, log, "\tcheck overlap between store hint %s (%p) and childStorageRefNode %s (%p)\n",
            ...
            if (self()->loadAndStoreMayOverlap(hintNode,
               ...
               trprintf(trace, log, "\toverlap=true (from %s test) -- privatize the source memref to a temp memref\n", useAliasing?"aliasing":"pattern");
               if (useAliasing && // checking useAliasing here because in the no info case the above loadAndStoreMayOverlap already did the pattern match
                   self()->storageMayOverlap(hintNode, hintNode->getSize(), childStorageRefNode, childStorageRefNode->getSize()) == TR_NoOverlap)
                  {
                  // get a second opinion -- the aliasing says the operations overlap but perhaps it is too conservative
                  // so do pattern matching based test to see if the operations are actually disjoint
                  trprints(trace, log, "\t\t but overlap=false (from 2nd opinion pattern test) -- set needsPrivitization to false\n");
               ...
               if (needsPrivitization)
                  ...
                  if (trace)
                     ...
                     if (hintNode->isInMemoryCopyProp())
                        log->printf("\ta^a : privatize needed due to isInMemoryCopyProp hintNode %s (%p) on line_no=%d (privatizeCase)\n",
                           hintNode->getOpCode().getName(),hintNode,comp->getLineNumber(hintNode));
                     if (isUsingStorageRefFromAnotherStore)
                        log->printf("\ta^a : privatize needed due to isUsingStorageRefFromAnotherStore childStorageRefNode %s (%p) on line_no=%d (privatizeCase)\n",
                           childStorageRefNode->getOpCode().getName(),childStorageRefNode,comp->getLineNumber(hintNode));
                     if (childRegHasDeadOrIgnoredBytes)
                        log->printf("\ta^a : privatize needed due to childRegHasDeadOrIgnoredBytes valueReg %s child %s (%p) on line_no=%d (privatizeCase)\n",
               ...
               {
               trprintf(trace, log, "\toverlap=false (from %s test) -- do not privatize the source memref\n", useAliasing?"aliasing":"pattern");
               }
            }
         else
            {
            trprintf(trace, log, "y^y : temp copy saved isInMemoryCopyProp = false on %s (%p) (privatizeCase)\n", hintNode->getOpCode().getName(), hintNode);
            }
         }
      }
   return outReg;
...
J9::Z::CodeGenerator::privatizeStorageReference(TR::Node *node, TR_OpaquePseudoRegister *reg, TR::MemoryReference *memRef)
   ...
   trprintf(trace, log, "privatizeStorageReference: %s (%p) refCount %d :: storageRef #%d, storageRefNode %s (%p) nodeRefCount %d, isNodeBased %s\n",
         node->getOpCode().getName(),
         node,
   ...
   if (force ||
      ...
      if (trace)
         {
         log->printf("\tnode %p (%s) with skipCopyOnLoad=false does need to be privatized for node based storageRef node %p (%s-based) (force=%s)\n",
            node,node->getOpCode().getName(),storageRef->getNode(),storageRef->getNode()->getOpCode().isStore()?"store":"load",force?"yes":"no");
         log->printf("\tb^b : gen memcpy of size = %d to privatizeStorageReference node %s (%p) with storageRef #%d (%s) on line_no=%d\n",
      ...
      trprintf(trace, log, "\t%s (%p) does NOT need to be privatised because isTemp (%s) and/or refCount %d <= 1 and/or skipCopyOnLoad=true (flag is %s)\n",
...
J9::Z::CodeGenerator::materializeFullBCDValue(TR::Node *node,
   ...
   trprintf(trace, log, "\tmaterializeFullBCDValue evaluated %s (%p) (nodeSize %d, requested resultSize %d) to reg %s (regSize %d), clearSize=%d, updateStorageReference=%s\n",
   ...
   if (clearSize == 0)
      ...
      trprintf(trace, log, "\tspecific clearSize not requested : set clearSize=resultSize=%d\n", resultSize);
      }
   else
      {
      ...
      trprintf(trace, log, "\tupdate clearSize %d to max(clearSize, regSize) = max(%d,%d) = %d\n", clearSize, clearSize, regSize, std::max(clearSize, regSize));
   ...
   if (regSize < resultSize &&
      ...
      {
      trprintf(trace, log, "\tbytes regSize->clearSize (%d->%d) are already clear -- no work to do to materializeFullBCDValue\n", regSize, clearSize);
      ...
      {
      trprintf(trace, log, "\treg->getSize() < resultSize (%d < %d) so check liveSymSize on reg\n", regSize, resultSize);
      ...
      if (reg->isInitialized() && reg->trackZeroDigits() && liveSymSize >= resultSize)
         {
         trprintf(trace, log, "\treg->getLiveSymbolSize() >= resultSize (%d >= %d) so call clearByteRangeIfNeeded\n", liveSymSize, resultSize);
         memRef = reuseS390RightAlignedMemoryReference(memRef, node, reg->getStorageReference(), self(), enforceSSLimitsForClear);
         ...
         {
         trprintf(trace, log, "\treg->getLiveSymbolSize() < resultSize (%d < %d) so call increaseTemporarySymbolSize but first check for already cleared bytes\n", liveSymSize, resultSize);
         ...
         trprintf(trace, log, "\tfound %d alreadyClearedBytes : adjust bytesToClear %d -> %d\n", alreadyClearedBytes, bytesToClear, bytesToClear-alreadyClearedBytes);
         ...
         if (clearSize == resultSize)
            ...
            trprintf(trace, log, "\tset leftAlignedZeroDigits to %d after temporarySymbolSize increase\n", newLeftAlignedZeroDigits);
            }
         ...
         {
         trprintf(trace, log, "\tstorageReference #%d is not tempBased (or is not packed) and reg->getLiveSymbolSize() < resultSize (%d < %d) so alloc a new temporary reference\n",
...
void genMVI(TR::MemoryReference *destMR, TR::Node *node, uint8_t value, TR::CodeGenerator *cg)
   {
   trprintf(cg->traceBCDCodeGen(), cg->comp()->log(), "\tgen MVI 0x%02x\n",value);
...
void genMVHHI(TR::MemoryReference *destMR, TR::Node *node, int16_t value, TR::CodeGenerator *cg)
   {
   trprintf(cg->traceBCDCodeGen(), cg->comp()->log(), "\tgen MVHHI 0x%04x\n", (uint16_t)value);
...
void genMVHI(TR::MemoryReference *destMR, TR::Node *node, int16_t value, TR::CodeGenerator *cg)
   {
   trprintf(cg->traceBCDCodeGen(), cg->comp()->log(), "\tgen MVHI 0x%04x\n", (uint16_t)value);
...
void genMVGHI(TR::MemoryReference *destMR, TR::Node *node, int16_t value, TR::CodeGenerator *cg)
   {
   trprintf(cg->traceBCDCodeGen(), cg->comp()->log(), "\tgen MVGHI 0x%04x\n", (uint16_t)value);
...
J9::Z::CodeGenerator::inlineSmallLiteral(size_t srcSize, char *srcLiteral, size_t destSize, bool trace)
   ...
   if (srcSize != destSize)
      {
      inlineLiteral = false;
      trprintf(trace, log, "\t\tinlineLiteral=false : srcSize %d != destSize %d\n", srcSize, destSize);
      }
   else if (srcSize == 1)
      {
      inlineLiteral = true;
      trprintf(trace, log, "\t\tinlineLiteral=true : srcSize == 1 (destSize %d)\n", destSize);
      }
   else if (destSize <= 2)
      {
      inlineLiteral = true;
      trprintf(trace, log, "\t\tinlineLiteral=true : destSize %d <= 2 (srcSize %d)\n", destSize, srcSize);
      }
   else if (self()->canCopyWithOneOrTwoInstrs(srcLiteral, srcSize))
      {
      inlineLiteral = true;
      trprintf(trace, log, "\t\tinlineLiteral=true : canCopyWithOneOrTwoInstrs = true (srcSize %d, destSize %d)\n", srcSize, destSize);
      }
   else
      {
      inlineLiteral = false;
      trprintf(trace, log, "\t\tinlineSmallLiteral=false : unhandled case (srcSize %d, destSize %d)\n", srcSize, destSize);
      }
...
J9::Z::CodeGenerator::evaluateBCDNode(TR::Node * node)
   ...
   if (isFirstTime)
      ...
      if (node->getOpCode().canHaveStorageReferenceHint() &&
         ...
         {
         trprintf(trace, log, "evaluateBCDNode: found temp based hint #%d on %s (%p)\n",
      ...
      if (node->getOpCode().canHavePaddingAddress())
         {
         trprintf(trace, log, "evaluateBCDNode: set UsedPaddingAnchorAddress flag to true on %s (%p)\n",
               node->getOpCode().getName(), node);
         }
      }
...
J9::Z::CodeGenerator::supportsPackedShiftRight(int32_t resultPrecision, TR::Node *shiftSource, int32_t shiftAmount)
   ...
   trprintf(self()->traceBCDCodeGen(), self()->comp()->log(), "%ssupportsPackedShiftRight = %s : shiftSource %s (%p) p=%d by shiftAmount=%d -> shiftedPrec=%d (resultPrec %d) on line_no=%d (offset=%06X)\n",
   ...
   if (node->getOpaquePseudoRegister())
      ...
      if (ref && ref->isNodeBased() && ref->getNodeReferenceCount() > 0)
         ...
         trprintf(self()->traceBCDCodeGen(), self()->comp()->log(),
               "\tnode %s (%p) with storageRef #%d (%s): increment nodeRefCount %d->%d when artificially incrementing ref count\n",
               node->getOpCode().getName(), node, ref->getReferenceNumber(), self()->comp()->getDebug()->getName(ref->getSymbol()),
               ref->getNodeReferenceCount(), ref->getNodeReferenceCount()+1);
