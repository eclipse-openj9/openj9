...
J9::Z::TreeEvaluator::DIVCHKEvaluator(TR::Node * node, TR::CodeGenerator * cg)
   ...
   if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::TR_z10) && divisorIsFieldAccess &&
...
VMarrayStoreCHKEvaluator(
   ...
   if (doObjectArrayCheck && (cg->wantToPatchClassPointer((TR_OpaqueClassBlock*)objectClass, node) || cg->needClassAndMethodPointerRelocations()))
      {
      if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::TR_z10) && cg->isLiteralPoolOnDemandOn())
         {
...
J9::Z::TreeEvaluator::VMmonentEvaluator(TR::Node * node, TR::CodeGenerator * cg)
   ...
   if (lwOffset <= 0)
      ...
      if (comp->getOption(TR_EnableMonitorCacheLookup))
         ...
         if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::TR_zEC12) && TR::Compiler->target.is64Bit())
            generateRIEInstruction(cg, TR::InstOpCode::RISBGN, node, lookupOffsetReg, objReg, start, end+0x80, shiftAmount);
         else if(TR::Compiler->target.cpu.getSupportsArch(TR::CPU::TR_z10) && TR::Compiler->target.is64Bit())
...
genInitObjectHeader(TR::Node * node, TR::Instruction *& iCursor, TR_OpaqueClassBlock * classAddress, TR::Register * classReg, TR::Register * resReg,
   ...
   if (!TR::Options::getCmdLineOptions()->realTimeGC())
      ...
      if (isStaticFlag)
         ...
         if (staticFlag != 0)
            {
            if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::TR_z10) && staticFlag >= MIN_IMMEDIATE_VAL && staticFlag <= MAX_IMMEDIATE_VAL)
               {
   ...
   if (doInline)
      ...
      if (isVariableLen)
         ...
         if (callLabel != NULL && (node->getOpCodeValue() == TR::anewarray ||
            ...
            if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::TR_z10) && TR::Compiler->target.is64Bit())
               {
      ...
      //////////////////////////////////////////////////////////////////////////////////////////////////////
      if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::TR_z10) && cg->enableTLHPrefetching())
         {
...
J9::Z::TreeEvaluator::ardbarEvaluator(TR::Node * node, TR::CodeGenerator * cg)
   ...
   if (TR::Compiler->om.readBarrierType() != gc_modron_readbar_none)
      ...
      {
      resultReg = OMR::TreeEvaluatorConnector::ardbarEvaluator(node, cg);
      }
