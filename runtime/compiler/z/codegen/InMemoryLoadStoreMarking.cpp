   ...
   for (TR::TreeTop *tt = comp()->getStartTree(); tt; tt = tt->getNextTreeTop())
      ...
      if (node->getOpCodeValue() == TR::BBStart)
         ...
         if (!block->isExtensionOfPreviousBlock())
            {
            trprintf(cg->traceBCDCodeGen(), comp()->log(), "block_%d is a new extension so clear all load and store lists\n", block->getNumber());
            clearAllLists();
            }
         }
...
void InMemoryLoadStoreMarking::refineConditionalCleanLoadList(TR::Node *child, TR::Node *parent)
   ...
   if (_BCDConditionalCleanLoadList.find(child))
      {
      trprintf(trace, log, "found %s (%p) in condCleanLoadList : examine parent %s (%p) to see if it relies on clean sign\n",
            child->getOpCode().getName(), child, parent->getOpCode().getName(), parent);
      if (cg->reliesOnAParticularSignEncoding(parent))
         {
         trprintf(trace, log, "\tz^z : parent %s (%p) does rely on a clean sign so remove child %s (%p) from condCleanLoadList\n",
               parent->getOpCode().getName(), parent, child->getOpCode().getName(), child);
         _BCDConditionalCleanLoadList.remove(child);
         }
      else
         {
         trprintf(trace, log, "\tparent %s (%p) does not rely on a clean sign so do not remove child %s (%p) from condCleanLoadList\n",
               parent->getOpCode().getName(), parent, child->getOpCode().getName(), child);
         }
      }
   }
...
void InMemoryLoadStoreMarking::examineFirstReference(TR::Node *node, TR::TreeTop* tt) // called the first time 'node' is encountered
   ...
   if ((node->getType().isAggregate() || node->getType().isBCD()) &&
      ...
      if (tt->getNode()->getOpCode().isBCDStore() &&
         ...
         {
         trprintf(trace, log, "store first encounter: %s (%p) under %s (%p), add store to list and set node futureUseCount %d\n",
      ...
      if (node->getOpCode().isBCDLoadVar())
         {
         trprintf(trace, log, "load var first encounter: %s (%p), add load to list and set node futureUseCount %d\n", node->getOpCode().getName(), node, node->getReferenceCount()-1);
         node->setFutureUseCount(node->getReferenceCount()-1);
...
void InMemoryLoadStoreMarking::examineCommonedReference(TR::Node *child, TR::Node *parent, TR::TreeTop *tt)
   ...
   trprintf(cg->traceBCDCodeGen(), comp()->log(), "looking at commoned reference to %s (%p) with parent %s (%p) and tt %s (%p) (isLastReference %s)\n",
...
bool InMemoryLoadStoreMarking::isConditionalCleanLoad(TR::Node *listNode, TR::Node *defNode)
   ...
   if (defNode->getOpCode().isBCDStore() &&
      ...
      trprintf(cg->traceBCDCodeGen(), comp()->log(), "\t\tfound conditional clean listNode %s (%p) under %s (%p) : do not consider a def but add to condCleanLoadList if not already present\n",
...
void InMemoryLoadStoreMarking::addToConditionalCleanLoadList(TR::Node *listNode)
   ...
   if (_BCDConditionalCleanLoadList.find(listNode))
      ...
      //
      trprintf(trace, log, "\t\tlistNode %s (%p) already present in condCleanLoadList : do not add again\n",
            listNode->getOpCode().getName(), listNode);
      }
   else
      {
      trprintf(trace, log, "\t\tlistNode %s (%p) not already present in condCleanLoadList : add to list\n",
            listNode->getOpCode().getName(), listNode);
...
void InMemoryLoadStoreMarking::processBCDAggrNodeList(List<TR::Node> &nodeList, TR::Node *defNode, TR_NodeListTypes listType)
   ...
   if (!nodeList.isEmpty())
      ...
      TR::Logger *log = comp()->log();
      bool trace = cg->traceBCDCodeGen();
      trprintf(trace, log, "\titerate through non-empty %s\n", getName(listType));
      ...
      for (TR::Node *listNode=listIt.getFirst(); listNode; listNode = listIt.getNext())
         ...
         if (isAnyTypeOfLoadList ||
            ...
            trprintf(trace, log, "\t\tintersect defNode %s (%p) #%d aliases with listNode %s (%p) #%d\n",
               ...
               if (isAnyTypeOfLoadList)
                  ...
                  if (isConditionalCleanLoad(listNode, defNode))
                     ...
                     if (isRegularLoadList)
                        {
                        trprintf(trace, log, "\t\t\tremove listNode %s (%p) from regular load list\n", listNode->getOpCode().getName(), listNode);
                     ...
                     trprintf(trace, log, "\t\t\tfound an intersection so remove listNode %s (%p) from %s and set setSkipCopyOnLoad flag to false (defNodeHasSymRef=%s)\n",
                  ...
                  {
                  trprintf(trace, log, "\t\t\tfound an intersection so remove listNode %s (%p) from %s and set setSkipCopyOnStore flag to false (defNodeHasSymRef=%s)\n",
                        listNode->getOpCode().getName(), listNode, getName(listType), defNodeHasSymRef?"yes":"no");
            ...
            trprintf(trace, log, "\t\tskipping current store treetop listNode %s (%p)\n", listNode->getOpCode().getName(), listNode);
         }
      }
   }
