...
J9::Z::PrivateLinkage::mapCompactedStack(TR::ResolvedMethodSymbol * method)
   ...
   trprintf(trace, log, "stackIndex after compaction = %d\n", stackIndex);
   ...
   trprintf(trace, log, "stackIndex after padding slots = %d\n", stackIndex);
      ...
      if (localCursor->getGCMapIndex() >= 0)
         ...
         if (NULL != igNode)
            ...
            if (localCursor->isInternalPointer() || localCursor->isPinningArrayPointer() || localCursor->holdsMonitoredObject())
               ...
               {
               trprintf(trace, log, "O^O COMPACT LOCALS: Sharing slot for local %p (colour = %d)\n", localCursor, colour);
   ...
   for (localCursor = automaticIterator.getFirst(); localCursor; localCursor = automaticIterator.getNext())
      ...
      while (variableSizeSymCursor != NULL)
         ...
         if (variableSizeSymCursor->isReferenced())
            {
            trprintf(traceBCD, log, "map variableSize sym %p (size %d) because isReferenced=true ", variableSizeSymCursor, variableSizeSymCursor->getSize());
            mapSingleAutomatic(variableSizeSymCursor, stackIndex);
            trprintf(traceBCD, log, "to auto offset %d\n", variableSizeSymCursor->getOffset());
            }
         else
            trprintf(traceBCD, log, "do not map variableSize sym %p (size %d) because isReferenced=false\n", variableSizeSymCursor, variableSizeSymCursor->getSize());
...
J9::Z::PrivateLinkage::mapStack(TR::ResolvedMethodSymbol * method)
   ...
   while (variableSizeSymCursor != NULL)
      ...
      if (variableSizeSymCursor->isReferenced())
         {
         trprintf(trace, log, "map variableSize sym %p (size %d) because isReferenced=true ", variableSizeSymCursor, variableSizeSymCursor->getSize());
         mapSingleAutomatic(variableSizeSymCursor, stackIndex);  //Ivan
         trprintf(trace, log, "to auto offset %d\n", variableSizeSymCursor->getOffset());
         }
      else
         trprintf(trace, log, "do not map variableSize sym %p (size %d) because isReferenced=false\n", variableSizeSymCursor, variableSizeSymCursor->getSize());
...
J9::Z::PrivateLinkage::createPrologue(TR::Instruction * cursor)
   ...
   if (!comp()->getOption(TR_DisableDoubleWordStackAlignment))
      {
      trprintf(trace, log, "Before stack alignment Framesize = %d, localSize = %d\n", size, localSize);
      ...
      trprintf(trace, log, "After stack alignment Framesize = %d, localSize = %d\n", size, localSize);
      }
...
J9::Z::PrivateLinkage::createEpilogue(TR::Instruction * cursor)
   ...
   if (getRaContextRestoreNeeded())
      ...
      {
      trprints(comp()->getOption(TR_TraceCG), comp()->log(), "No RAREG context restore needed in Epilog\n");
      }
...
getITableIterationsNumber(TR::Compilation * comp, TR::SymbolReference * methodSymRef, TR_OpaqueClassBlock *declaringClass)
   ...
   if (disableITableIterationsAfterLastITableCacheCheck)
      {
      trprints(trace, log, "ITable iteration after lastITable check is disabled.\n");
      return 0;
      }
   ...
   if (numITableIterationsAfterLastITableCacheCheck)
      ...
      trprintf(trace, log, "ITable iteration is %d because TR_NumITableIterationsAfterLastITableCacheCheck was set.\n", iterations);
      return iterations;
      }
   ...
   if (!comp->getOption(TR_DisableCHOpts))
      ...
      if ((numImplementers != 0) && (numImplementers <= MAX_IMPLEMENTERS_TO_EVALUATE))
         ...
         trprintf(trace, log, "ITable declaringClass %p numImplementers %d maxInterfaces %d iterations %d\n", declaringClass, numImplementers, maxInterfaces, iterations);
         }
      }
   else
      trprintf(trace, log, "ITable iteration is the default value of %d.\n", iterations);
   return iterations;
   }
...
J9::Z::PrivateLinkage::buildVirtualDispatch(TR::Node * callNode, TR::RegisterDependencyConditions * dependencies,
   ...
   trprints(trace, log, "Build Virtual Dispatch\n");
   ...
   if (methodSymbol->isVirtual())
      ...
      trprintf(trace, log, "Virtual call with offset %d\n", offset);
      ...
      if (methodSymRef->isUnresolved() || comp()->compileRelocatableCode())
         {
         trprints(trace, log, "... virtual call is unresolved\n");
         ...
         {
         trprints(trace, log, "...call resolved\n");
            ...
            if (resolvedMethod &&
               ...
               if (!resolvedMethod->virtualMethodIsOverridden() && !resolvedMethod->isAbstract())
                  ...
                  virtualGuard = TR_VirtualGuard::createGuardedDevirtualizationGuard(TR_NonoverriddenGuard,
                                                      comp(), callNode);
                  trprintf(trace, log, "Emit new Non-Overridden guard for call %s (%x) in %s\n", resolvedMethod->signature(trMemory()), callNode, comp()->signature());
                  }
               else
                  {
                  ...
                  if (devirtualizedCallInfo)
                     ...
                     trprintf(trace, log, "Found refined this class info %x for call %x in %s\n", refinedThisClass, callNode, comp()->signature());
                  ...
                  if (thisClass && TR::Compiler->cls.isAbstractClass(comp(), thisClass) && !comp()->compileRelocatableCode())
                     ...
                     if (method &&
                        ...
                        virtualGuard = TR_VirtualGuard::createGuardedDevirtualizationGuard(TR_AbstractGuard,
                                                            comp(), callNode);
                        trprintf(trace, log, "Emit new ABSTRACT guard for call %s (%x) in %s\n", resolvedMethod->signature(trMemory()), callNode, comp()->signature());
                        }
                     }
                     ...
                     if (resolvedMethod->virtualMethodIsOverridden())
                        ...
                        if (calleeMethod &&
                           ...
                           trprintf(trace, log, "Emit new HierarchyGuardguard for call %s (%x) in %s\n", resolvedMethod->signature(trMemory()), callNode, comp()->signature());
                           }
                        }
                     }
                  }
         ...
         if (!performGuardedDevirtualization &&
            ...
            if ( topValue )
               ...
               if (profiledVirtualMethod && !profiledVirtualMethod->isInterpreted())
                  {
                  trprintf(trace, log, "Profiled method {%s}\n",
         ...
         if (performGuardedDevirtualization || useProfiledValues)
            {
            trprints(trace, log, "Make direct call under devirtualization\n");
      ...
      if ( virtualLabel )
         {
         trprints(trace, log, "OOL vcall: generating Vcall dispatch sequence\n");
...
J9::Z::PrivateLinkage::buildDirectCall(TR::Node * callNode, TR::SymbolReference * callSymRef,
   ...
   trprints(trace, log, "Build Direct Call\n");
...
TR::Register * J9::Z::JNILinkage::buildDirectDispatch(TR::Node * callNode)
   ...
   trprintf(trace, log, "isPassReceiver: %d, isPassJNIThread: %d, isJNIGCPoint: %d, isJNICallOutFrame:%d, isReleaseVMAccess: %d, isCollapseJNIReferenceFrame: %d, isJNIGCPoint: %d\n",
...
J9::Z::PrivateLinkage::addSpecialRegDepsForBuildArgs(TR::Node * callNode, TR::RegisterDependencyConditions * dependencies, int32_t& from, int32_t step)
   ...
   if (specialArgReg != TR::RealRegister::NoReg)
      ...
      trprintf(comp()->getOption(TR_TraceCG), comp()->log(), "Special arg %s %s reg %s in %s\n",
...
J9::Z::PrivateLinkage::buildDirectDispatch(TR::Node * callNode)
   ...
   trprints(comp()->getOption(TR_TraceCG), comp()->log(), "\nbuildDirectDispatch\n");
