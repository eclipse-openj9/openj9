...
generateLastITableAndITableInstructions(TR::CodeGenerator * cg, TR::Node * callNode, TR::Register * vftReg,
   ...
   // Set the default state of enableLastITableCache.
   bool enableLastITableCache = !cg->profiledPointersRequireRelocation() && !comp->getOption(TR_DisableLastITableCache);

   static char* forceInterfaceDispatchUsingLastITable = feGetEnv("TR_forceInterfaceDispatchUsingLastITableState");
   if (forceInterfaceDispatchUsingLastITable)
      enableLastITableCache = static_cast<bool>(atoi(forceInterfaceDispatchUsingLastITable));
