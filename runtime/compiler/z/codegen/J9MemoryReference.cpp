...
void recursivelyIncrementReferenceCount(TR::Node *node, rcount_t increment, List<TR::Node> & incrementedNodesList, List<TR::Node> &nodesAlreadyEvaluatedBeforeFoldingList,
   ...
   trprintf(trace, log, "\t\t\trecAdjust node - %s (%p) and add to list, increment %d: refCount %d->%d\n",
   ...
   if (node->getRegister() && node->getRegister()->getOpaquePseudoRegister())
      ...
      trprintf(trace, log, "\tincrement temporary #%d (sym %p -- from loadaddr node %p) reference count %d->%d\n",
   ...
   if (node->getRegister() == NULL)
      {
      trprints(trace, log, "\t\t\tnode has no register so do recurse\n");
      ...
      trprints(trace, log, "\t\t\tnode has a register so do not recurse\n");
      }
   return;
   }
...
J9::Z::MemoryReference::tryForceFolding(TR::Node *& rootLoadOrStore, TR::CodeGenerator *& cg, TR_StorageReference *& storageReference, TR::SymbolReference *& symRef, TR::Symbol *& symbol,
   ...
   if (storageReference)
      ...
      trprintf(trace, log, "\t\tmr storageRef case: setting rootLoadOrStore from %s (%p) to storageRef->node %s (%p) (ref->nodeRefCount %d, symRef #%d (sym=%p), isIndirect %s, isConst %s)\n",
      ...
      if (isIndirect)
         ...
         if (storageReference->isNodeBasedHint())
            ...
            trprints(trace, log, "\t\tset isImpliedMemoryReference=true as ref isNodeBasedHint=true\n");
            }
         else
            {
            ...
            trprintf(trace, log, "\t\tset isImpliedMemoryReference=%s as ref->getNodeReferenceCount() %s 1\n",
                  isImpliedMemoryReference?"true":"false",isImpliedMemoryReference?">":"==");
            }
         }
      if (!storageReference->isNodeBasedHint())
         {
         trprintf(trace, log, "\t\tdec nodeRefCount %d->%d on storageRef #%d (storageRefNode %s (%p))\n",
      ...
      if (isImpliedMemoryReference)
         ...
         if (self()->forceFolding() || self()->forceFirstTimeFolding() || addressChild->getOpCodeValue() == TR::loadaddr)
            {
            trprintf(trace, log, "\t\tisImpliedMemoryReference=true and %s so recInc refCounts for rootLoadOrStore %s (%p) and addressChild %s (%p)\n",
            ...
            trprintf(trace, log, "\t\tisImpliedMemoryReference=true and forceFolding=false so increment addressChild %s (%p) refCount %d->%d and setForceEvaluation to true\n",
...
generateS390MemRefFromStorageRef(TR::Node *node, TR_StorageReference *storageReference, TR::CodeGenerator * cg, bool enforceSSLimits, bool isNewTemp)
   ...
   if (!isNewTemp &&
      ...
      trprintf(trace, log, "\tgenerateS390AlignedMemoryReference: adjust memRef->_offset for regDeadAndIgnoredBytes (%d->%d) from node %s (%p) and reg %s\n",
   ...
   if (storageReference->isTemporaryBased() && storageReference->getSymbolReference()->hasTemporaryNegativeOffset())
      {
      trprintf(trace, log, "\tgenerateS390AlignedMemoryReference mr %p: call addToTemporaryNegativeOffset flag for storageRef->symRef #%d (offset = %d) and node %s (%p) to memRef\n",
