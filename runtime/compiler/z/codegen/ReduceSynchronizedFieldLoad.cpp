...
ReduceSynchronizedFieldLoad::perform()
   ...
   if (comp->target().cpu.isAtLeast(OMR_PROCESSOR_S390_Z196))
      ...
      if (!comp->getOption(TR_DisableSynchronizedFieldLoad) && comp->getMethodSymbol()->mayContainMonitors())
         {
         trprints(comp->getOption(TR_TraceCG), comp->log(), "Performing ReduceSynchronizedFieldLoad\n");
   ...
   for (TR::TreeTopIterator iter(startTreeTop, comp); iter != endTreeTop; ++iter)
      ...
      if (iter.currentNode()->getOpCodeValue() == TR::monent ||
         ...
         if (cg->isMonitorValueBasedOrValueType(monentNode) != TR_no)
            continue;
         trprintf(trace, log, "Found monent [%p]\n", monentNode);
         ...
         for (++iter; iter != endTreeTop; ++iter)
            ...
            if (iter.currentNode()->getOpCodeValue() == TR::monexit ||
               ...
               TR::Node* monexitNode = iter.currentNode()->getOpCodeValue() == TR::monexit ?
                  iter.currentNode() :
                  iter.currentNode()->getFirstChild();
               trprintf(trace, log, "Found monexit [%p]\n", monexitNode);

               TR::Node* synchronizedObjectNode = monentNode->getFirstChild();

               if (synchronizedObjectNode == monexitNode->getFirstChild())
                  {
                  trprintf(trace, log, "Children of monent and monexit are synchronizing on the same object\n", monexitNode);
                  ...
                  if (loadNode != NULL)
                     ...
                     trprintf(trace, log, "Lock word offset = %d\n", lockWordOffset);
...
ReduceSynchronizedFieldLoad::findLoadInSynchornizedRegion(TR::TreeTop* startTreeTop, TR::TreeTop* endTreeTop, TR::TreeTop* monentTreeTop, TR::TreeTop* monexitTreeTop, TR::Node* synchronizedObjectNode)
   ...
   for (; iter != monentTreeTop->getNextTreeTop(); ++iter)
      ...
      trprintf(trace, log, "Iterating node [%p] outside the monitored region\n", currentNode);
      }
   ...
   for (; iter != monexitTreeTop; ++iter)
      ...
      trprintf(trace, log, "Iterating node [%p] inside the monitored region\n", currentNode);
      ...
      if (opcode.hasSymbolReference() || opcode.isBranch())
         ...
         if (loadNode == NULL &&
            ...
            {
            trprintf(trace, log, "Found load node [%p]\n", currentNode);

            loadNode = currentNode;
            }
         else
            {
            trprintf(trace, log, "Found sideeffect node [%p] within the monitored region\n", currentNode);
