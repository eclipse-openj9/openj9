...
TR_StorageReference::setTemporaryReferenceCount(rcount_t count)
   ...
   if (isTemporaryBased())
      ...
      trprintf(comp()->cg()->traceBCDCodeGen(), comp()->log(), "\tset temporary #%d (%s) reference count %d->%d\n",
...
TR_StorageReference::decrementTemporaryReferenceCount(rcount_t decrement)
   ...
   if (isTemporaryBased())
      ...
      trprintf(trace, log, "\tdecrement temporary #%d (%s) reference count %d->%d\n",
...
TR_StorageReference::incrementTemporaryReferenceCount(rcount_t increment)
   ...
   if (isTemporaryBased())
      ...
      TR_ASSERT(increment >= 0,"expecting a non-negative increment (%d)\n",increment);
      TR::AutomaticSymbol *sym = getTemporarySymbol();
      trprintf(comp()->cg()->traceBCDCodeGen(), comp()->log(), "\tincrement temporary #%d (%s) reference count %d->%d\n",
...
TR_StorageReference::increaseTemporarySymbolSize(int32_t sizeIncrement, TR_OpaquePseudoRegister *reg)
   ...
   if (isTemporaryBased())
      ...
      TR::AutomaticSymbol *sym = getTemporarySymbol();
      trprintf(trace, log, "\tincreaseTemporarySymbolSize : activeSize %d->%d (on reg %s and %s)\n",
      ...
      if (sym->getActiveSize() > sym->getSize())
         {
         trprintf(trace, log, "\t\tnew activeSize > symSize (%d > %d) so increment symSize %d->%d\n", sym->getActiveSize(), sym->getSize(), sym->getSize(), sym->getActiveSize());
         sym->setSize(sym->getActiveSize());
         }
      else
         trprintf(trace, log, "\t\tnew activeSize <= symSize (%d <= %d) so leave symSize at %d\n", sym->getActiveSize(), sym->getSize(), sym->getSize());
      ...
      if (getNodesToUpdateOnClobber())
         ...
         for (TR::Node *update = toUpdate.getFirst(); update; update = toUpdate.getNext())
            ...
            if (updateReg && updateReg != reg && updateReg->getStorageReference() == reg->getStorageReference())
               {
               trprintf(trace, log, "\ty^y : clear left-aligned state of reg %s with ref #%d (%s) on node %s (%p) refCount %d from _nodesToUpdateOnClobber\n",
...
TR_StorageReference::addSharedNode(TR::Node *node)
   ...
   trprintf(comp()->cg()->traceBCDCodeGen(), comp()->log(), "\tadding node %s (%p) to _sharedNodes on hint #%d\n", node->getOpCode().getName(), node, getReferenceNumber());
   }
...
TR_StorageReference::removeSharedNode(TR::Node *node)
   ...
   trprintf(comp()->cg()->traceBCDCodeGen(), comp()->log(), "\tremoving node %s (%p) from _sharedNodes on hint #%d\n", node->getOpCode().getName(), node, getReferenceNumber());
   }
...
TR_StorageReference::getMaxSharedNodeSize()
   ...
   trprintf(trace, log, "\tgetMaxSharedNodeSize() for ref #%d : setting initial maxSize=symSize=%d, _sharedNodes=%p\n", getReferenceNumber(), maxSize, _sharedNodes);
   if (_sharedNodes)
      {
      ...
      for (TR::Node *listNode = listIt.getFirst(); listNode; listNode = listIt.getNext())
         ...
         if (nodeSize > maxSize)
            {
            trprintf(trace, log, "\tupdating maxSize %d->%d from listNode %s (%p)\n", maxSize, nodeSize, listNode->getOpCode().getName(), listNode);
            maxSize = nodeSize;
            }
         }
      }
   trprintf(trace, log, "\treturning maxSize %d from _sharedNodes on hint #%d\n", maxSize, getReferenceNumber());
   return maxSize;
   }
...
TR_StorageReference::addNodeToUpdateOnClobber(TR::Node *node)
   ...
   if (node->getReferenceCount() > 1 || node->getRegister() != NULL)
      ...
      if (_nodesToUpdateOnClobber->find(node))
         {
         trprintf(trace, log, "\tNOT adding node %s (%p refCount %d) with reg %s to _nodesToUpdateOnClobber on ref #%d (%s) (already present in the list)\n",
               node->getOpCode().getName(), node, node->getReferenceCount(), node->getRegister()?comp()->getDebug()->getName(node->getRegister()):"NULL",
               getReferenceNumber(), comp()->getDebug()->getName(getSymbol()));
         }
      else
         {
         _nodesToUpdateOnClobber->add(node);
         trprintf(trace, log, "\tadding node %s (%p refCount %d) with reg %s to _nodesToUpdateOnClobber on ref #%d (%s)\n",
               node->getOpCode().getName(), node, node->getReferenceCount(), node->getRegister()?comp()->getDebug()->getName(node->getRegister()):"NULL",
               getReferenceNumber(), comp()->getDebug()->getName(getSymbol()));
         }
      }
...
TR_StorageReference::removeNodeToUpdateOnClobber(TR::Node *node)
   ...
   trprintf(comp()->cg()->traceBCDCodeGen(), comp()->log(), "\tremoving node %s (%p) with reg %s from _nodesToUpdateOnClobber on ref #%d (%s)\n",
...
TR_StorageReference::emptyNodesToUpdateOnClobber()
   ...
   trprintf(comp()->cg()->traceBCDCodeGen(), comp()->log(), "\temptying _nodesToUpdateOnClobber list on ref #%d (%s)\n", getReferenceNumber(), comp()->getDebug()->getName(getSymbol()));
   }
...
bool TR_StorageReference::mayOverlapWith(TR_StorageReference *ref2)
   ...
   if (ref1->isNonConstantNodeBased() && ref2->isNonConstantNodeBased() &&
      ...
      trprintf(trace, log, "\tmayOverlapWith storageRef : check overlap between ref1Node %s (%p) and ref2Node %s (%p)\n",
      ...
      bool mayOverlap = comp()->cg()->loadAndStoreMayOverlap(ref1Node,
                                                             ...
                                                             ref2Node,
                                                             ref2Node->getSize());
      trprintf(trace, log, "\toverlap=true (from %s storageRef test)\n", mayOverlap?"true":"false", "aliasing");
...
void TR_OpaquePseudoRegister::setStorageReference(TR_StorageReference *ref, TR::Node *node)
   ...
   if (_storageReference && (ref != _storageReference))
      ...
      if (_storageReference->isTemporaryBased())
         ...
         trprintf(trace, log, "\t\tdecrement storageRef #%d nodeRefCount by (node->refCount() - 1) = %d : %d->%d\n",
         ...
         if (storageRefNode->getOpCode().isLoadConst() || storageRefNode->getOpCode().isIndirect())
            {
            trprintf(trace, log, "\t\t_storageReference is non-hint nodeBased with nodeRefCount %d and addrChild %p\n",
            ...
            trprintf(trace, log, "\t\t\tdoRecursiveDecrement=%s on addrChild %p (refCount=%d), addrChild->firstChild %p (refCount %d) if storageRefNode %p == node %p (%s) or nodeRefCount %d == 0 (%s)\n",
      ...
      trprintf(trace, log, "\tsetting the new storageRef #%d (over existing storageRef #%d) on reg %s so reset leftAlignedZeroDigits and deadAndIgnoredBytes to 0\n",
      ...
      trprintf(trace, log, "\tdecrement owningRegisterCount %d->%d on _storageReference #%d (%s) as new ref is being set\n",
   ...
   if (node && // node may be NULL if calling to simply set the _storageReference = ref so state can be pulled off of the register
      ...
      trprintf(trace, log, "\t\tincrement owningRegisterCount %d->%d on ref #%d (%s) for reg %s and node %s (%p) refCount %d\n",
   ...
   if (node &&
      ...
      trprintf(trace, log, "\tsetting hintHasBeenUsed = true on new storageRef #%d\n", ref->getReferenceNumber());
      }
...
int32_t TR_PseudoRegister::getRangeStart(int32_t startDigit, int32_t endDigit)
   ...
   trprintf(trace, log, "\tgetRangeStart %s: startDigit %d, endDigit %d, symSize %d, symDigits %d\n",
   ...
   if (deadAndIgnoredBytes)
      ...
      trprintf(trace, log, "\t\tdeadAndIgnoredBytes = %d (digitOffset = %d) so inc startDigit %d -> %d and endDigit %d -> %d\n",
   ...
   trprintf(trace, log, "\t\treturning rangeStart %d\n", rangeStart);
...
int32_t TR_PseudoRegister::getRangeEnd(int32_t rangeStart, int32_t startDigit, int32_t endDigit)
   ...
   trprintf(comp()->cg()->traceBCDCodeGen(), comp()->log(), "\t\tgetRangeEnd %s returning %d\n", comp()->cg()->getDebug()->getName(this), rangeEnd);
...
int32_t TR_PseudoRegister::getDigitsToClear(int32_t startDigit, int32_t endDigit)
   ...
   trprintf(trace, log, "\tgetDigitsToClear %s (%s): %d -> %d\n",
   ...
   if (getLiveSymbolSize() < TR::DataType::getSizeFromBCDPrecision(getDataType(), endDigit))
      {
      trprintf(trace, log, "\tliveSymSize %d < endByte %d so return a conservative digitsToClear of %d\n",
   ...
   trprintf(trace, log, "\t\trangeStart %d, rangeEnd %d, leftAlignedZeroDigits = %d\n", rangeStart, rangeEnd, leftAlignedZeroDigits);
   if (rangeEnd > leftAlignedZeroDigits) // does the endDigit of range extend beyond the leftmost cleared digits (if so, then some digits will have to be cleared)
      {
      trprintf(trace, log, "\t\tsetting digitsToClear to %d (rangeSize) because rangeEnd %d > leftAlignedZeroDigits %d\n",
      ...
      if (rangeStart < leftAlignedZeroDigits) // is the startDigit of range is within the cleared range (i.e. an overlap)
         {
         trprintf(trace, log, "\t\tadjusting digitsToClear %d -> %d due to an overlap (rangeStart %d < leftAlignedZeroDigits %d)\n",
         ...
         trprintf(trace, log, "\t\tnot adjusting digitsToClear (remains at rangeSize = %d) as there is no overlap (rangeStart %d  >= leftAlignedZeroDigits %d)\n",
            digitsToClear, rangeStart, leftAlignedZeroDigits);
      }
   trprintf(trace, log, "\t\treturning digitsToClear %d\n", digitsToClear);
...
int32_t TR_PseudoRegister::getBytesToClear(int32_t startByte, int32_t endByte) // input range of bytes is right to left : startByte->endByte-1
   ...
   TR::Logger *log = comp()->log();
   bool trace = comp()->cg()->traceBCDCodeGen();
   trprintf(trace, log, "\tgetBytesToClear %s (%s): (startByte=%d, endByte=%d): defer to getDigitsToClear\n",
   ...
   if ((digitsToClear&0x1) != 0 &&  // if odd
      ...
      {
      trprintf(trace, log, "\tincrement digitsToClear %d->%d for halfByteType %s\n", digitsToClear, digitsToClear+1, TR::DataType::getName(getDataType()));
   ...
   trprintf(trace, log, "\t\treturning bytesToClear %d\n", bytesToClear);
   return bytesToClear;
   }
...
int32_t TR_PseudoRegister::getByteOffsetFromLeftForClear(int32_t startDigit, int32_t endDigit, int32_t &digitsToClear, int32_t resultSize)
   ...
   TR::Logger *log = comp()->log();
   bool trace = comp()->cg()->traceBCDCodeGen();
   trprintf(trace, log, "\tgetByteOffsetFromLeftForClear %s (%s): %d -> %d, digitsToClear %d, resultSize %d\n",
   ...
   if (leftAlignedZeroDigits > rangeStart)
      {
      trprintf(trace, log, "\t\tsetting digitOffset to %d (== leftAlignedZeroDigits) as leftAlignedZeroDigits %d > rangeStart %d (an overlap)\n",
      ...
      trprintf(trace, log, "\t\tsetting digitOffset to 0 as leftAlignedZeroDigits %d <= rangeStart %d (disjoint)\n", leftAlignedZeroDigits, rangeStart);
      digitOffset = 0;
      }
   ...
   if ((digitOffset&0x1) != 0 &&
      ...
      trprintf(trace, log, "\t\tdigitOffset is odd so decrement digitOffset %d -> %d and increment digitsToClear %d -> %d\n", digitOffset, digitOffset-1, digitsToClear, digitsToClear+1);
      digitOffset--;
      digitsToClear++;
      }
   ...
   if (digitOffset && (liveSymbolSize > resultSize))
      ...
      trprintf(trace, log, "\t\tdecrease digitOffset %d -> %d because liveSymbolSize > resultSize (%d > %d), liveSymbolSize is getSymbolSize() %d - deadAndIgnoredBytes %d\n",
            digitOffset, digitOffset-TR::DataType::bytesToDigits(getDataType(), liveSymbolSize-resultSize), liveSymbolSize, resultSize,
            getStorageReference()->getSymbolSize(), getRightAlignedDeadAndIgnoredBytes());
   ...
   trprintf(trace, log, "\t\treturning byte offset %d (from digitOffset %d and type %s)\n", byteOffset, digitOffset, TR::DataType::getName(getDataType()));
   return byteOffset;
   }
...
void TR_PseudoRegister::addRangeOfZeroDigits(int32_t startDigit, int32_t endDigit)
   ...
   TR::Logger *log = comp()->log();
   bool trace = comp()->cg()->traceBCDCodeGen();
   trprintf(trace, log, "\taddRangeOfZeroDigits %s (%s): %d -> %d\n", comp()->cg()->getDebug()->getName(this), TR::DataType::getName(getDataType()), startDigit, endDigit);
   int32_t rangeStart = getRangeStart(startDigit, endDigit);
   int32_t rangeEnd = getRangeEnd(rangeStart, startDigit, endDigit);
   int32_t leftAlignedZeroDigits = getLeftAlignedZeroDigits();
   trprintf(trace, log, "\t\trangeStart %d, rangeEnd %d, leftAlignedZeroDigits = %d\n", rangeStart, rangeEnd, leftAlignedZeroDigits);
   ...
   if (rangeStart <= leftAlignedZeroDigits && rangeEnd > leftAlignedZeroDigits)
      {
      trprintf(trace, log, "\t\tsetting leftAlignedZeroDigits to %d (leftAlignedZeroDigits %d + (rangeEnd %d - leftAlignedZeroDigits %d) because new range overlaps or is adjacent to current zero range\n",
      ...
      trprintf(trace, log, "\t\tnot setting leftAlignedZeroDigits because new range is not adjacent to or overlapping with the current zero range (rangeStart %d > leftAlignedZeroDigits %d)\n",
...
void TR_PseudoRegister::addRangeOfZeroBytes(int32_t startByte, int32_t endByte) // input range of bytes is right to left : startByte->endByte-1
   ...
   if (!trackZeroDigits()) return;
   trprintf(comp()->cg()->traceBCDCodeGen(), comp()->log(), "\taddRangeOfZeroBytes %s (%s): (startByte=%d, endByte=%d): defer to addRangeOfZeroDigits\n",
         comp()->cg()->getDebug()->getName(this), TR::DataType::getName(getDataType()), startByte, endByte);
   int32_t startDigit = TR::DataType::getBCDPrecisionFromSize(getDataType(), startByte);
   int32_t endDigit = TR::DataType::getBCDPrecisionFromSize(getDataType(), endByte);
...
void TR_PseudoRegister::removeRangeOfZeroDigits(int32_t startDigit, int32_t endDigit)
   ...
   bool trace = comp()->cg()->traceBCDCodeGen();
   int32_t leftAlignedZeroDigits = getLeftAlignedZeroDigits();
   trprintf(trace, log, "\tremoveRangeOfZeroDigits %s (%s): %d -> %d%s\n",
   ...
   if (rangeStart < leftAlignedZeroDigits)
      {
      trprintf(trace, log, "\t\tsetting leftAlignedZeroDigits to %d (leftAlignedZeroDigits %d - rangeStart %d) because rangeStart < leftAlignedZeroDigits\n",
      ...
      trprintf(trace, log, "\t\tnot setting leftAlignedZeroDigits because rangeStart %d >= leftAlignedZeroDigits %d\n", rangeStart, leftAlignedZeroDigits);
...
void TR_PseudoRegister::removeRangeOfZeroBytes(int32_t startByte, int32_t endByte) // input range of bytes is right to left : startByte->endByte-1
   ...
   if (!trackZeroDigits()) return;
   int32_t leftAlignedZeroDigits = getLeftAlignedZeroDigits();
   trprintf(comp()->cg()->traceBCDCodeGen(), comp()->log(), "\tremoveRangeOfZeroBytes %s (%s): (startByte=%d, endByte=%d)%s\n",
         comp()->cg()->getDebug()->getName(this), TR::DataType::getName(getDataType()), startByte, endByte,
...
void TR_PseudoRegister::removeByteRangeAfterLeftShift(int32_t operandByteSize, int32_t shiftDigitAmount)
   ...
   bool trace = comp()->cg()->traceBCDCodeGen();
   if (!trackZeroDigits()) return;
   trprintf(trace, log, "\tremoveByteRangeAfterLeftShift %s (%s): (operandByteSize=%d, shiftDigitAmount=%d)\n",
   ...
   if (rangeStart <= leftAlignedZeroDigits)
      ...
      trprintf(trace, log, "\t\tsetting leftAlignedZeroDigits to %d = std::max(leftAlignedZeroDigits %d - shiftDigitAmount %d, rangeStart %d) because rangeStart %d <= leftAlignedZeroDigits %d\n",
