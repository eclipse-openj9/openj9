...
J9::Z::TreeEvaluator::asciiAndUnicodeToPackedHelper(TR::Node *node,
   ...
   trprintf(trace, log, "\tasciiAndUnicodeToPackedHelper %p : op %s, isTruncation=%s, fixedDestSize %d, targetRegPrec %d, sourcePrecision %d, sourceEndByte %d, sourceSize %d, pkuSourceSize %d\n",
   ...
   trprintf(trace, log, "\tgen %s with fixed dest size of %d and source size %d. Set targetRegPrec to sourcePrec (%d)\n",isUnicode?"PKU":"PKA",destSize,pkxSourceSize,sourcePrecision);
...
J9::Z::TreeEvaluator::udsl2pdEvaluator(TR::Node *node, TR::CodeGenerator *cg)
   ...
   if (!node->getOpCode().isSetSign())
      ...
      trprintf(trace, log, "\tudsl2pdEvaluator %p : op %s, isTruncation=%s, targetReg->isInit=%s, targetRegSize=%d, targetRegPrec=%d, srcRegSize=%d, srcRegPrec=%d, sourceSignEndByte=%d\n",
      ...
      if (!isTruncation)
         ...
         trprintf(trace, log, "\tudsx2p is a truncation (srcRegPrec %d > nodePrec %d) so do not transfer any clean sign flags\n",
...
J9::Z::TreeEvaluator::pd2udslEvaluator(TR::Node *node, TR::CodeGenerator *cg)
   ...
   if (childReg->hasKnownSignCode())
      ...
      if (convertedSign == TR::DataType::getNationalSeparateMinus())
         {
         trprintf(trace, log, "\tchildReg has negative knownSignCode 0x%x so generate an MVI of the converted sign 0x%x\n", childReg->getKnownSignCode(), convertedSign);
         ...
         {
         trprintf(trace, log, "\tchildReg has positive knownSignCode 0x%x so no more codegen is needed (an MVC of 002B was already done)\n", childReg->getKnownSignCode());
...
J9::Z::TreeEvaluator::packedToUnicodeHelper(TR::Node *node,
   ...
   trprintf(trace, log, "\tpackedToUnicodeHelper %p : op %s, targetRegSize %d, targetRegPrec %d, srcRegSize %d, srcRegPrec %d\n",
   ...
   if (isSeparateSign)
      ...
      if (isImplicitValue)
         {
         trprintf(trace, log, "\tgen 2 MVIs of unicode sign with size of %d and destSignEndByte of %d\n", TR::DataType::getUnicodeSignSize(), destSignEndByte);
         ...
         trprintf(trace, log, "\tgen MVC of unicode sign with size of %d and destSignEndByte of %d\n", TR::DataType::getUnicodeSignSize(), destSignEndByte);
   ...
   trprintf(trace, log, "\tgen UNPKU: unpkuDestSize %d, destEndByte %d and fixed source size %d\n", unpkuDestSize, unpkuDestEndByte, fixedSourceSize);
...
J9::Z::TreeEvaluator::zonedToZonedSeparateSignHelper(TR::Node *node, TR_PseudoRegister *srcReg, TR_PseudoRegister *targetReg, TR::MemoryReference *sourceMR, TR::MemoryReference *destMR, TR::CodeGenerator * cg)
   ...
   trprintf(trace, log, "\tzonedToZonedSeparateSignHelper %p : op %s, isTruncation=%s, targetReg->knownSign=0x%x, trgSignIsZone=%s, targetReg->size=%d, targetRegPrec=%d, , digitsToClear=%d, (isSetSign=%s, sign 0x%x)\n",
   ...
   if (digitsToClear > 0)
      {
      trprintf(trace, log, "\tdigitsToClear > 0 (%d) so set upper bytes to 0x%x and set targetRegPrec to nodePrec %d\n",
   ...
   if (isSetSign || (srcReg->hasKnownSignCode() && srcReg->knownSignIsEmbeddedPreferredOrUnsigned()))
      ...
      trprintf(trace, log, "\t%s case so gen MVI to set target sign to 0x%x (from source sign 0x%x) and do %sgen OI because srcReg->knownOrAssumedSignIsZone() = %s\n",
...
J9::Z::TreeEvaluator::zonedSeparateSignToPackedOrZonedHelper(TR::Node *node, TR_PseudoRegister *targetReg, TR::MemoryReference *sourceMR, TR::MemoryReference *destMR, TR::CodeGenerator * cg)
   ...
   if (node->getOpCode().isSetSign())
      ...
      if (sign == TR::DataType::getIgnoredSignCode())
         ...
         trprintf(trace, log, "\tzonedSeparateSignToPackedOrZonedHelper %p : op %s, ignoredSetSign=true case, sign 0x%x\n", node, node->getOpCode().getName(), sign);
         ...
         trprintf(trace, log, "\t\ttargetReg->isInit=%s, targetRegSize=%d, targetRegPrec=%d, srcRegSize=%d, srcRegPrec=%d, sourceSignEndByte=%d\n",
         ...
         {
         trprintf(trace, log, "\tzonedSeparateSignToPackedOrZonedHelper %p : op %s, setSign=true case, sign 0x%x\n", node, node->getOpCode().getName(), sign);
      ...
      trprintf(trace, log, "\tzonedSeparateSignToPackedOrZonedHelper %p : op %s, targetReg->isInit=%s, targetRegSize=%d, targetRegPrec=%d, srcRegSize=%d, srcRegPrec=%d, sourceSignEndByte=%d\n",
...
J9::Z::TreeEvaluator::zdsls2zdEvaluator(TR::Node * node, TR::CodeGenerator * cg)
   ...
   trprintf(trace, log, "\tzdsls2zdEvaluator %p : op %s, isInitialized=%s, targetRegSize=%d, targetRegPrec=%d, srcRegSize=%d, srcRegPrec=%d, isEffectiveNop=%s (isSetSign %s, sign 0x%x)\n",
   ...
   if (!isEffectiveNop)
      ...
      if (!isInitialized)
         ...
         trprintf(trace, log, "\tisInit=false so gen MVC to init with size=%d and sourceOffset=%d, srcEndByte=%d\n", mvcSize, sourceOffset, srcEndByte);
         generateSS1Instruction(cg, TR::InstOpCode::MVC, node,
                                mvcSize-1,
                                generateS390RightAlignedMemoryReference(*destMR, node, 0, cg),
   ...
   if (isSrcTrailingSign)
      ...
      if (isEffectiveNop)
         ...
         trprintf(trace, log, "\tisSrcTrailingSign=true and isEffectiveNop=true (zdsls2zd) : increment targetReg %s ignoredBytes %d -> %d (by the TR::DataType::getZonedSignSize())\n",
         ...
         trprintf(trace, log, "\tisSrcTrailingSign=true and isInitialized=true (zdsls2zd) : increment targetReg %s deadBytes %d -> %d (by the TR::DataType::getZonedSignSize())\n",
...
J9::Z::TreeEvaluator::zd2zdslsEvaluator(TR::Node * node, TR::CodeGenerator * cg)
   ...
   trprintf(trace, log, "\tzd2zdslsEvaluator %p : op %s, targetReg->isInit=%s, targetRegSize=%d, targetRegPrec=%d\n",
         node, node->getOpCode().getName(), targetReg->isInitialized()?"yes":"no", targetReg->getSize(), targetReg->getDecimalPrecision());

   bool isTruncation = node->getDecimalPrecision() < srcReg->getDecimalPrecision();
   TR_ASSERT( !isTruncation,"a zd2zdsxs operation should not truncate\n");

   trprintf(trace, log, "\tset targetReg->prec to srcReg->prec %d\n", srcReg->getDecimalPrecision());
...
J9::Z::TreeEvaluator::zdsle2zdEvaluator(TR::Node * node, TR::CodeGenerator * cg)
   ...
   trprintf(trace, log, "\tzdsle2zdEvaluator %p : op %s, isEffectiveNop=%s, isTruncation=%s, srcSignIsZone=%s, srcReg->getSize()=%d, (isSetSign=%s, sign 0x%x)\n",
   ...
   if (!isEffectiveNop &&
      ...
      trprintf(trace, log, "\tperform an explicit widening (digitsToClear=%d, doWidening=yes, isEffectiveNop=no) set targetReg->prec to node->prec %d\n",
      ...
      trprintf(trace, log, "\tdo not perform an explicit widening (set digitsToClear=%d->0, doWidening=%s, isEffectiveNop=%s) set targetReg->prec to %d\n",
   ...
   if (!isEffectiveNop)
      ...
      if (isSetSign)
         ...
         if (sign == TR::DataType::getIgnoredSignCode())
            {
            trprintf(trace, log, "\tisSetSign=true with ignored sign=0x%x\n", sign);
            ...
            {
            trprintf(trace, log, "\tisSetSign=true : call genSignCodeSetting with sign=0x%x\n", sign);
      ...
      if (!isSetSign)
         {
         trprints(trace, log, "\tisSetSign=false : generate MVZ of size 1 to transfer left aligned zdsle sign to right aligned zd sign position\n");
      ...
      trprintf(trace, log, "\tcheck before resetting srcSignCode: srcSignWillBeIgnored %s, srcSignResetRedundant %s, srcSignResetIllegal %s\n",
      ...
      if (!(srcSignWillBeIgnored || srcSignResetRedundant || srcSignResetIllegal))
         {
            {
            trprintf(trace, log, "\tgenerate OI 0xF0 to force %s-aligned high nibble to 0xF\n", isTrailingSrc?"right":"left");
...
J9::Z::TreeEvaluator::zonedToPackedHelper(TR::Node *node, TR_PseudoRegister *targetReg, TR::MemoryReference *sourceMR, TR_PseudoRegister *childReg, TR::CodeGenerator * cg)
   ...
   trprintf(trace, log, "\tzonedToPackedHelper %p : op %s, destPrecision %d, destSize %d, sourcePrecision %d, sourceSize %d\n",
   ...
   if (node->getDecimalPrecision() < sourcePrecision)
      {
      trprintf(trace, log, "\tnodePrec <= sourcePrecision (%d <= %d) so set sourcePrecision=nodePrec=%d,isTruncation=true,sourceOffsetForLeftAlignment=%d\n",
   ...
   if (destSize > 16)
      {
      trprintf(trace, log, "\tdestSize %d > 16 so reduce destSize to 16 and destPrecision to 31 for PACK encoding and clear top %d byte(s)\n", destSize, (destSize-16));
   ...
   trprintf(trace, log, "\tsetting targetReg->prec to sourcePrecision %d\n", sourcePrecision);
   ...
   if (sourcePrecision <= 16)
      {
      trprintf(trace, log, "\tsourcePrecision %d <= 16 so generate a single PACK destSize %d, sourcePrecision %d, sourceEndByte %d\n",
      ...
      if (sourcePrecision == 31)
         ...
         destPrecision = 29;
         needsThirdPack = true;
         trprintf(trace, log, "\tsourcePrecision == 31 so reduce sourcePrecision and destPrecision to 29 and update sourceEndByte to %d\n", sourceEndByte);
         }

      trprintf(trace, log, "x^x : found large packed/zoned conv -- node %s (%p) prec %d, child %s (%p) prec %d (three=%s)\n",
      ...
      trprintf(trace, log, "\t\t1st PACK destSize=%d,srcSize=%d\n", pack1DestSize, pack1SourceSize);
      generateSS2Instruction(cg, TR::InstOpCode::PACK, node,
                             pack1DestSize-1,
                             destMR,
                             pack1SourceSize-1,
      ...
      trprintf(trace, log, "\t\t2nd PACK destSize=%d,destOffset=%d, srcSize=%d,srcOffset=%d\n", pack2DestSize, pack2DestOffset, pack2SourceSize, pack2SourceOffset);
      generateSS2Instruction(cg, TR::InstOpCode::PACK, node,
                             pack2DestSize-1,
      ...
      if (needsThirdPack)
         ...
         trprintf(trace, log, "\t\t3rd PACK destSize=%d,destOffset=%d, srcSize=%d,srcOffset=%d\n", pack3DestSize, pack3DestOffset, pack3SourceSize, pack3SourceOffset);
         generateSS2Instruction(cg, TR::InstOpCode::PACK, node,
                                pack3DestSize-1,
...
//   int32_t destPrecision = destSize;
   ...
   trprintf(trace, log, "\tpackedToZonedHelper %p : op %s, destPrecision %d, destSize %d, destEndByte %d, sourcePrecision %d, sourceSize %d\n",
   ...
   if (destPrecision < childReg->getDecimalPrecision())
      ...
      trprintf(trace, log, "\tisTruncation=true (dstPrec %d < srcPrec %d) reduce srcPrec %d->%d, srcSize %d->%d\n",
   ...
   if (destPrecision <= 16 || sourcePrecision <= 16)
      ...
      if (destPrecision > 16)
         ...
         trprintf(trace, log, "\tdestPrecision %d > 16, sourcePrecision %d <= 16 gen %d leftmost bytes of 0xF0\n", destPrecision, sourcePrecision, bytesToSet);
         ...
         trprintf(trace, log, "\treduce unpkDestOffset %d->%d and unpkDestSize %d->%d\n", unpkDestOffset, bytesToSet, unpkDestSize, sourcePrecision);
         unpkDestOffset = bytesToSet;
         unpkDestSize = sourcePrecision;
         }

      trprintf(trace, log, "\tdestPrecision %d <= 16 or sourcePrecision %d <= 16 so generate a single UNPK destPrecision %d, destOffset %d, unpkSourceSize %d\n",
      ...
      if (unpkDestSize > sourcePrecision)
         {
         trprintf(trace, log, "\tunpkDestSize %d > sourcePrecision %d adding range of zero digits for pd2zd op\n", unpkDestSize, sourcePrecision);
      ...
      if (sourcePrecision == 31)
         ...
         trprintf(trace, log, "\tsourcePrecision < 31 (%d) so reduce destPrecision to min(destPrecision,30) = min(%d,30) = %d ",
         ...
         destEndByte = isDestTrailingSign ? destPrecision + TR::DataType::getZonedSignSize() :
                                            destPrecision;
         targetReg->setDecimalPrecision(destPrecision);
         trprintf(trace, log, "and update targetReg->prec to new destPrecision %d and update destEndByte to %d\n", destPrecision, destEndByte);
         }
      ...
      trprintf(trace, log, "\t\t1st UNPK destSize=%d,srcSize=%d\n", unpk1DestSize, unpk1SourceSize);
      generateSS2Instruction(cg, TR::InstOpCode::UNPK, node,
                             unpk1DestSize-1,
                             destMR,
                             unpk1SourceSize-1,
      ...
      trprintf(trace, log, "\t\t2nd UNPK destSize=%d,destOffset=%d, srcSize=%d,srcOffset=%d\n", unpk2DestSize, unpk2DestOffset, unpk2SourceSize, unpk2SourceOffset);
      generateSS2Instruction(cg, TR::InstOpCode::UNPK, node,
                             unpk2DestSize-1,
      ...
      if (needsThirdUnpk)
         ...
         trprintf(trace, log, "\t\t3rd UNPK destSize=%d,destOffset=%d, srcSize=%d,srcOffset=%d\n", unpk3DestSize, unpk3DestOffset, unpk3SourceSize, unpk3SourceOffset);
         generateSS2Instruction(cg, TR::InstOpCode::UNPK, node,
                                unpk3DestSize-1,
...
J9::Z::TreeEvaluator::pd2zdVectorEvaluatorHelper(TR::Node * node, TR::CodeGenerator * cg)
   ...
   trprintf(trace, log, "gen VUKPZ, sizeOfZonedValue=%d, precision=%d\n", sizeOfZonedValue, precision);
   ...
   cg->decReferenceCount(child);
   targetReg->setIsInitialized();
   trprints(trace, log, "DAA: Leave pd2zdVectorEvaluatorHelper\n");
   return targetReg;
   }
...
J9::Z::TreeEvaluator::BCDCHKEvaluatorImpl(TR::Node * node,
   ...
   trprints(trace, log, "starting OOL section generation.\n");
   ...
   trprints(trace, log, "Finished OOL section generation.\n");
...
J9::Z::TreeEvaluator::BCDCHKEvaluator(TR::Node * node, TR::CodeGenerator * cg)
   ...
   switch (pdopNode->getOpCodeValue())
      ...
      default:
         ...
         trprintf(trace, log, "BCDCHK node n%dn has non-PD operation %s\n", node->getGlobalIndex(), pdopNode->getOpCode().getName());
         }
      }
   ...
   if (resultReg != NULL)
      ...
      trprintf(trace, log, "Skipped BCDCHK node n%dn\n", node->getGlobalIndex());
      }
   else
      {
...
J9::Z::TreeEvaluator::pdnegEvaluator(TR::Node * node, TR::CodeGenerator * cg)
   ...
   trprintf(trace, log, "\tpdnegEvaluator: isTruncation=%s, isWiden=%s, srcSign = 0x%x, srcSignIsValid=%s, isSimpleSignSet=%s, useRegBasedSequence=%s, needsFullInitialization=%s (== !useRegBasedSequence || isSimpleSignSet)\n",
         isTruncation ? "yes":"no",
         isWiden ? "yes":"no",
         srcSign,
   ...
   if (!needsFullInitialization && !targetReg->isInitialized() && targetReg->getSize() > 1)
      ...
      trprintf(trace, log, "\ttargetReg is not init and size %d > 1 so gen MVC with size targetRegSize-1 = %d and leftMostByte %d\n",
   ...
   if (isSrcSign0xF)
      ...
      trprintf(trace, log, "\tsrcReg has known preferred (%s) or known clean (%s) sign so gen XI 0x1 of sign byte to flip it\n",
      ...
      trprints(trace, log, "\ttargetReg has unknown but valid sign so generate register based decode sequence\n");
...
J9::Z::TreeEvaluator::evaluateValueModifyingOperand(TR::Node * node,
   ...
   trprintf(trace, log, "\tevaluateValueModifyingOperand for %s (%p) with targetReg %s and firstReg %s (#%d isInit %s), sourceSize=%d : initTarget=%s, alwaysLegalToCleanSign=%s\n",
   ...
   if (isInitialized && !useNewStoreHint)
      ...
      trprintf(trace, log, "\tisInitialized==true: liveSymSize %d (symSize %d - firstReg->deadAndIgnoredBytes %d), resultSize = %d (nodeSize %d)\n",
      ...
      if (savedLiveSymbolSize < resultSize)
         ...
         if (firstStorageReference->isTemporaryBased())
            ...
            if (trace)
               ...
               log->printf("\t\t * setting rightAlignedIgnoredBytes %d from firstReg %s to targetReg %s (valueMod incSize)\n",
            ...
            {
            trprintf(trace, log, "\t\tfirstStorageReference is not temporary based and liveSymSize < resultSize (%d < %d) so alloc and init a new temp slot and clear left most bytes\n",
                  savedLiveSymbolSize, resultSize);
         ...
         if (trace)
            ...
            log->printf("\t\t * setting rightAlignedDeadBytes %d from firstReg %s to targetReg %s (valueMod reuse)\n",
               savedRightAlignedDeadBytes,cg->getDebug()->getName(firstReg),cg->getDebug()->getName(targetReg));
            log->printf("\t\t * setting rightAlignedIgnoredBytes %d from firstReg %s to targetReg %s (valueMod reuse)\n",
               savedRightAlignedIgnoredBytes,cg->getDebug()->getName(firstReg),cg->getDebug()->getName(targetReg));
            log->printf("\t\t * setting savedLeftAlignedZeroDigits %d from firstReg %s to targetReg %s (valueMod reuse)\n",
      ...
      if (node->getOpCode().canHaveStorageReferenceHint() && node->getStorageReferenceHint())
         ...
         trprintf(trace, log, "\tcreated new targetStorageReference #%d on node %p\n", targetStorageReference->getReferenceNumber(), node);
         }
      ...
      if (destLength > 0)
         ...
         trprintf(trace, log, "\tsetting destLength and activeSize for initialization based on the smallest remaining node left on the temp based hint #%d : %d\n",
               targetStorageReference->getReferenceNumber(), destLength);
         }
      else if (destLength == 0)
         {
         destLength = targetStorageReference->getSymbolSize();
         trprintf(trace, log, "\tsetting destLength for initialization based on the current storageRef #%d size : %d\n", targetStorageReference->getReferenceNumber(), destLength);
         }
      else
         {
      ...
      if (initTarget)
         ...
         if (isBCD &&
            ...
            {
            trprintf(trace, log, "\ty^y : transfer leftAlignedZeroDigits %d from firstReg %s to targetReg %s (node %s %p)\n",
...
TR::Register *J9::Z::TreeEvaluator::pdloadEvaluatorHelper(TR::Node *node, TR::CodeGenerator *cg)
   ...
   if (isBCD)
      ...
      if (isConstant)
         {
         trprintf(trace, log, "\t%s (%p) is a constant load so set hasKnownValidSignAndData = true%s\n",
      ...
      if (!node->getOpCode().isSignlessBCDType() && node->hasKnownOrAssumedCleanSign())
         ...
         trprintf(trace, log, "\tsetting Has%sCleanSign (due to node flag) on targetPseudoReg %s on %s (%p)\n",
      ...
      if (storageRef->isTemporaryBased())
         ...
         trprintf(trace, log, "\tstorageRef is tempBased so set targetReg %s to isInitialized=true\n", cg->getDebug()->getName(targetPseudoReg));
      ...
      if (!node->hasSignStateOnLoad())
         ...
         trprintf(trace, log, "\tsetting SignStateInitialized due to hasSignStateOnLoad=false flag on %s (%p)\n", node->getOpCode().getName(), node);
         }
      }
   else
      {
...
J9::Z::TreeEvaluator::pdloadVectorEvaluatorHelper(TR::Node *node, TR::CodeGenerator *cg)
   ...
   trprintf(trace, log, "pdload Vector Evaluator, node=%p %d\n", node, __LINE__);
   ...
   trprintf(trace, log, "\tGen VLRL for %s node->size=%d\n", node->getOpCode().getName(), node->getSize());
...
isLegalOverlappingZAP(TR::Node *store, TR::CodeGenerator *cg)
   ...
   trprintf(trace, log, "\tisLegalOverlappingZAP check : store %s (%p), valueChild %s (%p)\n",
   ...
   if (storeEnd >= loadEnd)
      {
      trprintf(trace, log, "\t\tstoreEnd %lld >= loadEnd %lld : overlap ZAP is legal\n", storeEnd, loadEnd);
      return true;
      }
   else
      {
      trprintf(trace, log, "\t\tstoreEnd %lld < loadEnd %lld : overlap ZAP is NOT legal\n", storeEnd, loadEnd);
      return false;
      }
   }
...
TR::Register* J9::Z::TreeEvaluator::pdstoreEvaluatorHelper(TR::Node *node, TR::CodeGenerator *cg)
   ...
   if (valueChild->getSize() <= node->getSize() &&
      ...
      if (node->useStoreAsAnAccumulator()) // see comment above
         ...
         trprintf(trace, log, "\tsetting valueChild (%s) %p setSkipCopyOnLoad=true due to store with skipCopyOnStore=true (storeAccumCase)\n", valueChild->getOpCode().getName(), valueChild);
         }
         ...
         trprintf(trace, log, "\tsetting valueChild %s (%p) setSkipCopyOnLoad=true due to store with skipCopyOnStore=true (legalOverlappingZAPCase)\n", valueChild->getOpCode().getName(), valueChild);
         }
   ...
   if (valueStorageReference->isNonConstantNodeBased() &&
      ...
      trprintf(trace, log, "\tisInMemoryCopyProp=%s, isUsingStorageRefFromAnotherStore=%s, valueRegHasDeadOrIgnoredBytes=%s : node %s (%p), valueReg %s, storageRefNode %s (%p)\n",
      ...
      trprintf(trace, log, "\tallSizesMatch=%s (nodeSize=%d, valueRegSize=%d, storageRefNodeSize=%d)\n",
      ...
      if (node->isInMemoryCopyProp() || isUsingStorageRefFromAnotherStore || valueRegHasDeadOrIgnoredBytes || !allSizesMatch)
         ...
         trprintf(trace, log, "\tnode %s (%p) and source %s (%p) may overlap but first check if copy would be redundant\n",
         ...
         trprintf(trace, log, "\tgot copyIsRedundant=%s from first test\n", copyIsRedundant?"yes":"no");
         ...
         if (isAggr && !copyIsRedundant && !valueRegHasDeadOrIgnoredBytes && allSizesMatch)
            {
            trprints(trace, log, "\tperform test for definitelyNoDestructive overlap\n");
            ...
            if (cg->getStorageDestructiveOverlapInfo(valueStorageReference->getNode(), valueReg->getSize(), node, node->getSize()) == TR_DefinitelyNoDestructiveOverlap)
               ...
               trprintf(trace, log, "\t\tset copyIsRedundant=true : overlap check between node %s (%p) size=%d and valueStorageRefNode %s (%p) valueRegSize %d returns TR_DefinitelyNoDestructiveOverlap\n",
         ...
         trprintf(trace, log, "\t\tcopyIsRedundant=%s\n", copyIsRedundant?"yes":"no");
         ...
         if (!copyIsRedundant)
            ...
            trprintf(trace, log, "\tnode %s (%p) and source %s (%p) (uninitialized valueReg %s) may overlap -- must privatize valueReg\n",
            ...
            if (!valueReg->isInitialized() &&
               ...
               trprintf(trace, log, "\tset privatizedSize to storageRefNodeSize %d for uninit valueReg %s with mismatched storageRefNodeSize %d and valueRegSize %d\n",
               ...
               if (valueRegHasDeadOrIgnoredBytes)
                  ...
                  trprintf(trace, log, "\t\tgetRightAlignedIgnoredBytes %d > 0 so reduce privatizedSize %d -> %d\n",
            ...
            if (trace)
               ...
               if (isUsingStorageRefFromAnotherStore)
                  log->printf("\ta^a : privatize needed due to isUsingStorageRefFromAnotherStore storageRefNode %s (%p) on line_no=%d (storeCase)\n",
                     storageRefNode->getOpCode().getName(),storageRefNode,comp->getLineNumber(node));
               if (valueRegHasDeadOrIgnoredBytes)
                  log->printf("\ta^a : privatize needed due to valueRegHasDeadOrIgnoredBytes valueReg %s valueChild %s (%p) on line_no=%d (storeCase)\n",
         ...
         {
         trprintf(trace, log, "y^y : temp copy saved isInMemoryCopyProp = false on %s (%p) (storeCase)\n", node->getOpCode().getName(), node);
         }
      }
   ...
   trprintf(trace, log, "\tisPacked=%s, useZAP=%s, valueReg->signStateInit()=%s, valueReg->hasKnownOrAssumedCleanSign()=%s, isByteTruncation=%s, isByteWidening=%s, destSize=%d, sourceSize=%d\n",
         isPacked?"true":"false",
         useZAP?"true":"false",
   ...
   if (!isByteTruncation &&
      ...
      trprintf(trace, log, "\tsetting isLegalToChangeCommonedChildAddress=true for valueChild %s (%p) because isByteTruncation=false, isLeadingSignByteWidening=false, refCount %d > 1, skipCopyOnStore=true and savePreZappedValue=false\n",
   ...
   if (!valueStorageReference->isTemporaryBased() &&
      ...
      if (valueStorageReference->getNode()->getOpCode().isStore())
         {
         trprintf(trace, log, "found uninit storageRef node based STORE case valueChild %s (%p) and storageRefNode %s (%p)\n",
               valueChild->getOpCode().getName(),
               valueChild,
         ...
         {
         trprintf(trace, log, "found uninit storageRef node based LOAD case valueChild %s (%p) and storageRefNode %s (%p), skipCopyOnLoad storageRefNode is %s\n",
               valueChild->getOpCode().getName(),
               valueChild,
               valueStorageReference->getNode()->getOpCode().getName(),
               valueStorageReference->getNode(),
   ...
   if (valueStorageReference->isTemporaryBased() || (valueStorageReference->getNode() != node))
      ...
      if (isLegalToChangeCommonedChildAddress)
         ...
         if (useZAP)
            ...
            trprints(trace, log, "\t\tset changeCommonedChildAddress = true due to ZAP\n");
            }
         else if (isByteWidening)
            {
            changeCommonedChildAddress = true;
            trprints(trace, log, "\t\tset changeCommonedChildAddress = true due to byteWidening\n");
            }
/* // disable this case, not a good enough reason for potential operand store compare
            ...
            {
            changeCommonedChildAddress = true;
            trprints(trace, log, "\t\tset changeCommonedChildAddress = true due to cheaper addressability\n");
            }
*/
            ...
            {
            changeCommonedChildAddress = true;
            trprintf(trace, log, "\t\tset changeCommonedChildAddress = true due to uninitialized storageRefNode %p with skipCopyOnLoad that was forced to true\n", valueStorageReference->getNode());
            }
         else
            {
            trprints(trace, log, "\t\tleave changeCommonedChildAddress = false\n");
            }
         }
      else
         {
         trprints(trace, log, "\t\tisLegalToChangeCommonedChildAddress = false so do not attempt to look for cases to set changeCommonedChildAddress to true\n");
         }
      }
   else
      {
      ...
      changeCommonedChildAddress = true;
      trprints(trace, log, "\taccumulated hint case so unconditionally set changeCommonedChildAddress = true\n");
      }

   trprintf(trace, log, "\tbef legality check: changeCommonedChildAddress = %s and isLegalToChangeCommonedChildAddress=%s so final changeCommonedChildAddress=%s\n",
   ...
   trprintf(trace, log, "\tmustPrivatizeValueChild=%s\n", mustPrivatizeValueChild?"yes":"no");
   ...
   trprintf(trace, log, "\tcreate node based targetStorageReference #%d from %s (%p) and nodeRefCount %d (%s)\n",
   ...
   if (useZAP)
      {
      trprintf(trace, log, "\tuseZAP=true so gen ZAP but first determine the zapDestSize, initial size is destSize=%d\n", destSize);
      ...
      if (sourceNode)
         ...
         if (isByteWidening)
            {
            trprintf(trace, log, "\t\tdestSize > sourceSize (%d > %d) so check valueReg->getLiveSymbolSize() %d against destSize %d before checking if the upper bytes are clear\n",
            ...
            if (valueReg->getBytesToClear(sourceSize, destSize) == 0)
               ...
               trprintf(trace, log, "\t\tvalueReg bytes sourceSize->destSize (%d->%d) are already clear so set zapDestSize=sourceSize=%d\n", sourceSize, destSize, sourceSize);
               }
            }
         ...
         if (savePreZappedValue)
            ...
            trprintf(trace, log, "\tsavePreZappedValue=true so gen MVC with sourceSize %d to copy #%d on pdstore for valueChild %p with refCnt %d\n",
            ...
            if (savedRightAlignedDeadAndIgnoredBytes > 0)
               {
               trprintf(trace, log, "\tadd -savedRightAlignedDeadAndIgnoredBytes = -%d to sourceMR for savePreZappedValue copy\n", savedRightAlignedDeadAndIgnoredBytes);
         ...
         if (savedRightAlignedDeadAndIgnoredBytes > 0)
            {
            trprintf(trace, log, "\tadd -savedRightAlignedDeadAndIgnoredBytes = -%d to sourceMR for final ZAP\n", savedRightAlignedDeadAndIgnoredBytes);
      ...
      if (isByteTruncation)
         {
         trprintf(trace, log, "\tisByteTruncating ZAP so reduce sourceSize %d->%d\n", sourceSize, zapDestSize);
         sourceSize = zapDestSize;
         }

      trprintf(trace, log, "\tgen ZAP with zapDestSize=%d,sourceSize=%d\n", zapDestSize, sourceSize);
      ...
      if (sourceNode)
         {
         trprintf(trace, log, "\tuseZAP=false and sourceNode %s (%p) is non-NULL so gen MVC but first determine the mvcSize\n",
         ...
         if (isByteWidening)
            {
            needsClear = true;
            trprintf(trace, log, "\t\tdestSize > sourceSize (%d > %d) so try to reduce mvcSize by checking if the upper bytes are clear\n",
                  destSize, sourceSize, valueReg->getLiveSymbolSize(), destSize);
            if (valueReg->getBytesToClear(sourceSize, destSize) == 0)
               {
               needsClear=false;
               mvcSize=destSize;
               trprintf(trace, log, "\t\tvalueReg bytes sourceSize->destSize (%d->%d) are already clear so set mvcSize=destSize=%d\n", sourceSize, destSize, mvcSize);
               }
            }

         trprintf(trace, log, "\tsourceNode %s (%p) is non-NULL so gen MVC/memcpy with size %d to store (isByteTruncation=%s)\n",
         ...
         {
         trprints(trace, log, "\tuseZAP=false and sourceNode is NULL so just check if upper bytes need to be cleared\n");
         targetMR = generateS390RightAlignedMemoryReference(node, targetStorageReference, cg);
   ...
   if (valueChild->getReferenceCount() > 1)
      ...
      if (changeCommonedChildAddress)
         ...
         trprintf(trace, log, "\tchangeCommonedChildAddress=true so update storage reference on valueReg %s (leftAlignedZeroDigits=%d) and reset isInit to false\n",
         ...
         if (isByteWidening)
            ...
            if (childContainsAccumulatedResult &&
               ...
               {
               trprintf(trace, log, "\tset leftAlignedZeroDigits to %d on %s after setStorageReference\n", savedLeftAlignedZeroDigits, cg->getDebug()->getName(valueReg));
               ...
               trprintf(trace, log, "z^z : missed transferring zeroDigits %d to valueChild %s (%p) (accum=%s, valueRegSize %d, nodeSize %d\n",
         ...
         if (useZAP)
            ...
            trprintf(trace, log, "\t\tsetting HasKnownCleanSign (due to ZAP) on valueReg %s on valueChild %p\n", cg->getDebug()->getName(bcdValueReg), valueChild);
            }
         }
         ...
         trprintf(trace, log, "\tlate pdstore privatization of valueChild : so gen MVC/memcpy with sourceSize %d to copy #%d (%s) on %s for child %s (%p) with refCnt %d (mustPrivatizeValueChild %s)\n",
         ...
         if (childContainsAccumulatedResult && isLeadingSignByteWidening)
            ...
            trprintf(trace, log, "\tAccumulating a leading sign type: have to restore the sign code for the copy: signSize %d\n", signSize);
   ...
   if (changeCommonedChildAddress &&
      ...
      if (isIndirect && finalValueChildRefCount == 1)
         ...
         trprintf(trace, log, "\tfinalValueChildRefCount < origValueChildRefCount (%d < %d) and is 1 so recursively dec addrChild %s (%p) %d->%d\n",
               finalValueChildRefCount, origValueChildRefCount,
      ...
      trprintf(trace, log, "\tfinalValueChildRefCount < origValueChildRefCount (%d < %d) decrement the targetStorageReference nodeRefCount by the difference %d->%d\n",
            finalValueChildRefCount, origValueChildRefCount,
   ...
   if (targetMR == NULL)
      ...
      if (isIndirect)
         ...
         if (!changeCommonedChildAddress)
            {
            trprintf(trace, log, "\tno explicit store inst and changeCommonedChildAddress=false so recursively dec addrChild %p %d->%d\n",
                  node->getFirstChild(), node->getFirstChild()->getReferenceCount(), node->getFirstChild()->getReferenceCount()-1);
            cg->recursivelyDecReferenceCount(node->getFirstChild());
            }
         else
            {
            trprintf(trace, log, "\tno explicit store inst and changeCommonedChildAddress=true so do NOT recursively dec addrChild %p (refCount stays at %d)\n",
                  node->getFirstChild(), node->getFirstChild()->getReferenceCount());
            }
         }
      trprintf(trace, log, "\tno explicit store inst so decrement the targetStorageReference nodeRefCount %d->%d\n",
...
J9::Z::TreeEvaluator::pdstoreVectorEvaluatorHelper(TR::Node *node, TR::CodeGenerator *cg)
   ...
   TR::Logger *log = comp->log();
   bool trace = cg->traceBCDCodeGen();
   trprintf(trace, log, "DAA: Entering pdstoreVectorEvaluator %d\n", __LINE__);
   TR::Node * valueChild = node->getValueChild();
   ...
   trprintf(trace, log, "generating VSTRL for pdstore node->size = %d.\n", node->getSize());
   ...
   trprintf(trace, log, "DAA: Exiting pdstoreVectorEvaluator %d\n", __LINE__);
   return NULL;
   }
...
TR_OpaquePseudoRegister * J9::Z::TreeEvaluator::evaluateSignModifyingOperand(TR::Node *node,
   ...
   if (trace)
      ...
      if (isBCD)
         ...
         log->printf("\tevaluateSignModOperand for aggr type %s (%p) : firstReg %s (isInit %s, isEffectiveNop %s, initTarget %s)\n",
   ...
   if (isEffectiveNop)
      ...
      if (trace)
         ...
         if (isBCD)
            log->printf("\t * setting savedLeftAlignedZeroDigits %d from firstReg %s to targetReg %s (signMod nop)\n",
      ...
      if (firstReg->isInitialized())
         ...
         if (!comp->getOption(TR_DisableRefinedBCDClobberEval) && firstStorageReference->isTemporaryBased() && isNondestructiveNop)
            {
            trprintf(trace, log, "%sskipping ssrClobberEvaluate for %s (%p) with child %s (%p) refCount %d %s 1 owningRegisterCount %d %s 1-- %s mark #%d (%s) as readOnlyTemp (nondestructive nop case)\n",
      ...
      if (!firstReg->isInitialized() && targetStorageReference->isNodeBased())
         ...
         trprintf(trace, log, "\tisEffectiveNop=yes and firstReg->isInit=false case so increment the targetStorageReference nodeRefCount by (node->refCount() - 1) = %d : %d->%d\n",
      ...
      if (trace)
         ...
         log->printf("\t * setting rightAlignedIgnoredBytes %d from firstReg %s to targetReg %s (signMod isInit)\n",
            firstReg->getRightAlignedIgnoredBytes(),cg->getDebug()->getName(firstReg),cg->getDebug()->getName(targetReg));
         log->printf("\t * setting savedLeftAlignedZeroDigits %d from firstReg %s to targetReg %s (signMod isInit)\n",
            firstReg->getLeftAlignedZeroDigits(),cg->getDebug()->getName(firstReg),cg->getDebug()->getName(targetReg));
      ...
      if (!comp->getOption(TR_DisableRefinedBCDClobberEval) && firstReg->canBeConservativelyClobberedBy(node))
         ...
         //
         trprintf(trace, log, "%sskipping ssrClobberEvaluate for %s (%p) with child %s (%p) refCount %d %s 1 owningRegisterCount %d %s 1-- %s mark #%d (%s) as readOnlyTemp (isInit case)\n",
               child->getReferenceCount() > 1 ? "y^y : ":"",
               node->getOpCode().getName(),node,child->getOpCode().getName(),child,
               child->getReferenceCount(),child->getReferenceCount() > 1 ? ">":"<=",
               firstStorageReference->getOwningRegisterCount(), firstStorageReference->getOwningRegisterCount() > 1 ? ">" : "<=",
      ...
      if (initTarget)
         ...
         if (firstReg->trackZeroDigits() &&
            ...
            trprintf(trace, log, "\tupper %d bytes on srcReg %s are already clear so set mvcSize=%d\n", srcLiveSymbolSize-mvcSize,cg->getDebug()->getName(firstReg), srcLiveSymbolSize);
            ...
            {
            trprintf(trace, log, "\tfirstReg->getSize() <= node->getSize() (%d <= %d) so reduce mvcSize\n", firstReg->getSize(), node->getSize());
         ...
         if (isTruncation && node->getType().isSeparateSign())
            ...
            trprintf(trace, log, "\tnode %s is a truncating separateSign type so reduce mvcSize by sign size (%d->%d)\n",
                  node->getOpCode().getName(), mvcSize+node->getDataType().separateSignSize(), mvcSize);
            }

         trprintf(trace, log, "\tfirstReg->isInitialized()==false so gen MVC to init with mvcSize %d\n", mvcSize);
   ...
   if (firstReg->getSize() < node->getSize())
      {
      TR_ASSERT( isBCD, "this path should only be taken for BCD nodes (unless we extend support for aggr types)\n");
      trprintf(trace, log, "\twidening: firstRegSize < nodeSize (%d < %d) so set targetReg->getPseudoRegister()->prec to firstReg->prec (%d)\n",
...
J9::Z::TreeEvaluator::pdclearEvaluator(TR::Node *node, TR::CodeGenerator *cg)
   ...
   trprintf(trace, log, "\t%s (%p) : srcNode %s (%p) isInit=%s, digitClearRange %d->%d (leftMostByte=%d), digitsToClear = %d (isSetSign %s, sign 0x%x)\n",
         node->getOpCode().getName(), node,
   ...
   trprintf(trace, log, "\tset targetReg prec to %d (isTrucation %s)\n", targetRegPrec, isTruncation?"yes":"no");
   ...
   if (targetRegPrec < leftMostDigit)
      ...
      trprintf(trace, log, "\ttargetRegPrec %d < leftMostDigit %d : update leftMostDigit %d->%d, leftMostByte = %d, digitsToClear %d->%d, rightMostDigit = %d\n",
   ...
   if (isEven(leftMostDigit))
      {
      trprintf(trace, log, "\tleftMostDigit %d isEven : isInit=%s, truncatedIntoClearedDigits=%s, coincidentEvenDigitCorrection=%s -- adjust the leftMostNibble to preserve or clear the leftMostByte\n",
      ...
      if (isInitialized && !truncatedIntoClearedDigits && !coincidentEvenDigitCorrection) // full byte will be cleared if truncatedIntoClearedDigits or coincidentEvenDigitCorrection are true
         {
         trprintf(trace, log, "\t\tisInit=yes,truncatedIntoClearedDigits=no,coincidentEvenDigitCorrection=no so dec %d->%d to preserve initialized leftMostNibble\n", digitsToClear,digitsToClear-1);
         digitsToClear--; // must preserve the top byte and then clear just the top digit after the clearAndSetSign
         leftMostByteForClear--;
         }
      else
         {
         trprintf(trace, log, "\t\tisInit=no or truncatedIntoClearedDigits=yes or coincidentEvenDigitCorrection=yes so inc %d->%d to clear initialized leftMostNibble\n", digitsToClear,digitsToClear+1);
   ...
   if (!isTruncation && srcReg->isEvenPrecision() && srcReg->isLeftMostNibbleClear())
      {
      trprintf(trace, log, "\twidening with even srcRegPrec %d update targetReg with zero range for leftMostNibble %d->%d\n",
   ...
   if (!(truncatedIntoClearedDigits || coincidentEvenDigitCorrection))
      ...
      if (isEven(leftMostDigit))
         ...
         if (isInitialized)
            {
               {
               trprintf(trace, log, "\tisInit=yes : gen NI to clear right most nibble at byte %d\n", leftMostByte);
            ...
            {
            trprintf(trace, log, "\tisInit=no : gen MVZ to restore left most nibble at byte %d\n", leftMostByte);
            int32_t mvzSize = 1;
            generateSS1Instruction(cg, TR::InstOpCode::MVZ, node,
                                   mvzSize-1,
                                   reuseS390LeftAlignedMemoryReference(destMR, node, targetReg->getStorageReference(), cg, leftMostByte),
...
J9::Z::TreeEvaluator::correctPackedArithmeticPrecision(TR::Node *node, int32_t op1EncodingSize, TR_PseudoRegister *targetReg, int32_t computedResultPrecision, TR::CodeGenerator * cg)
   ...
   trprintf(cg->traceBCDCodeGen(), cg->comp()->log(), "\tset targetRegPrec to min(computedResultPrecision, nodePrec) = min(%d, %d) = %d (targetRegSize = %d)\n",
...
J9::Z::TreeEvaluator::pdaddsubEvaluatorHelper(TR::Node * node, TR::InstOpCode::Mnemonic op, TR::CodeGenerator * cg)
   ...
   trprintf(trace, log, "\t%s: produceOverflowMessage=%s, node->getSize()=%d, firstReg->getSize()=%d, secondReg->getSize()=%d, op1EncodingPrec=%d, op1EncodingSize=%d\n",
   ...
   trprintf(trace, log, "\tcomputedResultPrecision %s nodePrec (%d %s %d) -- mayOverflow = %s\n",
   ...
   if (mayOverflow)
      ...
      targetReg->setHasKnownPreferredSign();
      trprintf(trace, log, "\toverflow may occur so set HasKnownPreferredSign = true on reg %s\n", cg->getDebug()->getName(targetReg));
      ...
      if (trace)
         ...
         if (firstChild->isZero() || secondChild->isZero())
            ...
            log->printf("\t%s result prec %d is > both reg1 prec %d and reg2 prec %d so nibble clearing is NOT required and set HasKnownCleanSign = true on reg %s\n",
   ...
   if (isAdd &&
      ...
      {
      trprintf(trace, log, "\tfirstReg and secondReg have positive sign codes so set targetReg sign code to the preferred positive sign 0x%x\n", TR::DataType::getPreferredPlusCode());
...
J9::Z::TreeEvaluator::pdmulEvaluatorHelper(TR::Node * node, TR::CodeGenerator * cg)
   ...
   if (targetReg->getDecimalPrecision() < computedResultPrecision)
      ...
      trprintf(trace, log, "TR::InstOpCode::MP node %p targetRegPrec %d >= computedResultPrecision %d (firstRegPrec %d + secondRegPrec %d) so skip nibble clearing\n",
...
J9::Z::TreeEvaluator::pddivremEvaluatorHelper(TR::Node * node, TR::CodeGenerator * cg)
   ...
   trprintf(trace, log, "\t%s: gen DP dividendSize = %d, secondOpSize = secondRegSize = %d, targetRegSize = %d (firstRegPrec %d, secondRegPrec %d)\n",
   ...
   if (isRem)
      ...
      trprintf(trace, log, "\tpdrem: setting targetReg prec to divisor prec %d (node prec is %d), isTruncation=%s\n",
      ...
      if (firstReg->isEvenPrecision())
         {
         trprintf(trace, log, "\tfirstRegPrec (%d) isEven=true so reduce computedQuotientPrecision %d->%d\n", firstReg->getDecimalPrecision(), computedQuotientPrecision, computedQuotientPrecision-1);
      ...
      if (trace)
         ...
         log->printf("\tsetting targetReg prec to min(computedQuotPrec, nodePrec) = min(%d, %d) = %d (size %d), isTruncation=%s\n",
   ...
   if (!node->canSkipPadByteClearing() && targetReg->isEvenPrecision() && isTruncation)
      ...
      trprintf(trace, log, "\t%s: generating NI to clear top nibble with leftMostByte = targetReg->getSize() = %d\n", isRem ? "pdrem":"pddiv", targetReg->getSize());
   ...
   if (isRem)
      ...
      if (firstReg->hasKnownOrAssumedSignCode())
         ...
         trprintf(trace, log, "\tpdrem: firstReg has the knownSignCode 0x%x so set targetReg sign code to the preferred sign 0x%x\n",
      ...
      if (firstReg->hasKnownOrAssumedSignCode() && secondReg->hasKnownOrAssumedSignCode())
         ...
         if ((dividendSignIsPositive && divisorSignIsPositive) ||
            ...
            {
            targetReg->setKnownSignCode(TR::DataType::getPreferredPlusCode());
            trprintf(trace, log, "\tpddiv: dividendSign matches the divisorSign so set targetReg sign code to the preferred sign 0x%x\n", TR::DataType::getPreferredPlusCode());
            }
         else
            {
            targetReg->setKnownSignCode(TR::DataType::getPreferredMinusCode());
            trprintf(trace, log, "\tpddiv: dividendSign does not match the divisorSign so set targetReg sign code to the preferred sign 0x%x\n", TR::DataType::getPreferredMinusCode());
            }
         }
      }
...
J9::Z::TreeEvaluator::clearAndSetSign(TR::Node *node,
   ...
   trprintf(trace, log, "\tclearAndSetSign: digitsToClear %d, leftMostByte %d (isSetSign=%s, sign 0x%x)\n", digitsToClear, leftMostByteForClear, isSetSign?"yes":"no", sign);
   ...
   if (isSetSign)
      ...
      if (clearingNeeded)
         ...
         if (digitsToClear > 0 && (digitsToClear&0x1) && sign == TR::DataType::getIgnoredSignCode())
            ...
            targetReg->setHasKnownBadSignCode();
            trprintf(trace, log, "\tignored setSign case so inc digitsToClear %d->%d and setHasKnownBadSignCode=true on targetReg %s\n",
      ...
      /* if (digitsToClear == 1) // MVN done later is better then MVC/NI as the latter suffers from an OSC
         ...
         trprintf(trace, log, "\t\tdigitsToClear==1 case: gen MVC to initialize sign code\n");
         }
      else */
      if (clearingNeeded)
         {
         digitsToClear++;         // clear the sign code too and then MVN in the new sign code
         trprintf(trace, log, "\t\t init=false && isSetSign=false case : bump digitsToClear %d->%d to clear entire field\n", digitsToClear, digitsToClear+1);
         }
      }
   TR_ASSERT(digitsToClear >= 0,"digitsToClear %d should be >= 0\n",digitsToClear);
   if (digitsToClear > 0)
      {
      trprintf(trace, log, "\t\tdigitsToClear %d > 0 so call genClearLeftMostDigitsIfNeeded\n", digitsToClear);
   ...
   if (!signCodeIsInitialized)
      {
      trprintf(trace, log, "\t\tsignCodeIsInitialized=false after clearing of %d digits : init the sign now with an MVN of size 1\n", digitsToClear, isSetSign?"yes":"no");
...
J9::Z::TreeEvaluator::simpleWideningOrTruncation(TR::Node *node,
   ...
   TR::Logger *log = comp->log();
   bool trace = cg->traceBCDCodeGen();
   trprintf(trace, log, "\tsimple widening or truncating shift: srcRegPrecision %d, isSetSign=%s, sign 0x%x\n", srcReg->getDecimalPrecision(), isSetSign?"yes":"no", sign);
   ...
   if (!isDigitTruncation && srcReg->isEvenPrecision() && !srcReg->isLeftMostNibbleClear())
      ...
      if (targetPrecision != srcPrecision) // in case this routine starts doing explicit widenings at some point then !canSkipPadByteClearing alone is not valid
         ...
         {
         needsTopNibbleClearing = true;
         trprintf(trace, log, "z^z : new clear : simpleWide %p\n", node);
         }
      }
   ...
   TR_PseudoRegister *targetReg = NULL;
   TR::MemoryReference *sourceMR = NULL;
   trprintf(trace, log, "\tisDigitTruncation=%s, srcPrecision=%d, isPassThrough=%s, needsTopNibbleClearing=%s, initTargetAndSign=%s\n",
   ...
   if (!isInitialized && !isPassThrough)
      ...
      trprintf(trace, log, "\tisInit=false and isPassThru=false so gen initializing MVC with size %d. Do not clear after MVC just set targetReg->prec to srcPrecision %d\n", srcSize, srcPrecision);
      generateSS1Instruction(cg, TR::InstOpCode::MVC, node,
                             srcSize-1,
                             generateS390RightAlignedMemoryReference(*destMR, node, 0, cg),
                             generateS390RightAlignedMemoryReference(*sourceMR, node, 0, cg));
      ...
      trprintf(trace, log, "\tisInit=true (%s) or isPassThru=true (%s): no move needed just set targetReg->prec to srcPrecision %d\n", isInitialized?"yes":"no", isPassThrough?"yes":"no", srcPrecision);
   ...
   if (needsTopNibbleClearing)
      {
      trprintf(trace, log, "\tisDigitTruncation=true and targetReg->isEvenPrecision() (%d) so clear top nibble\n", targetReg->isEvenPrecision());
...
J9::Z::TreeEvaluator::pdshiftEvaluatorHelper(TR::Node *node, TR::CodeGenerator *cg, bool isRightShift)
   ...
   trprintf(cg->traceBCDCodeGen(), comp->log(), "\tGen packed decimal shift: %s %p : shift by %d, roundAmount=%d, result Size=%d, precision %d, sourceSize=%d, precision %d\n",
         node->getOpCode().getName(),
         node,
         shiftAmount,
         roundAmount,
...
J9::Z::TreeEvaluator::zdstoreiVectorEvaluatorHelper(TR::Node *node, TR::CodeGenerator *cg)
   {
   trprintf(cg->comp()->getOption(TR_TraceCG), cg->comp()->log(), "DAA: Entering zdstoreiVectorEvaluator %d\n", __LINE__);
