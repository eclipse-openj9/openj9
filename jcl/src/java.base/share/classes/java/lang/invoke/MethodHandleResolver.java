...
/*[IF OPENJDK_METHODHANDLES]*/
		VMLangAccess access = VM.getVMLangAccess();
		MethodType mt = MethodType.fromDescriptor(type, access.getClassloader(callerClass));

		Object[] result = new Object[2];

		if ((REF_invokeVirtual == refKind) && (VarHandle.class == definingClass) && (void.class == mt.returnType())) {
			VarHandle.AccessMode am;
			try {
				am = VarHandle.AccessMode.valueFromMethodName(name);
			} catch (IllegalArgumentException e) {
				throw new InternalError(e);
			}

			if (void.class != am.at.returnType) {
				try {
					result[0] = Invokers.varHandleInvokeLinkerMethod(mt);
					result[1] = new VarHandle.AccessDescriptor(mt, am.at.ordinal(), am.ordinal());
				} catch (Error e) {
					throw e;
				} catch (Throwable e) {
