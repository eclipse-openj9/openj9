		...
		if (messages.size() > 0 && !inlineMessages) {
			...
			if (parent.exists()) {
				try (OutputStream output = new PhantomOutputStream(outputFile, force);
					...
					put.println("#");
					put.println("#     SPDX-License-Identifier: EPL-2.0 OR Apache-2.0 OR GPL-2.0-only WITH Classpath-exception-2.0 OR GPL-2.0-only WITH OpenJDK-assembly-exception-1.0");
					put.println("#");
